...
int tqe_rx_multicast(void *queue, union topaz_tqe_cpuif_descr *desc,
...
uint32_t switch_tqe_multi_proc_sem_down(const char * funcname, int linenum);
uint32_t switch_tqe_multi_proc_sem_up(void);
#else
static inline uint32_t switch_tqe_multi_proc_sem_down(const char * funcname, int linenum)
{
	return 1;
}
static inline uint32_t switch_tqe_multi_proc_sem_up(void)
{
	return 1;
}
#endif
...
typedef unsigned int (*tqe_hbm_buf_append_meta_mcast_handler)(
...
extern atomic_t tqe_xmit_success_counter;

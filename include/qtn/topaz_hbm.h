...
#define topaz_hbm_attach_skb(buf_virt, pool, headroom)		\
	_topaz_hbm_attach_skb((buf_virt), (pool), (headroom)	\
			QTN_SKB_ALLOC_TRACE_ARGSRC)
#define topaz_hbm_attach_skb_for_qfp(buf_virt, buf_size, pool, data_ready, headroom)	\
	_topaz_hbm_attach_skb_for_qfp((buf_virt), (buf_size), (pool), (data_ready), (headroom)	\
			QTN_SKB_ALLOC_TRACE_ARGSRC)

#define QTN_HBM_MAX_FRAME_LEN	12000	/* greater than max for VHT */

struct sk_buff *_topaz_hbm_attach_skb(void *buf_virt, int8_t pool, uint8_t headroom
		QTN_SKB_ALLOC_TRACE_ARGS);
struct sk_buff *_topaz_hbm_attach_skb_for_qfp(void *buf_virt, uint32_t buf_size, uint8_t pool,
		uint8_t data_ready, uint8_t headroom QTN_SKB_ALLOC_TRACE_ARGS);
		...
		QTN_SKB_ALLOC_TRACE_ARGS)
{
	...
	return _topaz_hbm_attach_skb(buf_virt, pool, 0
...
extern void topaz_hbm_handle_deferred_level2(void);
#endif
...
#endif
extern void (*topaz_emac_timer_irq_cb)(unsigned int hbm_avail);

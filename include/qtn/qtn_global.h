...
#define QTN_AC_BE_INHERIT_MAX			QTN_AC_BE_INHERIT_VO_NO_STA
...
#define QTN_GLOBAL_MU_QM_BPASS_MODE_INIT_STATE	0
...
#define QTN_RX_REORDER_BUF_TIMEOUT_US	{200000, 200000, 200000, 200000} /* for different wmm ac */
#define QTN_RX_REORDER_BUF_TIMEOUT_US_VW	800000
#define QTN_DEAUTH_MAX_RETRY_COUNT	4
...
#define QTN_OPTI_DFLT_ID		0x80000000
...
/* mask of U-repeater tweaks */
#define QTN_REP_TWEAK_WMM_PARAMS		(0x1)
#define QTN_REP_TWEAK_DEFAULT			(QTN_REP_TWEAK_WMM_PARAMS)
...
/* If higher than this value, increase the CCA threshold */
#define QTN_AUTO_CCA_SHORT_PREAMBLE_THRESHOLD	15000
/* If higher than this value, increase the CCA threshold */
#define QTN_AUTO_CCA_INTF_THRESHOLD_HIGH	250
/* If lower than this value, decrease the CCA threshold */
#define QTN_AUTO_CCA_INTF_THRESHOLD_LOW		100
/* The max cca threshold we can set */
#define QTN_AUTO_CCA_THRESHOLD_MAX		0x10000
...
#define QTN_AUTO_CCA_FLAGS_SAMPLE_ONLY			0x4
	...
	uint32_t	cca_intf_threshold_low;
...
#define QTN_AUTO_CCA_PARARMS_DEFAULT		\
	{ QTN_AUTO_CCA_FLAGS_ENABLE, QTN_AUTO_CCA_SHORT_PREAMBLE_THRESHOLD, \
		QTN_AUTO_CCA_INTF_THRESHOLD_HIGH, QTN_AUTO_CCA_INTF_THRESHOLD_LOW, \
		QTN_AUTO_CCA_THRESHOLD_MAX}
...
struct qtn_global_param {
	...
	uint32_t	g_rx_agg_timeout[WMM_AC_NUM];
	uint32_t	g_muc_flags;
	struct qtn_scs_params scs_params;
	struct qtn_bgscan_params bgscan_params;
	...
	uint8_t		g_mu_qmat_bypass_mode;
	uint8_t		g_cot_tweaks;		/* Channel Occupancy Time tweaks */
	uint8_t		g_rep_tweaks;
	uint8_t		g_amsdu_security_check;
...
#define G_PARAMS_INIT	{			\
	...
	QTN_RX_BAR_SYNC_ALL,			\
	QTN_OPTI_DFLT_ID,			\
	QTN_GLOBAL_MU_QM_BPASS_MODE_INIT_STATE,	\
	0,					\
	QTN_REP_TWEAK_DEFAULT,			\
	QTN_AMSDU_SUBFRM_CHECK_INIT,		\
	"end"					\
}
...
#define QTN_CHAN_IS_2G(ch_num)	\
	(((ch_num) >= QTN_2G_FIRST_OPERATING_CHAN) && ((ch_num) <= QTN_2G_LAST_OPERATING_CHAN))
#define QTN_CHAN_IS_5G(ch_num)	\
	(((ch_num) >= QTN_5G_FIRST_OPERATING_CHAN) && ((ch_num) <= QTN_5G_LAST_OPERATING_CHAN))

#define IEEE80211_FDR_LOWBAND(ch)		(ch >= QTN_5G_FIRST_OPERATING_CHAN && \
			ch <= QTN_5G_LAST_OPERATING_CHAN_OF_LOWBAND)
#define IEEE80211_FDR_HIGHBAND(ch)		(ch >= QTN_5G_FIRST_OPERATING_CHAN_OF_HIGHBAND && \
 ...
 * comparisons in cases where they need to be distinguished
 */

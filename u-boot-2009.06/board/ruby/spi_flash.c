...
uint32_t spi_flash_get_flash_size(void)
{
#if defined(FLASH_SIZE_64KB)
	return SPI_FLASH_SIZE_64K;
#elif defined(FLASH_SIZE_128KB)
	return SPI_FLASH_SIZE_128K;
#elif defined(FLASH_SIZE_256KB)
	return SPI_FLASH_SIZE_256K;
#elif defined(FLASH_SIZE_512KB)
	return SPI_FLASH_SIZE_512K;
#elif defined(FLASH_SIZE_1MB)
	return SPI_FLASH_SIZE_1M;
#elif defined(FLASH_SIZE_2MB)
	return SPI_FLASH_SIZE_2M;
#elif defined(FLASH_SIZE_4MB)
	return SPI_FLASH_SIZE_4M;
#elif defined(FLASH_SIZE_8MB)
	return SPI_FLASH_SIZE_8M;
#elif defined(FLASH_SIZE_16MB)
	return SPI_FLASH_SIZE_16M;
#endif
	return 0;
}

void spi_flash_check_id(uint32_t jedec_id, uint32_t flash_size)
{
	int i;
	...
	for (i = 0; i < ARRAY_SIZE(flash_data); i++) {
		if (jedec_id == flash_data[i].jedec_id) {
			...
			if (g_spi_flash->jedec_id == SPI_GENERIC_MAGIC) {
				if (flash_size <= SPI_FLASH_SIZE_1M) {
					g_spi_flash->sector_size = SPI_SECTOR_4K;
					g_spi_flash->flags = SECTOR_ERASE_OP20;
				}
				if (g_spi_flash->sector_size)
					g_spi_flash->n_sectors = flash_size / g_spi_flash->sector_size;
			}
			break;
		}
	}
...
void spi_flash_probe(void)
{
	...
	flash_size = spi_flash_get_flash_size();

	spi_flash_check_id(jedec_id, flash_size);

	if (!g_spi_flash) {
		printf("%s: unsupported jedec found: 0x%x, using generic flash: ", __FUNCTION__, jedec_id);
		if (flash_size) {
			if (flash_size >= SPI_FLASH_SIZE_1M)
				printf("flash_size=%uMB\n", flash_size / (1024 * 1024));
			else
				printf("flash_size=%uKB\n", flash_size / 1024);
			jedec_id = SPI_GENERIC_MAGIC;
			spi_flash_check_id(jedec_id, flash_size);
		} else {
			printf("build u-boot with make uboot flash_size={64KB|256KB|...}\n");
		}
	}
}

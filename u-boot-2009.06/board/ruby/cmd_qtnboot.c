		...
		unsigned long *safety_size, unsigned long *live_size)
{
	...
	*live_size = 0;
	 ...
	 */
	if (safety_addr_str) {
		/* safety_addr_str is not tainted, seems coverity limit, add suppress comment */
	...
	if (safety_size_str) {
		/* safety_size_str is not tainted, seems coverity limit, add suppress comment */
		/* coverity[tainted_data] */
	...
	if (live_size_str) {
		/* live_size_str is not tainted, seems coverity limit, add suppress comment */
		/* coverity[tainted_data] */
...
int do_qtnboot (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (loadaddr == NULL) {
		printf("FATAL: Need 'loadaddr' set in uboot env\n");
		return -1;
	}
	/* loadaddr is not tainted, it seems a limit of coverity, add suppress comment */
	/* coverity[tainted_data] */
	mem_addr = simple_strtoul(loadaddr, NULL, 16);
#else
	mem_addr = QTNBOOT_COPY_DRAM_ADDR;
#endif
	if (get_qtnboot_envvars(&safety_addr, &live_addr, &safety_size, &live_size)) {
		return -1;
	}

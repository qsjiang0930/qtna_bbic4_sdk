...
int board_eth_init(bd_t *bis)
{
	...
	if (!((emac0_cfg & EMAC_IN_USE) || (emac1_cfg & EMAC_IN_USE))) {
		unsigned long reset_mask = RUBY_SYS_CTL_RESET_IOSS;

		/*
		 * FIXME
		 * For special no PHY design, IOSS is always in reset state, which seems to cause
		 * HBM operation hang in firmware.
		 */
		if ((readl(RUBY_SYS_CTL_CPU_VEC) & reset_mask) != reset_mask) {
			writel(reset_mask, RUBY_SYS_CTL_CPU_VEC_MASK);
			writel(reset_mask, RUBY_SYS_CTL_CPU_VEC);
			writel(0, RUBY_SYS_CTL_CPU_VEC_MASK);
			printf("IOSS brought out of reset\n");
		}

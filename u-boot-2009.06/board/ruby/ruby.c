...
void board_pmu_early_init(void);
void board_pmu_late_init(void);
...
static void init_ruby_crumbs(void)
{
	...
	if (crumb_env != NULL) {
		/* crump_env is not tainted, it seems a limit of coverity, add suppress comment */
...
#if RUBY_CRUMBS_PRESERVED_SIZE
	if (crumbs->preserved.magic == RUBY_CRUMBS_PRESERVED_MAGIC) {
		printf("found preserved data (len %d) at 0x%p\n", RUBY_CRUMBS_PRESERVED_SIZE,
		       &crumbs->preserved);

		/* clear the 'classic' crumbs structure part only */
		memset(((void*)crumbs) + RUBY_CRUMBS_PRESERVED_SIZE, 0, sizeof(*crumbs) - RUBY_CRUMBS_PRESERVED_SIZE);

		if (show_crumbs) {
			printf("preserved user data dump\n");
			print_buffer ((ulong)&crumbs->preserved, (void*)&crumbs->preserved, 1,
					RUBY_CRUMBS_PRESERVED_SIZE, 0);
		}
	} else {
		if (show_crumbs) {
			printf("unknown preservation magic code addr 0x%p: 0x%lx\n",
					&crumbs->preserved.magic, crumbs->preserved.magic);
		}
		/* clear the whole crumbs structure */
...
int board_late_init(void)
{
	...
	if (p != NULL)
		/* p is not tainted, it seems a limit of coverity, add suppress comment */
		/* coverity[tainted_data] */
...
static int get_board_id(void)
{
	...
	if (s == NULL)
		...
		/* s is not tainted, it seems a limit of coverity, add suppress comment */
		/* coverity[tainted_data] */
...
#define TOPAZ_PMU_CONFIG_1_COREBUCK_ENB	0x00400000 /* 0: enable 1: disable */
...
void board_pmu_late_init(void)
{
	u32 rdata;
	int board_id;
	int board_cfg_wifi_hw;

	board_id = get_board_id();
	board_cfg_wifi_hw = board_config(board_id, BOARD_CFG_WIFI_HW);

	if (board_cfg_wifi_hw != QTN_RUBY_WIFI_NONE) {
		rdata  = REG_READ(TOPAZ_PMU_CONFIG_1);
		rdata |= TOPAZ_PMU_CONFIG_1_COREBUCK_ENB;
		REG_WRITE(TOPAZ_PMU_CONFIG_1,rdata);
	}
}
...
void qtn_mini_hw_init(int board_id)
{
	...
	u32 ddr_param = DEFAULT_DDR_PARAM;
	int pcie_flags;
	...
	ddr_size = board_config(board_id,BOARD_CFG_DDR_SIZE);
	ddr_param = board_config(board_id, BOARD_CFG_DDR_PARAM);

	ddr_init(ddr_type, ddr_speed, ddr_size, ddr_param);

...
int do_pcieboot(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	 ...
	 *  two dummy read
	 */
	/* coverity[unused_value] - This is a clocking delay */
	...
	/* check host driver type */
#ifndef TOPAZ_EP_MINI_UBOOT
	if (PCIE_BDA_HOST_QLINK_DRV & arc_read_uncached_32((void *)&bda->bda_flags)) {
		RUN("setenv bootargs ${bootargs} qlink=1");
		if (PCIE_BDA_HOST_CALCMD & arc_read_uncached_32((void *)&bda->bda_flags)) {
			calstate = getenv("calstate");
			if (!calstate || strncmp(calstate, "1", 1))
				RUN("setenv bootargs ${bootargs} calstate=1");
		}
	}
#endif
...
void pcie_ep_init(size_t memsz, uint32_t flags)
{
	...
	arc_write_uncached_16(&bda->bda_version, QDPC_PCIE_BDA_VERSION);
...
#define PCIE_LINK_TIMEOUT	(65536)
	...
	if ((loop_timeout_str != NULL) && (loop_timeout_str[0] != '0'))

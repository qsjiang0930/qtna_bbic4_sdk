...
static int fwt_sw_remove_node(uint8_t port_num, uint8_t node_num);
...
{
	fwt_db_entry *entry = fwt_db_get_table_entry(src_index);
	fwt_db_table_insert(dst_index, entry);
	entry = fwt_db_get_table_entry(dst_index);
	entry->fwt_index = dst_index;
}
...
#endif
{
	...
	PRINT_FWT("FWT: [%pM] delete node, port:%s node:%u index:%u ip_alias:%u%s\n",
			db_ent->mac_id, g_port_names[port_id], node_num,
			db_ent->fwt_index, ip_alias, mcast_last_node ? " (empty)" : "");
...
#endif
{
	...
	if (new_port == entry->out_port
			&& new_node == old_node
			&& use_4addr == old_use_4addr)
		return -EPERM;
	...
	PRINT_FWT("FWT: [%pM] updated, port:%s->%s node:%d->%d index:%u 4addr:%u->%u\n",
			entry->mac_id,
			g_port_names[old_port], g_port_names[new_port],
			old_node, new_node, entry->fwt_index,
			old_use_4addr, use_4addr);
...
int fwt_sw_delete_device(const uint8_t *mac_be)
{
	...
	if (FWT_IF_SUCCESS(index)) {
		element = fwt_db_get_table_entry(index);
		if (!element) {
		...
		if (element->out_node != FWT_DB_INVALID_NODE) {
			...
			fwt_db_delete_index_from_node_table(
					element->out_node,
					index, FWT_DB_INVALID_IPV4, element->out_port);
		}
		g_fwt_ent_cnt--;
		PRINT_FWT("FWT: [%pM] delete entry, port:%s node:%u index:%d\n",
				mac_be, g_port_names[port], node, index);
...
int fwt_sw_set_4_address_support(const uint8_t *mac_be, int use_4addr)
{
	...
	fwt_db_entry->portal = use_4addr;

	/* Update the HW FWT table */
	topaz_set_portal(table_index, use_4addr);
...
EXPORT_SYMBOL(fwt_sw_get_timestamp);
...
void fwt_sw_unregister_node(uint16_t node_num)
{
	...
	/*
	 * Like IPFF, static multicast entries are created via FWT IF directly,
	 * they are invisible to Linux bridge, hence cannot be cleared by
	 * qdrv_remove_invalid_sub_port directly.
	 */
	fwt_sw_remove_node(TOPAZ_TQE_WMAC_PORT, node_num);
		...
		const struct br_ip *group)
{
	...
	if (FWT_IF_SUCCESS(new_device.fwt_index)) {
		...
		g_fwt_ent_cnt++;
		PRINT_FWT("FWT: [%pM] add entry, port:%s node:%d index:%u 4addr:%u entries:%d\n",
				mac_be, g_port_names[port_id], node_num,
				new_device.fwt_index, use_4addr, g_fwt_ent_cnt);
		/* set node list */
...
void fwt_sw_reset(void)
{
	...
	/*
	 * When deleting a primary entry that has one or more collision entries,
	 * the first collision entry is moved to the primary index.
	 * Traversing the table backwards avoids move operations and missing the moved entries.
	 */
	for (i = (TOPAZ_FWT_HW_TOTAL_ENTRIES - 1); i >= 0; i--) {
		if (topaz_get_mac_be_from_index(i, mac_be)) {
			int index = topaz_fwt_del_entry(mac_be);
			if (index >= 0)
				fwt_db_delete_table_entry(index);
		}
	}
...
fwt_sw_fast_get_ucast_entry(const unsigned char *src_mac_be, const unsigned char *dst_mac_be)
{
	...
	fwt_ent = fwt_db_get_table_entry(index);
	if (!fwt_ent || !fwt_ent->valid || fwt_ent->out_port == TOPAZ_TQE_DROP_PORT)
		return NULL;
...
static void __exit fwt_sw_exit(void)
{
	...
	tqe_register_macfwt_cbk(NULL, NULL, NULL);
	fwt_sw_reset();
	fwt_ipmac_uc_exit();
}
...
static int __init fwt_sw_init(void)
{
	...
	tqe_register_macfwt_cbk(fwt_sw_get_entry_from_mac, fwt_sw_add_device, fwt_sw_delete_device);

	return 0;
}

int fwt_sw_get_node_from_mac(const unsigned char *mac_be)
{
	const struct fwt_db_entry *fwt_ent;

	fwt_ent = fwt_sw_get_entry_from_mac(mac_be);
	if (!fwt_ent)
		return -1;

	return fwt_ent->out_node;
}
EXPORT_SYMBOL(fwt_sw_get_node_from_mac);

...
void topaz_fwt_hw_entry_read(uint16_t index, union topaz_fwt_entry *entry)
{
	...
	hw_ent = topaz_fwt_get_hw_entry(index);
	if (entry)
		topaz_fwt_copy_entry(entry, hw_ent, 4);
	topaz_fwt_cpu_access_unlock_irqrestore(&flags);
}

void topaz_fwt_sw_entry_copy(uint16_t dst_index, uint16_t src_index)
{
	union topaz_fwt_sw_entry *dst_swp = topaz_fwt_sw_entry_get(dst_index);
	union topaz_fwt_sw_entry *src_swp = topaz_fwt_sw_entry_get(src_index);

	arc_write_uncached_16(&dst_swp->raw, arc_read_uncached_16(&src_swp->raw));
}
	...
	for (loops = 0; loops < TOPAZ_FWT_HW_TOTAL_ENTRIES; loops++) {
		...
		if (memcmp(topaz_fwt_macaddr(&ent), mac_le, ETH_ALEN) == 0) {
			...
			if (prev_index < 0 && next_index == 0) {
				...
				topaz_fwt_sw_entry_copy(index, next_index);

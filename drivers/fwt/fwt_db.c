...
int fwt_db_delete_sw_mcast(struct fwt_db_entry *db, uint8_t ip_alias)
{
	...
	if (topaz_fwt_sw_alias_table_index_valid(alias_table_index)) {
		...
		if (topaz_fwt_sw_mcast_entry_index_valid(mcast_index)) {
			...
			/* This line most likely has no effect, but is retained to minimise risk. */
...
static int fwt_db_hw_ent_is_valid(int i, const union topaz_fwt_entry *hw_ent,
				const uint8_t *mac_be)
{
	if (g_fwt_db[i].valid != hw_ent->data.valid)
		return 0;

	if (!g_fwt_db[i].valid)
		return 1;

	if (!IEEE80211_ADDR_EQ(g_fwt_db[i].mac_id, mac_be))
		return 0;

	if (hw_ent->data.out_port != g_fwt_db[i].out_port)
		return 0;

	if (hw_ent->data.out_node_0 != g_fwt_db[i].out_node)
		return 0;

	return 1;
}

static void fwt_db_reverse_mac(uint8_t *out_mac, uint8_t *in_mac)
{
	int i;

	for (i = 0; i < ETH_ALEN; i++)
		out_mac[i] = in_mac[ETH_ALEN - i - 1];
}

static void fwt_db_print_hw_entry(int index, const union topaz_fwt_entry *hw_ent,
				const uint8_t *mac_be)
{
	printk("%-5u %pM                     %-7s %u v=%u n=%u hw mismatch\n",
		index,
		mac_be,
		fwt_db_port_names[hw_ent->data.out_port],
		hw_ent->data.out_node_0,
		hw_ent->data.valid,
		hw_ent->data.next_index);
}

static int fwt_db_print_unicast(void)
{
	const char *is_4addr[] = {"no", "yes"};
	int count = 0;
	int i;
	...
	for (i = 0; i < TOPAZ_FWT_HW_TOTAL_ENTRIES; i++) {
		memset(&hw_ent, 0, sizeof(hw_ent));
		topaz_fwt_hw_entry_read(i, &hw_ent);
		fwt_db_reverse_mac(mac_be, hw_ent.data.mac_le);
		...
		if (!fwt_db_hw_ent_is_valid(i, &hw_ent, mac_be))
			fwt_db_print_hw_entry(i, &hw_ent, mac_be);
	}

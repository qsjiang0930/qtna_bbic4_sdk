...
int ieee80211_beacon_create_param(struct ieee80211vap *vap)
{
	if (vap == NULL)
		return -EINVAL;
	if (vap->param == NULL) {
		vap->param = kmalloc(sizeof(struct ieee80211_beacon_param_t), GFP_ATOMIC);
		if (vap->param == NULL) {
	...
	if ((uint32_t)vap->param & (ARC_DCACHE_LINE_LEN - 1))
		printk_ratelimited("Beacon buffer address is not aligned to 32 bytes %p\n", vap->param);
...
uint8_t *ieee80211_add_beacon_desc_ie(struct ieee80211_node *ni, uint16_t ie_id, uint8_t *frm)
{
	...
	case IEEE80211_ELEMID_TIM:
			/* IBSS/TIM */
		tie = (struct ieee80211_tim_ie_full *) frm;
		...
		frm += sizeof(struct ieee80211_tim_ie_full);
		break;
	...
	case IEEE80211_ELEMID_CHANSWITCHANN:
		/*
		 * Add 1 to CSA count and make sure the first CSA beacon contains
		 * correct CSA count since CSA count would be decreased before every
		 * beacon transmitting
		 */
		frm = ieee80211_add_csa(frm, ic->ic_csa_mode,
				ic->ic_csa_chan->ic_ieee, ic->ic_csa_count + 1);
		break;
	...
	case IEEE80211_ELEMID_HTINFO:
		/* HT information */
		frm = ieee80211_add_htinfo_csa(ni, frm);
		break;
	...
	case IEEE80211_ELEMID_VHTCAP:
		if (IS_IEEE80211_VHT_ENABLED(ic) && !ap_pure_tkip) {
			...
			frm = ieee80211_add_vhtop_csa(ni, frm);
	...
	case IEEE80211_ELEMID_VENDOR_APP:
		...
		break;
	case IEEE80211_ELEMID_VENDOR_RPE:
		memcpy(frm, vap->rpe_appie[IEEE80211_APPIE_FRAME_BEACON].ie,
			vap->rpe_appie[IEEE80211_APPIE_FRAME_BEACON].length);
		frm += vap->rpe_appie[IEEE80211_APPIE_FRAME_BEACON].length;
		break;
	...
	case IEEE80211_ELEMID_MOBILITY_DOMAIN:
		...
		break;
	case IEEE80211_ELEMID_VENDOR_REPEATER:
		stavap = ieee80211_get_sta_vap(ic);
		KASSERT(stavap != NULL, ("STA on a Repeater must be present"));

		frm = ieee80211_add_qtn_repeater_ie(frm, vap->iv_dev->dev_id, ic->rep_curr_level,
			ic->rep_max_level, stavap->iv_myaddr);
		break;
	case IEEE80211_ELEMID_EXTCAP:
		frm = ieee80211_add_extcap(vap, frm);
		break;
	case IEEE80211_ELEMID_VENDOR_RP_INFO:
...
uint8_t *ieee80211_add_beacon_desc_ext_ie(struct ieee80211_node *ni, uint16_t ext_ie_id,
		uint8_t *frm)
{
	uint8_t *pre_frm = frm, *post_frm = frm;
	struct ieee80211com *ic = ni->ni_ic;
	struct ieee80211vap *vap = ni->ni_vap;
	struct ieee80211_beacon_param_t *param = vap->param;
	unsigned long chan_switch_time;
	unsigned long cac_duration_jiffies;

	if (param == NULL)
		return 0;

	switch (ext_ie_id) {
	case IEEE80211_ELEMID_MAX_CHAN_SWITCH_TIME:
		if (ieee80211_is_chan_available(ic->ic_csa_chan)) {
			chan_switch_time = vap->max_chan_switch_time;
		} else {
			cac_duration_jiffies =
				ic->ic_get_cac_duration_jiffies(ic, ic->ic_csa_chan);
			chan_switch_time = IEEE80211_JIFFIES_TO_TU(cac_duration_jiffies) +
				MAX_CHAN_SWITCH_DFS_SAFE_DELTA_TU;
		}
		frm = ieee80211_add_max_chan_switch_time(frm, chan_switch_time);
		break;
	default:
		break;
	}
...
EXPORT_SYMBOL(ieee80211_add_beacon_desc_ext_ie);

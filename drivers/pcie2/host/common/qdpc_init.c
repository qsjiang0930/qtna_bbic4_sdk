...
#endif
#define QDPC_TOPAZ_IMG		"ep/topaz-linux.lzma.img"
#define QDPC_TOPAZ_UBOOT	"ep/u-boot.bin"
...
static inline int qdpc_pcie_set_power_state(struct pci_dev *pdev, pci_power_t state)
{
	...
	case PCI_D0:
#ifdef QTN_RC_ENABLE_HDP
		if ((readl(RUBY_SYS_CTL_CSR) & 0xff) == TOPAZ_BOARD_REVA2)
			pci_write_config_word(pdev, TOPAZ_PCI_PM_CTRL_OFFSET,(pmcsr & ~PCI_PM_CTRL_STATE_MASK) | (PCI_D0 | PCI_PM_CTRL_PME_ENABLE));
		else
#endif

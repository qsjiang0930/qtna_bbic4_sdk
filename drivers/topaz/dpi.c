...
#define topaz_dpi_lock(info, flags)	spin_lock_irqsave(&(info)->lock, flags)
#define topaz_dpi_unlock(info, flags)	spin_unlock_irqrestore(&(info)->lock, flags)
		...
		const struct topaz_dpi_filter_request *req, uint8_t *used)
{
	...
	for (i = 0; i < req->field_count; i++) {
		struct topaz_dpi_field_def *new_field_def = &req->fields[i];
		int unused_index = -1;
		int found = 0;

		for (j = 0; !found && j < TOPAZ_EMAC_NUM_DPI_FIELDS; j++) {
			struct topaz_dpi_field *field = &info->fields[j];

			if (field->refcount == 0 && !isset(used, j) && unused_index < 0) {
				unused_index = j;
			} else if (field->refcount && memcmp(new_field_def, &field->def,
					sizeof(*new_field_def) - sizeof(new_field_def->field_index)) == 0) {
		...
		if (found) {
			...
			new_field_def->field_index = unused_index;
		} else {
			/* out of fields */
			return -1;
		}
		...
		const struct topaz_dpi_filter_request *req)
{
	...
	topaz_dpi_lock(info, flags);

	filter = topaz_dpi_get_filter(info, req);
	if (filter == NULL) {
		topaz_dpi_unlock(info, flags);
		return -1;
	}
...
void topaz_dpi_field_enable(unsigned int emac, int field_index, int enable)
{
	struct topaz_dpi_info *info = topaz_dpi_info_get(emac);
	unsigned long flags;
	union topaz_emac_rx_dpi_ctrl val;

	topaz_dpi_lock(info, flags);

	val.raw = readl(info->base + TOPAZ_EMAC_RX_DPI_FIELD_CTRL(field_index));
	val.data.enable = !!enable;
	writel(val.raw, info->base + TOPAZ_EMAC_RX_DPI_FIELD_CTRL(field_index));

	topaz_dpi_unlock(info, flags);
}
EXPORT_SYMBOL(topaz_dpi_field_enable);

void topaz_dpi_field_setval(unsigned int emac, int field_index, uint32_t val)
{
	struct topaz_dpi_info *info = topaz_dpi_info_get(emac);
	unsigned long flags;
	...
	writel(val, info->base + TOPAZ_EMAC_RX_DPI_FIELD_VAL(field_index));

	topaz_dpi_unlock(info, flags);
}
EXPORT_SYMBOL(topaz_dpi_field_setval);

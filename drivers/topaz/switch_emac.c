...
static int icmp_field_idx = -1;
static int localip_field_idx = -1;
...
module_param(stateless, int, 0644);
MODULE_PARM_DESC(stateless, "stateless mode");
...
static inline void topaz_emac_wait_rx_drain(void)
{
	/* Wait till emac rx settle down */
	udelay(200);
}
...
static inline int topaz_hbm_emac_rx_pool_uf_intr_dis(void)
{
	uint32_t tmp = readl(TOPAZ_HBM_CSR_REG);
...
static void topaz_emac_stop(struct net_device *dev)
{
	...
	topaz_hbm_emac_rx_pool_uf_intr_dis();
...
static void topaz_emac_init_rxp_set_default_port(struct emac_common *privc)
{
	...
	outport.data.mcast_en = 0;
	outport.data.mcast_port = TOPAZ_TQE_LHOST_PORT;	/* multicast redirect target node */
	outport.data.mcast_sel = 1;	/* 0 = multicast judgement based on DA */
	...
	outport.data.static_port_sel = (stateless ? TOPAZ_EMAC_RXP_OP_CTRL_STATIC_PORTSEL_BD : 0);
	outport.data.static_mode_en = !!stateless;
...
#if defined(TOPAZ_EMAC_DONT_USE_TXDONE_POOL)
	emac_wr(privc, TOPAZ_EMAC_BUFFER_POOLS,
		SM(TOPAZ_HBM_BUF_EMAC_RX_POOL, TOPAZ_EMAC_BUFFER_POOLS_RX_REPLENISH) |
		SM(TOPAZ_HBM_BUF_EMAC_RX_POOL, TOPAZ_EMAC_BUFFER_POOLS_TX_RETURN));
#else
	emac_wr(privc, TOPAZ_EMAC_BUFFER_POOLS,
		SM(TOPAZ_HBM_BUF_EMAC_RX_POOL, TOPAZ_EMAC_BUFFER_POOLS_RX_REPLENISH) |
...
#define SWITCH_EMAC_MTU_MAX	\
	(TOPAZ_HBM_BUF_EMAC_RX_SIZE	\
	 - TOPAZ_HBM_PAYLOAD_HEADROOM_MAX	\
 ...
 */
static int emac_ndo_change_mtu(struct net_device *ndev, int new_mtu)
{
	if (new_mtu < ETH_ZLEN || new_mtu > SWITCH_EMAC_MTU_MAX) {
		printk(KERN_ERR "set mtu %d rejected - must be between %d and %d\n",
				new_mtu, ETH_ZLEN, SWITCH_EMAC_MTU_MAX);
		return -EINVAL;
	}

	emac_lib_change_mtu(ndev, new_mtu);
...
static netdev_tx_t topaz_emac_ndo_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct topaz_emac_priv *priv = netdev_priv(dev);
...
static const struct net_device_ops topaz_emac_ndo = {
	...
	.ndo_change_mtu = emac_ndo_change_mtu,
...
int topaz_emac_descs_init(struct net_device *dev)
{
	...
	for (i = 0; i < privc->rx.desc_count; i++) {
		...
		bufsize = topaz_hbm_pool_buf_max_size(TOPAZ_HBM_BUF_EMAC_RX_POOL);
...
#define EMAC_RXP_STATIC_SEL(enable, select, port)	\
	(SM(enable, TOPAZ_EMAC_RXP_OP_CTRL_STATIC_ENABLE)	\
	| SM(select, TOPAZ_EMAC_RXP_OP_CTRL_STATIC_PORTSEL)	\
	| SM(port, TOPAZ_EMAC_RXP_OP_CTRL_STATIC_FAIL_PORT))

static const uint32_t rxp_static_cfg[EMAC_RXP_MODE_MAX] = {
	[EMAC_RXP_MODE_NORMAL]
		= EMAC_RXP_STATIC_SEL(0, 0, TOPAZ_TQE_LHOST_PORT),
	[EMAC_RXP_MODE_SDP]
		= EMAC_RXP_STATIC_SEL(1, TOPAZ_EMAC_RXP_OP_CTRL_STATIC_PORTSEL_BD, TOPAZ_TQE_LHOST_PORT),
	[EMAC_RXP_MODE_VLAN]
		= EMAC_RXP_STATIC_SEL(1, TOPAZ_EMAC_RXP_OP_CTRL_STATIC_PORTSEL_DPI, TOPAZ_TQE_LHOST_PORT),
	[EMAC_RXP_MODE_THROTTLE]
		= EMAC_RXP_STATIC_SEL(1, TOPAZ_EMAC_RXP_OP_CTRL_STATIC_PORTSEL_DPI, TOPAZ_TQE_DROP_PORT)
};

/* Must be called with IRQ disabled */
static void topaz_emac_set_static_outport(struct emac_common *privc, unsigned int new_state)
{
	union topaz_emac_rxp_outport_ctrl outport;

	outport.raw.word0 = emac_rd(privc, TOPAZ_EMAC_RXP_OUTPORT_CTRL);
	outport.raw.word0 &= (BIT(TOPAZ_EMAC_RXP_OP_CTRL_STATIC_FAIL_PORT_S) - 1);
	outport.raw.word0 |= rxp_static_cfg[new_state];
	emac_wr(privc, TOPAZ_EMAC_RXP_OUTPORT_CTRL, outport.raw.word0);

	privc->rxp_state = new_state;
}

static int topaz_emac_rxp_mode_switch(int enable, unsigned int new_mode, unsigned int excluded_mode)
{
	...
	for (i = 0; i < ARRAY_SIZE(iflist); i++) {
		...
		if (unlikely(privc->rxp_state == excluded_mode)) {
			ret = -EINVAL;
			break;
		}

		new_state = (enable ? new_mode : EMAC_RXP_MODE_NORMAL);

		if (privc->rxp_state != EMAC_RXP_MODE_THROTTLE)
			topaz_emac_set_static_outport(privc, new_state);

		privc->rxp_state_restore = new_state;
	}
...
int topaz_emac_vlan_enable(int enable)
{
	return topaz_emac_rxp_mode_switch(enable, EMAC_RXP_MODE_VLAN, EMAC_RXP_MODE_SDP);
}
EXPORT_SYMBOL(topaz_emac_vlan_enable);

int topaz_emac_sdp_enable(int enable)
{
	return topaz_emac_rxp_mode_switch(enable, EMAC_RXP_MODE_SDP, EMAC_RXP_MODE_VLAN);
}
EXPORT_SYMBOL(topaz_emac_sdp_enable);
...
/* In the context of Timer IRQ */
uint32_t topaz_emac_rxpkts_from_tqe(void)
{
	struct shared_params *sp = qtn_mproc_sync_shared_params_get();

	static unsigned int last_congq_success;
	static unsigned int last_wmacq_out;
	static unsigned int last_wmacq_drop;
	static unsigned int last_lhostq_out;
	static unsigned int last_lhostq_drop;
	static unsigned int last_inbss_wmacq;

	unsigned int congq_success = atomic_read(&tqe_xmit_success_counter);
	unsigned int wmacq_out = readl(TOPAZ_TQE_OUTPORT_WMAC_CNT);
	unsigned int wmacq_drop = readl(TOPAZ_TQE_DROP_WMAC_CNT);
	unsigned int lhostq_out = readl(TOPAZ_TQE_OUTPORT_LHOST_CNT);
	unsigned int lhostq_drop = readl(TOPAZ_TQE_DROP_LHOST_CNT);
	unsigned int inbss_wmacq = (likely(sp) ? sp->wmac_tqe_push : 0);

	unsigned int lhost_2_wmac = congq_success - last_congq_success;

	unsigned int wmacq_success = wmacq_out - last_wmacq_out;
	unsigned int wmacq_lost = wmacq_drop - last_wmacq_drop;
	unsigned int total_2_wmac =
		(unlikely(wmacq_success < wmacq_lost) ? 0 : wmacq_success - wmacq_lost);

	unsigned int lhostq_success = lhostq_out - last_lhostq_out;
	unsigned int lhostq_lost = lhostq_drop - last_lhostq_drop;
	unsigned int total_2_lhost =
		(unlikely(lhostq_success < lhostq_lost) ? 0 : lhostq_success - lhostq_lost);

	unsigned int inbss_wmacq_success = inbss_wmacq - last_inbss_wmacq;

	total_2_wmac = (unlikely(total_2_wmac < lhost_2_wmac + inbss_wmacq_success)
		? 0 : total_2_wmac - (lhost_2_wmac + inbss_wmacq_success));

	last_congq_success = congq_success;
	last_wmacq_out = wmacq_out;
	last_wmacq_drop = wmacq_drop;
	last_lhostq_out = lhostq_out;
	last_lhostq_drop = lhostq_drop;
	last_inbss_wmacq = inbss_wmacq;

	return total_2_lhost + total_2_wmac;
}

#define EMAC_RSV_BUF	(256)

/* In the context of Timer IRQ */
void  __attribute__((section(".sram.text"))) topaz_emac_timer_irq(unsigned int hbm_avail)
{
	struct net_device *dev;
	int i;
	static int drop = 0;
	uint32_t used = topaz_emac_rxpkts_from_tqe();
	struct emac_common *privc;

	if (used + EMAC_RSV_BUF >= hbm_avail) {
		if (!drop)
			drop = 1;
		else
			return;
	...
	for (i = 0; i < ARRAY_SIZE(iflist); i++) {
		...
		if (dev) {
			...
			topaz_dpi_field_enable(i, icmp_field_idx, !drop);
			topaz_dpi_field_enable(i, localip_field_idx, drop);
			topaz_emac_set_static_outport(privc,
				drop ? EMAC_RXP_MODE_THROTTLE : privc->rxp_state_restore);
		}
	}
}
...
{
	if (topaz_hbm_emac_rx_pool_uf_intr_dis()) {
		topaz_emac_stop_rx();
...
static void __init topaz_dpi_filter_arp(int port_num)
{
	...
	req.fields = &field;
	req.field_count = 1;
	req.out_port = TOPAZ_TQE_CTRLPKT_PORT;
	req.out_node = 0;
	req.tid = 0;
...
static void __init topaz_dpi_filter_icmp(int port_num)
{
	struct topaz_dpi_filter_request req;
	struct topaz_dpi_field_def field;
	...
	field.ctrl.data.anchor = TOPAZ_DPI_ANCHOR_IPV4;
	field.ctrl.data.cmp_op = TOPAZ_DPI_CMPOP_EQ;
	field.ctrl.data.offset = 2;
	field.val =  (uint32_t)(IPPROTO_ICMP << 16);
	field.mask = (uint32_t)(0xff << 16);

	topaz_dpi_filter_add(port_num, &req);

	icmp_field_idx = field.field_index;
}

static void __init topaz_dpi_filter_igmp(int port_num)
{
	struct topaz_dpi_filter_request req;
	struct topaz_dpi_field_def field;
	...
	field.ctrl.data.cmp_op = TOPAZ_DPI_CMPOP_EQ;
	field.ctrl.data.offset = 2;
	field.val =  (uint32_t)(IPPROTO_IGMP << 16);;
	field.mask = (uint32_t)(0xff << 16);

	topaz_dpi_filter_add(port_num, &req);
}

static void __init topaz_dpi_filter_localip(int port_num)
{
        struct topaz_dpi_filter_request req;
        struct topaz_dpi_field_def field;
	...
	req.out_port = TOPAZ_TQE_CTRLPKT_PORT;
        ...
        field.ctrl.data.enable = TOPAZ_DPI_DISABLE;
        field.ctrl.data.anchor = TOPAZ_DPI_ANCHOR_IPV4;
        field.ctrl.data.cmp_op = TOPAZ_DPI_CMPOP_EQ;
        field.ctrl.data.offset = 4;
	field.val = 0;
        field.mask = 0xffffffff;

        topaz_dpi_filter_add(port_num, &req);

	localip_field_idx = field.field_index;
}
...
static int topaz_emac_localip_event(struct notifier_block *this,
		unsigned long event, void *ptr)
{
	...
	switch (event) {
	case NETDEV_UP:
		if (!(dev->flags & IFF_SLAVE) && localip_field_idx != -1) {
			for (i = 0; i < ARRAY_SIZE(iflist); i++) {
				if (topaz_emacs[i])
					topaz_dpi_field_setval(i, localip_field_idx,
...
static struct notifier_block topaz_emac_localip_notifier = {
	.notifier_call = topaz_emac_localip_event,
};
...
static struct net_device * __init topaz_emac_init(int port_num)
{
	...
	privc->rxp_state = EMAC_RXP_MODE_NORMAL;
	privc->rxp_state_restore = EMAC_RXP_MODE_NORMAL;
	...
	topaz_dpi_filter_icmp(port_num);
	topaz_dpi_filter_igmp(port_num);
	topaz_dpi_filter_localip(port_num);
#ifdef CONFIG_IPV6
...
#ifdef TOPAZ_EMAC_NULL_BUF_WR
	topaz_emac_timer_irq_cb = topaz_emac_timer_irq;
#endif
	register_inetaddr_notifier(&topaz_emac_localip_notifier);
...
static void __exit topaz_emac_module_exit(void)
{
	...
	unregister_inetaddr_notifier(&topaz_emac_localip_notifier);
}

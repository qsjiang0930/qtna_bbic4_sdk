		...
		int reg, u32 mask, u32 val, unsigned long ms, const char *func)
{
	...
	if (((*read_func)(privc, reg) & mask) != val) {
		...
		if (func) {
			if (printk_ratelimit())
				printk(KERN_DEBUG "%s %s: warn: system is overloaded : spend more ~%lums!\n",
		...
		u32 txdescs, bool txdescs_sram)
{
	...
	if (ALIGNED_DMA_DESC_ALLOC(&priv->rx, rxdescs, EMAC_DMA_DESC_ALIGN, rxdescs_sram)) {
		goto bad;
	}

	if (ALIGNED_DMA_DESC_ALLOC(&priv->tx, txdescs, EMAC_DMA_DESC_ALIGN, txdescs_sram)) {
		goto bad;
	}
...
void emac_lib_change_mtu(struct net_device *dev, int mtu)
{
	dev->mtu = mtu;
}
EXPORT_SYMBOL(emac_lib_change_mtu);

#define EMAC_LIB_INIT_MTU	0xC80
#define EMAC_LIB_INIT_JABBER	0xCA0
...
void emac_lib_init_mac(struct net_device *dev)
{
	...
	emac_wr(privc, EMAC_MAC_MAX_FRAME_SIZE, EMAC_LIB_INIT_MTU);
	emac_wr(privc, EMAC_MAC_TX_JABBER_SIZE, EMAC_LIB_INIT_JABBER);
	emac_wr(privc, EMAC_MAC_RX_JABBER_SIZE, EMAC_LIB_INIT_JABBER);
...
/* Get Link Partner Advertisements/Capabilities */
static int emac_lib_get_lpa(struct phy_device *phy_dev)
{
	int ret;
	int adv = 0;

	ret = phy_read(phy_dev, MII_STAT1000);
	if (ret > 0) {
		if (ret & LPA_1000HALF)
			adv |= SUPPORTED_1000baseT_Half;
		if (ret & LPA_1000FULL)
			adv |= SUPPORTED_1000baseT_Full;
	}

	ret = phy_read(phy_dev, MII_LPA);
	if (ret > 0) {
		if (ret & LPA_10HALF)
			adv |= SUPPORTED_10baseT_Half;
		if (ret & LPA_10FULL)
			adv |= SUPPORTED_10baseT_Full;
		if (ret & LPA_100HALF)
			adv |= SUPPORTED_100baseT_Half;
		if (ret & LPA_100FULL)
			adv |= SUPPORTED_100baseT_Full;
	}
...
/* Get advertisements that can be cleared */
static int emac_pm_clear_adv(struct phy_device *phy_dev)
{
	int lpa;
	int adv;
	int low = 0;

	lpa = emac_lib_get_lpa(phy_dev);
	adv = phy_dev->advertising & lpa;

	if (adv & (SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full)) {
		if (adv & SUPPORTED_10baseT_Full)
			low |= SUPPORTED_10baseT_Full;
	} else if (adv & (SUPPORTED_100baseT_Half | SUPPORTED_100baseT_Full)) {
		if (adv & SUPPORTED_100baseT_Half)
			low |= SUPPORTED_100baseT_Half;
		if (adv & SUPPORTED_100baseT_Full)
			low |= SUPPORTED_100baseT_Full;
	}
	if (low == 0)
		return 0;
...
static void emac_pm_level(struct emac_common *arapc, int level)
{
	...
	if (level >= BOARD_PM_LEVEL_SUSPEND) {
		...
		if (!arapc->pm_adv_mode) {
			...
			if (phy_dev->autoneg == AUTONEG_ENABLE && phy_dev->link) {
				clear_adv = emac_pm_clear_adv(phy_dev);
			} else if (phy_dev->advertising & (adv_10M_F | adv_10M_H)) {

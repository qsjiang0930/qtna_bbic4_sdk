...
struct nd_opt_hdr *iputil_ipv6_ndisc_get_opt(struct nd_opt_hdr *nd_opt, int opt_len,
	uint8_t opt_type)
{
	if (opt_len < 0)
		return NULL;
...
EXPORT_SYMBOL(iputil_ipv6_ndisc_get_opt);

void *iputil_dhcp_find_option(void *payload, int len, uint8_t option_type)
{
	struct dhcp_option *option = (struct dhcp_option *)payload;

	while (len >= (int)sizeof(struct dhcp_option)) {
		if (len < (int)sizeof(struct dhcp_option) + option->len)
			return NULL;

		if (option->type == option_type)
			return option;

		option = (struct dhcp_option *)(((unsigned long)option)
			+ sizeof(struct dhcp_option)
			+ option->len);
		len -= (sizeof(struct dhcp_option) + option->len);
	}

	return NULL;
}
EXPORT_SYMBOL(iputil_dhcp_find_option);

struct dhcp_opt_client_identifier *iputil_dhcp_find_client_identifier(uint8_t *payload, int len)
{
	struct dhcp_opt_client_identifier *option;

	option = iputil_dhcp_find_option(payload, len, DHCP_OPTION_CLIENT_IDENTIFIER);
	if (!option)
		return NULL;

	if (option->hw_type != ARPHRD_ETHER
			|| option->hdr.len != sizeof(*option) - sizeof(option->hdr))
		return NULL;

	return option;
}
EXPORT_SYMBOL(iputil_dhcp_find_client_identifier);

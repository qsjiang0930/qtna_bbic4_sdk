...
int qtn_get_flash_size(void)
{
	return g_flash_size;
}
EXPORT_SYMBOL(qtn_get_flash_size);
...
void __init
parse_flash_config(char *cmdline)
{
	/* parse command line */
	int cmdline_len = strlen(cmdline);
	const char *var = "flash_size=";
	int var_len = strlen(var);
	char flash_size[10];
	while (cmdline_len > var_len) {
		if (!strncmp(cmdline, var, var_len)) {
			sscanf(cmdline + var_len, "%s", flash_size);
			break;
		}
		++cmdline;
		--cmdline_len;
	}
	if (!strcmp(flash_size, "64KB"))
		g_flash_size = SPI_FLASH_SIZE_64K;
	else if (!strcmp(flash_size, "128KB"))
		g_flash_size = SPI_FLASH_SIZE_128K;
	else if (!strcmp(flash_size, "256KB"))
		g_flash_size = SPI_FLASH_SIZE_256K;
	else if (!strcmp(flash_size, "512KB"))
		g_flash_size = SPI_FLASH_SIZE_512K;
	else if (!strcmp(flash_size, "1MB"))
		g_flash_size = SPI_FLASH_SIZE_1M;
	else if (!strcmp(flash_size, "2MB"))
		g_flash_size = SPI_FLASH_SIZE_2M;
	else if (!strcmp(flash_size, "4MB"))
		g_flash_size = SPI_FLASH_SIZE_4M;
	else if (!strcmp(flash_size, "8MB"))
		g_flash_size = SPI_FLASH_SIZE_8M;
	else if (!strcmp(flash_size, "16MB"))
		g_flash_size = SPI_FLASH_SIZE_16M;
	else if (!strcmp(flash_size, "32MB"))
		g_flash_size = SPI_FLASH_SIZE_32M;
	else if (!strcmp(flash_size, "64MB"))
		g_flash_size = SPI_FLASH_SIZE_64M;
	else
		return;

	printk_init("%s: flash_size = %s\n", __FUNCTION__, flash_size);
}

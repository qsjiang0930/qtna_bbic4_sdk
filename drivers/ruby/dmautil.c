...
{
	/* Descriptors should also be dcache line aligned to avoid memory corruption
	 * when sharing cache line with adjacent memory
	 */
	unsigned int align_cache_safe = max(align, (unsigned int)ARC_DCACHE_LINE_LENGTH);
	size_t remap_size = desc_size * desc_count;
	size_t alloc_size = remap_size + align_cache_safe - 1;
	void *p;
	...
	d->aligned_vdescs = align_val_up(d->unaligned_vdescs, align_cache_safe);

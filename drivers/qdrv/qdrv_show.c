...
static int qdrv_show_assoc_parse_group(const char *text, enum qdrv_show_assoc_group *group)
{
	...
	else if (strcmp(text, "caps") == 0)
		*group = QDRV_SHOW_ASSOC_CAPS;
	else
		retcode = -EINVAL;
...
static void qdrv_show_assoc_print_phy_header(struct seq_file *s)
{
	...
	const char* htx = "Pkts  Tstmp Qual  SNR MCS  Rate   BW Sca   RSSI Cost Fail AvgPER   Acks TxPwr";
	/*                  17     18   19   20    21  22  23 */
	const char* hrx = "Pkts  Tstmp MCS  Rate   BW Sym Cost";
...
static void qdrv_show_assoc_print_phy_detail(void *data, struct ieee80211_node *ni)
{
	...
	seq_printf(s, "%pM %4u %9u %6u %4u %4d %3s %5s %4s %3u %6s %4u %4u %6u %6u %5u",
			ni->ni_macaddr,
			IEEE80211_NODE_IDX_UNMAP(ni->ni_node_idx),
			tx->pkts,
			tx->last_pkt_timestamp,
			ni->ni_linkqual,
			ni->ni_snr,
			mi.mcs,
			mi.rate,
	...
	/*              17   18  19  20  21  22  23 */
	seq_printf(s, " %9u %6u %3s %5s %4s %3u %4u\n",
			rx->pkts,
			rx->last_pkt_timestamp,
			mi.mcs,
			mi.rate,
			mi.bw,
			rx->last_rxsym,
...
static void qdrv_show_assoc_capabilities(struct seq_file *s, struct ieee80211com *ic,
		struct ieee80211_node *ni)
{
	seq_printf(s, "%-17s %10s %14s %14s %14s %14s\n",
			"MAC", "VHT Caps", "MU Beamformer", "MU Beamformee", "SU Beamformer", "SU Beamformee");
	if (ni)
		get_node_capabilities(s, ni);
	else
		ic->ic_iterate_nodes(&ic->ic_sta, get_node_capabilities, (void*)s, 1);
}
		...
		struct ieee80211_node *ni)
{
	...
	seq_printf(s, "\nCapabilities:\n");
	qdrv_show_assoc_capabilities(s, ic, ni);
}

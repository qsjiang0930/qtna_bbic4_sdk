...
static __always_inline void
qdrv_mac_print_muc(void)
{
	shared_params *sp = qtn_mproc_sync_shared_params_get();

	printk("MuC debug info: jiffies %u, return address 0x%x, status32 0x%x"
			" ienable 0x%x, ipc_enable 0x%x, ipc_irq_mask 0x%x\n",
			sp->debug_info.jiffies,
			sp->debug_info.ret_addr,
			sp->debug_info.status32,
			sp->debug_info.ienable,
			sp->debug_info.ipc_reg_status,
			sp->debug_info.ipc_reg_mask);
}

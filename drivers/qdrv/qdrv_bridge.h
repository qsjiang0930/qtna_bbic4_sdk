...
#endif
#define QDRV_BR_DHCP_CHADDR_PROXY	BIT(0)
	unsigned long			flags;
	uint8_t				ic_addr[IEEE80211_ADDR_LEN];
	uint8_t				dhcp_chaddr[IEEE80211_ADDR_LEN];
};

void qdrv_br_create(struct qdrv_br *br, const uint8_t *ic_addr);
...
int qdrv_br_uc_update_from_dhcp(struct qdrv_br *br, struct sk_buff *skb,
...
#if defined(CONFIG_IPV6)
void qdrv_br_ipv6uc_update_from_dst_ipv6(struct qdrv_br *br,

...
struct qdrv_pktlogger {
	...
	void *mem_wps_read_buf;
	uint32_t mem_wps_read_buf_size;

	struct timer_list rate_timer;
	/* sysmsg work */
	struct delayed_work sysmsg_work;
	spinlock_t sysmsg_lock;

	uint32_t *stats_uc_phy_stats_ptr;
	...
	struct muc_stat_rates rx_rates[2];
	struct muc_stat_rates *rx_rate_pre;
	struct muc_stat_rates *rx_rate_cur;

	struct muc_stat_rates rx_ratelog[2];
	struct muc_stat_rates *rx_ratelog_pre;
	struct muc_stat_rates *rx_ratelog_cur;
	struct timer_list phy_stats_timer;
	struct timer_list dsp_stats_timer;

	/* node rate stats */
	struct timer_list node_rate_stats_timer;
	struct qtn_node_rate_stats node_ratelog[2][QTN_NODE_STAT_RATE_SLOTS];
	int node_ratelog_cur;

	/* vision stats */
	struct timer_list vision_timer;
};
...
#define TXSTAT(_qw, _member)	((_qw)->tx_stats._member += 1)
#define TXSTAT_SET(_qw, _member, _val)	((_qw)->tx_stats._member = (_val))
#define TXSTAT_ADD(_qw, _member, _val)	((_qw)->tx_stats._member += (_val))
#define RXSTAT(_qw, _member)	((_qw)->rx_stats._member += 1)
#define RXSTAT_SET(_qw, _member, _val)	((_qw)->rx_stats._member = (_val))
#define SMSTAT(_qw, _member)	((_qw)->sm_stats._member += 1)
...
struct qdrv_wlan_rx_stats {
	...
	unsigned int rx_decap_snap_drop;
};
...
#define QDRV_UNKNOWN_DP_MAX_PER_SEC 32   /* Rate limit per sec for unknown data pkts */
...
#define MAX_QDRV_UNKNOWN_DEST_DISCOVER_INTVAL 1000	/* Max unknown destination discovery interval */
	uint32_t discover_intval;		/* unknown destination discovery interval in millisecond */
...
#define QDRV_BR_ISOLATE_VLAN		BIT(1)
	...
	struct qtn_node_rate_stats *stats_uc_rates_per_node;
	uint8_t amsdu_security_check;
};
...
#define QDRV_NETDEBUG_RADAR_MAXPULSE		(116)
#define QDRV_NETDEBUG_RADAR_PULSESIZE		(12) /* sizeof(struct qtn_radar_sample) */

#define QDRV_NETDEBUG_TXBF_DATALEN		1024
#define QDRV_NETDEBUG_IWEVENT_LENGTH		256
#define QDRV_NETDEBUG_MEM_DATALEN		1024
#define QDRV_NETDEBUG_SYSMSG_LENGTH		4096
#define QDRV_NETDEBUG_BUILDSTRING_SIZE		32
#define QDRV_NETDEBUG_VISION_VAPS_NUM_MAX	8
...
struct qdrv_muc_rates {
	uint16_t			mcs[IEEE80211_HT_RATE_MAXSIZE];
	uint16_t			mcs_pad;
} __packed;

struct qdrv_muc_11acrates {
	uint16_t			mcs_11ac_su[MUC_VHT_NUM_RATES];
	uint16_t			mcs_11ac_mu[MUC_VHT_NUM_RATES];
} __packed;

/*
 * Please keep this structures synced with qdrv_muc_rates family of structures.
 * This copy-paste needed to have different names for different pktlogger statistics.
 */
...
struct qdrv_muc_rx_11acrates {
	u_int16_t			rx_11ac_su_mcs[MUC_VHT_NUM_RATES];
	u_int16_t			rx_11ac_mu_mcs[MUC_VHT_NUM_RATES];
} __packed;

struct qdrv_muc_per_node_rates {
	uint16_t flags;
	uint8_t macaddr[IEEE80211_ADDR_LEN];
	struct qdrv_muc_rates rates;
	struct qdrv_muc_11acrates rates_11ac;
} __packed;

/* Maximum number of rates is QTN_NODE_STAT_RATE_SLOTS */
/* @NDB@: type=13,payload=rates_per_node,payloadtype=vararray(node_count) */
struct qdrv_netdebug_node_rate_stats {
	struct qdrv_pktlogger_hdr	ndb_hdr;
	uint32_t			node_count;
	struct qdrv_muc_per_node_rates	rates_per_node[0];
} __packed;
...
struct qdrv_vision_vap_stats {
	uint32_t			index;
} __packed;

struct qdrv_vision_node_stats {
	uint32_t			vap_index;
	uint8_t				macaddr[IEEE80211_ADDR_LEN];
	uint64_t			tx_bytes;
	uint64_t			rx_bytes;
...
/* @NDB@: type=14,payload=node_data,payloadtype=vararray(node_count) */
struct qdrv_netdebug_vision {
	struct qdrv_pktlogger_hdr	ndb_hdr;
	uint32_t			vap_count;
	struct qdrv_vision_vap_stats	vap_data[QDRV_NETDEBUG_VISION_VAPS_NUM_MAX];
	uint32_t			node_count;
	struct qdrv_vision_node_stats	node_data[0];
} __packed;
...
/* @NDB@: type=12,payload=data,payloadtype=varstring */
...
int qdrv_hostlink_bgscan_end(struct qdrv_wlan *qw, struct qtn_scan_chan_info *);
#endif /* QTN_BG_SCAN */
...
void qdrv_wlan_vlan_drop_stag(struct ieee80211com *ic, int drop_stag);
...
static inline const char *qdrv_igmp_type_to_string(int igmp_type)
{
	switch(igmp_type) {
		...
		case IGMP_TYPE_MEMBERSHIP_REPORT1:
			return "IGMP Membership Report (v1)";
		case IGMP_TYPE_MEMBERSHIP_REPORT2:
			return "IGMP Membership Report (v2)";
		case IGMP_TYPE_MEMBERSHIP_REPORT3:
			return "IGMP Membership Report (v3)";
		case IGMP_TYPE_LEAVE_GROUP:
			return "IGMP Leave Group";
	}
...
static inline void qdrv_update_invalid_sub_port(struct ieee80211vap *vap,
		uint32_t sub_port, uint32_t new_sub_port)
{
	DBGPRINTF(DBG_LL_DEBUG, QDRV_LF_BRIDGE, "Renew subport[0x%x]\n", sub_port);

	if (sub_port == new_sub_port)
		return;

#if defined(CONFIG_BRIDGE) || defined(CONFIG_BRIDGE_MODULE)
	if (br_fdb_update_by_sub_port_hook && vap->iv_dev->br_port) {
		br_fdb_update_by_sub_port_hook(vap->iv_dev->br_port->br,
				vap->iv_dev->br_port, sub_port, new_sub_port);
	}
#endif
}
...
/* some special customer platform IDs */
#define MSMR_PLATFORM	415
#define MSFT_PLATFORM	425
#endif

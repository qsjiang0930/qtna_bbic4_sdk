...
#define QDRV_DEFAULT_SHORTRANGE_SCANCOUNT 4
...
#define QDRV_CMD_LENGTH	128
	...
	char cred_cust[QTN_CRED_CUST_LEN];
	char cred_plat[QTN_CRED_PLAT_LEN];
	char cred_feat[QTN_CRED_FEAT_LEN];
	char cred_sign[QTN_CRED_SIGN_LEN];
};

static __always_inline int sem_take(u32 sem, u32 bit, u32 *irqflags)
{
	return qtn_mproc_sync_set_hw_sem(sem, bit, irqflags);
}

static __always_inline void sem_give(u32 sem, u32 bit, u32 irqflags)
{
	qtn_mproc_sync_clear_hw_sem(sem , bit, irqflags);
}
...
u_int8_t get_bootcfg_scancnt(void);

...
static struct qtn_phy_stats host_phy_stats;
...
const uint8_t qdrv_legacy_rates[] = IEEE80211_RATES_LEGACY;
...
static const char g_header_line[]       = KERN_DEBUG "Tstamp"" SU/MU"" RxMPDU"" AMSDU"" RxG""   CRC""  Noise"" TxFrame"" Defers"" Touts""   H/SRtry"" ShPmbl""  LgPmbl"" Scale" " " " MCS-(TX/RX)""    RSSI / RCPI / EVM\n";
static const char g_stat_comm_fmt[]     = KERN_DEBUG    "%6d"   "%6s"   "%7d"   "%6d" "%4d"   "%6d"    "%7s"   "%8d"    "%7d"   "%6d"     "%5u/%4u"    "%7d"     "%8d"   "%6d"    " ";
static const char g_stat_comm_csv_fmt[] =               "%d,"   "%s,"   "%d,"   "%d," "%d,"   "%d,"    "%s,"   "%d,"    "%d,"   "%d,"     "%d,"    "%d,"    "%d,"     "%d,"   "%d,";

static void qtn_muc_stats_display_common(const struct qtn_stats *stats,
		int mu, int csv_format, int data)
{
	...
	printk(csv_format ? g_stat_comm_csv_fmt : g_stat_comm_fmt,
		stats->tstamp,
		mu ? "MU" : "SU",
		data ? rxstats[mu]->num_data_pkts : rxstats[mu]->num_pkts,
		...
		txstats[mu]->num_hwretries,
		txstats[mu]->num_swretries,
...
#ifdef PHY_STATS_PHY_DATE_RATE
#define STAT_RSSI_FMT "%4dM %4dM     %5s %5s %5s %6s dBm %6s avg RSSI\n"
...
#define STAT_EVM_FMT "%5d %5d %3d %5s %5s %5s %6s dB  %6s avg EVM\n"
...
static void display_log_info(const struct qtn_stats *stats, struct ieee80211com *ic, enum muc_stats_opt opt,
				int show_all_nodes, int csv_format, int data)
{
	...
	for (mu = start[opt]; mu < stop[opt]; mu++) {
		...
		ret = 0;
		qtn_muc_stats_display_common(stats, mu, csv_format, data);
		if (csv_format) {
...
#define STAT_ALL_CSV_FMT "%s," DBGMACVAR ",%u,%u,"
				printk(csv_format ? STAT_ALL_CSV_FMT : KERN_DEBUG STAT_ALL_FMT,
					...
					ni->ni_shared_stats->tx[mu].tx_mpdus);
				if (csv_format) {
...
#define COMMON_CSV_HEADER(x) "Tstamp_"#x","#x",RxPkt_"#x",AMSDU_"#x",RxG_"#x",CRC_"#x",Noise_"#x",TxPkt_"#x",Defers_"#x",Touts_"#x",HwRetries_"#x",SwRetries_"#x",ShPmbl_"#x",LgPmbl_"#x",Scale_"#x","
						  ...
						  int required_phy_stat_mode)
{
	...
	cr_indx = MOD(phy_stats->curr_buff + 2, NUM_LOG_BUFFS);
	curr_log_ptr = &phy_stats->stat_buffs[cr_indx];

	while (cr_indx != phy_stats->curr_buff) {
...
static void parse_log(struct qtn_phy_stats *phy_stats, struct ieee80211com *ic, enum muc_stats_opt opt,
			int show_all_nodes, int csv_format, int data)
{
	struct qtn_stats *curr_log_ptr;
	int cr_indx;

	/* If reading stats for first time or after NUM_LOG_BUFFS interval,
	 * reset next_buf_idx to phy_stats->curr_buff+2
	 */
	if ((last_tstamp && ((last_tstamp + 1) != phy_stats->stat_buffs[next_buf_idx].tstamp)) ||
		((next_buf_idx == 0) && (last_tstamp == 0)))
		next_buf_idx = MOD(phy_stats->curr_buff + 2, NUM_LOG_BUFFS);

	cr_indx = next_buf_idx;

	while (cr_indx != MOD(phy_stats->curr_buff + (NUM_LOG_BUFFS - 1), NUM_LOG_BUFFS)) {
		curr_log_ptr = &phy_stats->stat_buffs[cr_indx];
		cr_indx = MOD(cr_indx + 1, NUM_LOG_BUFFS);
		...
		next_buf_idx = cr_indx;
		display_log_info(curr_log_ptr, ic, opt, show_all_nodes, csv_format, data);
	}
}
...
static void dump_log(struct qtn_stats_log *log)
{
	...
	printk("Current Log indx %d\n", log->phy_stats->curr_buff);

	for(cr_indx=0;cr_indx < NUM_LOG_BUFFS; cr_indx++){
		printk("Indx %d, Tstamp %d\n",cr_indx,log->phy_stats->stat_buffs[cr_indx].tstamp);
	}
}
#endif

static void copy_log_to_local(u32 *muc_phy_stats_addr, struct qtn_phy_stats *host_phy_stats)
{
	memcpy(host_phy_stats, muc_phy_stats_addr, sizeof(struct qtn_phy_stats));
}
			    ...
			    char **argv)
{
	...
	copy_log_to_local((u32*)phy_stats, &host_phy_stats);
	parse_log(&host_phy_stats, ic, opt, show_all_nodes, csv_format, data_only);
 ...
 *
 * Currently they all return 0 to signal Failed to Get Requested Value.
 */
int qdrv_muc_get_noise(struct qdrv_mac *mac, const struct ieee80211com *ic)
{
	...
	if (address_current_log)
		return address_current_log->rx_phy_stats.hw_noise;
...
int qdrv_muc_stats_rssi_get(const struct ieee80211_node *ni, int smoothed, int type)
{
	int mu = STATS_SU;

	if (smoothed)
		return ni->ni_shared_stats->rx[mu].rssi_dbm_smoothed[type];
	else
		return ni->ni_shared_stats->rx[mu].last_rssi_dbm[type];
}

int qdrv_muc_stats_rssi(const struct ieee80211_node *ni)
{
	return qdrv_muc_stats_rssi_get(ni, 0, QTN_NODE_STATS_RSSI_AVE);
}

int qdrv_muc_stats_smoothed_rssi(const struct ieee80211_node *ni)
{
	return qdrv_muc_stats_rssi_get(ni, 1, QTN_NODE_STATS_RSSI_AVE);
}
...
int qdrv_muc_stats_rxtx_phy_rate(const struct ieee80211_node *ni, const int is_rx,
		uint8_t *nss, uint8_t *mcs, uint8_t *mcs_mode, uint8_t *bw, uint8_t *sgi, u_int32_t *phy_rate)
{
	...
	if (mcs_mode)
		*mcs_mode = MS(last_mcs, QTN_PHY_STATS_MCS_MODE);
	if (phy_rate) {
		*phy_rate = MS(last_mcs, QTN_PHY_STATS_MCS_PHYRATE);
		/*
		 * In case of legacy clients(11a/b/g), MAC hardware will not update receive mcs index
		 * value in rxvector and hence encoded last_mcs value will contain 0 as mcs index.
		 * So update mcs index value with legacy phy rate table index similar way done in
		 * tx path
		 */
		if (is_rx && mcs && IEEE80211_WIFI_MODE_LEGACY(ni->ni_wifi_mode)) {
			int i;
			for (i = 0; i < ARRAY_SIZE(qdrv_legacy_rates); i++) {
				if (*phy_rate == qdrv_legacy_rates[i] ) {
					*mcs = i;
					break;
				}
			}
			if (i == ARRAY_SIZE(qdrv_legacy_rates))
				*mcs = 0;
		}
	}
	if (bw)
		*bw = MS(last_mcs, QTN_PHY_STATS_MCS_BW);
				...
				enum qdrv_muc_stats_display_choice missing_type)
{
	...
	if (missing_type == QDRV_MUC_STATS_SHOW_RSSI) {
		...
		ps->last_rssi = prev_rx_stats->last_rssi_all;
	} else {
		prev_stats = qtn_muc_stats_get_addr_latest_stats(mac,
				...
				uint8_t all_stats)
{
	...
	ps->tx_retries	= tx_stats->num_swretries;
	...
	ps->last_tx_pkt_timestamp = node_txstats->last_pkt_timestamp;
	ps->last_rx_pkt_timestamp = node_rxstats->last_pkt_timestamp;
...
u_int32_t qdrv_muc_stats_tx_accum_retries(const struct ieee80211_node *ni)
{
	int mu = STATS_SU;
	return ni->ni_shared_stats->tx[mu].tx_accum_retries;
}
				...
				struct qtn_exp_cca_stats *cs)
{
	...
	cs->cca_tx = mac->cca_stats->cca_try_cnt ?
		(100 * (mac->cca_stats->cca_tx_cnt)
			/ mac->cca_stats->cca_try_cnt) : 0;
	if (cs->cca_tx > cs->cca_trfc)
		cs->cca_tx = cs->cca_trfc;
	cs->cca_rx = mac->cca_stats->cca_trfc ? mac->cca_stats->cca_trfc - cs->cca_tx : 0;
...
int qdrv_muc_get_last_cca_trfc(struct qdrv_mac *mac)
{
	if (!mac || !mac->cca_stats)
		return -EFAULT;

	return mac->cca_stats->cca_trfc;
}

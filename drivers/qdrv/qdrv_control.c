...
static int qdrv_command_set_dfs_available_channel(struct device *dev, int argc, char *argv[]);
static int qdrv_command_csi_est(struct device *dev, int argc, char *argv[]);
...
static dbg_module_table_t dbg_module_name_entry[] =
{
	{DBG_LM_QDRV, "qdrv"},
	{DBG_LM_QPCIE, "pcie"},
	{DBG_LM_QRADAR, "radar"},
	{DBG_LM_QBOOTCFG, "bootcfg"},
	{DBG_LM_QADM, "adm"},
	{DBG_LM_QWLAN, "wlan"},
	{DBG_LM_QMACFW, "macfw"}
};
...
static int qdrv_command_send_addba(struct device *dev, int argc, char *argv[]);
static int qdrv_command_send_opmode(struct device *dev, int argc, char *argv[]);
...
#endif
	{ "mu",         qdrv_command_mu },
	{ "send_addba", qdrv_command_send_addba },
	{ "send_opmode", qdrv_command_send_opmode },
	{ "rf_regdump",  qdrv_command_rf_reg_dump },
	{ "dfs_available_channel",	qdrv_command_set_dfs_available_channel },
	{ "csi_est",	qdrv_command_csi_est },
};
...
s_param_table[] =
{
	...
	{
		"cred_cust", P_FL_TYPE_STRING, NULL,
		offsetof(struct qdrv_cb, cred_cust), membersizeof(struct qdrv_cb, cred_cust)
	},
	{
		"cred_plat", P_FL_TYPE_STRING, NULL,
		offsetof(struct qdrv_cb, cred_plat), membersizeof(struct qdrv_cb, cred_plat)
	},
	{
		"cred_feat", P_FL_TYPE_STRING, NULL,
		offsetof(struct qdrv_cb, cred_feat), membersizeof(struct qdrv_cb, cred_feat)
	},
	{
		"cred_sign", P_FL_TYPE_STRING, NULL,
		offsetof(struct qdrv_cb, cred_sign), membersizeof(struct qdrv_cb, cred_sign)
	},
};
...
static struct qspdia_conf g_qspdia_conf[QSPDIA_NODES_NUM];
...
static int qdrv_is_mem_read_safe(u32 addr_begin, u32 addr_end)
{
	struct qdrv_reg_region {
		u16 reset_bit;
		u32 begin;
		u32 end;
	} reg_region[] = {
		{
			.reset_bit = RUBY_SYS_CTL_RESET_BB,
			.begin = RUBY_BB_BASE_ADDR,
			.end = TOPAZ_BB_END_ADDR + 1,
		},
		{
			.reset_bit = RUBY_SYS_CTL_RESET_PCIE,
			.begin = RUBY_PCIE_REG_BASE,
			.end = TOPAZ_PCIE_END_ADDR + 1,
		},
		{
			.reset_bit = RUBY_SYS_CTL_RESET_ENET0,
			.begin = RUBY_ENET0_BASE_ADDR,
			.end = TOPAZ_ENET0_END_ADDR + 1,
		},
		{
			.reset_bit = RUBY_SYS_CTL_RESET_ENET1,
			.begin = RUBY_ENET1_BASE_ADDR,
			.end = TOPAZ_ENET1_END_ADDR + 1,
		}
	};

	int i;

	unsigned long *reset_base = ioremap_nocache(RUBY_SYS_CTL_CPU_VEC, sizeof(*reset_base));
	unsigned long reset_vector = 0;

	if (!reset_base)
		return 0;

	reset_vector = *reset_base;
	iounmap(reset_base);

	for (i = 0; i < ARRAY_SIZE(reg_region); i++) {
		if (addr_end <= reg_region[i].begin || addr_begin >= reg_region[i].end)
			continue;

		if ((reset_vector & reg_region[i].reset_bit) == 0)
			return 0;

		if (addr_begin >= reg_region[i].begin && addr_end <= reg_region[i].end)
			break;
	}
...
static void qdrv_show_memory_safe(struct seq_file *s, void *data, size_t num)
{
	struct qdrv_cb *qcb = data;
	u32 addr_begin = qcb->read_addr;
	u32 addr_end = qcb->read_addr + sizeof(unsigned long) * qcb->values_per_line;

	if (!qdrv_is_mem_read_safe(addr_begin, addr_end))
		seq_printf(s, "%08x: unsafe addr\n", qcb->read_addr);
	else
		qdrv_show_memory(s, data, num);
}
...
static int qdrv_command_stop_one_vap(struct device *dev, struct qdrv_mac *mac, const char *vapname)
{
	...
	if (qdrv_command_start_block(dev, "VAP delete", &check_vap_deleted, (void *)wlanid)) {
		/* force detach as if we received "IOCTL_HLINK_DEVDETACH" from MuC */
		if (qdrv_vap_exit_muc_done(mac, vdev) >= 0) {
			struct qdrv_cb *qcb = dev_get_drvdata(dev);
			uint32_t resource = QDRV_RESOURCE_VAP(wlanid);
...
static struct reg_range qdrv_hw_regs[] = {
	{RUBY_SYS_CTL_BASE_ADDR,		RUBY_SYS_CTL_END_ADDR,		"System Control regs"},
	/*{0xE1007FFC,				0xE1038FFF,			"switch regs"},*/
	{TOPAZ_HBM_BASE_ADDR,			TOPAZ_HBM_END_ADDR,		"HBM regs"},
	/*{0xE2000000,				0xE200000F,			"SPI0 regs"},*/
	/*{0xE3100000,				0xE310009F,			"AHB regs"},*/
	{TOPAZ_PACKET_MEMORY_BASE_ADDR,		TOPAZ_PACKET_MEMORY_END_ADDR,	"Packet memory"},
	{TOPAZ_TCM_BASE_ADDR,			TOPAZ_TCM_END_ADDR,		"TCM"},
	{TOPAZ_GLOBAL_CONTROL_BASE_ADDR,	TOPAZ_GLOBAL_CONTROL_END_ADDR,	"Global control"},
	{TOPAZ_TX_FRAME_PROC_BASE_ADDR,		TOPAZ_TX_FRAME_PROC_END_ADDR,
		"Global control regs(1): TX frame processor regs"},
	{TOPAZ_RX_FRAME_PROC_BASE_ADDR,		TOPAZ_RX_FRAME_PROC_END_ADDR,
		"Global control regs(2): RX frame processor regs"},
	{TOPAZ_SHARED_FRAME_PROC_BASE_ADDR,	TOPAZ_SHARED_FRAME_PROC_END_ADDR,
		"Global control regs(3): shared frame processor regs"},
	{TOPAZ_GLOBAL_CONTROL_REGS_BASE_ADDR,	TOPAZ_GLOBAL_CONTROL_REGS_END_ADDR,
		"Global control regs(4)"},
	{TOPAZ_BB_1_BASE_ADDR,			TOPAZ_BB_1_END_ADDR,		"BB 1"},
	{TOPAZ_BB_2_BASE_ADDR,			TOPAZ_BB_2_END_ADDR,		"BB 2"},
	{TOPAZ_BB_3_BASE_ADDR,			TOPAZ_BB_3_END_ADDR,		"BB 3"},
	{TOPAZ_BB_4_BASE_ADDR,			TOPAZ_BB_4_END_ADDR,		"BB 4"},
	{TOPAZ_BB_5_BASE_ADDR,			TOPAZ_BB_5_END_ADDR,		"BB 5"},
	{TOPAZ_BB_6_BASE_ADDR,			TOPAZ_BB_6_END_ADDR,		"BB 6"},
	{TOPAZ_BB_7_BASE_ADDR,			TOPAZ_BB_7_END_ADDR,		"BB 7"},
	{TOPAZ_BB_8_BASE_ADDR,			TOPAZ_BB_8_END_ADDR,		"BB 8"},
	{TOPAZ_BB_9_BASE_ADDR,			TOPAZ_BB_9_END_ADDR,		"BB 9"},
	{TOPAZ_BB_10_BASE_ADDR,			TOPAZ_BB_10_END_ADDR,		"BB 10"},
	{TOPAZ_BB_11_BASE_ADDR,			TOPAZ_BB_11_END_ADDR,		"BB 11"},
	{TOPAZ_BB_12_BASE_ADDR,			TOPAZ_BB_12_END_ADDR,		"BB 12"},
	{TOPAZ_BB_13_BASE_ADDR,			TOPAZ_BB_13_END_ADDR,		"BB 13"},
	{TOPAZ_BB_14_BASE_ADDR,			TOPAZ_BB_14_END_ADDR,		"BB 14"},
	{TOPAZ_BB_15_BASE_ADDR,			TOPAZ_BB_15_END_ADDR,		"BB 15"},
	{TOPAZ_BB_16_BASE_ADDR,			TOPAZ_BB_16_END_ADDR,		"BB 16"},
	{TOPAZ_BB_17_BASE_ADDR,			TOPAZ_BB_17_END_ADDR,		"BB 17"},
	{TOPAZ_BB_18_BASE_ADDR,			TOPAZ_BB_18_END_ADDR,		"BB 18"},
	{TOPAZ_BB_19_BASE_ADDR,			TOPAZ_BB_19_END_ADDR,		"BB 19"},
	{TOPAZ_BB_20_BASE_ADDR,			TOPAZ_BB_20_END_ADDR,		"BB 20"},
	{TOPAZ_BB_21_BASE_ADDR,			TOPAZ_BB_21_END_ADDR,		"BB 21"},
	{TOPAZ_BB_22_BASE_ADDR,			TOPAZ_BB_22_END_ADDR,		"BB 22"},
	{TOPAZ_BB_23_BASE_ADDR,			TOPAZ_BB_23_END_ADDR,		"BB 23"},
	{TOPAZ_BB_24_BASE_ADDR,			TOPAZ_BB_24_END_ADDR,		"BB 24"},
	{TOPAZ_BB_25_BASE_ADDR,			TOPAZ_BB_25_END_ADDR,		"BB 25"},
	{TOPAZ_BB_26_BASE_ADDR,			TOPAZ_BB_26_END_ADDR,		"BB 26"},
	{TOPAZ_EMAC1_BASE_ADDR,			TOPAZ_EMAC1_END_ADDR,		"EMAC1 Control regs"},
	/*{0xE9000000,				0xE9000bFF,			"PCIE regs"},*/
	/*{0xEA000000,				0xEA0003FF,			"DMAC Conrol regs"},*/
	{TOPAZ_EMAC0_BASE_ADDR,			TOPAZ_EMAC0_END_ADDR,		"EMAC0 Control regs"},
	{TOPAZ_UART1_CTRL_BASE_ADDR,		TOPAZ_UART1_CTRL_END_ADDR,	"UART1 Control regs"},
	{TOPAZ_GPIO_BASE_ADDR,			TOPAZ_GPIO_END_ADDR,		"GPIO regs"},
	{TOPAZ_SPI1_BASE_ADDR,			TOPAZ_SPI1_END_ADDR,		"SPI1 regs"},
	{TOPAZ_TIMER_CTRL_BASE_ADDR,		TOPAZ_TIMER_CTRL_END_ADDR,	"Timer Control regs"},
	{TOPAZ_UART2_CTRL_BASE_ADDR,		TOPAZ_UART2_CTRL_END_ADDR,	"UART2 Control regs"},
	{TOPAZ_DDR_BASE_ADDR,			TOPAZ_DDR_END_ADDR,		"DDR Control regs"},
					...
					unsigned int param, unsigned int val)
{
	if (param == QDRV_CMD_MUC_MEMDBG_MSG_SEND) {
		...
		qdrv_pktlogger_start_or_stop(qw, "sysmsg", mac->params.mucdbg_netdbg, 0);
	}
...
#define QDRV_HWREG_OVERRUN_MASK		(0xFF)
#define QDRV_HWREG_OVERRUN_DELAY_MSECS	(500)
static void qdrv_dump_hwreg_printk_output(struct qdrv_hwreg_print_data *sets)
{
	...
	/*
	 * print buffer overruns at ~400 registers - enable delay
	 * to prevent overrun errors on large blocks
	 */
	if (len > QDRV_HWREG_OVERRUN_MASK)
		overrun_delay = 1;
	...
	for (i = 1; i <= len; i++) {
		...
		if (overrun_delay && ((i & QDRV_HWREG_OVERRUN_MASK) == 0))
			msleep(QDRV_HWREG_OVERRUN_DELAY_MSECS);
	}
}
...
static int qdrv_command_set_dfs_available_channel(struct device *dev, int argc, char *argv[])
{
	uint32_t dfs_available_channel;
	if (argc != 2)
		goto usage;

	dfs_available_channel = _atoi(argv[1]);
	qdrv_set_dfs_available_channel(dfs_available_channel);

	return 0;
usage:
	printk("usage: %s (<DFS available channel list>)\n", argv[0]);
	return 0;
}
...
static int qdrv_command_read(struct device *dev, int argc, char *argv[])
{
	...
	if(strcmp(argv[1], "addr_safe") == 0)
		safe_read = 1;

	if(safe_read || strcmp(argv[1], "addr") == 0) {
		if (sscanf(argv[2], "%x", &addr) != 1)
			goto error;
	...
	if (safe_read)
		qdrv_control_set_show(qdrv_show_memory_safe, (void *) qcb, num, 1);
	else
 ...
 * buffer for too long.
 */
void qdrv_control_sysmsg_send(void *data, char *sysmsg, u_int32_t text_len, int force_flush_now)
{
...
#define QDRV_CMD_SYSMSG_PREF_LEN	25
	...
	if ((msgbuf != NULL) &&
			((force_flush_now != 0) || ((msg_len + line_len + 1) > (QDRV_NETDEBUG_SYSMSG_LENGTH - 20)))) {
		int udp_len = sizeof(msgbuf->ndb_hdr) + msg_len;
	...
	if (mac->params.mucdbg_netdbg) {
		...
		snprintf(msgbuf->msg + msg_len, line_len + 1, fmt, U64_HIGH32(tsf), U64_LOW32(tsf), sysmsg),
		msg_len += line_len;
	} else if (qw->pktlogger.flag & BIT(QDRV_NETDEBUG_TYPE_SYSMSG)) {
		snprintf(msgbuf->msg + msg_len, text_len + 1, "%s", sysmsg);
		msg_len += text_len;
	}
	BUG_ON(strlen(msgbuf->msg) + 1 > QDRV_NETDEBUG_SYSMSG_LENGTH);
	spin_unlock(&qw->pktlogger.sysmsg_lock);
}
...
static int qdrv_command_memdebug(struct device *dev, int argc, char *argv[])
{
	...
	if (strncmp(argv[2], "add", 3) == 0) {
		...
		if (qw->pktlogger.mem_wps_read_buf_size < (wp->size * sizeof(u32))
			|| (qw->pktlogger.mem_wps_read_buf == NULL)) {
			if (qw->pktlogger.mem_wps_read_buf) {
				kfree(qw->pktlogger.mem_wps_read_buf);
			}
			qw->pktlogger.mem_wps_read_buf_size = wp->size * sizeof(u32);
			qw->pktlogger.mem_wps_read_buf =
				kmalloc(qw->pktlogger.mem_wps_read_buf_size, GFP_KERNEL);
			if (qw->pktlogger.mem_wps_read_buf == NULL) {
				DBGPRINTF_E("unable to alloc buf %s line:%d\n", __FILE__, __LINE__);
				return -1;
			}
		}
...
{
	struct qdrv_cb *qcb;
	uint16_t *buf = (uint16_t *) (ARC_SRAM_SAFE_AREA_BASE);
	...
	if (qdrv_crash_log == NULL) {
		...
		if (*buf == HEADER_CORE_DUMP) {
			...
			if (len > ARC_SRAM_SAFE_AREA_SIZE) {
	...
	if (qcb->resources == 0) {
		...
		crumbs = (struct ruby_crumbs *)p_uncached;
		/*
		 * only clear 'classic' crumbs segment, to avoid overwriting preserved segment (if supported by uboot),
		 * and to avoid accessing at all the (non-defined) 'preserved' segment if uboot is not knowing about it
		*/
		memset(((void*)crumbs) + RUBY_CRUMBS_PRESERVED_SIZE, 0, sizeof(*crumbs) - RUBY_CRUMBS_PRESERVED_SIZE);
		iounmap(p_uncached);
...
static void
qdrv_control_get_3addr_br_config(struct seq_file *s, void *data, u32 num)
{
	struct qdrv_br *qbr = (struct qdrv_br *)data;
	uint32_t val;

	val = !!(qbr->flags & QDRV_BR_DHCP_CHADDR_PROXY);
...
static int qdrv_command_bridge(struct device *dev, int argc, char *argv[])
{
	...
	if (strcmp(argv[2], "enable") == 0) {
		if (QDRV_FLAG_3ADDR_BRIDGE_ENABLED()) {
			...
			qdrv_br_create(&qw->bridge_table, qw->ic.ic_myaddr);
	...
	if (strcmp(argv[2], "showmacs") == 0) {
		qdrv_br_show(&qw->bridge_table);
	} else if (strcmp(argv[2], "clear") == 0) {
		qdrv_br_clear(&qw->bridge_table);
	} else if (strcmp(argv[2], "dhcp_chaddr_enable") == 0) {
		qdrv_br_flags_set(&qw->bridge_table, QDRV_BR_DHCP_CHADDR_PROXY, 1);
	} else if (strcmp(argv[2], "dhcp_chaddr_disable") == 0) {
		qdrv_br_flags_set(&qw->bridge_table, QDRV_BR_DHCP_CHADDR_PROXY, 0);
	} else if (strcmp(argv[2], "dhcp_chaddr_get") == 0) {
		 qdrv_control_set_show(qdrv_control_get_3addr_br_config, &qw->bridge_table, 1, 1);
	} else {
		goto usage;
	}
...
usage:
	printk("Usage: bridge 0 {showmacs | enable | disable | clear | \
		dhcp_chaddr_enable | dhcp_chaddr_disable}\n");
	return -1;
}
...
static int qdrv_control_vlan_enable(struct qdrv_mac *mac, int enable)
{
	struct qdrv_wlan *qw;
	...
	if (vlan_enabled != enable) {
		vlan_enabled = enable;
#if !defined(CONFIG_TOPAZ_PCIE_HOST) && !defined(CONFIG_TOPAZ_PCIE_TARGET)
		ret = topaz_emac_vlan_enable(enable);
		if (ret) {
...
int qdrv_control_set_vlan_enable(struct qdrv_mac *mac, const char *cmd)
{
	...
	if (strcmp(cmd, "enable") == 0) {
		ret = qdrv_control_vlan_enable(mac, 1);
	} else if (strcmp(cmd, "disable") == 0) {
		ret = qdrv_control_vlan_enable(mac, 0);
		if (ret == 0)
			switch_vlan_reset();
	} else {
		ret = -EINVAL;
...
static int qdrv_control_vlan_drop_stag(struct qdrv_mac *mac, int drop_stag)
{
	struct qdrv_wlan *qw;

	if (vlan_drop_stag != drop_stag) {
		vlan_drop_stag = drop_stag;
		qw = qdrv_mac_get_wlan(mac);
		qdrv_wlan_vlan_drop_stag(&qw->ic, drop_stag);
	}

	return 0;
}

static int qdrv_control_set_vlan_drop_stag(struct qdrv_mac *mac, const char *cmd)
{
	int ret;
	...
	if (strcmp(cmd, "drop_stag") == 0)
		ret = qdrv_control_vlan_drop_stag(mac, 1);
	else if (strcmp(cmd, "undrop_stag") == 0)
		ret = qdrv_control_vlan_drop_stag(mac, 0);
	else
		ret = -EINVAL;
			...
			int add)
{
	if (mode == QVLAN_MODE_DYNAMIC) {
		...
		if (add == 1)
			return qdrv_control_vlan_enable(mac, 1);
		return 0;
	}
			...
			int add)
{
	...
	} else if (vlanid == QVLAN_VID_ALL || qtn_vlan_is_pvid(vdev, vlanid)) {
...
static int qdrv_control_vlan_config(struct qdrv_mac *mac, int argc, char *argv[])
{
	...
	if (argc == 3) {
		...
		else if (!strcmp(argv[2], "drop_stag") || !strcmp(argv[2], "undrop_stag"))
			return qdrv_control_set_vlan_drop_stag(mac, argv[2]);
		else
			return -EINVAL;
	}
...
static int qdrv_control_set_power_table_checksum(struct qdrv_cb *qcb, char *fname,
							char *checksum)
{
	...
	if (qcb->power_table_ctrl.checksum_list_locked) {
		pr_err("QDRV: power table checksum list has been locked\n");
		return -EINVAL;
	}

	if (strlen(fname) > QDRV_POWER_TABLE_FNAME_MAX_LEN) {
		pr_err("QDRV: power table filename is too long\n");
		return -EINVAL;
	}

	if (strlen(checksum) != QDRV_POWER_TABLE_CHECKSUM_LEN) {
		pr_err("QDRV: power table checksum length is invalid\n");
		return -EINVAL;
	}
	...
	while (p_entry) {
		if (strcmp(p_entry->fname, fname) == 0) {
			pr_err("QDRV: power table checksum for %s exists\n", fname);
			return -EINVAL;
		}
		if (p_entry->next) {
	...
	if (!n_entry) {
		pr_err("QDRV: set power table checksum malloc failed\n");
		return -ENOMEM;
	}
...
static int qdrv_control_set_node_rate_stats(struct qdrv_cb *qcb, int argc, char *argv[])
{
	struct qdrv_wlan *qw = NULL;
	struct ieee80211com *ic = NULL;
	struct ieee80211vap *vap = NULL;
	const char *usage = "Usage: node_rate_stats <unit_id> <node_mac_addr> { tx | rx } { 0 | 1 }\n";
	const char *macid_str;
	const char *mac_str;
	const char *dir_str;
	uint32_t mac_id;
	...
	mac_str = argv[2];
	dir_str = argv[3];
	is_add = STR2L(argv[4]) ? 1 : 0;

	mac_id = STR2L(macid_str);
	if (mac_id >= MAC_UNITS) {
		DBGPRINTF_E("Invalid WMAC id %u\n", mac_id);
		return -EINVAL;
	}

	if (strcmp("tx", dir_str) == 0) {
		is_tx = 1;
	} else if(strcmp("rx", dir_str) == 0) {
		is_tx = 0;
	} else {
		DBGPRINTF_E("%s", usage);
		return -EINVAL;
	...
	if (qdrv_parse_mac(mac_str, mac) < 0) {
		DBGPRINTF_E("%s", usage);
		return -EINVAL;
	}

	if (is_tx)
		state |= QTN_NODE_STAT_SLOT_IS_TX;

	if (is_add)
		state |= QTN_NODE_STAT_SLOT_ATTACHED;

	if (qcb == NULL) {
		DBGPRINTF_E("qcb is NULL\n");
		return -EINVAL;
	}

	qw = (struct qdrv_wlan *)qcb->macs[mac_id].data;
	if (qw == NULL) {
		DBGPRINTF_E("qw is NULL\n");
		return -EINVAL;
	}
	...
	if (vap == NULL) {
		DBGPRINTF_E("vap is NULL\n");
		return -EINVAL;
	}

	ieee80211_param_to_qdrv(vap, IEEE80211_PARAM_NODE_RATE_STATS, state, mac, IEEE80211_ADDR_LEN);

	return 0;
}

static char g_l2_ext_filter_dev[IFNAMSIZ];
static int qdrv_control_set_l2_ext_filter_port(int argc, char *argv[])
{
	struct net_device *vap_dev;
	struct net_device *ext_dev;
	struct ieee80211vap *vap;
	...
	vap_dev = dev_get_by_name(&init_net, argv[0]);
	if (!vap_dev)
		return -ENODEV;
	vap = netdev_priv(vap_dev);
	dev_put(vap_dev);

	ext_dev = dev_get_by_name(&init_net, argv[1]);
	if (!ext_dev)
		return -ENODEV;

	ic = vap->iv_ic;
	if (ic->ic_set_l2_ext_filter_port) {
		ret = ic->ic_set_l2_ext_filter_port(vap, ext_dev->if_port);
		if (ret < 0)
			goto out;
	}

	ret = br_set_l2_ext_filter_port(ext_dev);
	if (ret < 0)
		goto out;

	strncpy(g_l2_ext_filter_dev, ext_dev->name, IFNAMSIZ - 1);
out:
	dev_put(ext_dev);
	return ret;
}

static int qdrv_command_val_required(const char *name)
{
...
static int qdrv_command_set(struct device *dev, int argc, char *argv[])
{
	...
	value = argv[2];
	if (value == NULL && qdrv_command_val_required(name)) {
	...
	if (strcmp(name, "power_table_checksum") == 0) {
		...
		if (argc != 4) {
			pr_err("QDRV: set power_table_checksum command requires 2 arguments\n");
			return -EINVAL;
		}

		checksum = argv[3];

		return qdrv_control_set_power_table_checksum(qcb, fname, checksum);
	}
	...
	if (strcmp(name, "opt_txpower_path") == 0) {
		if (strcmp(value, "0") == 0)
			value = NULL;
		if (qdrv_soc_set_tx_pwr_file_path(value) < 0)
			return -EINVAL;
		else
			return 0;
	}

	if (strcmp(name, "node_rate_stats") == 0)
		return qdrv_control_set_node_rate_stats(qcb, argc - 1, &argv[1]);
	...
	if (strcmp(name, "enable_emac_sdp") == 0) {
		if (argc != 3)
			return -EINVAL;

		int enable_sdp;

		enable_sdp = simple_strtol(argv[2], NULL, 10);
		if (enable_sdp < 0)
			return -EINVAL;

		return topaz_emac_sdp_enable(enable_sdp);
	}
#endif

	if (strcmp(name, "l2_ext_filter_port") == 0)
		return qdrv_control_set_l2_ext_filter_port(argc - 2, &argv[2]);
	...
	if(strcmp(name, "test1") == 0) {
		...
		if (argc >= 8) {
			...
			rx_gain_params.cca_prim_dbm_scs_off = (int16_t)STR2L(argv[6]);
			rx_gain_params.cca_prim_dbm_scs_on = (int16_t)STR2L(argv[7]);
			rx_gain_params.cca2_dbm_scs_off = (int16_t)STR2L(argv[8]);
			rx_gain_params.cca2_dbm_scs_on = (int16_t)STR2L(argv[9]);
			rx_gain_params.cca40_dbm_scs_off = (int16_t)STR2L(argv[10]);
			rx_gain_params.cca40_dbm_scs_on = (int16_t)STR2L(argv[11]);
...
#define QDRV_SWVER_STR_MAX	20
	...
	QDRV_SHOW_PRINT(s, "Recommended U-Boot:    %s\n", RUBY_UBOOT_VERSION);
}
...
qdrv_show_dsp_stats(struct seq_file *s, void *data, u32 num)
{
	...
	seq_printf(s, "%-*s %-3s %-5s %-4s %-4s %-4s %-8s\n", QDRV_UC_STATS_DESC_LEN,
		"dsp_mu_grcand", "grp", "valid", "aid0", "aid1", "rank", "last_upd_time");

	for (i = 0; i < ARRAY_SIZE(txbf_mbox->dsp_stats.dsp_mu_grcand_valid); i++) {
		seq_printf(s, "%-*s %-3u %-5u %-4u %-4u %-4d %08lu\n", QDRV_UC_STATS_DESC_LEN, "dsp_mu_grcand",
				(unsigned)i,
				txbf_mbox->dsp_stats.dsp_mu_grcand_valid[i],
				txbf_mbox->dsp_stats.dsp_mu_grcand_aid0[i],
				txbf_mbox->dsp_stats.dsp_mu_grcand_aid1[i],
				txbf_mbox->dsp_stats.dsp_mu_grcand_rank[i],
				(long unsigned int)txbf_mbox->dsp_stats.dsp_mu_grcand_last_upd_time[i]);
	}
...
static void
qdrv_show_uc_rates(struct seq_file *s, struct muc_stat_rates *uc_rates)
{
	int i;
	uint8_t nss;

	/* 11n rates */
	for (i = 0; i < ARRAY_SIZE(uc_rates->mcs_11n); i++) {
		if ((i % 10) == 0) {
			seq_printf(s, "\nmcs_11n ");
		}
		seq_printf(s, " %2d:%-9u", i, uc_rates->mcs_11n[i]);
	}

	/* 11ac rates */
	for (nss = 0; nss <= (IEEE80211_AC_MCS_NSS_MAX - 1); ++nss) {
		seq_printf(s, "\nmcs_11ac");
		for (i = 0; i < ARRAY_SIZE(uc_rates->mcs_11ac_su[nss]); i++) {
			seq_printf(s, " %2d:%-9u",
					nss * IEEE80211_AC_MCS_MAX + i,
					uc_rates->mcs_11ac_su[nss][i]);
		}
	}
	...
	for (nss = 0; nss <= (IEEE80211_AC_MCS_NSS_MAX - 1); ++nss) {
		seq_printf(s, "\nmu      ");
		for (i = 0; i < ARRAY_SIZE(uc_rates->mcs_11ac_mu[nss]); i++) {
			seq_printf(s, " %2d:%-9u",
					nss * IEEE80211_AC_MCS_MAX + i,
					uc_rates->mcs_11ac_mu[nss][i]);
		}
	}
	seq_printf(s, "\n");
}
...
qdrv_show_uc_rx_stats(struct seq_file *s, void *data, u32 num)
{
	struct qdrv_mac *mac = (struct qdrv_mac *) data;
	struct qdrv_wlan *qw;
	struct muc_stat_rates *uc_rx_rates;
	...
	uc_rx_rates = (struct muc_stat_rates *)qw->pktlogger.stats_uc_rx_rate_ptr;
	/* Beamforming */
	uc_rx_bf_stats = (struct muc_rx_bf_stats *)qw->pktlogger.stats_uc_rx_bf_ptr;

	if (!rx_stats || !uc_rx_rates || !uc_rx_bf_stats) {
		return;
	}
	...
	seq_printf(s, "%-*s %u\n", QDRV_UC_STATS_DESC_LEN, "BF rx_bad_bf_act_token",
		uc_rx_bf_stats->rx_bad_bf_act_token);

	/* display RX rates */
	if (uc_rx_rates)
		qdrv_show_uc_rates(s, uc_rx_rates);
}

static void
qdrv_show_muc_sreset_stats(struct seq_file *s, void *data, u32 num)
{
	struct qdrv_mac *mac = (struct qdrv_mac *) data;
	struct qdrv_wlan *qw;
	int i;
	const u32 *rx_stats;
	const int rx_stats_index[MUC_SRESET_STATS_NUM] = { MUC_STATS_RX_TYPE_SRESET_BEGIN,
	...
	for (i = 0; i < ARRAY_SIZE(rx_stats_index); i++)
		seq_printf(s, "%u ", rx_stats[rx_stats_index[i]]);
	seq_printf(s, "\n");
}

static void
qdrv_show_uc_pta_stats(struct seq_file *s, void *data, u32 num)
{
	struct qdrv_mac *mac = (struct qdrv_mac *) data;
	...
	const u32* pta_stats;
	int i;
	const char *pta_stats_names[] = MUC_PTA_STATS_NAMES_TABLE;
	...
	pta_stats = ioremap_nocache(muc_to_lhost((u32)iw_stats_log->pta_muc_stats),
					sizeof(struct muc_pta_stats));
	if (!pta_stats)
		return;

	for (i = 0; i < ARRAY_SIZE(pta_stats_names); i++) {
		seq_printf(s, "%-*s %u\n", QDRV_UC_STATS_DESC_LEN, pta_stats_names[i], pta_stats[i]);
	}
	seq_printf(s, "\n");
	iounmap(pta_stats);
}
...
static void
qdrv_show_node_rate_stats(struct seq_file *s, void *data, u32 num)
{
	struct qdrv_mac *mac = (struct qdrv_mac *) data;
	struct qdrv_wlan *qw = qdrv_control_wlan_get(mac);
	struct qtn_stats_log *iw_stats_log;
	...
	if (qw->stats_uc_rates_per_node == NULL) {
		qw->stats_uc_rates_per_node = ioremap_nocache(
					muc_to_lhost((u32)iw_stats_log->rates_per_node),
					sizeof(struct qtn_node_rate_stats)*QTN_NODE_STAT_RATE_SLOTS);
	...
	for (i = 0; i < QTN_NODE_STAT_RATE_SLOTS; i++) {
		uc_rates_per_node = &qw->stats_uc_rates_per_node[i];

		/* display Tx rates */
		if (uc_rates_per_node->slot_flags & QTN_NODE_STAT_SLOT_ALLOCATED) {
			seq_printf(s, "%s rate stats for %pM (%s)\n",
				uc_rates_per_node->slot_flags & QTN_NODE_STAT_SLOT_IS_TX ? \
					"Tx" : "Rx",
				uc_rates_per_node->macaddr,
				uc_rates_per_node->slot_flags & QTN_NODE_STAT_SLOT_ATTACHED ? \
					"attached" : "detached");
			qdrv_show_uc_rates(s, &uc_rates_per_node->rates);
			seq_printf(s, "\n");
		}
	}
}
...
static void qdrv_control_show_vapname(struct seq_file *s, void *data, uint32_t num)
{
	struct qdrv_wlan *qw = (struct qdrv_wlan *)data;
	struct ieee80211com *ic = &qw->ic;
	struct ieee80211vap *vap;
	struct qdrv_vap *qv;
	unsigned int vapid = num - 1;

	IEEE80211_LOCK_BH(ic);
	TAILQ_FOREACH(vap, &ic->ic_vaps, iv_next) {
		qv = container_of(vap, struct qdrv_vap, iv);
		if (vapid == QDRV_WLANID_FROM_DEVID(qv->devid)) {
			seq_printf(s, "%s", vap->iv_dev->name);
			seq_putc(s, '\0');
			break;
		}
	}
...
static void
qdrv_show_opt_txpower_path(struct seq_file *s, void *data, u32 num)
{
	seq_printf( s, "%s\n", (char *) data);
}

static void
qdrv_show_l2_ext_filter_port(struct seq_file *s, void *data, u32 num)
{
	seq_printf(s, "%s", (char *)data);
}
...
static int qdrv_command_get(struct device *dev, int argc, char *argv[])
{
	...
	mac = qdrv_control_mac_get(dev, argv[1]);
	if (!mac) {
		DBGPRINTF(DBG_LL_ALL, QDRV_LF_TRACE, "mac %s not found\n", argv[1]);
		return -1;
	}
	...
	if(strcmp(argv[2], "stats") == 0) {
		...
		if (sp->ext_lna_gain != QTN_EXT_LNA_GAIN_INVALID)
			printk("ext_lna_gain : %d.", sp->ext_lna_gain);
		else
			printk("ext_lna_gain(%d) is invalid.", sp->ext_lna_gain);

		if (sp->ext_lna_bypass_gain != QTN_EXT_LNA_GAIN_INVALID)
			printk("ext_lna_bypass_gain : %d.\n", sp->ext_lna_bypass_gain);
		else
			printk("ext_lna_bypass_gain(%d) is invalid.\n", sp->ext_lna_bypass_gain);

	} else if (strcmp(argv[2], "ext_lna_gain_2g") == 0) {
		shared_params *sp = qtn_mproc_sync_shared_params_get();

		if (!sp)
			return -1;

		if (sp->ext_lna_gain_2g != QTN_EXT_LNA_GAIN_INVALID)
			printk("ext_lna_gain_2g : %d.", sp->ext_lna_gain_2g);
		else
			printk("ext_lna_gain_2g(%d) is invalid.", sp->ext_lna_gain_2g);

		if (sp->ext_lna_bypass_gain_2g != QTN_EXT_LNA_GAIN_INVALID)
			printk("ext_lna_bypass_gain_2g : %d.\n", sp->ext_lna_bypass_gain_2g);
		else
			printk("ext_lna_bypass_gain_2g(%d) is invalid.\n", sp->ext_lna_bypass_gain_2g);
	...
	} else if (strcmp(argv[2], "node_rate_stats") == 0) {
		qdrv_control_set_show(qdrv_show_node_rate_stats, (void *)mac, 1, 1);
	...
	} else if (strcmp(argv[2], "muc_sreset_stats") == 0) {
		qdrv_control_set_show(qdrv_show_muc_sreset_stats, (void *)mac, 1, 1);
	} else if (strcmp(argv[2], "uc_pta_stats") == 0) {
		qdrv_control_set_show(qdrv_show_uc_pta_stats, (void *)mac, 1, 1);
...
#endif
	} else if (strcmp(argv[2], "vapname_by_idx") == 0) {
		int vap_idx;

		if (argc != 4)
			return -1;

		vap_idx = simple_strtol(argv[3], NULL, 10);
		if (vap_idx < 0)
			return -1;

		qdrv_control_set_show(qdrv_control_show_vapname, qw, vap_idx + 1, vap_idx + 1);
	} else if (strcmp(argv[2], "opt_txpower_path") == 0) {
		char *filepath = qdrv_soc_get_tx_pwr_file_path(NULL);
		qdrv_control_set_show(qdrv_show_opt_txpower_path,(void *)filepath, 1, 1);
#if defined(CONFIG_QTN_BSA_SUPPORT)
	} else if (strcmp(argv[2], "show_erw_stats") == 0) {
		qdrv_wlan_get_bsa_erw_stats(qw);
#endif
	} else if (strcmp(argv[2], "l2_ext_filter_port") == 0) {
		qdrv_control_set_show(qdrv_show_l2_ext_filter_port, g_l2_ext_filter_dev, 1, 1);
	} else {
...
static int qdrv_command_dbg(struct device *dev, int argc, char *argv[])
{
	...
	if (strcmp(argv[1], "set") == 0) {
		...
		if (module_id != DBG_LM_QMACFW) {
			unsigned int prev_func = g_dbg_log_func[module_id - 1];

			g_dbg_log_module |= BIT(module_id - 1);
			g_dbg_log_func[module_id - 1] = dbg_func_mask;
			g_dbg_log_level[module_id - 1] = dbg_log_level;

			if (module_id == DBG_LM && ((prev_func ^ dbg_func_mask) &
			    (QDRV_LF_DFS_DONTCAREDOTH | QDRV_LF_DFS_DISALLOWRADARDETECT))) {
				u32 mask = 0;

				if (dbg_func_mask & QDRV_LF_DFS_DONTCAREDOTH)
					mask |= RADAR_DEBUG_DONTCAREDOTH;

				if (dbg_func_mask & QDRV_LF_DFS_DISALLOWRADARDETECT)
					mask |= RADAR_DEBUG_DISALLOWRADARDETECT;

				qdrv_hostlink_radar_set_param(qw, RADAR_PARAM_DEBUG_FLAGS, mask);
			}
		} else {
...
uint8_t qdrv_get_num_macs(void)
{
	return soc_shared_params->lh_num_devices;
}
EXPORT_SYMBOL(qdrv_get_num_macs);

uint16_t qdrv_get_max_ap_assoc_sta(uint8_t mac_id)
{
	if (mac_id >= MAC_UNITS)
		return 0;

	return QTN_ASSOC_LIMIT;
}
EXPORT_SYMBOL(qdrv_get_max_ap_assoc_sta);

uint8_t qdrv_get_num_rx_chains(unsigned int unit)
{
	if (unit >= MAC_UNITS)
		return 0;

	if (g_qcb->macs[unit].data == NULL)
		return 0;

	return qdrv_wlan_get_hw_rx_chains(g_qcb->macs[unit].unit);
}
EXPORT_SYMBOL(qdrv_get_num_rx_chains);

uint8_t qdrv_get_num_tx_chains(unsigned int unit)
{
	if (unit >= MAC_UNITS)
		return 0;
	...
	return qdrv_wlan_get_hw_tx_chains(g_qcb->macs[unit].unit);
}
EXPORT_SYMBOL(qdrv_get_num_tx_chains);

uint32_t qdrv_get_fw_version(void)
{
	return QDRV_BLD_VER;
}
EXPORT_SYMBOL(qdrv_get_fw_version);

void qdrv_get_mac_addr(unsigned int unit, void *addr_buf)
{
	if (unit >= MAC_UNITS)
		return;

	IEEE80211_ADDR_COPY(addr_buf, g_qcb->macs[unit].mac_addr);
}
EXPORT_SYMBOL(qdrv_get_mac_addr);

struct ieee80211com *qdrv_get_ic(unsigned int unit)
{
	struct qdrv_wlan *qw;
	...
	qw = qdrv_control_wlan_get(&g_qcb->macs[unit]);
	if (!qw)
		return NULL;

	return &qw->ic;
}
EXPORT_SYMBOL(qdrv_get_ic);
...
int qdrv_csi_est_enabled(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_qspdia_conf); i++) {
		if (g_qspdia_conf[i].interval != 0 && g_qspdia_conf[i].flags != 0)
			return 1;
	}
...
static void qdrv_csi_est_usage(struct seq_file *s, void *data, u32 num)
{
	seq_puts(s, "Usage:\n");
	seq_puts(s, "    csi_est show\n");
	seq_puts(s, "    csi_est <MAC> add\n");
	seq_puts(s, "    csi_est <MAC> remove\n");
	seq_puts(s, "    csi_est <MAC> mode { data | ndp | none }\n");
	seq_puts(s, "    csi_est <MAC> interval <ms>\n");
	seq_puts(s, "    csi_est <MAC> ng { 0 | 1 | 2 }\n");
	seq_puts(s, "    csi_est <MAC> reorder { 0 | 1 }\n");
	seq_puts(s, "    csi_est <MAC> smooth { none | ramp | en }\n");
}

static void qdrv_csi_est_show(struct seq_file *s, void *data, u32 num)
{
	struct qspdia_conf *csi_est_conf_arr = (struct qspdia_conf *) data;
	int i;

	for (i = 0; i < ARRAY_SIZE(g_qspdia_conf); i++) {
		struct qspdia_conf *csi_est_conf = csi_est_conf_arr + i;

		seq_printf(s, "\nBucket %d\n", i);
		if (csi_est_conf->flags & QSPDIA_NDP)
			seq_printf(s, "mode:     ndp\n");
		else if (csi_est_conf->flags & QSPDIA_DATA)
			seq_printf(s, "mode:     data\n");
		else
			seq_printf(s, "mode:     none\n");

		seq_printf(s, "interval: %u\n", csi_est_conf->interval);
		seq_printf(s, "ng:       %u\n", csi_est_conf->ng);
		seq_printf(s, "reorder:  %u\n", csi_est_conf->tone_reorder);

		switch (csi_est_conf->smooth) {
		case QSPDIA_SMOOTH_NONE:
			seq_puts(s, "smooth:   none\n");
			break;
		case QSPDIA_SMOOTH_RAMP_ONLY:
			seq_puts(s, "smooth:   ramp\n");
			break;
		case QSPDIA_SMOOTH_EN:
			seq_puts(s, "smooth:   en\n");
			break;
		default:
			break;
		}
		seq_printf(s, "mac:      %pM\n", csi_est_conf->mac_addr);
	}
}
...
static int qdrv_command_csi_est(struct device *dev, int argc, char *argv[])
{
	...
	struct qdrv_wlan *qw;
	struct qspdia_conf *qspdia_conf;
	struct qspdia_conf qspdia_conf_bkp;
	const struct qspdia_conf qspdia_conf_init = QSPDIA_CONF_INIT;

	char *cmd;
	char *param;
	int conf_num = -1;
	uint8_t conf_mac[IEEE80211_ADDR_LEN] = {0};
	int i;
	int hlink_rc;
	...
	if (strcmp(argv[1], "show") == 0) {
		qdrv_control_set_show(qdrv_csi_est_show, &g_qspdia_conf, 1, 1);
		return 0;
	}

	qcb = dev_get_drvdata(dev);
	if (!qcb) {
		DBGPRINTF_E("qcb NULL\n");
		return -1;
	}
	...
	if (qdrv_parse_mac(argv[1], conf_mac) < 0)
		goto usage;

	for (i = 0; i < ARRAY_SIZE(g_qspdia_conf); i++) {
		if (IEEE80211_ADDR_EQ(conf_mac, g_qspdia_conf[i].mac_addr)) {
			conf_num = i;
			break;
		}

		if (IEEE80211_ADDR_NULL(g_qspdia_conf[i].mac_addr) && conf_num < 0)
			conf_num = i;
	}

	if (conf_num < 0) {
		DBGPRINTF_E("Can not find bucket for %pM\n", conf_mac);
		return -1;
	}

	cmd = argv[2];

	qspdia_conf = &g_qspdia_conf[conf_num];
	qspdia_conf_bkp = *qspdia_conf;

	if (strcmp(cmd, "add") == 0) {
		if (!IEEE80211_ADDR_NULL(qspdia_conf->mac_addr) &&
				!IEEE80211_ADDR_EQ(qspdia_conf->mac_addr, conf_mac)) {
			goto usage;
		}

		if (IEEE80211_ADDR_NULL(qspdia_conf->mac_addr))
			*qspdia_conf = qspdia_conf_init;

		memcpy(qspdia_conf->mac_addr, conf_mac, sizeof(qspdia_conf->mac_addr));
		goto send_conf;
	...
	if (!qdrv_csi_est_enabled()) {
		...
		if (qdrv_spdia_setup()) {
			DBGPRINTF_E("Can not setup SPDIA\n");
			return -1;
		}
		ieee80211_param_to_qdrv(vap, IEEE80211_PARAM_PHY_STATS_MODE,
					MUC_PHY_STATS_RSSI_RCPI_ONLY,
					NULL, 0);
	}

	if (strcmp(cmd, "interval") == 0) {
		qspdia_conf->interval = _atoi(param);

		if (qspdia_conf->interval != 0 && qspdia_conf->interval < QSPDIA_MIN_INTERVAL)
			qspdia_conf->interval = QSPDIA_MIN_INTERVAL;

		if (qspdia_conf->interval != 0 && qspdia_conf->flags == 0)
			qspdia_conf->flags = QSPDIA_DATA;
		else if (qspdia_conf->interval == 0 && qspdia_conf->flags != 0)
			qspdia_conf->flags = 0;

	} else if (strcmp(cmd, "mode") == 0) {
		if (strcmp(param, "data") == 0)
			qspdia_conf->flags = QSPDIA_DATA;
		else if (strcmp(param, "ndp") == 0)
			qspdia_conf->flags = QSPDIA_NDP;
		else if (strcmp(param, "none") == 0)
			qspdia_conf->flags = 0;
		else
			goto usage;

	} else if (strcmp(cmd, "ng") == 0) {
		uint32_t ng;
		...
		uint32_t tone_reorder;

		tone_reorder = _atoi(param);
		if (tone_reorder > 1)
			goto usage;

		qspdia_conf->tone_reorder = tone_reorder;

	} else if (strcmp(cmd, "smooth") == 0) {
		if (strcmp(param, "en") == 0)
			qspdia_conf->smooth = QSPDIA_SMOOTH_EN;
		else if (strcmp(param, "ramp") == 0)
			qspdia_conf->smooth = QSPDIA_SMOOTH_RAMP_ONLY;
		else if (strcmp(param, "none") == 0)
			qspdia_conf->smooth = QSPDIA_SMOOTH_NONE;
		else
			goto usage;
...
send_conf:
	hlink_rc = qdrv_hostlink_spdia_set_conf(qw, qspdia_conf, conf_num);
	if (hlink_rc & QTN_HLINK_RC_ERR) {
		DBGPRINTF_E("Can not send configuration to %pM\n", conf_mac);
		*qspdia_conf = qspdia_conf_bkp;
		return -1;
	}

	return 0;
usage:
	qdrv_control_set_show(qdrv_csi_est_usage, &g_qspdia_conf, 1, 1);
	return 0;
}
...
int qdrv_control_input(struct device *dev, char *buf, unsigned int count)
{
	...
	/* Parse the arguments */
	argc = qdrv_parse_args(qcb->command, ARRAY_SIZE(argv), argv);
...
EXPORT_SYMBOL(qdrv_control_input);

int qdrv_set_ipc_irq_handler(void (*fn)(void *arg1, void *arg2))
{
	struct int_handler handler = {  .handler = fn, .arg1 = NULL, .arg2 = NULL };
	struct qdrv_mac *mac = &g_qcb->macs[0];

	return qdrv_mac_set_handler(mac, RUBY_M2L_IRQ_LO_PCIE_HOST, &handler);
}
EXPORT_SYMBOL(qdrv_set_ipc_irq_handler);

int qdrv_clear_ipc_irq_handler(void)
{
	struct qdrv_mac *mac = &g_qcb->macs[0];

	return qdrv_mac_clear_handler(mac, RUBY_M2L_IRQ_LO_PCIE_HOST);
}
EXPORT_SYMBOL(qdrv_clear_ipc_irq_handler);

void qdrv_enable_ipc_irq(void)
{
	struct qdrv_mac *mac = &g_qcb->macs[0];

	qdrv_mac_enable_irq(mac, RUBY_M2L_IRQ_LO_PCIE_HOST);
}
EXPORT_SYMBOL(qdrv_enable_ipc_irq);

void qdrv_disable_ipc_irq(void)
{
	struct qdrv_mac *mac = &g_qcb->macs[0];

	qdrv_mac_disable_irq(mac, RUBY_M2L_IRQ_LO_PCIE_HOST);
}
EXPORT_SYMBOL(qdrv_disable_ipc_irq);

int qdrv_set_hbm_buf_append_meta(int meta_is_needed)
{
	struct qdrv_wlan *qw = qdrv_control_wlan_get(&g_qcb->macs[0]);
	int ret;

	if (!qw)
		return -1;

	qdrv_rx_register_mcast_meta_cb(meta_is_needed);

	ret = qdrv_hostlink_config_hbm_meta(qw, meta_is_needed);
	if (ret < 0) {
		DBGPRINTF_E("HBM meta append configuration failed (%d)\n", ret);
		return ret;
	}

	return 0;
}
EXPORT_SYMBOL(qdrv_set_hbm_buf_append_meta);

uint16_t qdrv_get_rfchip_info(unsigned int unit)
{
	struct qdrv_wlan *qw;
	...
	return ((qw->rf_chipid << 8) | qw->rf_chip_verid);
}
EXPORT_SYMBOL(qdrv_get_rfchip_info);

void qdrv_get_build_info(const struct device *dev,
		const char **name, const char **revision,
		const char **type, const char **label,
		u32 *tmstamp, u32 *plat_id, const char **hw_id,
		const char **calibration_ver, const char **uboot_ver,
		u32 *hw_ver)
{
	struct qdrv_cb *qcb = dev_get_drvdata(dev);
	...
	if (name)
		*name = QDRV_BLD_NAME;
	if (revision)
		*revision = QDRV_BLD_REV;
	if (type)
		*type = qdrv_control_bld_type_str(QDRV_BLD_TYPE);
	if (label)
		*label = QDRV_BLD_LABEL;
	if (tmstamp)
		*tmstamp = QDRV_BUILDDATE;
	if (plat_id)
		*plat_id = QDRV_CFG_PLATFORM_ID;
	if (hw_id)
		*hw_id = qdrv_soc_get_hw_id(0);
	if (calibration_ver)
		*calibration_ver = qcb->algo_version;
	if (uboot_ver)
		*uboot_ver = RUBY_UBOOT_VERSION;
	if (hw_ver)
		*hw_ver = soc_shared_params->hardware_options |
			(soc_shared_params->rf_chip_id << 24);
}
EXPORT_SYMBOL(qdrv_get_build_info);
...
static int qdrv_command_send_opmode(struct device *dev, int argc, char *argv[])
{
	...
	if (argc < 4) {
		printk("Wrong number of arguments %d\n", argc);
		goto exit;
	}

	if (qdrv_parse_mac(argv[1], mac) < 0) {
		printk("Error mac address %s\n", argv[1]);
		goto exit;
	}
	...
	ieee80211_send_vht_opmode_action(vap, ni, bw, rx_nss);
	opmode = SM(bw, IEEE80211_VHT_OPMODE_CHWIDTH) |
		SM(rx_nss, IEEE80211_VHT_OPMODE_RXNSS) |
		SM(0, IEEE80211_VHT_OPMODE_RXNSS_TYPE);
	ic->ic_setparam(ni, IEEE80211_PARAM_NODE_OPMODE, opmode, ni->ni_macaddr, IEEE80211_ADDR_LEN);

	res = 0;
exit:
	if (res == -1) {
		printk("Usage: send_opmode mac bw rx_nss\n");
	}
...
static int qdrv_command_send_addba(struct device *dev, int argc, char *argv[])
{
	...
	ic->ic_setparam(ni, IEEE80211_PARAM_HT_ADDBA, tid, 0, 0);

	res = 0;
exit:
	if (res == -1) {
		printk("Usage: send_addba mac tid\n");
	}
...
int qdrv_control_init(struct device *dev)
{
	...
	/* init to capacity max */
	qcb->instances = MAC_UNITS;

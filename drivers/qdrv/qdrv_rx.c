...
static int skb_need_meta_info = 0;
...
static __sram_text int qdrv_rx_decap_data_ratelimit(struct qdrv_wlan *qw)
{
	...
	qw->unknown_dp_count++;
	if (qw->unknown_dp_count > QDRV_UNKNOWN_DP_MAX_PER_SEC) {
		...
		struct ieee80211vap *vap, const struct ether_header *eh, void *p_iphdr)
{
	 ...
	 */
	if (vap->iv_bss && (vap->iv_bss->ni_ext_flags & IEEE80211_NODE_LNCB_4ADDR) &&
...
static void qdrv_rx_data_unauthed(struct ieee80211vap *vap, struct ieee80211_node *ni,
		unsigned char *mac, int found_vap)
{
	if (found_vap) {
		uint8_t reason;
		...
		IEEE80211_DPRINTF(vap, IEEE80211_MSG_AUTH | IEEE80211_MSG_INPUT,
				"%s: send deauth to node "MACSTR" for rxing data when state=%d\n",
				__func__, MAC2STR(mac), vap->iv_state);
...
struct qdrv_rx_decap_context {
	...
	uint32_t skb_done;
#define QTN_RX_WH_4ADDR		1
#define QTN_RX_CHECK_3ADDR_BR	2
	int addr_flags;
};
...
void wowlan_wakeup_host(struct ieee80211vap *vap, uint32_t reason)
{
	volatile uint32_t reg;
	struct ieee80211com *ic = vap->iv_ic;

	IEEE80211_DPRINTF(vap, IEEE80211_MSG_POWER, "%s: WoWLAN: Wake up host, reason 0x%x\n",
			  __func__, reason);

	switch (ic->ic_wowlan.wakeup_type) {
	case WOWLAN_GPIO_PIN_HOST_WAKEUP:
		gpio_wowlan_output(WOWLAN_GPIO_OUTPUT_PIN, 1);
		mdelay(10);
		gpio_wowlan_output(WOWLAN_GPIO_OUTPUT_PIN, 0);
		break;
	case WOWLAN_PCIE_PME_HOST_WAKEUP:
		reg = readl(RUBY_SYS_CTL_PCIE_CFG0);
#define TOPAZ_PME_EVENT	BIT(8)
		writel(reg | TOPAZ_PME_EVENT, RUBY_SYS_CTL_PCIE_CFG0);
		reg &= ~TOPAZ_PME_EVENT;
		writel(reg, RUBY_SYS_CTL_PCIE_CFG0);
		break;
	default:
		IEEE80211_DPRINTF(vap, IEEE80211_MSG_POWER, "%s: WoWLAN: Unknown wakeup type: %d\n",
				  __func__, ic->ic_wowlan.wakeup_type);
		break;
	}
}
...
int wowlan_magic_process(struct sk_buff *skb, struct ieee80211vap *vap)
{
	...
	if ((ic->ic_wowlan.mask & WOWLAN_TRIG_PATTERN) && ic->ic_wowlan.pattern.len != 0) {
		wake_flag = wowlan_magic_match(skb->data, skb->len,
				ic->ic_wowlan.pattern.magic_pattern,
				ic->ic_wowlan.pattern.len);
		if (wake_flag) {
			wowlan_wakeup_host(vap, WOWLAN_TRIG_PATTERN);
			return 1;
		}
	}

	if (ic->ic_wowlan.mask & WOWLAN_TRIG_MAGIC) {

		/* maintain legacy behavior: custom pattern masks standard magic pattern */
		if ((ic->ic_wowlan.mask == WOWLAN_TRIG_ALL) && (ic->ic_wowlan.pattern.len != 0))
			return 0;
		...
		if (wake_flag) {
			wowlan_wakeup_host(vap, WOWLAN_TRIG_MAGIC);
			return 1;
		}
	}
...
static inline void qdrv_rx_set_addr_flags(struct qdrv_rx_decap_context *ctx,
	int is_4addr)
{
	...
	if (is_4addr)
		ctx->addr_flags = QTN_RX_WH_4ADDR;
	else if (vap->iv_opmode == IEEE80211_M_STA &&
			(!ieee80211_node_is_qtn(ni) ||
			!(vap->iv_flags_ext & IEEE80211_FEXT_WDS)))
		ctx->addr_flags = QTN_RX_CHECK_3ADDR_BR;
	else
		ctx->addr_flags = 0;
}

static inline int qdrv_rx_should_input_all(struct ieee80211_node *ni,
		struct ieee80211_qosframe_addr4 *wh, uint8_t type, uint8_t subtype)
{
	struct ieee80211vap *vap = ni->ni_vap;

	if (type == IEEE80211_FC0_TYPE_MGT) {
		/* currently only Probe Request cared */
		if (subtype == IEEE80211_FC0_SUBTYPE_PROBE_REQ) {
			/* broadcast BSSID */
			if (IEEE80211_ADDR_BCAST(wh->i_addr3) &&
					(vap->iv_opmode == IEEE80211_M_HOSTAP ||
					 ieee80211_is_repeater_sta(vap) ||
		...
		struct sk_buff *skb)
{
	...
	if (qtn_vlan_should_drop_stag(vdev, di->start, vlan_drop_stag))
		return 0;
...
/*
 * If in AP mode with 4-address mode configured and a 4-address frame is
 * received from a STA that is not flagged as supporting 4-address mode,
 * notify the lhost, so that the FWT entries for the station and its endpoints
 * can be marked for 4-address handling.
 */
static __inline__ void
qdrv_rx_check_cfg_4addr(struct qdrv_rx_decap_context *ctx)
{
	struct ieee80211_node *ni = ctx->ni;
	struct ieee80211vap *vap = &ctx->qv->iv;
	struct qtn_node_shared_stats *data = ni->ni_shared_stats;

	if (ctx->addr_flags != QTN_RX_WH_4ADDR)
		return;		/* not a 4-address header */

	if (!IEEE80211_CFG_4ADDR_ENABLED(vap))
		return;

	if (vap->iv_opmode != IEEE80211_M_HOSTAP)
		return;		/* always on in STA mode */

	if (unlikely(!data->cfg_4addr)) {
		data->cfg_4addr = 1;
		vap->iv_ic->ic_setparam(ni, IEEE80211_PARAM_CFG_4ADDR_UPDATE,
			0, ni->ni_macaddr, IEEE80211_ADDR_LEN);
	}
}

/*
 * Assume max of one VLAN header.
 */
static void qdrv_rx_remove_eapol_vlan(struct qdrv_vap *qv, struct sk_buff *skb)
{
	struct vlan_ethhdr *veth = (struct vlan_ethhdr *)(skb->data);
	struct ether_header eth;

	if (!IEEE80211_ADDR_EQ(qv->iv.iv_myaddr, veth->h_dest))
		return;

	if (veth->h_vlan_proto == htons(ETH_P_8021Q) &&
			veth->h_vlan_encapsulated_proto == htons(ETH_P_PAE)) {
		memcpy(&eth, skb->data, sizeof(eth));
		eth.ether_type = htons(ETH_P_PAE);
		skb_pull(skb, sizeof(struct vlan_ethhdr));
		memcpy(skb_push(skb, sizeof(eth)), &eth, sizeof(eth));

		QTN_SKB_CB_ETHERTYPE(skb) = htons(ETH_P_PAE);
	}
}
...
static int __sram_text qdrv_rx_decap_callback(struct qtn_rx_decap_info *di, void *_ctx)
{
	...
	skb->priority = TID_TO_WMM_AC(di->tid);
	QTN_SKB_CB_ETHERTYPE(skb) = di->l3_ether_type;

	qdrv_rx_remove_eapol_vlan(qv, skb);
	...
	if (handle_rx_msdu(qw, qv, ni, ctx->wh_copy, skb,
			ctx->addr_flags == QTN_RX_CHECK_3ADDR_BR) == 0) {
...
static void qdrv_rx_reject(struct qdrv_wlan *qw, struct ieee80211vap *vap,
				struct ieee80211_qosframe_addr4 *wh_copy, int found_vap)
{
	struct ieee80211_node *ni;
	...
	if ((dir != IEEE80211_FC1_DIR_NODS) && qdrv_rx_decap_data_ratelimit(qw)) {
		if (found_vap) {
			...
			if (ni) {
				IEEE80211_DPRINTF(vap, IEEE80211_MSG_AUTH | IEEE80211_MSG_INPUT,
					"%s: receive data packet from unknown node "MACSTR
					" send deauth with reason %u\n", __func__,
					MAC2STR(wh_copy->i_addr2), IEEE80211_REASON_NOT_AUTHED);
...
void qdrv_rx_more_data_frame(struct ieee80211_qosframe_addr4 *wh,
	struct ieee80211vap *vap, struct ieee80211_node *ni)
{
	int more_data = 0;
...
static int __sram_text qdrv_rx_decap(struct qdrv_vap *qv, struct sk_buff *skb,
		struct qdrv_rx_skb_list *skb_rcv_list, uint32_t pseudo_rssi, int found_vap)
{
	uint8_t dir;
	uint8_t type;
	uint8_t subtype;
	...
	frag_more = wh_copy.i_fc[1] & IEEE80211_FC1_MORE_FRAG;
	frag_num = le16_to_cpu(*(__le16 *) wh_copy.i_seq) & IEEE80211_SEQ_FRAG_MASK;
...
#ifdef ARTSMNG_SUPPORT
		/* set ni as null to proceed processing of probe req packets received from WDS */
		if (QTN_IS_WDS_NI(ni) && (type == IEEE80211_FC0_TYPE_MGT) &&
	...
	if (!ni) {
		...
		if (ni) {
			...
			if (type == IEEE80211_FC0_TYPE_DATA) {
				qdrv_rx_reject(qw, vap, &wh_copy, found_vap);
				dev_kfree_skb(skb);
				return 0;
...
#ifdef ARTSMNG_SUPPORT
	if ((vap->iv_opmode == IEEE80211_M_HOSTAP) && (type == IEEE80211_FC0_TYPE_MGT) &&
			((subtype == IEEE80211_FC0_SUBTYPE_ASSOC_REQ) ||
			(subtype == IEEE80211_FC0_SUBTYPE_REASSOC_REQ) ||
			(subtype == IEEE80211_FC0_SUBTYPE_PROBE_REQ) ||
			(subtype == IEEE80211_FC0_SUBTYPE_AUTH))) {
		if (artsmng_blacklist_check(vap, wh_copy.i_addr2)) {
			if (ni) {
	...
	if (ni) {
		...
		if (type == IEEE80211_FC0_TYPE_DATA) {
			...
			if (unlikely(vap->iv_state < IEEE80211_S_RUN) &&
				...
				if (qdrv_rx_decap_data_ratelimit(qw))
					qdrv_rx_data_unauthed(vap, ni, wh_copy.i_addr2, found_vap);
	...
	frag = ((type != IEEE80211_FC0_TYPE_CTL) && (frag_more || frag_num));
	...
	if ((type != IEEE80211_FC0_TYPE_DATA) || frag ||
		...
		if (skb_need_meta_info) {
			if ((type == IEEE80211_FC0_TYPE_DATA) && frag && (frag_num == 0)) {
				skb->qtn_cb.radio_info.macid = vap->iv_ic->ic_unit;
				skb->qtn_cb.radio_info.ifidx = qv->qv_vap_idx;
				M_FLAG_SET(skb, M_HAS_RADIO_INFO);
			}
		}
		...
		if ((ni == NULL) || qdrv_rx_should_input_all(ni, &wh_copy, type, subtype)) {
			RXSTAT(qw, rx_input_all);
	...
	qdrv_rx_more_data_frame(&wh_copy, vap, ni);
	...
	ctx.wh_copy = &wh_copy;
	ctx.skb_done = 0;
	qdrv_rx_set_addr_flags(&ctx, dir == IEEE80211_FC1_DIR_DSTODS);
	snap_check = qw->amsdu_security_check & QTN_AMSDU_SUBFRM_SNAP_CHECK;
	ret = qtn_rx_decap(&wh_copy, skb->data, skb->len, def_vlan_tci, &qtn_vlan_info,
				vlan_enabled, &qdrv_rx_decap_callback, &ctx, NULL, snap_check);

	if (ret == QTN_RX_DECAP_TRAINING) {
		RXSTAT(qw, rx_rate_train_invalid);
	} else if (unlikely(ret == QTN_RX_DECAP_SNAP_DROP)) {
		RXSTAT(qw, rx_decap_snap_drop);
		dev_kfree_skb(skb);
		ieee80211_check_free_node(node_reference_held, ni);
		return 0;
	}
		...
		struct sk_buff *skb)
{
	...
	if (br_get_ap_isolate() || QTN_FLAG_IS_INTRA_BSS(skb->dev->qtn_flags))
		return 0;
	if (QTN_FLAG_IS_L2_EXT_FILTER(skb->dev->qtn_flags))
		return 0;
...
static struct qdrv_vap *qdrv_find_vap(struct qdrv_wlan *qw, struct sk_buff *skb)
{
	...
	/*
	 * find AP VAP based on bssid, if cannot find AP VAP,
	 * check RA to find STA VAP
	 */
	vap = ieee80211_find_vap(ic, bssid);
	if (!vap && (bssid != wh->i_addr1))
		vap = ieee80211_find_vap(ic, wh->i_addr1);
	if (vap)
...
int __sram_text qdrv_rx_poll(struct napi_struct *napi, int budget)
{
	...
	while (processed < budget) {
		...
		if (ni) {
			...
			/*
			 * Using the frame's bssid to find out the vap, if still
			 * cannot find, using the default vap0
			 */
			KASSERT(qw->mac->vnet[0] != NULL, ("Primary VAP is null\n"));
			qv = qdrv_find_vap(qw, skb);
			if (!qv) {
		...
		pn_lo = rxdesc->rx_status_pnlo;
		pn_hi = 0x0000ffff & rxdesc->rx_status_pnhi;
		skb->qtn_cb.pn = pn_lo + (pn_hi << 32);

		processed += qdrv_rx_decap(qv, skb, &skb_rcv_list, pseudo_rssi, found_vap);
...
static unsigned int qdrv_mcast_do_frame_meta_append(const union topaz_tqe_cpuif_descr *desc)
{
	unsigned int data_len = desc->data.length;
	const uint16_t misc_user = desc->data.misc_user;
	const uint16_t node_idx_unmapped = MS(misc_user, TQE_MISCUSER_M2L_DATA_NODE_IDX);
	struct ieee80211_node *ni;
	struct qdrv_vap *qv;
	...
	ni = ic->ic_node_idx_ni[node_idx_unmapped];
	if (!ni) {
		DBGPRINTF_E("Failed to node\n");
		goto out;
	}

	qv = container_of(ni->ni_vap, struct qdrv_vap, iv);
	hbm_buf_append_meta_info(desc->data.pkt, &data_len, ic->ic_unit, qv->qv_vap_idx);
...
void qdrv_rx_register_mcast_meta_cb(int do_register)
{
	if (do_register) {
		tqe_register_mcast_do_frame_meta_append(&qdrv_mcast_do_frame_meta_append);
		skb_need_meta_info = 1;
	} else {
		tqe_register_mcast_do_frame_meta_append(NULL);
		skb_need_meta_info = 0;
	}
}
...
int qdrv_rx_init(struct qdrv_wlan *qw, struct host_ioctl_hifinfo *hifinfo)
{
	...
	qw->amsdu_security_check = QTN_AMSDU_SUBFRM_CHECK_INIT;
	DBGPRINTF(DBG_LL_INFO, QDRV_LF_PKT_RX,
			"hi_rxfifo %p %p (virt)\n",

...
void hal_get_tsf(uint64_t *ret)
{
	...
	/*
	 * Mutex here is protection against soft resets.
	 * MuC can do MAC soft reset any time, and accessing registers while MAC in reset may cause
	 * bus hang. So MuC when does soft reset it locks mutex, and so Lhost here does the same
	 * when access register.
	*/
	qtn_bb_mutex_enter(QTN_LHOST_SOC_CPU);

	/*
	 * Wrap-around protection for reading a 64 bit register.
	 * HW updates the TSF register when:
	 * - Timer increments every 1 us, and
	 * - Beacon received: TSF can be incremented or decremented.
	 */
	do {
		readhi1 = readl(HAL_REG(HAL_REG_TSF_HI));
		readlo1 = readl(HAL_REG(HAL_REG_TSF_LO));
		readhi2 = readl(HAL_REG(HAL_REG_TSF_HI));
		readlo2 = readl(HAL_REG(HAL_REG_TSF_LO));
		r1 = ((((uint64_t)readhi1) << 32) | readlo1);
		r2 = ((((uint64_t)readhi2) << 32) | readlo2);
	} while ((r2 - r1) > 1);

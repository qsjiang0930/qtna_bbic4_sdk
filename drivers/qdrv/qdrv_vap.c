...
static int vnet_init(struct net_device *dev)
{
	...
	if (vap->iv_btm_term.flags & IEEE80211_VAP_BTM_REQ_SEND) {
		DBGPRINTF_E("Interface %s not ready due to BTM Terminate - try again\n", dev->name);
		return -1;
	}
...
static int vnet_stop(struct net_device *dev)
{
	...
	if ((vap->iv_btm_term.flags & IEEE80211_VAP_BTM_IN_EXTCAP) &&
			(vap->iv_btm_term.delay != IEEE80211_BSS_BTM_INVALID_TERM_DELAY)) {
		if (ieee80211_send_btm_req_terminate_bss(vap))
			return 0;
	}
		...
		const uint32_t sub_port)
{
	...
	return (ni && ieee80211_node_is_running(ni) && ieee80211_node_is_authorized(ni));
}
#endif

static void qdrv_vap_reset_mu_caps(struct ieee80211com *ic)
{
	ic->ic_vhtcap.cap_flags &= ~IEEE80211_VHTCAP_C_MU_BEAM_FORMXX_CAP_MASK;

	if (ic->ic_mu_enable) {
		uint32_t mu_bf_cap_flag = 0;
...
#ifdef CONFIG_QVSP
static struct ieee80211_node *qdrv_vap_find_node(void *token, const u_int8_t *macaddr)
{
	struct qdrv_wlan *qw = (struct qdrv_wlan *)token;
	struct ieee80211com *ic = &qw->ic;

	return ieee80211_find_node(&ic->ic_sta, macaddr);
}
#endif

static int
qdrv_should_disable_rxpn_check(struct ieee80211com *ic)
{
	...
	u32 arg1, u32 arg2)
{
	...
	if (qdrv_should_disable_rxpn_check(ic))
		ieee80211_enable_rxpn_check(vap, QTN_DISABLE_PN_VALIDATION);
	...
	ic->ic_qtn_bgscan.duration_msecs_obss = QDRV_WLAN_QTN_BGSCAN_DURATION_OBSS;
	...
	if (qw->qvsp == NULL) {
		qw->qvsp = qvsp_init(&qdrv_qvsp_ioctl, qw, vdev, stamode,
			ic->ic_vsp_cb_cfg, ic->ic_vsp_cb_strm_ctrl,
			ic->ic_vsp_cb_strm_ext_throttler, qdrv_vap_find_node,
...
int qdrv_vap_exit_muc_done(struct qdrv_mac *mac, struct net_device *vdev)
{
	...
	/* Destroy it ... */
	rtnl_lock();
	if (qw->pktlogger.netdev_q_ptr_w && qw->pktlogger.netdev_q_ptr_w->dev == vdev)

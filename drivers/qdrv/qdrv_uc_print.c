 ...
 */
static unsigned uc_print(struct shared_print_consumer* shared_buf, const char *prefix, unsigned max_lines)
{
#define FW_PRINT_MAX_CHAR_PER_LINE	128
	...
	if (uc_print_qcb) {
		qw = uc_print_qcb->macs[0].data;
	}

	while(took_line) {
		u32 chars_to_consume = shared_buf->producer->produced - shared_buf->consumed;
		u32 i;
		char stackbuf[FW_PRINT_MAX_CHAR_PER_LINE];

		took_line = 0;
		for (i = 0; i < chars_to_consume && (i < FW_PRINT_MAX_CHAR_PER_LINE - 1); i++) {
			char c;

			if (completed_lines >= max_lines)
				break;

			c = shared_buf->buf[(shared_buf->consumed + i) % shared_buf->producer->bufsize];
			stackbuf[i] = c;

			if (!c || i == FW_PRINT_MAX_CHAR_PER_LINE - 2 || c == '\n') {
				took_line = 1;
				...
				if (qw)
					qdrv_control_sysmsg_send(qw, stackbuf, i + 1, 0);
...
{
	unsigned max_lines = FW_PRINT_MAX_DEFAULT_LINES;
	struct qdrv_wlan *qw = NULL;
	...
	if (uc_print_qcb) {
		...
		if (qw) {
			ic = &qw->ic;
			max_lines = ic->ic_fw_print_max_lines;
		}
	}
	...
	if (muc_printbuf.producer || muc_print_initialise_shared_data()) {
		max_lines = uc_print(&muc_printbuf, MUC_PREFIX, max_lines);
	}
	if (auc_printbuf.producer || auc_print_initialise_shared_data()) {
#if AUC_LHOST_PRINT_FORMAT
		if (uc_print_auc_cb)
			uc_print_auc_cb(&auc_printbuf, max_lines);
#else
			uc_print(&auc_printbuf, AUC_PREFIX, max_lines);
#endif
	}
}

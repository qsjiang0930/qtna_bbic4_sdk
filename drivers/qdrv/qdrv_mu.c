			...
			if (!is_station)
			{
				...
				for (j = 0; j < ARRAY_SIZE(mu_grp_tbl[i].aid); j++) {
					if ( mu_grp_tbl[i].aid[j] != 0) {
						struct ieee80211_node *user = ieee80211_find_node_by_aid(ic, mu_grp_tbl[i].aid[j]);
						if (user != NULL) {
							p += sprintf(p, "user %d: AID: %.x IDX: %u, MAC: %pM\n",
								j, mu_grp_tbl[i].aid[j], mu_grp_tbl[i].ncidx[j], user->ni_macaddr);
							ieee80211_free_node(user);
						}
					}
				}

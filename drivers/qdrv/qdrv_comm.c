...
static int msg_radar_detected_handler(struct qdrv_cb *qcb, void *msg);
static int msg_radar_new_samples_handler(struct qdrv_cb *qcb, void *msg);
...
static int (*s_msg_handler_table[])(struct qdrv_cb *qcb, void *msg) =
{
	...
	msg_radar_detected_handler,	/* IOCTL_HLINK_RADAR_DETECTED */
	msg_radar_new_samples_handler,	/* IOCTL_HLINK_RADAR_NEW_SAMPLES */
};
...
{
	struct host_ioctl *mp = (struct host_ioctl *) msg;
	struct qdrv_mac *mac = &qcb->macs[mp->ioctl_arg1];
	...
	if (mac->mac_sys_stats != NULL) {
		iounmap(mac->mac_sys_stats);
	}

	mac->mac_sys_stats = ioremap_nocache(muc_to_lhost((u32) mp->ioctl_arg2),
						sizeof(*mac->mac_sys_stats));

	ret = qdrv_pktlogger_map(qw);
...
static int msg_radar_new_samples_handler(struct qdrv_cb *qcb, void *msg)
{
	...
	qdrv_radar_handle_samples(mp->ioctl_arg1);
...
static void comm_work(struct work_struct *work)
{
	...
	/* Take the semaphore */
	while (!sem_take(RUBY_SYS_CTL_L2M_SEM, QTN_SEM_HOST_LINK_SEMNUM, &irqflags));
	...
	sem_give(RUBY_SYS_CTL_L2M_SEM, QTN_SEM_HOST_LINK_SEMNUM, irqflags);

...
show_mtdparts()
{
	if [ $# -gt 0 ]; then
		...
		echo "mtdparts is not present in $cmdfile" 1>&2
	fi
}

if [ "$env_bak" -ne 0 ]; then
	echo  "uboot_env_bak is already present" 1>&2
...
if [ -z "$device_env" ]; then
	echo "uboot_env does not exist" 1>&2
	show_mtdparts $mtdparts_orig
	exit 2
fi
...
if [ "$size_env" -eq 0 -o "$erase_sz" -eq 0 ]; then
	echo "uboot_env size or sector is invalid" 1>&2
	show_mtdparts $mtdparts_orig
        exit 3
fi
...
needed_size_env=$(($new_size_env * 2))

if [ $size_env -lt $needed_size_env ]; then
	# no enough number of blocks
	echo "Not enough space; Needed=$needed_size_env, Avail=$size_env" 1>&2
	show_mtdparts $mtdparts_orig
        exit 4
fi
...
for s in ${sizes}; do
	...
	if [ -z "$name" ]; then
		echo "mtd$i is not present in $mtdfile" 1>&2
		exit 5
	fi

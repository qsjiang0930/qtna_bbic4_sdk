...
copy_toolchain_lib_root =									\
	...
	if test -z "$${LIB_DIR}"; then								\
		...
		exit -1;									\
												...
												\
	LIB="$(strip $1)";									\
	LINKS=`cd $${LIB_DIR}; find . -type l -name "$${LIB}*" -maxdepth 3`;			\
	FILES=`cd $${LIB_DIR}; find . -type f -name "$${LIB}" -maxdepth 3`;			\
	for FILE in $${LINKS} $${FILES}; do							\
		LIB=$${FILE};									\
		LIB_DIRNAME=""; 								\
		while test \! -z "$${LIB}"; do							\
...
BR2_TOOLCHAIN_EXTERNAL_LIBS += libstdc++.so libgcc_s.so libgcc_s.so.1
endif

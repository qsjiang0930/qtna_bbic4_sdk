...
static const qwe_cmd_entry qweaction_cmd_list[] = {
	...
	{"sta_info", {e_qwe_param_non_special, "<interface>"},
			{e_qwe_param_none, ""}, e_qwe_handle_sta_info, e_qwe_output_string},
...
#define QWE_IF_NUM (5)
const char *qwe_if_identifier[QWE_IF_NUM] = {"ap", "vap0", "vap1", "vap2", "vap3"};

static int validation_check(const char *param1, const char *param2)
{
	char *p = NULL;
	int i, ret = 0;

	/* backward compatible */
	if (!strcasecmp(param1, param2))
		return 1;

	p = strstr(param1, param2);
	if (!p) {
		ret = 0;
	} else {
		p = p + strlen(param2) + 1;
		for (i = 0; i < QWE_IF_NUM; i++)
			if (strstr(p, qwe_if_identifier[i]))
				ret = 1;
	}
	return ret;
}
...
static int match_param(const char *param, const qwe_param *require)
{
	...
	case e_qwe_param_specific:
		return ((param == NULL) ? 0 : validation_check(param, require->param));
...
{
	int fd[2];
	int fd_stdout_bak = -1, fd_stdout_new = -1;
	int fd_stderr_bak = -1, fd_stderr_new = -1;
	int result, len;
	char *tail;
...
static int qwe_handle_sta_info(const qwe_handle_type handle,
	const char *device, const char *index, char *output, const unsigned int max_len)
{
#define INFO_EXPIRE_TIME 300
	...
	if (handle == e_qwe_handle_sta_info) {
		filename = STA_INFO_FILE;
		snprintf(qwecmd, sizeof(qwecmd), "qweaction %s stainfo %s", device, index);
	} else {
		snprintf(output, max_len, "Function %s get unknown handle", __func__);
		return 1;
	}
	...
	system(cmd);
	snprintf(cmd, sizeof(cmd), "%s >> %s", qwecmd, filename);
	system_adv(cmd, CATCH_STDERR, output, max_len);
	...
	if (time(NULL) > timestamp + INFO_EXPIRE_TIME) {
		snprintf(output, max_len,
			"The \"%s\" already was operated over %d seconds, please execute again",
			 qwecmd, INFO_EXPIRE_TIME);
		fclose(fp);
		return 1;
	}
...
static int qwe_handle_ap_info(const qwe_handle_type handle,
	const char *device, const char *index, char *output, const unsigned int max_len)
{
#define INFO_EXPIRE_TIME 300
	char qwecmd[128];
...
int qwe_command(const char *cmd, const char *param1, const char *param2, const char *param3, char *output, const unsigned int max_len)
{
	...
	case e_qwe_handle_ap_info:
		return qwe_handle_ap_info(entry->handle, param1, param3, output, max_len);
	case e_qwe_handle_sta_info:
		return qwe_handle_sta_info(entry->handle, param1, param3, output, max_len);
	default:
		return -EINVAL;
	}
}

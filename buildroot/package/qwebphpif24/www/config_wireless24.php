...
function get_value()
{
	global $curr_mode,$curr_5gmode,$curr_fixed_ap,$curr_ssid,$curr_channel,$curr_channel_list,$curr_proto,$curr_psk,$curr_band,$curr_broadcast,$curr_wmm,$curr_bintval,$curr_dtim_period,$curr_short_gi,$curr_bw,$curr_coexist,$curr_frag_threshold,$curr_rts_threshold;
	//==========================================================================================================
	$curr_mode=exec("qweconfig get mode.wlan1");
	if ($curr_mode == "0")//AP
	{
		$curr_5gmode=exec("call_qcsapi get_mode wifi0");
		if ($curr_5gmode == "Station")
		{
			$ssidmode=exec("qweconfig get apmode.wlan1");
			if ($ssidmode == "fixed")
				$curr_fixed_ap=1;
			else
				$curr_fixed_ap=0;
...
function set_value()
{
	global $curr_mode,$curr_fixed_ap,$curr_ssid,$curr_channel,$curr_channel_list,$curr_proto,$curr_psk,$curr_band,$curr_broadcast,$curr_wmm,$curr_bintval,$curr_dtim_period,$curr_short_gi,$curr_bw,$curr_coexist,$curr_frag_threshold,$curr_rts_threshold;
	//Save
	if ($_POST['action'] == 0)
	{
		$new_mode=$_POST['cmb_devicemode'];

		$new_mode_esc = escapeshellarg($new_mode);

		if ($new_mode != $curr_mode)
		{
			exec("qweconfig set mode.wlan1 $new_mode_esc");
		...
		else
		{
			...
			if ($curr_mode == "0")//AP
			{
				...
				$new_fixed_ap=$_POST['chk_fixed_ap'];
				$new_fixed_ap=($new_fixed_ap == "1")?"1":"0";
				if ($new_fixed_ap != $curr_fixed_ap)
				{
					$change_flag++;
					if ($new_fixed_ap == "1")
					{
						exec("qweconfig set apmode.wlan1 fixed");
					}
					else
					{
						exec("qweconfig set apmode.wlan1 clone");
					}
				}
				if ($new_fixed_ap == "1")
				{
					...
					{
						$new_proto_esc = escapeshellarg($new_proto);
						$change_flag++;
						exec("qweconfig set encryption.wlan1 $new_proto_esc");
					}
					//Set Passphrase
					if ($new_psk != $curr_psk)
					{
				...
				if ($new_channel != $curr_channel)
				{
					...
					exec("qweconfig set channel.wlan1 $new_channel_esc");
				}
				//Set band
				if ($new_band!=$curr_band)
				{
					...
					exec("qweconfig set band.wlan1 $new_band_esc");
				}
				//set bw
				if ($new_bw!=$curr_bw)
				{
					...
					exec("qweconfig set bandwidth.wlan1 $new_bw_esc");
				}
				//Set Broadcast SSID
				if ($new_broadcast != $curr_broadcast)
				{
					...
					exec("qweconfig set broadcastssid.wlan1 $new_broadcast_esc");
				}
				//Set Beacon Interval
				if ($new_bintval != $curr_bintval){
					$new_bintval_esc = escapeshellarg($new_bintval);
					exec("qweconfig set bcnint.wlan1 $new_bintval_esc");
				}
				//Set DTIM Period
				if ($new_dtim_period != $curr_dtim_period){
					$new_dtim_period_esc = escapeshellarg($new_dtim_period);
					exec("qweconfig set dtimperiod.wlan1 $new_dtim_period_esc");
				}
				//Set Short GI
				if ($new_short_gi != $curr_short_gi)
				{
					...
					exec("qweconfig set shortgi.wlan1 $new_short_gi_esc");
				}
				//Set 20/40 coexist
				if ($new_coexist != $curr_coexist)
				{
					$change_flag++;
					$new_coexist_esc = escapeshellarg($new_coexist);
					exec("qweconfig set coexist.wlan1 $new_coexist_esc");
				}
				//Set Fragmentation threshold
				if ($new_frag_threshold != $curr_frag_threshold)
				{
					$change_flag++;
					$new_frag_threshold_esc = escapeshellarg($new_frag_threshold);
					exec("qweconfig set fragthres.wlan1 $new_frag_threshold_esc");
				}
				//Set RTS/CTS  threshold
				if ($new_rts_threshold != $curr_rts_threshold)
				{
					$change_flag++;
					$new_rts_threshold_esc = escapeshellarg($new_rts_threshold);
					exec("qweconfig set rtsthres.wlan1 $new_rts_threshold_esc");
				}
			}
			else if ($curr_mode == "1" || $curr_mode == "2")
			{
				...
				if ($new_proto != $curr_proto)
				{
					$change_flag++;
					$new_proto_esc = escapeshellarg($new_proto);
					exec("qweconfig set encryption.sta.wlan1 $new_proto_esc");
				}
				//Set Passphrase
				if ($new_psk != $curr_psk)
				{
...
function modechange(obj)
{
	...
	if (obj.name == "chk_fixed_ap")
	{
		if (document.mainform.chk_fixed_ap.checked)
		{
			set_disabled('txt_essid',false);
			set_disabled('cmb_encryption',false);
			set_disabled('txt_passphrase',false);
		}
		else
		{
			set_disabled('txt_essid',true);
			set_disabled('cmb_encryption',true);
			set_disabled('txt_passphrase',true);
		}
	}
}
...
function validate(action_name)
{
	...
	if (action_name==0)//Save Button
	{
		...
		if(mode == cmb_devicemode.value )
		{
			...
			if(mode=="0" && ((rts_threshold.value > 2347) || (rts_threshold.value < 1)))
			{
...
function onload_event()
{
	...
	var curr_5gmode = "<?php echo $curr_5gmode; ?>";
	if (curr_5gmode == "Access point")
		set_visible("tr_fixed_ap", false);

	var curr_fixed_ap = "<?php echo $curr_fixed_ap; ?>";
	if (curr_fixed_ap == "0")
	{
		set_disabled('txt_essid',true);
		set_disabled('cmb_encryption',true);
	...
	set_control_value('chk_fixed_ap','<?php echo $curr_fixed_ap; ?>', 'checkbox');
...
<div class="container">
	<div class="left">
		<script type="text/javascript">
			createMenu('<?php $tmp=exec("call_qcsapi verify_repeater_mode");if($tmp == 1)    $tmp = "Repeater";else    $tmp = exec("call_qcsapi get_mode wifi0"); echo $tmp;?>','<?php $tmp=exec("qweconfig get mode.wlan1"); echo $tmp;?>',privilege);
		</script>
	</div>
				...
				<div class="TabbedPanelsContent">
					<div class="rightmain">
						...
						<tr id="tr_fixed_ap">
							<td>Fixed AP:</td>
							<td>
								<input name="chk_fixed_ap" id="chk_fixed_ap" type="checkbox"  class="checkbox" value="1" onchange="modechange(this);"/>
							</td>
						</tr>
						<tr>
							<td>ESSID:</td>

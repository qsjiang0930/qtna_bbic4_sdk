...
function read_mode($interface)
{
	...
	if($curr_mode=="Station") {
        ...
        $res = exec("call_qcsapi get_mode $interface");
    }
	else
	{
        $res = "Access Point (AP)";
...
function read_band($interface)
{
	//Get Current band
	$curr_band = trim(shell_exec("iwpriv $interface get_mode | sed 's?^.*:??'| cut -c1-4"));
	if(strcmp($curr_band, "11na") == 0)
		$curr_band = "11an";
	else if(strcmp($curr_band, "11ng") == 0)
		$curr_band = "11gn";
	else if(strcmp($curr_band, "11g") == 0)
		$curr_band = "11bg";
...
function read_channel($interface)
{
	...
	if(!(strpos($channel, "QCS API error") === FALSE))
	{
		$res="Not Associated";
	}
	else
...
<div class="container">
	...
	<div class="right">
		<div class="righttop">STATUS - 5G WI-FI</div>
		<div class="rightmain">
			<table class="tablemain">
				...
				<tr>
					<td>Device Mode:</td>
					<td><?php $tmp=read_mode($interface_id); echo $tmp;?></td>
				</tr>
				<tr>
					<td>Wireless Band:</td>
					<td><?php $tmp=read_band($interface_id); echo $tmp;?></td>
				</tr>
				<tr>
					<td>Bandwidth:</td>
				...
				<tr>
					<td>
					<?php
                    $tmp=exec("call_qcsapi get_mode $interface_id");
					if($tmp=="Station")
...
<input type="button" name="btn_assoc_table" id="btn_assoc_table" value="Association Table" class="button" style="width:150px; height:23px;" onclick="validate(<?php echo $passed_id;?>);"/>
				...
				<tr id="tr_rssi" <?php $tmp=exec("call_qcsapi get_mode $interface_id"); if($tmp == "Access point") echo "style=\"display: none;\""; ?>>
					<td>RSSI:</td>
					<td><?php $tmp=read_rssi($interface_id); echo $tmp;?></td>
				</tr>
				<tr>
					<td>Packets Received Successfully:</td>
					<td><?php $tmp=exec("call_qcsapi get_counter $interface_id RX_packets"); echo $tmp;?></td>
				</tr>
				<tr>
					<td>Bytes Received:</td>
					<td><?php $tmp=exec("call_qcsapi get_counter $interface_id RX_bytes"); echo $tmp;?></td>
				</tr>
				<tr>
					<td>Packets Transmitted Successfully:</td>
					<td><?php $tmp=exec("call_qcsapi get_counter $interface_id TX_packets"); echo $tmp;?></td>
				</tr>
				<tr>
					<td>Bytes Transmitted:</td>
					<td><?php $tmp=exec("call_qcsapi get_counter $interface_id TX_bytes"); echo $tmp;?></td>
				</tr>
				<tr>
					<td class="divline" colspan="2";></td>
				</tr>

...
function set_value()
{
	...
	$mac_esc = escapeshellarg($mac);
	$interface_esc = escapeshellarg($interface);
	...
	if($action == "save")
	{
		...
		$filter_esc = escapeshellarg($filter);

		if($old_filter != $filter)
		{
			exec("call_qcsapi set_macaddr_filter $interface_esc $filter_esc");
		}
	}
	else if($action == "add")
	{
		...
		{
			exec("call_qcsapi deny_macaddr $interface_esc $mac_esc");
		}
		else if ($curr_filter == "2") //white
		{
			exec("call_qcsapi authorize_macaddr $interface_esc $mac_esc");
		}
	}
...
<div class="container">
	...
	<div class="right">
	<div class="righttop">5G Wi-Fi - MAC FILTER</div>
		<div class="rightmain">
					...
					<td width="70%">
						<select name="cmb_interface" class="combox" id="cmb_interface" style="width:180px;" onchange="interface_switch(this);">
							...
							for ($i=0;$i<8;$i++)
							{
								...
								if(strpos($bssid, "QCS API error") === FALSE)
								{

...
function getValue()
{
	global $chk0,$chk1,$chk2,$chk3,$chk4,$chk5,$chk6,$chk7,$wds0,$wds1,$wds2,$wds3,$wds4,$wds5,$wds6,$wds7,$psk0,$psk1,$psk2,$psk3,$psk4,$psk5,$psk6,$psk7,$vlan0,$vlan1,$vlan2,$vlan3,$vlan4,$vlan5,$vlan6,$vlan7,$file_vlan;
	//==============Get WDS Peer=======================
	$wds0=exec("call_qcsapi wds_get_peer_address wifi0 0");
	if(!(strpos($wds0, "QCS API error") === FALSE))
	{
		$chk0=0;
		$wds0="N/A";
	}
	...
	if(!(strpos($wds1, "QCS API error") === FALSE))
	{
		$wds1="N/A";
		$chk1=0;
	}
	...
	if(!(strpos($wds2, "QCS API error") === FALSE))
	{
		$wds2="N/A";
		$chk2=0;
	}
	...
	if(!(strpos($wds3, "QCS API error") === FALSE))
	{
		$wds3="N/A";
		$chk3=0;
	}
	...
	if(!(strpos($wds4, "QCS API error") === FALSE))
	{
		$wds4="N/A";
		$chk4=0;
	}
	...
	if(!(strpos($wds5, "QCS API error") === FALSE))
	{
		$wds5="N/A";
		$chk5=0;
	}
	...
	if(!(strpos($wds6, "QCS API error") === FALSE))
	{
		$wds6="N/A";
		$chk6=0;
	}
	...
	if(!(strpos($wds7, "QCS API error") === FALSE))
	{
		$wds7="N/A";
		$chk7=0;
	}
...
function setValue($old_chk,$new_chk,$old_wds,$new_wds,$new_psk,$old_vlan,$new_vlan,$wds_if)
{
	...
	$new_psk_esc = escapeshellarg($new_psk);
	...
	if($new_chk!=$old_chk)
	{
		...
		else
		{
			...
			if($new_vlan!="")
			{
				exec("qvlan bind $wds_if $new_vlan_esc");
		...
		if ($old_vlan!=$new_vlan)
		{
			...
			else
			{
				if($old_vlan=="")
				{
					...
					$tmp=exec("qvlan bind $wds_if $new_vlan_esc");
					$content_vlan=str_replace("#$wds_if:","$wds_if:$new_vlan",$content_vlan);
				}
				else
				{
...
if(isset($_POST['action']))
{
	...
	if (isset($_POST['chk_wds0']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass0=$_POST['txt_pass0'].$_POST['txt_pass0'];
	...
	if (isset($_POST['chk_wds1']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass1=$_POST['txt_pass1'].$_POST['txt_pass1'];
	...
	if (isset($_POST['chk_wds2']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass2=$_POST['txt_pass2'].$_POST['txt_pass2'];
	...
	if (isset($_POST['chk_wds3']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass3=$_POST['txt_pass3'].$_POST['txt_pass3'];
	...
	if (isset($_POST['chk_wds4']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass4=$_POST['txt_pass4'].$_POST['txt_pass4'];
	...
	if (isset($_POST['chk_wds5']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass5=$_POST['txt_pass5'].$_POST['txt_pass5'];
	...
	if (isset($_POST['chk_wds6']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass6=$_POST['txt_pass6'].$_POST['txt_pass6'];
	...
	if (isset($_POST['chk_wds7']))
	{
		...
		//WAR: API only accept 64 chars as input passphrase, but ONLY 32 bit take effect. UI made the workaround to limit ONLY 32 bit input.
		$new_pass7=$_POST['txt_pass7'].$_POST['txt_pass7'];
...
function test(wdsid)
{
	...
	if(chk0.checked==true)
	{
		...
		if(txtpass0.value.length != 32 && txtpass0.value.length != 0 && txtpass0.value != "********")
		{
			txtpass0.focus();
			res.innerHTML="Passphrase must contain 32 ASCII characters";
			res.style.visibility="visible";
			return false;
		}
	}
...
<div class="container">
	...
	<div class="right">
		<div class="righttop">5G WI-FI - WDS</div>
		<div class="rightmain">

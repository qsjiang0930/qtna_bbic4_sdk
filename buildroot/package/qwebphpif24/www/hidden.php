...
else
{
	if($_POST['passed_id']!="")
	{
		...
		$interface_id="wifi".escapeshellarg($passed_id);
	}
	else
	{
...
function load_value()
{
	...
	$curr_vsp_status = exec("call_qcsapi vsp wifi0 show") == "VSP is not enabled"? 0:1;
	...
	if ($curr_mode == "Access point" || $curr_mode=="Repeater")
	{
		...
		if(!(strpos($curr_pp_name, "QCS API error") === FALSE)||$curr_pp_status==0)
		{
			$curr_pp_name="";
		}
		...
		if(!(strpos($curr_mspp_status, "QCS API error") === FALSE))
		{
			$curr_mspp_status=0;
		}
		$curr_mspp_name=exec("call_qcsapi get_pairing_id $interface_id");
		if(!(strpos($curr_mspp_name, "QCS API error") === FALSE)||$curr_mspp_status==0)
		{
			$curr_mspp_name="";
		}
	...
	if(!(strpos($file_region, "QCS API error") === FALSE))
	{
		$file_region="";
	}
}
...
function set_value()
{
	global $curr_hw_wps_pbc,$curr_mode,$curr_pp_name,$curr_pp_status,$curr_nonwps_pp_status,$curr_mspp_name,$curr_mspp_status,$curr_region,$file_region,$curr_scs_status,$curr_vsp_status,$curr_bf_status,$arr,$curr_coc_status,$interface_id,$curr_ap_interface;
	//set att pp status
	...
	//set VSP
	$new_vsp_status = $_POST['vspStatus'];
	if(strcmp($new_vsp_status,$curr_vsp_status)!=0)
	{
		$contents = file_get_contents("/mnt/jffs2/wireless_conf.txt");
		if(0==$new_vsp_status)
		{
			exec("call_qcsapi vsp wifi0 set enabled 0");
		}
		else
		{
			exec("call_qcsapi vsp wifi0 set enabled 1");
		}
		$curr_vsp_status = "vsp=".$curr_vsp_status;
		$new_vsp_status = "vsp=".$new_vsp_status;
		$contents = str_replace($curr_vsp_status, $new_vsp_status, $contents);
		file_put_contents("/mnt/jffs2/wireless_conf.txt", $contents);
	}
...
function validate()
{
	...
	if (cf.enblvsp[0].checked)
	{
		cf.vspStatus.value = 0;
	}
	else
	{
		cf.vspStatus.value = 1;
	}
		...
		<div class="rightmain">
			<table class="tablemain" style=" height:auto">
				...
				<tr>
					<td>VSP:</td>
					<td>
						<input type="radio" name="enblvsp" id="enblvsp"  <?php if(0==$curr_vsp_status){echo "checked=\"checked\"";}?>>
							Disable&nbsp;&nbsp;
						<input type="radio" name="enblvsp" id="enblvsp"  <?php if(1==$curr_vsp_status){echo "checked=\"checked\"";}?>>
							Enable
						<input type="hidden" name="vspStatus" />
					</td>
				</tr>

		...
		<table class="tablemain">
			...
			<pre><?php
				...
				if($current_wifi_mode == "0")
					...
					$assoc_count=exec("qweaction wlan1 status rootap|grep mac|wc -l");
				if( $assoc_count > 0)
				{
					$index = 0;
					while( $index<$assoc_count )
					{
						...
						if ($current_wifi_mode=="0")
						{
							...
							$station_mac = exec("qweaction wlan1 status rootap | grep mac | awk '{print $2}'");
							$tmp_rssi = exec("qweaction wlan1 status rootap | grep rssi | awk '{print $2}'");
							$rssi=$tmp_rssi-100;
							$rx_bytes = exec("qweaction wlan1 status rootap | grep rx_bytes | awk '{print $2}'");
							$tx_bytes = exec("qweaction wlan1 status rootap | grep tx_bytes | awk '{print $2}'");
							$bw_assoc = exec("qweaction wlan1 status rootap | grep bandwidth | awk '{print $2}'");
							$time_associated = exec("qweaction wlan1 status rootap | grep duration | awk '{print $2}'");
						}
						$index++;
						echo "<tr>";

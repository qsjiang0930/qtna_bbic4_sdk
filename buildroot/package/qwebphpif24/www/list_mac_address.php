...
{
	$res="";
	$mac_filter = trim(exec("call_qcsapi get_macaddr_filter $interface"));
	if ($mac_filter==0)
	{$res="NONE";}
...
if(isset($_POST['action']))
{
	...
	if($action == 0) //Action0: Save
	{
		if(isset($_POST['macaddr']) && isset($_POST['settype']))
		{
			...
			if ($macaddr != "NONE")
			{
				if($deny_or_auth == 0)
				{
					$tmp="call_qcsapi authorize_macaddr $interface ".escapeshellarg($macaddr);
				}
				else
				{
					$tmp="call_qcsapi deny_macaddr $interface ".escapeshellarg($macaddr);
				}
				exec($tmp);
			}
		}
		if(isset($_POST['cmb_macaddrfilter']))
		{
			...
			$new_filter_esc = escapeshellarg($new_filter);

			if ($mac_filter != $new_filter)
			{
				exec("call_qcsapi set_macaddr_filter $interface $new_filter_esc");
			}
		}
	}
	else if($action == 1) //Action2: Verify MAC address
	{
		...
		if(isset($_POST['macaddr']))
		{
			...
			$macaddr_esc = escapeshellarg($macaddr);

			$result = exec("call_qcsapi is_macaddr_authorized $interface $macaddr_esc");
			if (!(strpos($result, "QCS API error") === FALSE))
			{
...
if($mac_filter == 1)
{
	$result = exec("call_qcsapi get_denied_macaddr $interface 400");
}
else if($mac_filter == 2)
{
	$result = exec("call_qcsapi get_authorized_macaddr $interface 400");
}
else
{
	$result = "";

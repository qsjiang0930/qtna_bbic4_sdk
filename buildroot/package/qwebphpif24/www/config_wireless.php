...
function get_sta_proto($ssid)
{
	...
	if(!(strpos($encryption, "QCS API error") === FALSE) && $authentication != "OPENandOWEAuthentication")
		$proto="NONE";
...
function set_sta_proto($ssid,$proto)
{
	...
	elseif ($proto == "SAE")
	{
		exec("call_qcsapi SSID_set_authentication_mode wifi0 $ssid SAEAuthentication");
		exec("call_qcsapi SSID_set_proto wifi0 $ssid $WPAand11i");
		exec("call_qcsapi SSID_set_encryption_modes wifi0 $ssid AESEncryption");
	}
	elseif ($proto == "SAE-WPA-PSK")
	{
		exec("call_qcsapi SSID_set_authentication_mode wifi0 $ssid SAEandPSKAuthentication");
		exec("call_qcsapi SSID_set_proto wifi0 $ssid $WPAand11i");
		exec("call_qcsapi SSID_set_encryption_modes wifi0 $ssid AESEncryption");
	}
	elseif ($proto == "OWE")
	{
		exec("call_qcsapi SSID_set_authentication_mode wifi0 $ssid OPENandOWEAuthentication");
	}
}
...
function get_value()
{
	...
	{
		//Get PMF
		$curr_pmf=trim(shell_exec("call_qcsapi SSID_get_pmf wifi0 $curr_ssid"));
		...
		if($curr_proto == "DPP") {
			...
			if ($curr_proto != "NONE" && $curr_proto != "OWE")
			{
				...
				$curr_encryption = trim(shell_exec("call_qcsapi SSID_get_encryption_modes wifi0 $curr_ssid"));
				$curr_ssid_cpy = escape_any_characters($curr_ssid);
				$curr_psk = trim(shell_exec("call_qcsapi SSID_get_key_passphrase wifi0 \"$curr_ssid_cpy\" 0"));
				if($curr_psk == "")
					$curr_psk = trim(shell_exec("call_qcsapi SSID_get_pre_shared_key wifi0 \"$curr_ssid_cpy\" 0"));
			}
		}
	}
	else if($curr_mode == "Repeater")
	{
		...
		else
		{
			...
			if (!(strpos($curr_sta_pmf, "QCS API error") === FALSE)) {
				$curr_sta_pmf="0";
			}
		}
		//Get Current Proto and PSK
	...
	$curr_soniq=read_wireless_conf("qsteer");
	if ($curr_soniq == "")
		$curr_soniq="1";
	$pid = exec("pidof csmd");
	if($pid != "")
...
function set_value()
{
	...
	global $curr_radius_ipaddr,$curr_radius_port,$curr_shared_key,$curr_group_key_interval;
	global $curr_soniq,$curr_soniq_role;
	global $curr_ap_interface,$curr_sta_ssid,$curr_sta_pmf,$curr_sta_proto,$curr_sta_encryption,$curr_sta_psk;
	...
	if ($_POST['action'] == 0)
	{
		...
		if ($new_mode != $curr_mode)
		{
			if($curr_mode == "Station")
				$s_curr_mode = "sta";
			else if($curr_mode == "Access point")
				$s_curr_mode = "ap";
			else if($curr_mode == "Repeater")
				$s_curr_mode = "repeater";
			if($new_mode == "Station")
				$s_new_mode = "sta";
		...
		else
		{
			...
			if ($curr_mode == "Access point")
			{
				...
				if ($new_proto == "11i" || $new_proto == "WPAand11i" || $new_proto == "WPA2-EAP" || $new_proto == "11i_pmf"|| $new_proto == "SAE" || $new_proto == "SAE-WPA-PSK")
				{
			...
			else if ($curr_mode == "Station")
			{
				if ($new_proto == "DPP") {
					...
					if ($new_pmf!=$curr_pmf)
					{
						$new_ssid_esc = escapeshellarg($new_ssid);
						exec("call_qcsapi update_persistent_param wifi0 pmf $new_pmf_esc");
				...
				if ($new_bw!=$curr_bw)
				{
					exec("iwpriv wifi0 setparam 71 $new_bw_esc");
					write_wireless_conf("bw",$new_bw);
				}
			}
			else if ($curr_mode == "Repeater")
			{
				...
				if ($new_ssid != $curr_ssid)
				{
					if ($curr_soniq_role == "master")
					{
						...
						exec("qweb set_inactive Device.WiFi.SSID.{1}.SSID \"\\\"$new_ssid_esc\\\"\"");
					}
					else
					{
						$new_ssid_esc = escape_any_characters($new_ssid);
				...
				if ($new_proto != $curr_proto)
				{
					if ($curr_soniq_role == "master")
					{
						if ($new_proto == "NONE")
						{
							exec("qweb set_inactive Device.WiFi.AccessPoint.{1}.Security.ModeEnabled \\\"None\\\"");
						}
						else if ($new_proto == "11i")
						{
							exec("qweb set_inactive Device.WiFi.AccessPoint.{1}.Security.ModeEnabled \\\"WPA2-Personal\\\"");
						}
					}
					else
					{
				...
				if (strcmp($new_psk, $curr_psk) != 0 && $new_proto != "DPP")
				{
					if ($curr_soniq_role == "master")
					{
						...
						if($new_is_psk == 1)
							exec("qweb set_inactive Device.WiFi.AccessPoint.{1}.Security.PreSharedKey \"\\\"$new_psk_esc\\\"\"");
						else
							exec("qweb set_inactive Device.WiFi.AccessPoint.{1}.Security.KeyPassphrase \"\\\"$new_psk_esc\\\"\"");
					}
					else
					{
...
var curr_sta_saegroups = "<?php echo $curr_sta_saegroups; ?>";
var curr_sta_owegroups = "<?php echo $curr_sta_owegroups; ?>";
...
function populate_sta_encryptionlist(sta_pmf_value)
{
	...
	if (mode == "Repeater")
	{
		if (sta_pmf_value != "0")
		{
			...
			cmb_sta_encryption.options[5].text = "DPP"; cmb_sta_encryption.options[5].value = "DPP";
		}
		else
		{
			...
			if (curr_region != "us")
			{
				cmb_sta_encryption.options.length = 4;
				cmb_sta_encryption.options[2].text = "WPA2 + WPA (mixed mode)"; cmb_sta_encryption.options[2].value = "WPAand11i";
				cmb_sta_encryption.options[3].text = "DPP"; cmb_sta_encryption.options[3].value = "DPP";
			}
			else {
				cmb_sta_encryption.options.length = 3;
				cmb_sta_encryption.options[2].text = "DPP"; cmb_sta_encryption.options[2].value = "DPP";
			}
		}
	}
	var encryption_in_list = false;
...
function modechange(obj)
{
	...
	if(obj.name == 'cmb_encryption')
	{
		...
		else
		{
			...
			if (proto != "NONE" && proto != "OWE" && mode != "Station") {
				set_visible('tr_group_key_interval', true);
			}
			if(mode == "Access point" || mode == "Repeater") {
			...
			if (mode == "Station") {
				...
				if (proto == "DPP") {
					...
					set_visible('dpp_field', true);
					set_visible('dpp_qrcode_field', true);
					if(document.mainform.cmb_dpp_role.selectedIndex == 1) {
						set_visible('dpp_enrollee_field', true);
					}
					else if(curr_dpp_qrcode == "") {
						set_visible('dpp_qrcode_field', false);
	...
	if (action_name==0)//Save Button
	{
		...
		if(mode == cmb_devicemode.value )
		{
			...
			if(cmb_encryption_value != "DPP") {
				...
				{
					if ((cmb_encryption_value != "NONE" && cmb_encryption_value != "OWE" && cmb_encryption_value != "DPP") && (mode == "Access point" || mode == "Repeater"))
					{
			...
			if((mode=="Access point" || mode=="Repeater") && cmb_devicemode.selectedIndex=="0" && dtim.value == "")
			{
...
function onload_event()
{
	...
	if (curr_proto == "NONE" || curr_proto == "" || curr_proto == "WPA2-EAP" || curr_proto =="WPAand11i-EAP" || curr_proto == "OWE" || curr_proto == "DPP")
	{
		set_visible('tr_passphrase', false);
	}
	if (curr_proto == "WPA2-EAP" || curr_proto == "WPAand11i-EAP")

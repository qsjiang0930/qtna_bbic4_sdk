...
function get_value()
{
	...
	if (!(strpos($curr_11u_status, "QCS API error") === FALSE))
		$curr_11u_status="0";
	//Get Internet Access
	...
	if (!(strpos($curr_ip_addr_avail, "QCS API error") === FALSE))
	{
		$curr_ipv4 = "";
		$curr_ipv6 = "";
	}
	...
	if (!(strpos($curr_domain, "QCS API error") === FALSE))
		$curr_domain="";

	//Get Nai Realm
	$nai_realm=trim(shell_exec("call_qcsapi get_nai_realms wifi0"));
        if (!(strpos($nai_realm, "QCS API error") === FALSE))
        {
                $nai_realm_arr_len = 0;
        }
        else
	...
	if (!(strpos($curr_venue_gr, "QCS API error") === FALSE))
	{
		$curr_venue_gr=0;
		$curr_venue_type=0;
	}
	...
	$curr_venue_name=trim(shell_exec("call_qcsapi get_venue_name wifi0"));
        if (!(strpos($curr_venue_name, "QCS API error") === FALSE))
        {
                $venue_arr_len = 0;
        }
        else
	...
	$cell_net_list=trim(shell_exec("call_qcsapi get_80211u_params wifi0 anqp_3gpp_cell_net"));
        if (!(strpos($cell_net_list, "QCS API error") === FALSE))
        {
                $cell_net_arr_len = 0;
        }
        else
...
function set_value()
{
	...
	if ($_POST['action'] == 0)
	{
		...
		$new_11u_status_esc = escapeshellarg($new_11u_status);
		$new_internet_access_esc = escapeshellarg($new_internet_access);
		$new_net_type_esc = escapeshellarg($new_net_type);
		$new_net_auth_type_esc = escapeshellarg($new_net_auth_type);
		$new_hessid_esc = escapeshellarg($new_hessid);
		$new_comeback_delay_esc = escapeshellarg($new_comeback_delay);
		$new_ipv4_esc = escapeshellarg($new_ipv4);
		$new_ipv6_esc = escapeshellarg($new_ipv6);
		$new_domain_esc = escapeshellarg($new_domain);
		$new_venue_gr_esc = escapeshellarg($new_venue_gr);
		$new_venue_type_esc = escapeshellarg($new_venue_type);


		//Set 11u Status
		if ($new_11u_status != $curr_11u_status)
		{
			exec("call_qcsapi set_interworking wifi0 $new_11u_status_esc");
		}
		//Set Internet Access
		if ($new_internet_access != $curr_internet_access)
		{
			exec("call_qcsapi set_80211u_params wifi0 internet $new_internet_access_esc");
		}
		//Set Network Type
		if ($new_net_type != $curr_net_type)
		{
			exec("call_qcsapi set_80211u_params wifi0 access_network_type $new_net_type_esc");
		}
		//Set Network Auth Type
		if ($new_net_auth_type != $curr_net_auth_type)
		{
			 exec("call_qcsapi set_80211u_params wifi0 network_auth_type $new_net_auth_type_esc");
		}
		//Set Hessid
		if ($new_hessid != $curr_hessid)
		{
			exec("call_qcsapi set_80211u_params wifi0 hessid $new_hessid_esc");
		}
		...
		{
			exec("call_qcsapi set_80211u_params wifi0 gas_comeback_delay $new_comeback_delay_esc");
		}
		...
		if ($add_roaming == 1) {
			for($i = 0; $i < $count; $i++) {
				...
				exec("call_qcsapi add_roaming_consortium wifi0 $add_val_esc");
			}
		}
		...
		if ($del_roaming == 1) {
			for($i = 0; $i <= $count; $i++) {
				if ($chkbox_arr[$i] == "true")
				{
					...
					exec("call_qcsapi del_roaming_consortium wifi0 $del_val_esc");
				}
			}
		}
		...
		{
			exec("call_qcsapi set_80211u_params wifi0 ipaddr_type_availability $new_ipv4_esc $new_ipv6_esc");
		}
		//Set Domain Name
		if ($new_domain != $curr_domain)
		{
			exec("call_qcsapi set_80211u_params wifi0 domain_name $new_domain_esc");
		}
		...
		if ($del_nai_realm == 1) {
			for($i = 0; $i <= $nai_realm_count; $i++) {
				if ($nai_realm_chkbox_arr[$i] == "true")
				{
					...
					$eap_auth_list = substr_replace($eap_auth_list, "", -1);
					exec("call_qcsapi del_nai_realm wifi0 $realm_val[0],$realm_val[1],$eap_auth_list");
				}
			}
		}
		//Set Venue Group
                ...
                {
                        exec("call_qcsapi set_80211u_params wifi0 venue_group $new_venue_gr_esc");
                }
                //Set Venue Type
                if ($new_venue_type != $curr_venue_type)
                {
                        exec("call_qcsapi set_80211u_params wifi0 venue_type $new_venue_type_esc");
                }
...
<div class="container">
	...
	<div class="right">
		<div class="righttop">5G WI-FI - 802.11u</div>

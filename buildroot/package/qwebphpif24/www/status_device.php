...
{
	global $curr_mode;
	if((strpos($curr_mode, "API error") === FALSE))
	{
		if($curr_mode == "Repeater"){
            		$res="[&nbsp;] Access Point (AP)&nbsp;&nbsp;[&nbsp;] Station (STA)&nbsp;&nbsp;<a style=\"font-weight:bold\">[X] Repeater </a>";
        	}
        	else if($curr_mode=="Station")
		{
			$res="[&nbsp;] Access Point (AP)&nbsp;&nbsp;<a style=\"font-weight:bold\">[X] Station (STA) </a>&nbsp;&nbsp;[&nbsp;] Repeater ";
		}
		else
		{
			$res="<a style=\"font-weight:bold\">[X] Access Point (AP)</a>&nbsp;&nbsp;[&nbsp;] Station (STA)&nbsp;&nbsp;[&nbsp;] Repeater ";
		}
	}
	else
	{$res="";}
...
{
	$info=trim(exec("qweconfig get mode.wlan1"));
	if($info == "0")
	{
		$res="<a style=\"font-weight:bold\">[X] Access Point</a>&nbsp;&nbsp;[&nbsp;] Station";
	}
	else if($info == "1")
	{
		$res="[&nbsp;] Access Point&nbsp;&nbsp;<a style=\"font-weight:bold\">[X] Station</a>";
	}
	else
	{$res="";}
	return $res;

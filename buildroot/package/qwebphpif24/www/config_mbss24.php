	...
	if ($_POST['action'] == "1")
	{
		...
		if ($new_chk != $chk)
		{
			...
			if ($new_chk == "1")
			{
				...
				if($new_proto!=$proto)
				{
					exec("qweconfig set encryption.$interface_id.wlan1 $new_proto_esc");
				}
				if($new_psk!=$psk)
				{
				...
				if($new_br!=$br)
				{
					exec("qweconfig set broadcastssid.$interface_id.wlan1 $new_br_esc");
				}
			}
			else
			{
...
function onload_event()
{
	...
	set_control_value('txt_ssid', decodeURIComponent("<?php echo rawurlencode($ssid); ?>"), 'text');
	set_control_value('cmb_proto', proto, 'combox');
	set_control_value('txt_psk', decodeURIComponent("<?php echo rawurlencode($psk); ?>"), 'text');

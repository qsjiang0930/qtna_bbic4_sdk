...
#define IW_PRINTBUF_SZ        256

struct iwevent_net_s {
	int  iw_s; /*Socketdescriptor*/
	char iw_buffer[ETHER_MAX_LEN];
	int  iw_is_sendto_pktlogger;
	int  iw_is_sendto_syslog;
};
static struct iwevent_net_s iwval;
...
static void
print_buf(const char *fmt, ...)
{
	va_list ap;
	char buffer[IW_PRINTBUF_SZ + 2] = { 0 };    // +2 for additional \n and \0 chars

	va_start(ap, fmt);
	vsnprintf(buffer, sizeof(buffer), fmt, ap);
	va_end(ap);

	if (iwval.iw_is_sendto_pktlogger == 0 && iwval.iw_is_sendto_syslog == 0) {
		printf("%s", buffer);
	} else {
		unsigned int free_space = 0;
		unsigned int bytes_to_write = 0;

		free_space = sizeof(iwval.iw_buffer) - strlen(iwval.iw_buffer) - 1;
		bytes_to_write = strlen(buffer);
		if (bytes_to_write > free_space)
			bytes_to_write = free_space;

		strncat(iwval.iw_buffer, buffer, bytes_to_write);
	}
}
		  ...
		  int			has_range)
{
  ...
  switch(event->cmd)
	...
	print_buf("Set NWID:off/any\n");
      else
	print_buf("Set NWID:%X\n", event->u.nwid.value);
      break;
    case SIOCSIWFREQ:
    case SIOCGIWFREQ:
      {
	...
	if(event->u.essid.flags)
	  {
	    ...
	    if((event->u.essid.flags & IW_ENCODE_INDEX) > 1)
	      print_buf("%s ESSID:\"%s\" [%d]\n", prefix, essid,
		     (event->u.essid.flags & IW_ENCODE_INDEX));
	    else
	      print_buf("%s ESSID:\"%s\"\n", prefix, essid);
	  }
	else
	  print_buf("%s ESSID:off/any\n", prefix);
      }
      break;
    case SIOCSIWENCODE:
      {
	...
	if(event->u.data.flags & IW_ENCODE_DISABLED)
	...
	else
	  {
	    ...
	    if((event->u.data.flags & IW_ENCODE_INDEX) > 1)
	      print_buf(" [%d]", event->u.data.flags & IW_ENCODE_INDEX);
	    if(event->u.data.flags & IW_ENCODE_RESTRICTED)
	      print_buf("   Security mode:restricted");
	    if(event->u.data.flags & IW_ENCODE_OPEN)
	      print_buf("   Security mode:open");
	    print_buf("\n");
	  }
      }
      break;
    ...
    case SIOCGIWAP:
      print_buf("New Access Point/Cell address:%s\n",
	     iw_sawap_ntop(&event->u.ap_addr, buffer));
      break;
    case SIOCGIWSCAN:
      print_buf("Scan request completed\n");
      break;
    case IWEVTXDROP:
      print_buf("Tx packet dropped:%s\n",
    ...
    case IWEVCUSTOM:
      {
		...
		if ((event->u.data.pointer) && (event->u.data.length)) {
			...
			bytes_to_copy = event->u.data.length;
			if (bytes_to_copy > (sizeof(custom) - 1))
				bytes_to_copy = sizeof(custom) - 1;
			memcpy(custom, event->u.data.pointer, bytes_to_copy);
		}
		print_buf("Custom driver event:%s\n", custom);
	}
      break;
    case IWEVREGISTERED:
      print_buf("Registered node:%s\n",
	     iw_saether_ntop(&event->u.addr, buffer));
      break;
    case IWEVEXPIRED:
      print_buf("Expired node:%s\n",
	     iw_saether_ntop(&event->u.addr, buffer));
      break;
    case SIOCGIWTHRSPY:
      {
	...
	if((event->u.data.pointer) && (event->u.data.length))
	  {
	    ...
	    print_buf("Spy threshold crossed on address:%s\n",
	  ...
	  print_buf("Invalid Spy Threshold event\n");
      }
      break;
      /* ----- driver WPA events ----- */
      ...
      if(event->u.data.length >= sizeof(struct iw_michaelmicfailure))
	{
	  ...
	  print_buf("Michael MIC failure flags:0x%X src_addr:%s tsc:%s\n",
		 mf.flags,
      ...
      break;
    case IWEVASSOCREQIE:
      print_buf("Association Request IEs:%s\n",
	     iw_hexdump(buffer, sizeof(buffer),
			event->u.data.pointer, event->u.data.length));
      break;
    case IWEVASSOCRESPIE:
      print_buf("Association Response IEs:%s\n",
	     iw_hexdump(buffer, sizeof(buffer),
			event->u.data.pointer, event->u.data.length));
      break;
    case IWEVPMKIDCAND:
      if(event->u.data.length >= sizeof(struct iw_pmkid_cand))
	{
	  ...
	  print_buf("PMKID candidate flags:0x%X index:%d bssid:%s\n",
    ...
    case SIOCGIWRATE:
      ...
      print_buf("New Bit Rate:%s\n", buffer);
      break;
    case SIOCGIWNAME:
      print_buf("Protocol:%-1.16s\n", event->u.name);
      break;
    case IWEVQUAL:
      {
    ...
    default:
      print_buf("(Unknown Wireless event 0x%04X)\n", event->cmd);
    }	/* switch(event->cmd) */

  return(0);
}

static void iw_send_to_pktlogger(void)
{
...
#define QDRV_NETLINK_PKTLOGGER 30
	sock_fd = socket(PF_NETLINK, SOCK_RAW, QDRV_NETLINK_PKTLOGGER);
	if (sock_fd < 0) {
	...
	nlh = calloc(1, NLMSG_SPACE(ETHER_MAX_LEN));
	if (nlh == NULL) {
		close(sock_fd);
		return;
	}
	buffer_len = strlen(iwval.iw_buffer);
	memcpy(NLMSG_DATA(nlh), iwval.iw_buffer, buffer_len);
	nlh->nlmsg_pid = getpid();
	nlh->nlmsg_len = buffer_len;
#define QDRV_NETDEBUG_TYPE_IWEVENT 5
	nlh->nlmsg_type = QDRV_NETDEBUG_TYPE_IWEVENT;
	iov.iov_base = (void *)nlh;
	iov.iov_len = NLMSG_SPACE(buffer_len);
...
static void iw_send_syslog(void)
{
	static char temp_buffer[sizeof(iwval.iw_buffer) + 1];
	unsigned int buffer_len;

	buffer_len = strlen(iwval.iw_buffer);
	memcpy(temp_buffer, iwval.iw_buffer, buffer_len);
	temp_buffer[buffer_len] = '\n';
	temp_buffer[buffer_len + 1] = 0;
	syslog(LOG_NOTICE, "%s", temp_buffer);
}
		   ...
		   int		len)
{
  ...
  iw_init_event_stream(&stream, data, len);
	do {
		...
		{
			if (i++ == 0)
				print_buf("%s   %-8.16s ", buffer, wireless_data->ifname);
			else
				print_buf("                           ");
			if (ret > 0)
				print_event_token(&iwe, &wireless_data->range,
							wireless_data->has_range);
			else
				print_buf("(Invalid event)\n");
			/* Push data out *now*, in case we are redirected to a pipe */
			fflush(stdout);

			if (iwval.iw_is_sendto_syslog)
				iw_send_syslog();

			if (iwval.iw_is_sendto_pktlogger)
				iw_send_to_pktlogger();

			if (iwval.iw_is_sendto_pktlogger != 0 || iwval.iw_is_sendto_syslog != 0)
				memset(iwval.iw_buffer, 0, sizeof(iwval.iw_buffer));
		}
	}
  while(ret > 0);
...
iw_usage(int status)
{
  fputs("Usage: iwevent [OPTIONS]\n"
	...
	"     -p,--pktlogger Send output to pktlogger.\n",
	status ? stderr : stdout);
  exit(status);
}
/* Command line options */
static const struct option long_opts_iwevent[] = {
  { "help", no_argument, NULL, 'h' },
  { "version", no_argument, NULL, 'v' },
  { "syslog", no_argument, NULL, 's' },
  { "pktlogger", no_argument, NULL, 'p' },
  { NULL, 0, NULL, 0 }
};
     ...
     char *	argv[])
{
  ...
  memset(&iwval, 0, sizeof(iwval));

  /* Check command line options */
  while((opt = getopt_long(argc, argv, "hvsp", long_opts_iwevent, NULL)) > 0)
    {
      switch(opt)
	{
	case 'h':
	...
	case 's':
	  /* open syslog */
	  iwval.iw_is_sendto_syslog = 1;
	  openlog("iwevent", LOG_CONS, LOG_DAEMON);
	  break;

	case 'p':
	  iwval.iw_is_sendto_pktlogger = 1;
	  break;

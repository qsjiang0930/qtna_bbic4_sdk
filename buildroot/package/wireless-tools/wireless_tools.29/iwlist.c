...
/* Values for the IW_IE_WFA_KEY_MGMT_* in GENIE */
static const char *iw_ie_wfa_key_mgmt_name[] = {
	"unknown",
	"unknown",
	"DPP",
};
#define	IW_IE_WFA_KEY_MGMT_NUM	IW_ARRAY_LEN(iw_ie_wfa_key_mgmt_name)
		...
		int		buflen)
{
  ...
  for(i = 0; i < cnt; i++)
  {
	if (memcmp(&iebuf[offset], wpa_oui, 3) == 0)
		...
		iw_print_value_name(iebuf[offset+3],
			iw_ie_wfa_key_mgmt_name, IW_IE_WFA_KEY_MGMT_NUM);
	else
		printf(" Proprietary");
  ...
  while(1)
	...
	realloc:
	  ...
	  if(iw_get_ext(skfd, ifname, SIOCGIWSCAN, &wrq) < 0)
		...
		{
		  if (buflen >= MAX_SCAN_BUFFER_LEN) {
		    printf("Found scan results over the limit. "
				    "Scan result would be truncated\n");
		    break;
		  }

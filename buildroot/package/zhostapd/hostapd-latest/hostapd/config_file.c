...
static int hostapd_config_default_vlan(struct hostapd_bss_config *bss)
{
	struct hostapd_vlan *vlan;

	if (bss->ssid.dynamic_vlan == DYNAMIC_VLAN_DISABLED)
		return 0;

	vlan = os_malloc(sizeof(*vlan));
	if (vlan == NULL) {
		wpa_printf(MSG_ERROR, "Out of memory while create default VLAN group\n");
		return -1;
	}

	os_memset(vlan, 0, sizeof(*vlan));
	vlan->vlan_id = VLAN_ID_WILDCARD;
	os_strlcpy(vlan->ifname, "vlan#", sizeof(vlan->ifname));
	vlan->next = bss->vlan;
	bss->vlan = vlan;
...
#ifdef CONFIG_QTNA_WIFI
	if (conf->num_bss == g_max_bssid) {
		wpa_printf(MSG_ERROR, "Maximum BSS count (%d) reached",
				g_max_bssid);
		return -1;
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	/* If bss->nai_realm_count is 0, os_calloc return NULL in Quantenna
	 * platform. This function was written assuming that os_calloc returns
	 * pointereven when count is zero. */
	offsets = NULL;
	if (bss->nai_realm_count) {
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
static int parse_multiap_backhaul_bss_wpa_psk(struct hostapd_bss_config *bss, char *auth)
{
	if (os_strcmp(auth, "WPA-PSK") == 0)
		bss->multiap_backhaul_bss.wpa_psk = WPS_AUTH_WPAPSK;
	else if (os_strcmp(auth, "WPA2-PSK") == 0)
		bss->multiap_backhaul_bss.wpa_psk = WPS_AUTH_WPA2PSK;
	else
		return -1;
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
...
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(buf, "accept_oui_file") == 0) {
		if (hostapd_config_read_maclist(pos, &bss->accept_oui,
						&bss->num_accept_oui)) {
			wpa_printf(MSG_ERROR, "Line %d: Failed to read deny_mac_file '%s'",
					line, pos);
			return 1;
...
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(buf, "intra_bss_isolate") == 0) {
		bss->intra_bss_isolate = atoi(pos);
	} else if (os_strcmp(buf, "bss_isolate") == 0) {
		bss->bss_isolate = atoi(pos);
#endif /* CONFIG_QTNA_WIFI */
		...
		if (bss->max_num_sta < 0 ||
			...
			bss->set_assoc_limit_required = 1;
		}
	} else if (os_strcmp(buf, "total_assoc_limit") == 0) {
		conf->total_assoc_limit = atoi(pos);
		if (conf->total_assoc_limit < 0 ||
			conf->total_assoc_limit > MAX_STA_COUNT) {
			wpa_printf(MSG_ERROR, "Line %d: Invalid total_assoc_limit=%d",
					line, conf->total_assoc_limit);
			return 1;
		}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
		if (qtn_hapd_cfg_wpa_passphrase(bss, pos, 0, line))
			return 1;
#else
		int len = os_strlen(pos);
		if (len < 8 || len > 63) {
...
#ifdef CONFIG_QTNA_WIFI
		if (qtn_hapd_cfg_wpa_psk(bss, pos, 0, line))
			return 1;
#else
		hostapd_config_clear_wpa_psk(&bss->ssid.wpa_psk);
	...
	} else if (os_strcmp(buf, "ieee80211r") == 0) {
		if (atoi(pos)) {
			if (bss->wpa_key_mgmt == WPA_KEY_MGMT_PSK)
				bss->wpa_key_mgmt |= WPA_KEY_MGMT_FT_PSK;
			if (bss->wpa_key_mgmt == WPA_KEY_MGMT_IEEE8021X)
				bss->wpa_key_mgmt |= WPA_KEY_MGMT_FT_IEEE8021X;
			if (bss->wpa_key_mgmt == WPA_KEY_MGMT_SAE)
				bss->wpa_key_mgmt |= WPA_KEY_MGMT_FT_SAE;
			if (bss->wpa_key_mgmt == (WPA_KEY_MGMT_PSK | WPA_KEY_MGMT_SAE))
				bss->wpa_key_mgmt |= (WPA_KEY_MGMT_FT_PSK | WPA_KEY_MGMT_FT_SAE);
		} else {
			bss->wpa_key_mgmt &= ~WPA_KEY_MGMT_FT_PSK;
			bss->wpa_key_mgmt &= ~WPA_KEY_MGMT_FT_IEEE8021X;
			bss->wpa_key_mgmt &= ~WPA_KEY_MGMT_FT_SAE;
...
#ifdef CONFIG_QTNA_WIFI
		os_free(conf->hapd_ctrl_interface);
		conf->hapd_ctrl_interface = os_strdup(pos);
#endif
...
#ifdef CONFIG_QTNA_WIFI
		hostapd_config_default_vlan(bss);
#endif /* CONFIG_QTNA_WIFI */
		...
		bss->okc = atoi(pos);
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(buf, "pairing_enable") == 0) {
		if (atoi(pos) >= 0 && atoi(pos) <= 2) {
			bss->pairing_enable = atoi(pos);
		} else {
			wpa_printf(MSG_ERROR, "Line %d: pairing_enable not right", line);
			return 1;
		}
	} else if (os_strcmp(buf, "pairing_id") == 0) {
		if (os_strlen(pos) > PAIRING_ID_MAX_LEN) {
			wpa_printf(MSG_ERROR, "Line %d: too long pairing_id", line);
			return 1;
		}
		os_free(bss->pairing_id);
		bss->pairing_id = os_strdup(pos);
#endif /* CONFIG_QTNA_WIFI */
#ifdef CONFIG_WPS
...
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(buf, "multiap_backhaul_bss_ssid") == 0) {
		bss->multiap_backhaul_bss.ssid_len = os_strlen(pos);
		if (bss->multiap_backhaul_bss.ssid_len > SSID_MAX_LEN ||
				bss->ssid.ssid_len < 1) {
			wpa_printf(MSG_ERROR, "Line %d: invalid SSID '%s'",
					line, pos);
			return 1;
		}
		os_memcpy(bss->multiap_backhaul_bss.ssid, pos,
				bss->multiap_backhaul_bss.ssid_len);
	} else if (os_strcmp(buf, "multiap_backhaul_bss_wpa_psk") == 0) {
		if (parse_multiap_backhaul_bss_wpa_psk(bss, pos) < 0) {
			wpa_printf(MSG_ERROR, "Line %d: invalid multiap_backhaul_bss_wpa_psk '%s'",
					line, pos);
			return 1;
		}
	} else if (os_strcmp(buf, "multiap_backhaul_bss_wpa_passphrase") == 0) {
		int len = os_strlen(pos);

		if (len < MIN_PASSPHRASE_LEN || len > MAX_PASSPHRASE_LEN) {
			wpa_printf(MSG_ERROR,
				"Line %d: invalid multiap_backhaul_bss_wpa_passphrase len %d",
				line, len);
			return 1;
		}
		os_memcpy(bss->multiap_backhaul_bss.wpa_passphrase, pos, os_strlen(pos));
	} else if (os_strcmp(buf, "wps_cred_processing_multiap_backhaul") == 0) {
		bss->wps_cred_processing_multiap_backhaul = atoi(pos);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(buf, "wps_pp_devname") == 0) {
		if (os_strlen(pos) > MAX_WPS_PP_DEVICE_NAME_LEN) {
			wpa_printf(MSG_ERROR, "Line %d: Too long wps_pp_devname", line);
			return 1;
		} else {
			os_free(bss->wps_pp_devname);
			if (os_strlen(pos) == 0)
				bss->wps_pp_devname = NULL;
			else
				bss->wps_pp_devname = os_strdup(pos);
		}
	} else if (os_strcmp(buf, "wps_pp_devname_blacklist") == 0) {
		if (os_strlen(pos) > MAX_WPS_PP_DEVICE_NAME_LEN) {
			wpa_printf(MSG_ERROR, "Line %d: Too long wps_pp_devname", line);
			return 1;
		} else {
			os_free(bss->wps_pp_devname_bl);
			if (os_strlen(pos) == 0)
				bss->wps_pp_devname_bl = NULL;
			else
				bss->wps_pp_devname_bl = os_strdup(pos);
		}
	} else if (os_strcmp(buf, "wps_pp_enable") == 0) {
		if (atoi(pos) == 0)
			bss->pp_enable = WPS_PP_DISABLE;
		else
			bss->pp_enable = WPS_PP_ENABLE;
#endif /* CONFIG_QTNA_WIFI */
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
...
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(buf, "sae_max_auth_attempts") == 0) {
		bss->sae_max_auth_attempts = atoi(pos);
	} else if (os_strcmp(buf, "sae_auth_lockout_period") == 0) {
		bss->sae_auth_lockout_period = atoi(pos);
	} else if (os_strcmp(buf, "sae_auth_monitor_period") == 0) {
		bss->sae_auth_monitor_period = atoi(pos);
#endif /* CONFIG_QTNA_WIFI */
#endif /* CONFIG_SAE */
...
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(buf, "wps_vendor_spec") == 0) {
		if (strlen(pos) == 0)
			bss->wps_vendor_spec = NULL;
		else
			bss->wps_vendor_spec = os_strdup(pos);
	} else if (os_strcmp(buf, "ap_pin_fail_method") == 0) {
		bss->wps_lockdown = WPS_LOCKDOWN_DEFAULT;
		if (os_strcmp(pos, "auto_lockdown") == 0)
			bss->wps_lockdown = WPS_LOCKDOWN_AUTO;
		else if (os_strcmp(pos, "default") == 0)
			bss->wps_lockdown = WPS_LOCKDOWN_DEFAULT;
		else
			wpa_printf(MSG_WARNING, "Line %d: unknown ap pin "
					"fail method '%s', use Default", line, pos);
	} else if (os_strcmp(buf, "auto_lockdown_max_retry") == 0) {
		bss->auto_ld_max_retry = atoi(pos);
		if (bss->auto_ld_max_retry < 0) {
			wpa_printf(MSG_ERROR, "Line %d: Invalid "
					"auto_lockdown_max_retry value '%s'", line, pos);
			return 1;
		}
	} else if (os_strcmp(buf, "wps_allow_pbc_overlap") == 0) {
		int i;

		if (atoi(pos) != 0) {
			for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++) {
				if (bss->wps_vendor_ext[i] == NULL)
					break;
			}
			if (i == MAX_WPS_VENDOR_EXTENSIONS) {
				wpa_printf(MSG_ERROR, "Line %d: WPS vendor extension full", line);
				return 1;
			}
			bss->wps_vendor_ext[i] = wpabuf_alloc(6);
			if (bss->wps_vendor_ext[i] == NULL) {
				wpa_printf(MSG_ERROR, "Line %d: fail to allocate "
						"WPS vendor extension buffer", line);
				return 1;
			}
			wpabuf_put_be24(bss->wps_vendor_ext[i], 0x2686);/* Quantena vendor ID */
			wpabuf_put_u8(bss->wps_vendor_ext[i], 0);	/* WPS allow PBC overlap sub ID */
			wpabuf_put_u8(bss->wps_vendor_ext[i], 1);	/* length */
			wpabuf_put_u8(bss->wps_vendor_ext[i], 1);	/* 1 - enable; 0 - disable */
		}
	} else if (os_strcmp(buf, "pbc_detect_enhance") == 0) {
		bss->pbc_detect_enhance = atoi(pos);
	} else if (os_strcmp(buf, "pbc_in_srcm") == 0) {
		conf->pbc_in_srcm = atoi(pos);
#if 0
	} else if (os_strcmp(buf, "qlink_iface") == 0) {
		os_strlcpy(conf->bss[0]->qlink_iface, pos,
		sizeof(conf->bss[0]->qlink_iface));
	} else if (os_strcmp(buf, "qlink_server_addr") == 0) {
		if (hwaddr_aton(pos, bss->qlink_server_addr)) {
			wpa_printf(MSG_ERROR, "Invalid MAC address '%s' at line %d", pos, line);
			return 1;
		}
		if ((bss->qlink_server_addr[0] & 0x01) != 0) {
			wpa_printf(MSG_ERROR, "Invalid MAC address '%s' at "
					"line %d - multicast address is not allowed",
			pos, line);
			return 1;
		}
		if (is_zero_ether_addr(bss->qlink_server_addr)) {
			wpa_printf(MSG_ERROR, "Invalid MAC address '%s' at "
					"line %d - zero address is not allowed",
			pos, line);
			return 1;
		}
#endif
		...
		val = atoi(pos);
		if (val == 1) {
			g_max_bssid = MAX_BSSID - 1;
		} else if (val == 0) {
			g_max_bssid = MAX_BSSID;
		} else {
			wpa_printf(MSG_ERROR, "Invalid repeater parameter '%s' at"
				"line %d", pos, line);
			return 1;
		}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_HS20
	/*
	 * The following settings are required for Hotspot 2.0 according to section 4.1
	 * of the Wi-Fi Certified Passpoint Release 1 Interoperability Test Plan v1.0.5.
	 */
	if (bss->hs20) {
		bss->p2p |= P2P_MANAGE;
		bss->p2p &= ~P2P_ALLOW_CROSS_CONNECTION;

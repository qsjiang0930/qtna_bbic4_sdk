...
static int qtn_cli_cmd_reconfigure(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	return wpa_ctrl_command(ctrl, "RECONFIGURE");
}


static int qtn_cli_cmd_wps_pin_bss(struct wpa_ctrl *ctrl, int argc,
				   char *argv[])
{
	char buf[256];
	if (argc < 3) {
		printf("Invalid 'wps_pin_bss' command - at least three arguments, "
		       "interface, UUID and PIN, are required.\n");
		return -1;
	}
	if (argc > 4)
		snprintf(buf, sizeof(buf), "WPS_PIN_BSS %s %s %s %s %s",
			 argv[0], argv[1], argv[2], argv[3], argv[4]);
	else if (argc > 3)
		snprintf(buf, sizeof(buf), "WPS_PIN_BSS %s %s %s %s",
			 argv[0], argv[1], argv[2], argv[3]);
	else
		snprintf(buf, sizeof(buf), "WPS_PIN_BSS %s %s %s", argv[0], argv[1], argv[2]);
	return wpa_ctrl_command(ctrl, buf);
}


static int qtn_cli_cmd_wps_pbc(struct wpa_ctrl *ctrl, int argc,
			   char *argv[])
{
	char cmd[256];
	int res;
	...
	if (argc == 0) {
		res = os_snprintf(cmd, sizeof(cmd), "WPS_PBC");
	} else if (argc == 1) {
		res = os_snprintf(cmd, sizeof(cmd), "WPS_PBC %s",
				  argv[0]);
	} else {
		printf("WPS_PBC [BSS name]\n");
		return -1;
	}
...
static int qtn_cli_cmd_wps_ap_pin_bss(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	char buf[64];
	if (argc < 2) {
		printf("Invalid 'wps_ap_pin_bss' command - at least two arguments "
		       "are required.\n");
		return -1;
	}
	if (argc > 3)
		snprintf(buf, sizeof(buf), "WPS_AP_PIN_BSS %s %s %s %s",
			 argv[0], argv[1], argv[2], argv[3]);
	else if (argc > 2)
		snprintf(buf, sizeof(buf), "WPS_AP_PIN_BSS %s %s %s",
			 argv[0], argv[1], argv[2]);
	else
		snprintf(buf, sizeof(buf), "WPS_AP_PIN_BSS %s %s", argv[0], argv[1]);
	return wpa_ctrl_command(ctrl, buf);
}


#ifdef CONFIG_WPS_UPNP
static int qtn_cli_cmd_wps_upnp_enable(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;
	int option;
	...
	if (argc != 1) {
		printf("Invalid WPS_UPNP_ENABLE command:needs one argument (variable "
			"1=enable 0=disable)\n");
		return -1;
	}

	option = atoi(argv[0]);
	if ((!check_str_digit(argv[0])) || (option != 0 && option != 1)) {
		printf("usage: wps_upnp <0 | 1>\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "WPS_UPNP_ENABLE %d", option);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
...
static int qtn_cli_cmd_wps_upnp_status(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	return wpa_ctrl_command(ctrl, "WPS_UPNP_STATUS");
}
#endif /* CONFIG_WPS_UPNP */

static int qtn_cli_cmd_wps_cancel(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 1) {
		printf("Invalid 'wps_cancel' command: needs one argument (variable "
		       "interface)\n");
		return -1;
	}
...
static int qtn_cli_cmd_wps_status(struct wpa_ctrl *ctrl, int argc,
                                   char *argv[])
{
        return wpa_ctrl_command(ctrl, "WPS_STATUS");
}


static int qtn_cli_cmd_wps_configured_state(struct wpa_ctrl *ctrl,
					        int argc, char *argv[])
{
	char cmd[256];

	if (argc == 0) {
		snprintf(cmd, sizeof(cmd), "WPS_CONFIGURED_STATE");
	} else if (argc == 1) {
		snprintf(cmd, sizeof(cmd), "WPS_CONFIGURED_STATE %s", argv[0]);
	} else {
		printf("Usage: hostapd_cli wps_configured_state [interface]\n");
		return -1;
	}
...
static int qtn_cli_cmd_wps_timeout(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 1) {
		printf("Invalid WPS_TIMEOUT command: needs one argument (variable "
		       "timeout_value)\n");
		return -1;
	}

	if (!check_str_digit(argv[0])) {
		printf("Invalid WPS_TIMEOUT command: invalide value %s\n", argv[0]);
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "WPS_TIMEOUT %s", argv[0]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long wps_timeout command.\n");
		return -1;
	}
...
static int qtn_cli_cmd_wps_pbc_in_srcm(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int option;
	int res;
	...
	if (argc != 1) {
		printf("usage: wps_pbc_in_srcm <0 | 1>\n");
		return -1;
	}
	...
	res = os_snprintf(cmd, sizeof(cmd), "WPS_PBC_IN_SRCM %d", option);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long wps_pbc_in_srcm command.\n");
		return -1;
	}
...
static int qtn_cli_cmd_wps_ignore_hw_pbc(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;
	int option;
	...
	if (argc != 1) {
		printf("Invalid IGNORE_HW_PBC command: needs one argument (variable "
			"1=ignore 0=accept)\n");
		return -1;
	}
	...
	option = atoi(argv[0]);
	if ((!check_str_digit(argv[0])) || (option != 0 && option != 1)) {
		printf("usage: wps_ignore_hw_pbc <1 | 0>\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "IGNORE_HW_PBC %d", option);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long parameter.\n");
		return -1;
	}
...
static int qtn_cli_cmd_update_bss(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char buf[64];

	if (argc == 0) {
		printf("Invalid 'update_bssconfig' command - need to specify the name"
		       "of BSS.\n");
		return -1;
	}

	if (snprintf(buf, sizeof(buf), "UPDATE_BSSCONFIG %s", argv[0]) > sizeof(buf)) {
...
static int qtn_cli_cmd_create_bss(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char buf[64];
	int offset;

	if (argc == 0) {
		printf("Invalid 'create_bssconfig' command - need to specify the name"
		       "of BSS.\n");
		return -1;
	}

	if (snprintf(buf, sizeof(buf), "CREATE_BSSCONFIG %s", argv[0]) > sizeof(buf)) {
		printf("parameter too long\n");
		return -1;
	}

	if (argc > 1) {
		if (os_strcmp(argv[1], "disabled") == 0) {
			offset = os_strlen(buf);
			if (snprintf(buf + offset, sizeof(buf) - offset, " %s", argv[1])
					> sizeof(buf) - offset) {
				printf("parameter too long\n");
				return -1;
			}
		} else {
			printf("Invalid 'create_bssconfig' command parameter.\n");
			return -1;
		}
	}
...
static int qtn_cli_cmd_remove_bss(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char buf[64];

	if (argc == 0) {
		printf("Invalid 'remove_bssconfig' command - need to specify the name"
		       "of BSS.\n");
		return -1;
	}

	if (snprintf(buf, sizeof(buf), "REMOVE_BSSCONFIG %s", argv[0]) > sizeof(buf)) {
		printf("parameter too long\n");
		return -1;
	}
...
static int qtn_cli_cmd_bss_set(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 3) {
		printf("Invalid BSS SET command: needs three arguments: "
		       "<param> <interface> <value>\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "BSS_SET %s %s %s", argv[0], argv[1], argv[2]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long SET command.\n");
		return -1;
	}
...
static int qtn_cli_cmd_bss_get(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 2) {
		printf("Invalid BSS GET command: needs two arguments: "
		       "<param> <interface>\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "BSS_GET %s %s", argv[0], argv[1]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long GET command.\n");
		return -1;
	}
...
static int qtn_cli_cmd_wpa_status(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 1) {
		printf("Invalid WPA_STATUS command: needs one argument (variable "
		       "mac_addr)\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "STATUS %s", argv[0]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long STATUS command.\n");
		return -1;
	}
...
static int local_cmd_level(struct wpa_ctrl *ctrl, int argc, char *argv[], char *log_type)
{
    ...
    pos = cmd;
    ret = os_snprintf(pos, end - pos, log_type);
    ...
    for (iter = 0; iter < argc; iter++) {
        ret = os_snprintf(pos, end - pos, " %s", argv[iter]);
        if (ret < 0 || ret >= end - pos) {
            printf("Too long %s command.\n", log_type);
            return -1;
        }
...
static int qtn_cli_cmd_log_level(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	return local_cmd_level(ctrl, argc, argv, "LOG_LEVEL");
}


static int qtn_cli_cmd_logger_level(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	return local_cmd_level(ctrl, argc, argv, "LOGGER_LEVEL");
}


static int qtn_cli_cmd_add_filter_mac(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 3) {
		printf("Invalid ADD_FILTER_MAC command: needs three arguments"
		       " (variable <bss_name> <accept|deny> <mac_addr>)\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "ADD_FILTER_MAC %s %s %s",
			argv[0], argv[1], argv[2]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long STATUS command.\n");
		return -1;
...
static int qtn_cli_cmd_del_filter_mac(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 3) {
		printf("Invalid DEL_FILTER_MAC command: needs three arguments"
		       " (variable <bss_name> <accept|deny> <mac_addr>)\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "DEL_FILTER_MAC %s %s %s",
			argv[0], argv[1], argv[2]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long STATUS command.\n");
		return -1;
...
static int qtn_cli_cmd_get_filter_mac(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 2) {
		printf("Invalid DEL_FILTER_MAC command: needs two arguments"
		       " (variable <bss_name> <accept|deny>)\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "GET_FILTER_MAC %s %s",
			argv[0], argv[1]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long STATUS command.\n");
		return -1;
...
static int qtn_cli_cmd_deauth_all(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	return wpa_ctrl_command(ctrl, "DEAUTH_ALL");
}


static int qtn_cli_cmd_get_max_bss_num(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc >= 1) {
		printf("Invalid get_max_bss_num command: no arguments required\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "GET_MAX_BSS_NUM");

	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long GET_MAX_BSS_NUM command.\n");
		return -1;
	}
...
#ifdef CONFIG_QTNA_WIFI
static int qtn_cli_cmd_show_sta_wpa_keyid(struct wpa_ctrl *ctrl, int argc,
					  char *argv[])
{
	return wpa_ctrl_command(ctrl, "SHOW_STA_WPA_KEYID");
}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
static struct hostapd_cli_cmd hostapd_qtn_commands[] = {
	{ "reconfigure", qtn_cli_cmd_reconfigure, NULL,
	  "= reload the configuration" },
	{ "reconfig", qtn_cli_cmd_reconfigure, NULL,
	  "= reload the configuration" },
#ifdef CONFIG_WPS
	{ "wps_pin_bss", qtn_cli_cmd_wps_pin_bss, NULL,
	  "<interface> <uuid> <pin> [timeout] [addr]\n"
	  "  = add WPS Enrollee PIN for the specified interface" },
	{ "wps_pbc", qtn_cli_cmd_wps_pbc, NULL,
	  "[BSS] = indicate button pushed to initiate PBC per-SSID" },
	{ "wps_ap_pin_bss", qtn_cli_cmd_wps_ap_pin_bss, NULL,
	  "<interface> <cmd> [params..]\n"
	  "  = enable or disable AP PIN per-SSID" },
#ifdef CONFIG_WPS_UPNP
	{ "wps_upnp_enable", qtn_cli_cmd_wps_upnp_enable, NULL,
	  "[0|1] = enable or disable upnp feature" },
	{ "wps_upnp_status", qtn_cli_cmd_wps_upnp_status, NULL,
	  "= get upnp status" },
#endif /* CONFIG_WPS_UPNP */
	{ "wps_cancel", qtn_cli_cmd_wps_cancel, NULL,
	  "<BSS> = cancel the pending WPS per-SSID" },
	{ "wps_status", qtn_cli_cmd_wps_status, NULL,
	  "= get the current status of WPS" },
	{ "wps_configured_state", qtn_cli_cmd_wps_configured_state, NULL,
	  "= get the wps configured state [diabled | not configured | configured]" },
	{ "wps_timeout", qtn_cli_cmd_wps_timeout, NULL,
	  "<val> = set the wps timeout value. The valid range is 120 to 600 seconds" },
	{ "wps_pbc_in_srcm",  qtn_cli_cmd_wps_pbc_in_srcm, NULL,
	  "[0|1] = disable or enable PBC methods in SRCM" },
	{ "ignore_hw_pbc",  qtn_cli_cmd_wps_ignore_hw_pbc, NULL, NULL},
#endif /* CONFIG_WPS */
	{ "update_bss", qtn_cli_cmd_update_bss, NULL,
	  "<BSS> = update the specified BSS according to the new configuration file" },
	{ "create_bss", qtn_cli_cmd_create_bss, NULL,
	  "<BSS> [disabled]\n"
	  "  = create the specified BSS according to the configuration file.\n"
          "    the BSS is enabled unless the argument \"disabled\" is present" },
	{ "remove_bss", qtn_cli_cmd_remove_bss, NULL,
	  "<BSS> = remove the specified BSS" },
	{ "bss_set", qtn_cli_cmd_bss_set, NULL,
	  "<param> <BSS> <value> = set the parameter per-SSID "},
	{ "bss_get", qtn_cli_cmd_bss_get, NULL,
	  "<param> <BSS> = get the parameter per-SSID" },
	{ "status", qtn_cli_cmd_wpa_status, NULL,
	  "<mac_addr> = get the wpa handshaking state" },
	{ "log_level", qtn_cli_cmd_log_level, NULL,
	  "[level] = get the current log level or set log level" },
	{ "logger_level", qtn_cli_cmd_logger_level, NULL,
	  "[level] = get the current logger level or set logger level" },
	{ "deauth_all", qtn_cli_cmd_deauth_all },
	{ "add_filter_mac", qtn_cli_cmd_add_filter_mac, NULL,
	  "<BSS> <accept|deny> <mac_addr>\n"
	  "  = add the specified mac address into accept/deny mac filter table" },
	{ "del_filter_mac", qtn_cli_cmd_del_filter_mac, NULL,
	  "<BSS> <accept|deny> <mac_addr>\n"
	  "  = delete the specified mac address from accept/deny mac filter table" },
	{ "get_filter_mac", qtn_cli_cmd_get_filter_mac, NULL,
	  "<BSS> <accept|deny>\n"
	  "  = get the list of mac addresses from accept/deny mac filter table" },
	{ "get_max_bss_num", qtn_cli_cmd_get_max_bss_num, NULL,
	  "\n"
	  "  = get the max number of BSSes one Radio can support" },
	{ "show_sta_wpa_keyid", qtn_cli_cmd_show_sta_wpa_keyid, NULL,
	  " = list stations and WPA keyids" },
	{ NULL, NULL, NULL, NULL },
};
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	fprintf(stream, "\n");
	fprintf(stream, "Quantenna specific commands:\n");
	for (n = 0; hostapd_qtn_commands[n].cmd; n++) {
		if (cmd == NULL || str_starts(hostapd_qtn_commands[n].cmd, cmd))
			print_cmd_help(stream, &hostapd_qtn_commands[n], "  ");
	}
#endif /* CONFIG_QTNA_WIFI */
}
...
#ifdef CONFIG_QTNA_WIFI
	/* quantenna commands
	 * will replace hostapd_cli commands with quantenna version if necessary
	 */
	cmd = hostapd_qtn_commands;
	while (cmd->cmd) {
		if (os_strcasecmp(cmd->cmd, argv[0]) == 0) {
			if (match != NULL)
				printf("##Replace with Quantenna versoin \"%s\"\n", match->cmd);
			match = cmd;
			count = 1;
			break;
		}
			...
			if (dir) {
				...
				while ((dent = readdir(dir))) {
					if (os_strcmp(dent->d_name, ".") == 0
						...
						continue;
#ifdef CONFIG_QTNA_WIFI
					if (!d_name || strcmp(d_name, dent->d_name) >= 0) {
						free(d_name);
						d_name = os_strdup(dent->d_name);
					}
#else

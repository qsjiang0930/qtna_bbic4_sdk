...
static int hostapd_driver_init(struct hostapd_iface *iface)
{
	...
	if (hapd->driver->get_capa &&
		...
		if (capa.hw_capab_24g)
			iface->qtna_drv_flag |= QTNA_DRIVER_FLAG_24G_CAPABLE;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
		"   -r   hostapd run in repeater mode\n"
#endif /* CONFIG_QTNA_WIFI */
...
int main(int argc, char *argv[])
{
...
#ifdef CONFIG_QTNA_WIFI
	interfaces.update_config = hostapd_update_bss;
#endif /* CONFIG_QTNA_WIFI */
	...
	for (;;) {
#ifdef CONFIG_QTNA_WIFI
		c = getopt(argc, argv, "b:Bde:f:hi:KP:sSTtu:vg:G:r");
#else

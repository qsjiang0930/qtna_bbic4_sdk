...
#ifdef CONFIG_DPP
	} else if (os_strcmp(cmd, "DPP_AUTH_ROLE") == 0) {
		hapd->dpp_auth_role = atoi(value);
	} else if (os_strcmp(cmd, "DPP_PROV_ROLE") == 0) {
		hapd->dpp_prov_role = atoi(value);
	} else if (os_strcmp(cmd, "DPP_BOOTSTRAP_METHOD") == 0) {
		hapd->dpp_bootstrap_method = atoi(value);
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_BOOTSTRAP") == 0) {
		hapd->dpp_ecc_curve_bootstrap = atoi(value);
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_CSIGN") == 0) {
		hapd->dpp_ecc_curve_csign = atoi(value);
	} else if (os_strcmp(cmd, "DPP_NETROLE") == 0) {
		hapd->dpp_netrole = atoi(value);
	} else if (os_strcmp(cmd, "DPP_AUTHDIR") == 0) {
		hapd->dpp_authdir = atoi(value);
#endif /* CONFIG_DPP */
#endif /* CONFIG_QTNA_WIFI */
	} else {
				  ...
				  char *buf, size_t buflen)
{
	...
	if (os_strcmp(cmd, "version") == 0) {
		...
		res = os_snprintf(buf, buflen, "%d", hapd->dpp_auth_role);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_PROV_ROLE") == 0) {
		res = os_snprintf(buf, buflen, "%d", hapd->dpp_prov_role);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_BOOTSTRAP_METHOD") == 0) {
		res = os_snprintf(buf, buflen, "%d", hapd->dpp_bootstrap_method);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_BOOTSTRAP") == 0) {
		res = os_snprintf(buf, buflen, "%d", hapd->dpp_ecc_curve_bootstrap);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_CSIGN") == 0) {
		res = os_snprintf(buf, buflen, "%d", hapd->dpp_ecc_curve_csign);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_NETROLE") == 0) {
		res = os_snprintf(buf, buflen, "%d", hapd->dpp_netrole);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_AUTHDIR") == 0) {
		res = os_snprintf(buf, buflen, "%d", hapd->dpp_authdir);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcasecmp(cmd, "DPP_CONFIGURATOR_PARAMS") == 0) {
		if (hapd->dpp_configurator_params) {
			res = os_snprintf(buf, buflen, "%s", hapd->dpp_configurator_params);
			if (os_snprintf_error(buflen, res))
				return -1;
		} else {
			return -1;
...
#ifdef CONFIG_QTNA_WIFI
static int qtn_ctrl_iface_show_sta_wpa_keyid(struct hostapd_data *hapd,
					     char *buf, size_t buflen)
{
	struct sta_info *sta;
	const char *keyid;
	...
	ret = os_snprintf(pos, end - pos, "MAC / key-id\n");
	if (os_snprintf_error(end - pos, ret))
		return pos - buf;
	pos += ret;
	...
	for (sta = hapd->sta_list; sta; sta = sta->next) {
		...
		ret = os_snprintf(pos, end - pos, MACSTR " %s\n",
				  MAC2STR(sta->addr), keyid);
		if (os_snprintf_error(end - pos, ret))
			return pos - buf;
...
static int hostapd_ctrl_iface_logger_level(struct hostapd_data *hapd, char *cmd,
                           char *buf, size_t buflen)
{
	char *pos, *end;
	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "force_broadcast_uuid") == 0) {
		...
		if (sel_bss->wps_third_party_band != wps_third_party_band) {
			...
			if (!sel_bss->wps || !sel_bss->wps->registrar || !hapd->conf || !hapd->iconf)
				return ret;

			if (wps_third_party_band) {
				sel_bss->wps->dev.rf_bands |= hapd->wps_third_party_band;
			} else {
				if (hapd->conf->wps_rf_bands) {
					sel_bss->wps->dev.rf_bands = hapd->conf->wps_rf_bands;
				} else {
					sel_bss->wps->dev.rf_bands =
						hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211A ?
						WPS_RF_50GHZ : WPS_RF_24GHZ; /* FIX: dualband AP */
				}
			}
			if ((sel_bss->wps->dev.rf_bands & (WPS_RF_50GHZ | WPS_RF_24GHZ))
					== (WPS_RF_50GHZ | WPS_RF_24GHZ))
				wps_registrar_set_dualband(sel_bss->wps->registrar, 1);
			else
				wps_registrar_set_dualband(sel_bss->wps->registrar, 0);
			ret = wps_registrar_update_ie(sel_bss->wps->registrar);
		}
	} else {
		ret = -1;
...
static int hostapd_ctrl_iface_ignore_hw_pbc(struct hostapd_data *hapd, char *value)
{
	if (os_strcmp(value, "0") == 0)
		hapd->ignore_hw_pbc = 0;
	else if (os_strcmp(value, "1") == 0)
		hapd->ignore_hw_pbc = 1;
	else
		return -1;
...
static int hostapd_ctrl_iface_wps_upnp_enable(struct hostapd_data *hapd, char *value,
		  char *buf, size_t buflen)
{
	int enable;
	struct hostapd_data *sel_bss;
	...
	/* Format of the value: bss_name option */
	option = os_strchr(value, ' ');
	if (NULL == option)
		return -1;
	*option++ = '\0';

	if ((sel_bss = hostapd_get_bss(hapd->iface, value)) == NULL)
		return -1;
	...
	if (enable && sel_bss->wps_upnp == NULL) {
		if (sel_bss->conf->upnp_iface == NULL) {
			sel_bss->conf->upnp_iface = os_strdup("br0");
		}
		hostapd_wps_upnp_init(sel_bss, sel_bss->wps);
	}

	if (!enable && sel_bss->wps_upnp != NULL) {
		hostapd_wps_upnp_deinit(sel_bss);
	}

	return 0;
}

static int hostapd_ctrl_iface_wps_upnp_status(struct hostapd_data *hapd, char *bss_name,
			char *buf, size_t buflen)
{
	char *response[] = {"0\n", "1\n"};
	int select_resp;
...
static int hostapd_ctrl_iface_wpa_status(struct hostapd_data *hapd, char *cmd,
		  char *buf, size_t buflen)
{
	struct sta_info *sta;
	...
	/* Format of the cmd: bss_name mac_addr */
	...
	*mac_addr++ = '\0';
	...
	if ((sel_bss = hostapd_get_bss(hapd->iface, cmd)) == NULL)
		return res;

	for (sta = sel_bss->sta_list; sta; sta = sta->next) {
...
static int hostapd_ctrl_iface_add_filter_mac(struct hostapd_data *hapd, char *buf)
{
	struct hostapd_data *sel_bss = NULL;
	struct hostapd_bss_config *conf = NULL;
	struct mac_acl_entry *newacl = NULL;
	...
	*pos++ = '\0';
	...
	if (strlen(macaddr) != MAC_ADDR_STR_LEN ||
			hwaddr_aton(macaddr, addr)) {
		wpa_printf(MSG_ERROR, "%s: invalid mac address %s",
				__func__, macaddr);
		return -1;
	}

	sel_bss = hostapd_get_bss(hapd->iface, ifname);
	if (!sel_bss) {
		wpa_printf(MSG_ERROR, "%s: fail to find bss %s",
				__func__, ifname);
		return -1;
	}

	conf = sel_bss->conf;
	if (os_strcmp(accept_deny, "accept") == 0) {
		acl = &(conf->accept_mac);
		aclnum = &conf->num_accept_mac;
	} else if (os_strcmp(accept_deny, "deny") == 0) {
		acl = &(conf->deny_mac);
		aclnum = &conf->num_deny_mac;
	} else {
		wpa_printf(MSG_ERROR, "%s: invalid mac filter type %s",
				__func__, accept_deny);
		return -1;
	}

	newacl = os_realloc_array(*acl, *aclnum + 1, sizeof(**acl));
	if (!newacl) {
		wpa_printf(MSG_ERROR, "%s: fail to allocate new mac"
				" filter table, restore the old mac "
				"filter table from files", __func__);
		hostapd_update_bss_filer_mac_list(hapd->iface, ifname);

		return -1;
	}

	os_memcpy(newacl[*aclnum].addr, addr, ETH_ALEN);

#if 0 /* FIXME: Modified for compilation */
	newacl[*aclnum].vlan_id = 0;
#else
	newacl[*aclnum].vlan_id.notempty = 0;
#endif

	*acl = newacl;
	(*aclnum)++;
	qsort(*acl, *aclnum, sizeof(**acl), hostapd_acl_comp);

	hostapd_set_acl(sel_bss);
	hostapd_flush_filtered_stations(sel_bss);

	return 0;
}

static int hostapd_ctrl_iface_del_filter_mac(struct hostapd_data *hapd, char *buf)
{
	struct hostapd_data *sel_bss = NULL;
	struct hostapd_bss_config *conf = NULL;
	struct mac_acl_entry *newacl = NULL;
	...
	*pos++ = '\0';
	...
	if (*aclnum > 1) {
		for (j = i; j < *aclnum - 1; j++)
			os_memcpy(&((*acl)[j]), &((*acl)[j + 1]), sizeof(**acl));
...
static int hostapd_ctrl_iface_get_filter_mac(struct hostapd_data *hapd,
	char *buf, char *reply, size_t reply_size)
{
	struct hostapd_data *sel_bss = NULL;
	struct hostapd_bss_config *conf = NULL;
	struct mac_acl_entry *acl;
	...
	*pos++ = '\0';
	...
	for (i = 0; i < aclnum; i++) {
		if (((reply_len + strlen(MACSTR) + 1) >= reply_size) ||
				(reply_len < 0)) {
			wpa_printf(MSG_ERROR, "%s: invalid reply length %d",
				__func__, reply_len);
			break;
		}
		...
		reply_len += snprintf(&reply[reply_len], reply_size - reply_len,
				MACSTR, MAC2STR(acl[i].addr));
	}
...
#endif
{
	...
	if (os_strcmp(buf, "RECONFIGURE") == 0) {
		...
		if (hostapd_del_bss(hapd->iface, buf + 17))
			reply_len = -1;
	} else if (os_strncmp(buf, "UPDATE_FILTER_MAC_LIST ", 23) == 0) {
		if (hostapd_update_bss_filer_mac_list(hapd->iface, buf + 23))
			reply_len = -1;
#ifdef CONFIG_WPS
		...
		reply_len = hostapd_ctrl_iface_wps_upnp_status(hapd, buf + strlen("WPS_UPNP_STATUS "),
								reply, reply_size);
		...
		if (reply_len < 0 || (unsigned int) reply_len >= reply_size)
			reply_len = -1;
	} else if (os_strncmp(buf, "IGNORE_HW_PBC ", strlen("IGNORE_HW_PBC ")) == 0) {
		if (hostapd_ctrl_iface_ignore_hw_pbc(hapd, buf + strlen("IGNORE_HW_PBC ")) < 0)
			reply_len = -1;
		...
		reply_len = qtn_non_wps_pp_status(hapd, buf + strlen("NON_WPS_PP_STATUS "),
						reply, reply_size);
#endif /* CONFIG_WPS */
	...
	} else if (os_strcmp(buf, "DEAUTH_ALL") == 0) {
		if (hostapd_deauth_all_iface(hapd->iface) < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "ADD_FILTER_MAC ", strlen("ADD_FILTER_MAC ")) == 0) {
		if (hostapd_ctrl_iface_add_filter_mac(hapd, buf + strlen("ADD_FILTER_MAC ")) < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "DEL_FILTER_MAC ", strlen("DEL_FILTER_MAC ")) == 0) {
		if (hostapd_ctrl_iface_del_filter_mac(hapd, buf + strlen("DEL_FILTER_MAC ")) < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "GET_FILTER_MAC ", strlen("GET_FILTER_MAC ")) == 0) {
		reply_len = hostapd_ctrl_iface_get_filter_mac(hapd,
					buf + strlen("GET_FILTER_MAC "), reply, reply_size);
	} else if (os_strncmp(buf, "GET_MAX_BSS_NUM", strlen("GET_MAX_BSS_NUM")) == 0) {
		reply_len = os_snprintf(reply, reply_size, "%d", g_max_bssid);
		if (reply_len < 0 || (unsigned int) reply_len >= reply_size)
			reply_len = -1;
	} else if (os_strcmp(buf, "SHOW_STA_WPA_KEYID") == 0) {
		reply_len = qtn_ctrl_iface_show_sta_wpa_keyid(hapd, reply, reply_size);
	} else {
...
#ifdef CONFIG_QTNA_WIFI
		reply_len = hostapd_qtn_ctrl_iface_process(hapd, buf, reply, reply_size);
#else
		os_memcpy(reply, "UNKNOWN COMMAND\n", 16);
		reply_len = 16;
#endif /* CONFIG_QTNA_WIFI */

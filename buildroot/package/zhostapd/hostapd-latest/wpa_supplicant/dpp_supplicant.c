...
static void wpas_dpp_pkex_alloc_timeout(void *eloop_data, void *user_ctx)
{
				    ...
				    struct dpp_authentication *auth)
{
	...
	if (wpa_config_write(wpa_s->confname, wpa_s->conf))
		wpa_printf(MSG_ERROR, "DPP: could not write config to file");
#endif /* CONFIG_QTNA_WIFI */
...
{
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->qtna_drv_flag & QTNA_DRIVER_FLAG_24G_CAPABLE) {
		if (pkex->freq == QTN_DPP_PKEX_DEFAULT_FREQ_24G) {
			wpa_printf(MSG_DEBUG, "DPP: Dual-band platform; try next frequency");
			pkex->freq = QTN_DPP_PKEX_DEFAULT_FREQ_5G;
			return 0;
		}

		wpa_printf(MSG_DEBUG, "DPP: Dual-band platform; indicate PKEX failure");
		return -1;
	}

	wpa_printf(MSG_DEBUG, "DPP: 5G only platform; indicate PKEX failure");
	return -1;
#else
	if (pkex->freq == 2437)
		pkex->freq = 5745;
...
#ifdef CONFIG_QTNA_WIFI
	eloop_register_timeout(QTN_DPP_PKEX_TIMEOUT, 0, wpas_dpp_pkex_alloc_timeout, wpa_s, NULL);
#endif
...
#ifdef CONFIG_QTNA_WIFI
	eloop_cancel_timeout(wpas_dpp_pkex_alloc_timeout, wpa_s, NULL);
#endif /* CONFIG_QTNA_WIFI */
	dpp_pkex_free(pkex);
	wpa_s->dpp_pkex = NULL;
...
#ifdef CONFIG_QTNA_WIFI
		if (wpa_s->qtna_drv_flag & QTNA_DRIVER_FLAG_24G_CAPABLE)
			timeout = QTN_DPP_PKEX_TIMEOUT * 2;
		else
			timeout = QTN_DPP_PKEX_TIMEOUT;

		wpa_printf(MSG_DEBUG, "DPP: PKEX cleanup timeout %d", timeout);
		eloop_register_timeout(timeout, 0, wpas_dpp_pkex_alloc_timeout, wpa_s, NULL);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
		if (wpa_s->qtna_drv_flag & QTNA_DRIVER_FLAG_24G_CAPABLE)
			pkex->freq = QTN_DPP_PKEX_DEFAULT_FREQ_24G;
		else
			pkex->freq = QTN_DPP_PKEX_DEFAULT_FREQ_5G;
#else
		pkex->freq = 2437;
...
#ifdef CONFIG_QTNA_WIFI
	wpa_s->dpp_auth_role = 0;
	wpa_s->dpp_prov_role = 0;
	wpa_s->dpp_bootstrap_method = 0;
	wpa_s->dpp_netrole = 0;
	wpa_s->dpp_ecc_curve_bootstrap = 0;
	wpa_s->dpp_ecc_curve_csign = 0;
#endif /* CONFIG_QTNA_WIFI */
	return 0;
}

...
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcasecmp(cmd, "blacklistFailMax") == 0) {
		if (wpa_blacklist_set_fail_max(wpa_s, atoi(value)))
			ret = -1;
	} else if (os_strcasecmp(cmd, "blacklistTimeout") == 0) {
		if (wpa_blacklist_set_timeout(wpa_s, atoi(value)))
			ret = -1;
	} else if (os_strcasecmp(cmd, "non_wps_pp_enable") == 0) {
		int enable = atoi(value);
		if (enable)
			wpa_s->non_wps_pp_enable = 1;
		else
			wpa_s->non_wps_pp_enable = 0;
...
#if defined(CONFIG_QTNA_WIFI) && defined(CONFIG_DPP)
	} else if (os_strcmp(cmd, "DPP_AUTH_ROLE") == 0) {
		wpa_s->dpp_auth_role = atoi(value);
	} else if (os_strcmp(cmd, "DPP_PROV_ROLE") == 0) {
		wpa_s->dpp_prov_role = atoi(value);
	} else if (os_strcmp(cmd, "DPP_BOOTSTRAP_METHOD") == 0) {
		wpa_s->dpp_bootstrap_method = atoi(value);
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_BOOTSTRAP") == 0) {
		wpa_s->dpp_ecc_curve_bootstrap = atoi(value);
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_CSIGN") == 0) {
		wpa_s->dpp_ecc_curve_csign = atoi(value);
	} else if (os_strcmp(cmd, "DPP_NETROLE") == 0) {
		wpa_s->dpp_netrole = atoi(value);
	} else if (os_strcmp(cmd, "DPP_AUTHDIR") == 0) {
		wpa_s->dpp_authdir = atoi(value);
#endif /* CONFIG_QTNA_WIFI && CONFIG_DPP */
	} else {
		value[-1] = '=';
...
#ifdef CONFIG_QTNA_WIFI
static int wpa_supplicant_ctrl_iface_wps_status(struct wpa_supplicant *wpa_s,
						char *buf,
						const size_t buf_len)
{
	int	ret = wpas_wps_get_ext_status(wpa_s, buf, buf_len);

	return ret;
}

static int wpa_supplicant_ctrl_iface_specific_scan(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	int specific_scan = atoi(cmd);

	if (specific_scan < 0 || specific_scan > 1)
		return -1;

	wpa_s->conf->specific_scan = specific_scan;

	return 0;
}


static int wpa_supplicant_ctrl_iface_scan_buf_max_size(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	int max_size = atoi(cmd);

	if ((max_size < 0) || (max_size > DEFAULT_MAX_SCAN_RESULT_BUF))
		return -1;

	wpa_s->conf->scan_buf_max_size = max_size;

	return 0;
}

static int wpa_supplicant_ctrl_iface_wps_timeout(struct wpa_supplicant *wpa_s, char *txt,
							char *reply, int size)
{
...
static int wpa_supplicant_ctrl_iface_reconnect(struct wpa_supplicant *wpa_s, char *cmd)
{
	int delay_msec;
	int noscan = 0;

	if (os_strcasecmp(cmd, "noscan") == 0) {
		noscan = 1;
	} else {
		delay_msec = atoi(cmd);
		if (delay_msec < 0)
	...
	if (noscan)
		...
		wpa_supplicant_req_scan(wpa_s, delay_msec / 1000, (delay_msec % 1000) * 1000);

	return 0;
}


static int wpa_supplicant_ctrl_iface_ignore_hw_pbc(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	if (os_strcmp(cmd, "0") == 0)
		wpa_s->ignore_hw_pbc = 0;
	else if (os_strcmp(cmd, "1") == 0)
		wpa_s->ignore_hw_pbc = 1;
	else
		return -1;
					 ...
					 char *cmd, char *buf, size_t buflen)
{
	...
	if (os_strcmp(cmd, "version") == 0) {
...
#endif /* CONFIG_TESTING_OPTIONS */
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strcmp(cmd, "non_wps_pp_enable") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->non_wps_pp_enable);
		if (res < 0)
			return -1;
		return res;
	} else if (os_strcmp(cmd, "specific_scan") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->conf->specific_scan);
		if (res < 0)
			return -1;
		return res;
	} else if (os_strcmp(cmd, "device_name") == 0) {
		res = os_snprintf(buf, buflen, "%s", wpa_s->conf->device_name);
		if (res < 0)
			return -1;
		return res;
	} else if (os_strcmp(cmd, "scan_buf_max_size") == 0) {
		res = os_snprintf(buf, buflen, "%d\n", wpa_s->conf->scan_buf_max_size);
		if (res < 0)
			return -1;
		return res;
	} else if (os_strcmp(cmd, "uuid") == 0) {
		res = uuid_bin2str(wpa_s->conf->uuid, buf, MIN(UUID_HEX_STR_LEN, buflen));
		if (!res)
			res = strlen(buf);
		else
			return -1;
		...
		res = os_snprintf(buf, buflen, "%d", wpa_s->dpp_auth_role);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_PROV_ROLE") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->dpp_prov_role);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_BOOTSTRAP_METHOD") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->dpp_bootstrap_method);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_BOOTSTRAP") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->dpp_ecc_curve_bootstrap);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_ECC_CURVE_CSIGN") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->dpp_ecc_curve_csign);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_NETROLE") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->dpp_netrole);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
	} else if (os_strcmp(cmd, "DPP_AUTHDIR") == 0) {
		res = os_snprintf(buf, buflen, "%d", wpa_s->dpp_authdir);
		if (os_snprintf_error(buflen, res))
			return -1;
		return res;
#endif /* CONFIG_DPP */
...
#ifdef CONFIG_QTNA_WIFI
		ssid->from_wps = wpa_check_wps_ssid(wpa_s->confname, (const char *)ssid->ssid);
#endif /* CONFIG_QTNA_WIFI */
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
...
#ifdef CONFIG_QTNA_WIFI
		if (wpa_supplicant_ctrl_iface_reconnect(wpa_s, buf + 10))
			reply_len = -1;
#else
			wpas_request_connection(wpa_s);
#endif /* CONFIG_QTNA_WIFI */
		...
		if (wpa_supplicant_ctrl_iface_wps_reg(wpa_s, buf + 8))
			reply_len = -1;
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strncmp(buf, "WPS_STATUS", 10) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_wps_status( wpa_s, reply, reply_size );
	} else if (os_strncmp(buf, "SPECIFIC_SCAN ", 14) == 0) {
		if (wpa_supplicant_ctrl_iface_specific_scan(wpa_s, buf + 14))
			reply_len = -1;
	} else if (os_strncmp(buf, "SCAN_BUF_MAX_SIZE ", 18) == 0) {
		if (wpa_supplicant_ctrl_iface_scan_buf_max_size(wpa_s, buf + 18))
			reply_len = -1;
	} else if (os_strncmp(buf, "WPS_TIMEOUT ", strlen("WPS_TIMEOUT ")) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_wps_timeout(wpa_s, buf + strlen("WPS_TIMEOUT "),
								reply, reply_size);
	} else if (os_strncmp(buf, "GET_PSK_AUTH_FAILURE ",
			strlen("GET_PSK_AUTH_FAILURE ")) == 0) {
		reply_len = os_snprintf(reply, reply_len, "%u",
			 wpa_s->wpa->dot11RSNA4WayHandshakeFailures);
#ifdef CONFIG_WNM
	} else if (os_strncmp(buf, "WNM_BSS_QUERY ", 10) == 0) {
		if (wpas_ctrl_iface_wnm_bss_query(wpa_s, buf + 10))
				reply_len = -1;
#endif /* CONFIG_WNM */
	} else if (os_strncmp(buf, "IGNORE_HW_PBC ", 14) == 0) {
		if (wpa_supplicant_ctrl_iface_ignore_hw_pbc(wpa_s, buf + 14))
			reply_len = -1;
	} else if (os_strncmp(buf, "ADD_WPS_PBC_SSID_FILTER ", 24) == 0) {
		if (wpas_wps_pbc_ssid_filter_add(wpa_s, buf + 24))
			reply_len = -1;
	} else if (os_strncmp(buf, "DEL_WPS_PBC_SSID_FILTER ", 24) == 0) {
		if (wpas_wps_pbc_ssid_filter_del(wpa_s, buf + 24))
			reply_len = -1;
	} else if (os_strcmp(buf, "SHOW_WPS_PBC_SSID_FILTERS") == 0) {
		reply_len = wpas_wps_pbc_ssid_filters_show(wpa_s, reply, reply_size);
#endif /* CONFIG_QTNA_WIFI */
#ifdef CONFIG_AP

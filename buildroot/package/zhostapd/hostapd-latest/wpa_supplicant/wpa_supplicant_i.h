...
#define WPS_PBC_SSID_FILTERS_MAX 10
...
#ifdef CONFIG_QTNA_WIFI
/* structure for AP that use MD5 PSK*/
struct psk_md5_ap {
	u8	addr[ETH_ALEN];	/* AP's mac addr */
	int psk_md5_used;	/* md5 used flag */
	int connect_ok;		/* connecting status flag */
};
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	int wps_fast_reassoc;	/*For fast reassociation */
	int ignore_hw_pbc;
	struct dl_list wps_pbc_ssid_filters; /* struct wps_pbc_ssid_filter::list */
	size_t wps_pbc_ssid_filter_cnt;
#define QTNA_DRIVER_FLAG_24G_CAPABLE		0x00000001
	u32 qtna_drv_flag;
#endif /* CONFIG_QTNA_WIFI */
	...
	struct {
		...
		u16 num_modes;
#ifdef CONFIG_QTNA_WIFI
#define HOSTAPD_HW_FLAG_SKIP_RATE_CHECK    BIT(0)
#endif /* CONFIG_QTNA_WIFI */
		u16 flags;
	} hw;
	enum local_hw_capab {
...
#define MAC_ADDR_RAND_ALL        (MAC_ADDR_RAND_SCAN | \
	 ...
	 * This flag indicates only quantenna AP
	 * which contains qtn ext attribute is found.
	 */
	int wps_only_qtn_attr_fud;

	int blacklist_timer;
	int blacklist_fail_max;
	int blacklist_timeout;

	u8 pairing_hash[PAIRING_HASH_LEN];
	int non_wps_pp_enable;
	struct wpa_ssid *wps_credential_ssid;

	/* Non WPS mode related parameters */
#define NON_WPS_MODE_NONE	0
#define NON_WPS_MODE_MIX	1
#define NON_WPS_MODE_STRICT	2
	int non_wps_mode;
	struct psk_md5_ap md5_ap;
#define QTN_MAX_REASSOC_WITHOUT_SCAN	3
	int reassoc_without_scan_count;
#endif /* CONFIG_QTNA_WIFI */

#if defined(CONFIG_QTNA_WIFI) && defined(CONFIG_OWE)
	int try_default_owe_group;
#endif /* CONFIG_QTNA_WIFI && CONFIG_OWE*/
#ifdef CONFIG_QTNA_WIFI
	int dpp_auth_role;
	int dpp_prov_role;
	int dpp_bootstrap_method;
	int dpp_netrole;
	int dpp_ecc_curve_bootstrap;
	int dpp_ecc_curve_csign;
	int dpp_authdir;

	int is_map_fronthaul_bss;
	struct wpa_ssid *map_backhaul_ssid;
#endif /* CONFIG_QTNA_WIFI */
};

...
#endif /* CONFIG_FILS && IEEE8021X_EAPOL */
#ifdef CONFIG_QTNA_WIFI
		system("killall -SIGUSR1 dhclient");
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->conf && wpa_s->conf->pairing_id &&
			wpa_supplicant_setup_pairing_hash(wpa_s->conf->pairing_id,
							wpa_s->own_addr, wpa_s->pairing_hash)) {
		wpa_printf(MSG_ERROR, "Pairing hash initializing failed");
		return -1;
	}

	if (wpa_s->conf->pairing_id) {
		if (wpa_drv_set_pairing_hash_ie(wpa_s,
				wpa_s->pairing_hash, PAIRING_HASH_LEN) < 0) {
			wpa_printf(MSG_ERROR, "Setting pairing hash IE failed");
			return -1;
		}
	} else {
		if (wpa_drv_set_pairing_hash_ie(wpa_s,
				wpa_s->pairing_hash, 0) < 0) {
			wpa_printf(MSG_ERROR, "Removing pairing hash IE failed");
			return -1;
		}
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
		if (wpa_s->non_wps_pp_enable && !ssid->from_wps) {
			wpa_s->non_wps_mode = NON_WPS_MODE_MIX;
			wpa_printf(MSG_DEBUG, "from wps is %d, use mix mode", ssid->from_wps);
		} else {
			wpa_s->non_wps_mode = NON_WPS_MODE_NONE;
			wpa_printf(MSG_DEBUG, "from wps is %d, use none mode", ssid->from_wps);
		}

		if (wpa_s->non_wps_mode) {
			wpa_sm_set_pmk_md5(wpa_s->wpa, ssid->psk_md5, PMK_LEN);
		}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
/**
 * wpa_supplicant_clear_fast_reassoc - Releasing the fast reassoiation
 * @wpa_s: Pointer to wpa_supplicant data
 *
 * This function is used to clear the fast reassoication.So now we can able
 * to scan on all channels.
 */
void wpa_supplicant_clear_fast_reassoc(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->wps_fast_reassoc != WPS_REASSOC_DEFAULT) {
		wpa_drv_set_fast_reassoc(wpa_s, 0, NULL);
...
#ifdef CONFIG_QTNA_WIFI
	/*
	 * Trigger the 'sme-connect' work (through 'sme_authenticate()')
	 * only if SAE auth is enabled and also when the target BSS supports SAE.
	 * Otherwise follow the regular 'connect' work path.
	 */
#ifdef CONFIG_SAE
	if (wpa_key_mgmt_sae(ssid->key_mgmt)) {
		const u8 *rsn;
		...
		if (rsn) {
			if (wpa_parse_wpa_ie(rsn, 2 + rsn[1], &ied) == 0 &&
					wpa_key_mgmt_sae(ied.key_mgmt))
				qtn_use_sme_auth = 1;
		}
	}
	wpa_dbg(wpa_s, MSG_DEBUG, "qtn_use_sme_auth = %d", qtn_use_sme_auth);
#endif /* CONFIG_SAE */
	if (qtn_use_sme_auth &&
#else
	if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) &&
	...
	enum wpa_drv_update_connect_params_mask *mask)
{
	...
	if (algs == WPA_AUTH_ALG_OPEN &&
		...
		if (ssid->owe_group) {
#ifdef CONFIG_QTNA_WIFI
			if (wpa_s->try_default_owe_group)
				group = OWE_DH_GROUP;
			else
#endif /* CONFIG_QTNA_WIFI */
...
{
#ifdef CONFIG_QTNA_WIFI
	if (scan_interval == QTNA_INVALID_SCAN_INTERVAL) {
		wpa_msg(wpa_s, MSG_DEBUG, "Disabling scan");
	} else {
#endif /* CONFIG_QTNA_WIFI */
		if (scan_interval < 0) {
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->bridge_ifname[0]) {
		...
		if (wpa_s->l2_br)
		{
			wpa_dbg(wpa_s, MSG_DEBUG, "Closing previous bridge interface l2"
...
#ifdef CONFIG_QTNA_WIFI
int wpa_supplicant_setup_pairing_hash(const u8 *pairing_id,
			const u8 *own_addr, u8 *pairing_hash)
{
	int ret = 0;
	const u8 *addr[2];
	size_t len[2];
	...
	addr[0] = pairing_id;
	len[0] = os_strlen((char *)pairing_id) + 1;
	addr[1] = own_addr;
	len[1] = ETH_ALEN;
...
#ifdef CONFIG_QTNA_WIFI
#define WPA_SUPPLICANT_BACKUP_NAME "/mnt/jffs2/wpa_supplicant.conf.bk"
		wpa_s->conf = wpa_config_read(wpa_s->confname, NULL);
		if (wpa_s->conf == NULL) {
			char confname[128];
			wpa_msg(wpa_s, MSG_ERROR, "Failed to parse the configuration "
				"file '%s' - trying the backup one", wpa_s->confname);
			/* If the wpa_supplicant.conf is corrupt, use default setting */
			strcpy(confname, WPA_SUPPLICANT_BACKUP_NAME);
			wpa_s->conf = wpa_config_read(confname, NULL);
			if (wpa_s->conf == NULL) {
				wpa_msg(wpa_s, MSG_ERROR, "Failed to parse the backup configuration "
				"file '%s' - trying the default one", wpa_s->confname);
				strcpy(confname, "/scripts/wpa_supplicant.conf");
				wpa_s->conf = wpa_config_read(confname, NULL);
				if (wpa_s->conf == NULL) {
					wpa_msg(wpa_s, MSG_ERROR, "Failed to parse the default configuration "
					"file '%s' - exit", wpa_s->confname);
					return -1;
				} else {
					char cmd[256];
					/* Fix the corrupt file */
					sprintf(cmd, "cp %s %s", "/scripts/wpa_supplicant.conf", wpa_s->confname);
					system(cmd);
				}
			} else {
				char cmd[256];
				/* Fix the corrupt file */
				sprintf(cmd, "cp %s %s", WPA_SUPPLICANT_BACKUP_NAME, wpa_s->confname);
				system(cmd);
			}
		} else {
			/* Backup the good configuration in case it is corrupted. */
			char cmd[256];
			sprintf(cmd, "cp %s %s", wpa_s->confname, WPA_SUPPLICANT_BACKUP_NAME);
			system(cmd);
		}
#else
		wpa_s->conf = wpa_config_read(wpa_s->confname, NULL);
...
#ifdef CONFIG_QTNA_WIFI
		if (capa.hw_capab_24g)
			wpa_s->qtna_drv_flag |= QTNA_DRIVER_FLAG_24G_CAPABLE;
#endif /* CONFIG_QTNA_WIFI */
	}
...
#ifdef CONFIG_QTNA_WIFI
	wpa_blacklist_set_default_config(wpa_s);
#endif /* CONFIG_QTNA_WIFI */

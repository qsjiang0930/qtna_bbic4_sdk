...
static char ** wpa_cli_complete_set(const char *str, int pos)
{
	...
	const char *fields[] = {
		...
		"blacklistFailMax", "blacklistTimeout",
...
#ifdef CONFIG_QTNA_WIFI
static int wpa_cli_cmd_specific_scan(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 1) {
		printf("Invalid SPECIIFC_SCAN command: needs one argument (specific_scan "
		       "value)\n");
		return -1;
	}
	res = os_snprintf(cmd, sizeof(cmd), "SPECIFIC_SCAN %s", argv[0]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long SPECIIFC_SCAN command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}

static int wpa_cli_cmd_scan_buf_max_size(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;

	if (argc != 1) {
		printf("Invalid SCAN_BUF_MAX_SIZE command: needs one argument"
			   " (scan_buf_max_size value)\n");
		return -1;
	}
	res = os_snprintf(cmd, sizeof(cmd), "SCAN_BUF_MAX_SIZE %s", argv[0]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long SCAN_BUF_MAX_SIZE command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}

static int wpa_cli_cmd_wps_status( struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	return wpa_ctrl_command(ctrl, "WPS_STATUS" );
}

static int wpa_cli_cmd_wps_timeout( struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;
				  ...
				  char *argv[])
{
	...
	if (argc == 1) {
		res = os_snprintf(cmd, sizeof(cmd), "RECONNECT %s", argv[0]);
	} else {
		res = os_snprintf(cmd, sizeof(cmd), "RECONNECT");
	}
...
#ifdef CONFIG_QTNA_WIFI
static int wpa_cli_cmd_ignore_hw_pbc(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	char cmd[256];
	int res;
	...
	if (argc != 1) {
		printf("Invalid IGNORE_HW_PBC command: needs one argument "
		       "(0/1 = accept/ignore hw pbc event)\n");
		return -1;
	}
	res = os_snprintf(cmd, sizeof(cmd), "IGNORE_HW_PBC %s", argv[0]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long IGNORE_HW_PBC command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}

static int wpa_cli_cmd_add_wps_pbc_ssid_filter(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	char cmd[256];
	int res;
	...
	if (argc != 1) {
		printf("Invalid ADD_WPS_PBC_SSID_FILTER command: needs one argument "
		       "(SSID sub-string)\n");
		return -1;
	}
	res = os_snprintf(cmd, sizeof(cmd), "ADD_WPS_PBC_SSID_FILTER %s", argv[0]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long ADD_WPS_PBC_SSID_FILTER command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}

static int wpa_cli_cmd_del_wps_pbc_ssid_filter(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	char cmd[256];
	int res;
	...
	if (argc != 1) {
		printf("Invalid DEL_WPS_PBC_SSID_FILTER command: needs one argument "
		       "(SSID sub-string)\n");
		return -1;
	}
	res = os_snprintf(cmd, sizeof(cmd), "DEL_WPS_PBC_SSID_FILTER %s", argv[0]);
	if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
		printf("Too long DEL_WPS_PBC_SSID_FILTER command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}

static int wpa_cli_cmd_show_wps_pbc_ssid_filters(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	return wpa_ctrl_command(ctrl, "SHOW_WPS_PBC_SSID_FILTERS");
}
#endif /* CONFIG_QTNA_WIFI */
...
static const struct wpa_cli_cmd wpa_cli_commands[] = {
	...
	{ "reconnect", wpa_cli_cmd_reconnect, NULL,
	  cli_cmd_flag_none,
	  "[noscan] | [scan delay time] = an optional argument is used to \n"
...
#endif /* CONFIG_DPP */
#ifdef CONFIG_QTNA_WIFI
	{ "ignore_hw_pbc", wpa_cli_cmd_ignore_hw_pbc, NULL, cli_cmd_flag_none,
	  "<0/1> = accept/ignore hw pbc event" },
	{ "add_wps_pbc_ssid_filter", wpa_cli_cmd_add_wps_pbc_ssid_filter, NULL,
	  cli_cmd_flag_none,
	  "<SSID sub-string> = add SSID sub-string to match" },
	{ "del_wps_pbc_ssid_filter", wpa_cli_cmd_del_wps_pbc_ssid_filter, NULL,
	  cli_cmd_flag_none,
	  "<SSID sub-string> = del SSID sub-string" },
	{ "show_wps_pbc_ssid_filters", wpa_cli_cmd_show_wps_pbc_ssid_filters, NULL,
	  cli_cmd_flag_none,
	  "= show SSID filters" },
	{ "wps_status", wpa_cli_cmd_wps_status, NULL,
	  cli_cmd_flag_none,
	  "= get WPS status" },
	{ "wps_timeout", wpa_cli_cmd_wps_timeout, NULL,
	  cli_cmd_flag_none,
	  "<timeout> = set WPS timeout value" },
	{ "specific_scan", wpa_cli_cmd_specific_scan, NULL,
	  cli_cmd_flag_none,
	  "<value> = set specific_scan parameter" },
	{ "scan_buf_max_size", wpa_cli_cmd_scan_buf_max_size, NULL,
	  cli_cmd_flag_none,
	  "<value> = scan results buffer max size" },
#endif /* CONFIG_QTNA_WIFI */

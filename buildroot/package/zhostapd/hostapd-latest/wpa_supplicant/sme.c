...
#ifdef CONFIG_QTNA_WIFI
	if (data->auth.auth_event_source == AUTH_FROM_CUSTOM_EVENT) {
		if (data->auth.status_code == WLAN_STATUS_REJECTED_WITH_SUGGESTED_BSS_TRANSITION) {

			struct wpa_bss *bss = NULL;
			struct wpa_ssid *ssid = wpa_s->current_ssid;
			struct neighbor_report_trans_item *nr_item = &data->auth.nr_item;
			int freq;
			...
			bss = wpa_get_bss_by_bssid_freq_essid(wpa_s, nr_item->bssid, freq,
			...
			if (bss){
				wpa_printf(MSG_DEBUG, "Neighbor bss found (SSID='%s' freq=%d MHz)"
						MACSTR,
...
#ifdef CONFIG_QTNA_WIFI
	wpa_dbg(wpa_s, MSG_DEBUG, "%s: mode=%d auth_type=0x%x", __func__, mode, auth_type);
...
#ifdef CONFIG_QTNA_WIFI
	params.auth_alg = wpa_s->sme.auth_alg;
#endif /* CONFIG_QTNA_WIFI */
#ifdef CONFIG_HT_OVERRIDES

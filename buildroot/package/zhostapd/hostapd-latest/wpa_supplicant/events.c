...
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->hw.flags & HOSTAPD_HW_FLAG_SKIP_RATE_CHECK) {
		/*
		 * The Quantenna driver does not provide supported rates IE
		 * in scan results right now, so allow the connection attempt
		 * to continue.
		 */
		return 1;
	}
				     ...
				     int only_first_ssid, int debug_print)
{
...
#ifdef CONFIG_QTNA_WIFI
	if (bss->flags & WPA_SCAN_FREQ_NOT_AVAILABLE) {
		wpa_dbg(wpa_s, MSG_DEBUG, "   skip - frequency not support");
		return NULL;
	}
#endif /* CONFIG_QTNA_WIFI */
	...
	if ((match_ssid_len == 0)
#if defined (CONFIG_QTNA_WIFI) && defined (CONFIG_WPS)
		&& !wpas_wps_in_progress(wpa_s)
#endif /* CONFIG_QTNA_WIFI && CONFIG_WPS*/
		) {
		if (debug_print)
...
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->map_backhaul_ssid) {
		selected = wpa_supplicant_select_bss(
			wpa_s, wpa_s->map_backhaul_ssid, selected_ssid, 1);
		if (selected == NULL) {
			wpa_dbg(wpa_s, MSG_DEBUG, "Backhaul SSID not yet available in scan list");
			return NULL;
		}
	}
#endif
...
#if defined(CONFIG_WPS) && defined(CONFIG_QTNA_WIFI)
		    && wpas_wps_in_progress(wpa_s)
#endif /* CONFIG_WPS && CONFIG_QTNA_WIFI*/
...
#if (defined CONFIG_QTNA_WIFI) && (defined CONFIG_IEEE80211W)
		wpa_msg(wpa_s, MSG_DEBUG, "ssid->ieee80211w=%d", ssid->ieee80211w);
		if (ssid->ieee80211w == MGMT_FRAME_PROTECTION_DEFAULT) {
			int curr_pmf;

			curr_pmf = wpa_drv_get_pmf(wpa_s);
			if (curr_pmf < 0) {
				wpa_msg(wpa_s, MSG_ERROR, "Can't get pmf from driver");
				return -1;
			}
			ssid->ieee80211w = curr_pmf;
		} else {
			if (wpa_drv_set_pmf(wpa_s, ssid->ieee80211w)) {
				wpa_msg(wpa_s, MSG_ERROR, "Can't set pmf %d", ssid->ieee80211w);
				return -1;
			}
		}
#endif /* CONFIG_QTNA_WIFI and CONFIG_IEEE80211W */
...
#ifdef CONFIG_QTNA_WIFI
static int wpa_supplicant_pairing_hash_check(struct wpa_supplicant *wpa_s, struct wpa_bss *selected)
{
	int ret = 0;
	u8 bssid[ETH_ALEN];
	u8 hash_ie[PAIRING_HASH_LEN];
	u8 pairing_hash[PAIRING_HASH_LEN];
	int pairing_mismatch = 0;

	/* PPS2 option for station was disabled */
	if (wpa_s->conf->pairing_enable == PPS2_DISABLE) {
		return ret;
	}
	...
	if (!selected) {
		...
		os_memcpy(bssid, &selected->bssid[0], ETH_ALEN);
	}
	...
	if (ret > 0) {
		if (!wpa_supplicant_setup_pairing_hash(wpa_s->conf->pairing_id,
				bssid, pairing_hash)) {
			pairing_mismatch = os_memcmp(pairing_hash, hash_ie,
								PAIRING_HASH_LEN);

			if (((wpa_s->conf->pairing_enable == PPS2_MODE_ACCEPT) &&
					pairing_mismatch) ||
				((wpa_s->conf->pairing_enable == PPS2_MODE_DENY) &&
					(!pairing_mismatch))) {
				wpa_drv_disassociate(wpa_s, bssid,
							WLAN_REASON_INVALID_IE);
				ret = -1;
			}
		}
	} else {
		/* Whitelist was set and no pairing id from AP */
		if (wpa_s->conf->pairing_enable == PPS2_MODE_ACCEPT) {
					      ...
					      int new_scan, int own_request)
{
	...
	if (selected) {
		...
		if (wpa_s->conf->pairing_id &&
				wpa_supplicant_pairing_hash_check(wpa_s, selected) < 0) {
...
#ifdef CONFIG_QTNA_WIFI
static void wpa_map_process_assoc_resp(struct wpa_supplicant *wpa_s,
					    const u8 *ies, size_t ies_len)
{
	struct ieee802_11_elems elems;
	...
	if (!elems.map || (elems.map_len < MAP_OUI_IE_MIN_LEN))
		return;

	if ((elems.map[4] == MAP_EXT_SUBELE_ID) &&
			(elems.map[6] & MAP_EXT_SUBELE_FRONTHAUL_BSS))
		wpa_s->is_map_fronthaul_bss = 1;

	wpa_dbg(wpa_s, MSG_WARNING, "MAP fronthaul BSS=%d", wpa_s->is_map_fronthaul_bss);
}
#endif /* CONFIG_QTNA_WIFI */
					  ...
					  union wpa_event_data *data)
{
	...
	if (data->assoc_info.resp_ies) {
		...
		if (wpa_s->conf->easymesh_backhaul_sta) {
			wpa_map_process_assoc_resp(wpa_s, data->assoc_info.resp_ies,
							data->assoc_info.resp_ies_len);
		}
#endif
...
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->conf->pairing_id &&
			wpa_supplicant_pairing_hash_check(wpa_s, NULL) < 0) {
		wpa_printf(MSG_DEBUG, "wpa_supplicant: pairing hash check fail, deauth");
		return;
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	wpa_s->reassoc_without_scan_count = 0;
#endif /* CONFIG_QTNA_WIFI */
}
						 ...
						 int locally_generated)
{
...
#ifdef CONFIG_QTNA_WIFI
#if defined(CONFIG_DPP) || defined(CONFIG_OWE)
	if (wpa_s->current_ssid &&
		(wpa_key_mgmt_dpp(wpa_s->current_ssid->key_mgmt) ||
		 wpa_key_mgmt_owe(wpa_s->current_ssid->key_mgmt)) &&
		!locally_generated && reason_code == WLAN_REASON_INVALID_IE) {
		wpa_dbg(wpa_s, MSG_WARNING, "OWE/DPP: Disconnect reason INVALID_IE");
		wpa_dbg(wpa_s, MSG_WARNING, "OWE/DPP: Drop PMKSA cache entry");
		wpa_sm_aborted_cached(wpa_s->wpa);
		wpa_sm_pmksa_cache_flush(wpa_s->wpa, wpa_s->current_ssid);
	}
#endif /* CONFIG_DPP || CONFIG_OWE */
#endif /* CONFIG_QTNA_WIFI */
	...
	if (!wpa_s->disconnected &&
		...
		if (wpa_s->wpa_state == WPA_COMPLETED &&
			...
			wpa_dbg(wpa_s, MSG_DEBUG,
				"Disconnect reason recoverable; trying fast reconnect ");
			/*
...
#ifdef CONFIG_QTNA_WIFI
		/*
		 * The default reason code is set to WLAN_REASON_DISASSOC_DUE_TO_INACTIVITY,
		 * to initiate reassoc without scanning when PMKID is available
		 */
		if (reason_code == WLAN_REASON_DISASSOC_STA_HAS_LEFT) {
			reason_code = 0;
			/* add for sae-owe 5.2.2 if PMK caching is avaliable  */
...
#ifdef CONFIG_QTNA_WIFI
	if ((wpa_s->current_ssid) &&
			(wpa_s->current_ssid->key_mgmt & (WPA_KEY_MGMT_SAE | WPA_KEY_MGMT_OWE))) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Deauthenticate to clear driver state");
...
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->current_ssid &&
		reason_code == WLAN_REASON_STA_REQ_ASSOC_WITHOUT_AUTH &&
		wpa_s->reassoc_without_scan_count < QTN_MAX_REASSOC_WITHOUT_SCAN) {
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		struct wpa_bss *bss = wpa_s->current_bss;

		if (!bss) {
			bss = wpa_supplicant_get_new_bss(wpa_s, addr);
			if (!bss) {
				wpas_connection_failed(wpa_s, addr);
				wpa_supplicant_mark_disassoc(wpa_s);
				return;
			}
		}
		...
		wpa_dbg(wpa_s, MSG_DEBUG,
			"Recoverable disconnect: trying re-association without scan");
		wpa_s->reassoc_without_scan_count++;
				    ...
				    union wpa_event_data *data)
{
	...
	if (data->assoc_reject.status_code ==
	    ...
	    wpa_s->current_ssid &&
#ifdef CONFIG_QTNA_WIFI
	    ((wpa_s->current_ssid->owe_group == 0 && wpa_s->last_owe_group != 21) ||
	     (wpa_s->current_ssid->owe_group != 0 && !wpa_s->try_default_owe_group))
#else
		...
		if (wpa_s->current_ssid->owe_group != 0) {
			wpa_printf(MSG_DEBUG, "OWE: Try default DH group");
			wpa_s->try_default_owe_group = 1;
		}
	...
	if (data->assoc_reject.status_code ==
		...
		struct wpa_bss *bss = NULL;
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		struct neighbor_report_trans_item *nr_item = &data->assoc_reject.nr_item;
		int freq;

		if (!nr_item || !ssid)
			return;

		freq = ieee80211_chan_to_freq(NULL, nr_item->operating_class,nr_item->channel);

		wpas_connect_work_done(wpa_s);
		wpa_supplicant_mark_disassoc(wpa_s);

		wpa_printf(MSG_INFO, "Neighbor bssid=" MACSTR
					"operating_class=%d channel=%d freq=%d",
						MAC2STR(nr_item->bssid),
						nr_item->operating_class,
						nr_item->channel, freq);
		bss = wpa_get_bss_by_bssid_freq_essid(wpa_s, nr_item->bssid, freq, ssid->ssid, ssid->ssid_len);
		if (!bss && ssid && ssid->ssid_len > 0)
			bss = wpa_bss_get(wpa_s, nr_item->bssid, ssid->ssid, ssid->ssid_len);
		if (!bss)
			bss = wpa_bss_get_bssid(wpa_s, nr_item->bssid);
		if(bss){
			wpa_printf(MSG_DEBUG, "Neighbor bss found (SSID='%s' Freq=%d MHz) Bssid=" MACSTR,
							wpa_ssid_txt(bss->ssid, bss->ssid_len),
							bss->freq, MAC2STR(bss->bssid));
			wpa_supplicant_connect(wpa_s, bss, ssid);
		} else {
			wpa_printf(MSG_INFO, "Neighbor bss not found");
		}
		return;
	}
#endif
...
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->current_ssid &&
		data->assoc_reject.status_code == WLAN_STATUS_ASSOC_REJECTED_TEMPORARILY &&
		wpa_s->reassoc_without_scan_count < QTN_MAX_REASSOC_WITHOUT_SCAN) {
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		struct wpa_bss *bss = wpa_s->current_bss;
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_RX_ACTION:
		...
		if (data->rx_action.category == WLAN_ACTION_WNM) {
			ieee802_11_rx_wnm_action(wpa_s,
						(struct ieee80211_mgmt *)data->rx_action.data,
						data->rx_action.len);
			break;
		}
		break;

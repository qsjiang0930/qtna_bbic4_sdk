			  ...
			  u16 proto, const u8 *buf, size_t len)
{
	...
	if (wpa_s->l2) {
#ifdef CONFIG_QTNA_WIFI
		WPA_ASSERT(len + sizeof(*eth) <= sizeof(hdr_buf));

		eth = (struct l2_ethhdr *)hdr_buf;
		os_memcpy(eth->h_dest, dest, ETH_ALEN);
		os_memcpy(eth->h_source, wpa_s->own_addr, ETH_ALEN);
		eth->h_proto = host_to_be16(proto);
		os_memcpy(hdr_buf + sizeof(*eth), buf, len);

		return l2_packet_send(wpa_s->l2, dest, proto, hdr_buf, len + sizeof(*eth));
#else

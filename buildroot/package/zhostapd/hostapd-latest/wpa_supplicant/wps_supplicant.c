				   ...
				   const struct wps_credential *cred)
{
	...
	os_free(ssid->ssid);
#ifdef CONFIG_QTNA_WIFI
	wpa_s->wps_credential_ssid = NULL;
#endif
...
#ifdef CONFIG_QTNA_WIFI
		wpa_s->wps_credential_ssid = ssid;
#endif
	}
...
#if defined(CONFIG_SAE) && defined(CONFIG_QTNA_WIFI)
		if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SAE &&
				wpa_s->conf->easymesh_backhaul_sta &&
				(wpa_s->conf->easymesh_backhaul_sta_profile >=
							MAP_BACKHAUL_STA_PROFILE_2)) {
	...
	if (wpa_s->conf->easymesh_backhaul_prefer_ssid_len > 0) {
		os_free(wpa_s->conf->easymesh_backhaul_prefer_ssid);
		wpa_s->conf->easymesh_backhaul_prefer_ssid_len = 0;
		wpa_s->conf->easymesh_backhaul_prefer_ssid = NULL;
	}
	if (wpa_s->is_map_fronthaul_bss) {
		wpa_printf(MSG_WARNING, "WPS: MAP fronthaul BSS ");
		wpa_s->conf->easymesh_backhaul_prefer_ssid = os_malloc(cred->ssid_len);
		if (wpa_s->conf->easymesh_backhaul_prefer_ssid) {
			os_memcpy(wpa_s->conf->easymesh_backhaul_prefer_ssid, cred->ssid,
				cred->ssid_len);
			wpa_s->conf->easymesh_backhaul_prefer_ssid_len = cred->ssid_len;
			wpa_hexdump_ascii(MSG_DEBUG, "Backhaul SSID",
				(u8 *)wpa_s->conf->easymesh_backhaul_prefer_ssid,
				wpa_s->conf->easymesh_backhaul_prefer_ssid_len);
		}
	} else {
		wpa_printf(MSG_DEBUG, "WPS: non MAP fronthaul BSS ");
	}
#endif /* CONFIG_QTNA_WIFI */
	...
	if (wpa_s->non_wps_pp_enable && wpa_s->conf->update_config &&
		(wpa_supplicant_record_wps_ssid(wpa_s, (char *)ssid->ssid, ssid->ssid_len) < 0)) {
...
static const char *wps_ext_state_to_str( enum wps_external_state external_state )
{
	...
	for (iter = 0; state_to_str[iter].wps_external_state != WPS_EXT_STATE_UNKNOWN &&
			retaddr == NULL; iter++) {
...
#ifdef CONFIG_QTNA_WIFI
	os_memset(msg, 0, sizeof(msg));
	os_snprintf(msg, sizeof(msg)-1, msg_format, wps_state_to_str(fail->msg), fail->config_error,
			wps_m2d_to_str(fail->config_error));
	wpa_supplicant_send_wlan_msg(wpa_s, msg);
#endif /* CONFIG_QTNA_WIFI */
...
static void wpa_supplicant_wps_event_success(struct wpa_supplicant *wpa_s)
{
#ifdef CONFIG_QTNA_WIFI
	char msg[MAX_WLAN_MSG_LEN];
	const char *msg_format = "STA interface %s WPS success with AP [" MACSTR "] received SSID %s";
#endif /* CONFIG_QTNA_WIFI */
	...
	os_memset(msg, 0, sizeof(msg));
	os_snprintf(msg, sizeof(msg)-1, msg_format,
			wpa_s->ifname, MAC2STR(wpa_s->bssid),
			wpa_s->wps_credential_ssid && wpa_s->wps_credential_ssid->ssid ?
			wpa_ssid_txt(wpa_s->wps_credential_ssid->ssid,
				wpa_s->wps_credential_ssid->ssid_len) : "");
	wpa_supplicant_send_wlan_msg(wpa_s, msg);
#endif /* CONFIG_QTNA_WIFI */
 ...
 */
	...
	case WPS_EV_TIMEOUT:
		...
		break;
	/**
	 * FIXME: The following events are not handled in 'wpa_supplicant-1.1' codebase.
	 * Adding them here to resolve compiler warnings.
				       ...
				       struct wps_context *wps)
{
	...
	if (wpa_s->conf->easymesh_backhaul_sta == 0) {
		wpa_set_app_ie(wpa_s, IEEE80211_APPIE_FRAME_ASSOC_REQ, NULL);
		return;
	}

	appie = wpabuf_alloc(MAP_EXT_IELEN_MAX);

	if (appie == NULL)
		return;

	wpabuf_put_u8(appie, WLAN_EID_VENDOR_SPECIFIC);
	len = wpabuf_put(appie, 1);
	wpabuf_put_be24(appie, OUI_WFA);
	wpabuf_put_u8(appie, MAP_OUI_TYPE);

	wpabuf_put_u8(appie, MAP_EXT_SUBELE_ID);
	wpabuf_put_u8(appie, MAP_EXT_SUBELE_ID_LEN);
	wpabuf_put_u8(appie, MAP_EXT_SUBELE_BACKHAUL_STA);

	if (wpa_s->conf->easymesh_backhaul_sta_profile >= MAP_BACKHAUL_STA_PROFILE_2) {
		wpabuf_put_u8(appie, MAP_PROFILE_SUBELE_ID);
		wpabuf_put_u8(appie, MAP_PROFILE_SUBELE_ID_LEN);
		wpabuf_put_u8(appie, wpa_s->conf->easymesh_backhaul_sta_profile);
	}

	/* Vendor MAP IE payload length */
	*len = wpabuf_len(appie) - 2;
	wpa_set_app_ie(wpa_s, IEEE80211_APPIE_FRAME_ASSOC_REQ, appie);
	wpabuf_free(appie);
...
int wpas_wps_init(struct wpa_supplicant *wpa_s)
{
	...
	dl_list_init(&wpa_s->wps_pbc_ssid_filters);
	wpa_s->wps_pbc_ssid_filter_cnt = 0;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
static void wpas_wps_pbc_ssid_filters_flush(struct wpa_supplicant *wpa_s)
{
	struct wps_pbc_filter *filter, *n;

	if (wpa_s->wps_pbc_ssid_filters.next == NULL)
		return; /* WPS PBC SSID filter list is empty */

	dl_list_for_each_safe(filter, n, &wpa_s->wps_pbc_ssid_filters, struct wps_pbc_filter, list) {
		dl_list_del(&filter->list);
		wpa_s->wps_pbc_ssid_filter_cnt--;
		wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Delete PBC SSID filter %s", filter->ssid_substr);
		os_free(filter);
	}
}
...
{
	struct wps_pbc_filter *filter;

	if (wpa_s->wps_pbc_ssid_filter_cnt >= WPS_PBC_SSID_FILTERS_MAX)
		return -1;
	if (os_strlen(ssid_substr) == 0 || os_strlen(ssid_substr) >= sizeof(filter->ssid_substr))
		return -1;

	/* Do not add the SSID if it contains one of the existing filters. */
	if (!dl_list_empty(&wpa_s->wps_pbc_ssid_filters)) {
		dl_list_for_each(filter, &wpa_s->wps_pbc_ssid_filters, struct wps_pbc_filter, list) {
			if (os_strstr(ssid_substr, filter->ssid_substr))
				return -1;
		}
	}
	...
	if (filter == NULL)
		return -1;
	os_strncpy(filter->ssid_substr, ssid_substr, sizeof(filter->ssid_substr));

	dl_list_add_tail(&wpa_s->wps_pbc_ssid_filters, &filter->list);
	wpa_s->wps_pbc_ssid_filter_cnt++;
	wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Add PBC SSID filter %s", filter->ssid_substr);
	return 0;
}
...
{
	struct wps_pbc_filter *filter, *n;
	int is_filter_found = 0;

	if (dl_list_empty(&wpa_s->wps_pbc_ssid_filters))
		return -1;
	if (os_strlen(ssid_substr) >= sizeof(filter->ssid_substr))
		return -1;
	if (os_strlen(ssid_substr) == 0) {
		wpas_wps_pbc_ssid_filters_flush(wpa_s);
		return 0;
	}

	/* Delete all filters that contain the SSID substring. */
	dl_list_for_each_safe(filter, n, &wpa_s->wps_pbc_ssid_filters, struct wps_pbc_filter, list) {
		if (os_strstr(filter->ssid_substr, ssid_substr)) {
			is_filter_found = 1;
			dl_list_del(&filter->list);
			wpa_s->wps_pbc_ssid_filter_cnt--;
			wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Delete PBC SSID filter %s",
				filter->ssid_substr);
			os_free(filter);
		}
	}
...
{
	struct wps_pbc_filter *filter;
	char *next_filter_pos = filters_str;
	size_t remained_len = filters_len;
	int i = 0;
	...
	if (dl_list_empty(&wpa_s->wps_pbc_ssid_filters)) {
		os_snprintf(filters_str, filters_len, "none\n");
		return os_strlen("none\n");
	}

	dl_list_for_each(filter, &wpa_s->wps_pbc_ssid_filters, struct wps_pbc_filter, list) {
		if (remained_len < os_strlen(filter->ssid_substr) + 1) {
			os_snprintf(filters_str, filters_len,
					"%zu bytes is not enough for the filters list.\n",
					filters_len);
			break;
		}
		if (++i == wpa_s->wps_pbc_ssid_filter_cnt) {
			os_snprintf(next_filter_pos, remained_len, "%s\n", filter->ssid_substr);
			break;
		} else {
			os_snprintf(next_filter_pos, remained_len, "%s,", filter->ssid_substr);
		}

		next_filter_pos = next_filter_pos + os_strlen(filter->ssid_substr) + 1;
		remained_len = remained_len - os_strlen(filter->ssid_substr) - 1;
	}

	return os_strlen(filters_str);
}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
		/* SSID substring filter */
		if (wpa_s->wps_pbc_ssid_filter_cnt) {
			ssid_txt = wpa_ssid_txt(ssid_, ssid_len);
			match_pbc_filter = 0;
			dl_list_for_each(filter, &wpa_s->wps_pbc_ssid_filters,
					struct wps_pbc_filter, list) {
				if (os_strstr(ssid_txt, filter->ssid_substr)) {
					match_pbc_filter = 1;
					break;
				}
			}
			if (!match_pbc_filter) {
				wpa_printf(MSG_DEBUG, "   skip - WPS AP "
					"PBC Registrar SSID '%s' not in the filter pool", ssid_txt);
				wpabuf_free(wps_ie);
				return 0;
			}
		}
			      ...
			      struct wpa_bss *selected, struct wpa_ssid *ssid)
{
	...
	for (i = 0; i < wpa_s->num_wps_ap; i++) {
		 ...
		 */
		if (sel_uuid != NULL && ap->uuid != NULL &&
			os_memcmp(sel_uuid, ap->uuid, UUID_LEN) == 0) {
			wpa_printf(MSG_INFO, "No PBC overlap - same UUID\n");
		} else if (os_memcmp(ap->bssid, selected->bssid, ETH_ALEN) != 0) {
			ret = 1; /* PBC overlap */

...
#ifdef CONFIG_QTNA_WIFI
static int wpa_set_ssids_from_backhaul_ssid(struct wpa_supplicant *wpa_s,
				       struct wpa_driver_scan_params *params)
{
	...
	if (!wpa_s->conf->easymesh_backhaul_sta || !wpa_s->conf->easymesh_backhaul_prefer_ssid ||
			(wpa_s->conf->easymesh_backhaul_prefer_ssid_len == 0))
		return 0;

	for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next)
		if ((wpa_s->conf->easymesh_backhaul_prefer_ssid_len == ssid->ssid_len) &&
			!os_memcmp(wpa_s->conf->easymesh_backhaul_prefer_ssid, ssid->ssid,
				ssid->ssid_len))
			break;

	if (ssid) {
		wpa_s->map_backhaul_ssid = ssid;
...
#ifdef CONFIG_QTNA_WIFI
	wpa_s->map_backhaul_ssid = NULL;
	if (wpa_s->last_scan_req != MANUAL_SCAN_REQ && !wpas_wps_in_progress(wpa_s) &&
		wpa_set_ssids_from_backhaul_ssid(wpa_s, &params)) {
		wpa_printf(MSG_DEBUG, "Use specific SSIDs from backhaul network");
		goto ssid_list_set;
	}
#endif
...
#ifdef CONFIG_QTNA_WIFI
	/*
	 * Just perform specific SSID scan if 'specific_scan' is set
	 */
	if ((wpa_s->conf->specific_scan == 1) &&
...
#ifdef CONFIG_QTNA_WIFI
	if (sec != QTNA_INVALID_SCAN_INTERVAL) {
#endif /* CONFIG_QTNA_WIFI */
		new_int.sec = sec;
		new_int.usec = 0;
...
#ifdef CONFIG_QTNA_WIFI
	/* MBS/RBS has higer priority than others */
	if ((wa->qtn_extender_role != IEEE80211_EXTENDER_ROLE_NONE) &&
			(wb->qtn_extender_role == IEEE80211_EXTENDER_ROLE_NONE))
		return -1;
	else if ((wa->qtn_extender_role == IEEE80211_EXTENDER_ROLE_NONE) &&
			(wb->qtn_extender_role != IEEE80211_EXTENDER_ROLE_NONE))
		return 1;
#endif /* CONFIG_QTNA_WIFI */
...
/*
 * Compare function for sorting scan results. Return >0 if @b is considered
 * better, returns 0 if all measurable parameters are equal.
 */
static int wpa_scan_result_rssi_compar(const void *a, const void *b)
{
	struct wpa_scan_res **_wa = (void *) a;
	struct wpa_scan_res **_wb = (void *) b;
	struct wpa_scan_res *wa = *_wa;
	struct wpa_scan_res *wb = *_wb;
	int maxrate_a, maxrate_b;

#define MIN_RECOG_SIGNAL_LEVEL	5
#define MIN_RECOG_SIGNAL_QUAL	10
	/* best/max rate preferred if signal level close enough XXX */
	if ((wa->level && wb->level &&
		abs(wb->level - wa->level) < MIN_RECOG_SIGNAL_LEVEL) ||
		(wa->qual && wb->qual &&
		 abs(wb->qual - wa->qual) < MIN_RECOG_SIGNAL_QUAL)) {
...
struct wpa_scan_results *
wpa_supplicant_get_qtn_ext_attr_scan_results(struct wpa_supplicant *wpa_s,
		struct wpa_scan_results *scan_res)
{
	size_t i;
	...
	int non_qtn_ext_pbc_sle_bss_fud = 0;

	wpa_printf(MSG_DEBUG, "Try to find quantenna extension "
			"attribute contained WPA-enabled AP");

	wpa_s->wps_only_qtn_attr_fud = 0;

	wps_qtn_ext_attr_bss = os_zalloc(sizeof(*wps_qtn_ext_attr_bss));
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if ((wps_is_selected_pbc_registrar(wps_ie)) &&
				(!wps_is_allowed_pbc_overlap(wps_ie))) {
			wpa_printf(MSG_DEBUG, "None qtn extension attribute PBC"
					" selected bss found, need to check PBC overlap");
			non_qtn_ext_pbc_sle_bss_fud = 1;
			wpabuf_free(wps_ie);
			break;
		}

		if (wps_is_allowed_pbc_overlap(wps_ie)) {
			tmp = os_realloc(wps_qtn_ext_attr_bss->res,
				(wps_qtn_ext_attr_bss->num + 1) * sizeof(struct wpa_scan_res *));
			if (tmp == NULL) {
				wpabuf_free(wps_ie);
			...
			tmp[wps_qtn_ext_attr_bss->num++] = bss;
			wps_qtn_ext_attr_bss->res = tmp;
		}
		wpabuf_free(wps_ie);
	}

	if ((!non_qtn_ext_pbc_sle_bss_fud) &&
		(wps_qtn_ext_attr_bss) && (wps_qtn_ext_attr_bss->num > 0)) {
		wpa_s->wps_only_qtn_attr_fud = 1;
		qsort(wps_qtn_ext_attr_bss->res, wps_qtn_ext_attr_bss->num,
				sizeof(struct wpa_scan_res *), wpa_scan_result_rssi_compar);

		for (i = 0; i < wps_qtn_ext_attr_bss->num; i++) {
			bss = wps_qtn_ext_attr_bss->res[i];
			wpa_printf(MSG_DEBUG, MACSTR ": qual=%d, level=%d",
					MAC2STR(bss->bssid), bss->qual, bss->level);
		}

		return wps_qtn_ext_attr_bss;
	}

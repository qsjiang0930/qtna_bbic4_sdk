...
#ifdef CONFIG_QTNA_WIFI
static char *wpa_config_write_str_extended(const struct parse_data *data,
				   struct wpa_ssid *ssid)
{
	size_t len;
	char **src;
...
#ifdef CONFIG_QTNA_WIFI
	strncpy(temp, value, PMK_LEN * 2);
	qtn_util_md5_convert_passphrase(temp, psk_md5);
	if (hexstr2bin(psk_md5, ssid->psk_md5, PMK_LEN)) {
		wpa_printf(MSG_ERROR, "Line %d: Invalid PSK_MD5 '%s'.",
			   line, psk_md5);
		return -1;
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	wpa_hexdump_key(MSG_MSGDUMP, "PSK_MD5", ssid->psk_md5, PMK_LEN);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
#define _STRx(f) #f, wpa_config_parse_str, OFFSET(f)
#endif /* CONFIG_QTNA_WIFI */
#define _STRe(f) #f, wpa_config_parse_str, OFFSET(eap.f)
#else /* NO_CONFIG_WRITE */
#define _STR(f) #f, wpa_config_parse_str, wpa_config_write_str, OFFSET(f)
#ifdef CONFIG_QTNA_WIFI
#define _STRx(f) #f, wpa_config_parse_str, wpa_config_write_str_extended, OFFSET(f)
#endif /* CONFIG_QTNA_WIFI*/
...
#ifdef CONFIG_QTNA_WIFI
#define STRx(f) _STRx(f), NULL, NULL, NULL, 0
#define _STR_LENx(f) _STRx(f), OFFSET(f ## _len)
#define _STR_RANGEx(f, min, max) _STR_LENx(f), (void *) (min), (void *) (max)
#define STR_RANGEx(f, min, max) _STR_RANGEx(f, min, max), 0
#endif /* CONFIG_QTNA_WIFI*/
...
#ifdef CONFIG_QTNA_WIFI
	/*
	 * STR_RANGE do not allow ASCII >= 127 (field is written to file as hexadecimal);
	 * STR_RANGEx means allow ASCII chars >= 127 (field is written to file in normal ASCII).
	 */
	{ STR_RANGEx(ssid, 0, SSID_MAX_LEN) },
#else
...
#endif /* CONFIG_MBO */
#ifdef CONFIG_QTNA_WIFI
	os_free(config->easymesh_backhaul_prefer_ssid);
#endif
...
#ifdef CONFIG_QTNA_WIFI
	ssid->scan_ssid = DEFAULT_SSID_SCAN;
#endif /* CONFIG_QTNA_WIFI */
}
...
#ifdef CONFIG_QTNA_WIFI
int wpa_check_wps_ssid(const char *confname, const char *ssid)
{
	int conf_len;
	char *pp_confname;
	FILE *stream;
	...
	conf_len = strlen(confname);
	pp_confname = (char *)os_malloc(conf_len + 4);
	if (!pp_confname)
		return -1;

	memset(pp_confname, 0, (conf_len + 4));
	os_strncpy(pp_confname, confname, (conf_len + 1));
	strcat(pp_confname, ".pp");
	wpa_printf(MSG_DEBUG, "pp_confname is %s \n", pp_confname);

	stream = fopen(pp_confname, "r");
	if (!stream) {
		wpa_printf(MSG_DEBUG, "%s not exist \n", pp_confname);
		os_free(pp_confname);
		return ssid_in;
	}
	...
	while(fgets(buf, sizeof(buf), stream)) {
		...
		if (!os_strncmp(buf, ssid, sizeof(buf))) {
			wpa_printf(MSG_DEBUG, "%s is found", ssid);
			ssid_in = 1;
			break;
		}
	}
...
#ifndef CONFIG_NO_PBKDF2
#ifdef CONFIG_QTNA_WIFI
	string_64 passphrase_md5 = {0};

	qtn_util_md5_convert_passphrase(ssid->passphrase, passphrase_md5);
	ssid->passphrase_md5 = os_strdup(passphrase_md5);
	wpa_printf(MSG_DEBUG, "passphrase is %s, and passphrase_md5 is %s", ssid->passphrase,
			passphrase_md5);
#endif /* CONFIG_QTNA_WIFI*/
...
#ifdef CONFIG_QTNA_WIFI
	pbkdf2_sha1(passphrase_md5,
		    ssid->ssid, ssid->ssid_len, 4096,
		    ssid->psk_md5, PMK_LEN);
	wpa_hexdump(MSG_DEBUG, "PSK MD5 (from passphrase_md5)",
			ssid->psk_md5, PMK_LEN);
#endif /* CONFIG_QTNA_WIFI*/
...
#ifdef CONFIG_QTNA_WIFI
	config->specific_scan = DEFAULT_SPECIFIC_SCAN;
	config->scan_buf_max_size = DEFAULT_MAX_SCAN_RESULT_BUF;
#ifdef CONFIG_WPS
	config->wps_state = DEFAULT_WPS_STATE;
	config->wps_allow_pbc_overlap = WPS_FORBID_PBC_OVERLAP;
#endif /* CONFIG_WPS */
#endif /* CONFIG_QTNA_WIFI*/
...
#ifdef CONFIG_QTNA_WIFI
	config->easymesh_backhaul_sta_profile = DEFAULT_MAP_BACKHAUL_STA_PROFILE;
#endif
#ifdef CONFIG_MBO
...
#ifdef CONFIG_QTNA_WIFI
static int wpa_global_config_process_ssid(
	const struct global_parse_data *data,
	struct wpa_config *config, int line, const char *value)
{
	char *tmp, *dup_tmp;
	...
	if (!value || (os_strcmp(value, "NULL") == 0)) {
		wpa_printf(MSG_ERROR, "Unset configuration string '%s'", data->name);
		return -1;
	}
	...
	if ((res_len <= 0) || (res_len > SSID_MAX_LEN)) {
		os_free(tmp);
		wpa_printf(MSG_ERROR, "Line %d: invalid %s", line, data->name);
		return -1;
	}

	dup_tmp = os_strdup(tmp);
	if (dup_tmp == NULL) {
		os_free(tmp);
		return -1;
	}
	...
	os_free(tmp);
	dst_ssid = (char **) (((u8 *) config) + (long) data->param1);
	os_free(*dst_ssid);
	*dst_ssid = dup_tmp;
	dst_len = (int *) (((u8 *) config) + (long) data->param2);
	*dst_len = res_len;
	wpa_hexdump_ascii(MSG_MSGDUMP, data->name, (u8 *) dup_tmp, res_len);
...
#define IPV4(f) #f, wpa_global_config_parse_ipv4, wpa_config_get_ipv4,  \
	OFFSET(f), NULL, NULL
#ifdef CONFIG_QTNA_WIFI
#define FUNC_SSID(f) #f, wpa_global_config_process_ssid, NULL, \
	OFFSET(f), OFFSET(f ## _len), NULL
#endif
...
#ifdef CONFIG_QTNA_WIFI
	{ INT(specific_scan), 0 },
	{ INT(scan_buf_max_size), 0 },
	{ STR_RANGE(pairing_id, 0, 32) },
	{ INT_RANGE(pairing_enable, 0, 2), 0 },
#endif /* CONFIG_QTNA_WIFI */
	{ FUNC(bgscan), 0 },
#ifdef CONFIG_MESH
	{ INT(user_mpm), 0 },
...
#ifdef CONFIG_WPS
#ifdef CONFIG_QTNA_WIFI
	{ INT_RANGE(wps_state, 0, 1) },
	{ INT_RANGE(wps_allow_pbc_overlap, 0, 1), 0 },
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	{ INT_RANGE(easymesh_backhaul_sta, MAP_BACKHAUL_STA_MIN, MAP_BACKHAUL_STA_MAX),
				CFG_CHANGED_EASYMESH_BACKHAUL_STA},
	{ INT_RANGE(easymesh_backhaul_sta_profile, MAP_BACKHAUL_STA_PROFILE_MIN,
				MAP_BACKHAUL_STA_PROFILE_MAX), CFG_CHANGED_EASYMESH_BACKHAUL_STA},
#endif /* CONFIG_QTNA_WIFI */
#endif /* CONFIG_WPS */
#ifdef CONFIG_P2P
...
#ifdef CONFIG_QTNA_WIFI
	{ FUNC_SSID(easymesh_backhaul_prefer_ssid), 0 },
#endif /* CONFIG_QTNA_WIFI */
};
...
#ifdef CONFIG_QTNA_WIFI
char *wpa_config_get_global_str(struct wpa_config *config, const char *var)
{
	size_t i;
	size_t len;
	char **src;
	const struct global_parse_data *field = NULL;
	size_t flen;

	if (config == NULL || var == NULL)
		return NULL;

	for (i = 0; i < NUM_GLOBAL_FIELDS; i++) {
		field = &global_fields[i];
		flen = os_strlen(field->name);

		if (os_strncmp(var, field->name, flen) == 0)
			break;
	}
	...
	src = (char **) (((u8 *) config) + (long) field->param1);
	if (*src == NULL)
		return NULL;

	if (field->param2)
		len = *((size_t *) (((u8 *) config) + (long) field->param2));
	else
		len = os_strlen(*src);

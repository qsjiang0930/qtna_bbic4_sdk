...
struct wpa_config * wpa_config_read(const char *name, struct wpa_config *cfgp)
{
	...
	if (!config->easymesh_backhaul_sta && (config->easymesh_backhaul_prefer_ssid_len > 0)) {
		wpa_printf(MSG_WARNING, "Due to non backhaul station, clear prefer backhaul BSS");
		os_free(config->easymesh_backhaul_prefer_ssid);
		config->easymesh_backhaul_prefer_ssid_len = 0;
		config->easymesh_backhaul_prefer_ssid = NULL;
	}
...
#define INT_DEFe(t, def) write_int(f, #t, ssid->eap.t, def)
	...
	STR(id_str);
#ifdef CONFIG_QTNA_WIFI
	INT_DEF(flags, NETWORK_DEFAULT_FLAGS);
#endif /* CONFIG_QTNA_WIFI */
#ifdef CONFIG_P2P
...
#ifdef CONFIG_QTNA_WIFI
static void wpa_config_write_global_str(FILE *f, const char *field, struct wpa_config *config)
{
	char *value = wpa_config_get_global_str(config, field);
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
	...
	if (config->ap_scan != DEFAULT_AP_SCAN)
		fprintf(f, "ap_scan=%d\n", config->ap_scan);
#ifdef CONFIG_QTNA_WIFI
	if (config->specific_scan != DEFAULT_SPECIFIC_SCAN)
		fprintf(f, "specific_scan=%d\n", config->specific_scan);
	if (config->pairing_id)
		fprintf(f, "pairing_id=%s\n", config->pairing_id);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_WPS
#ifdef CONFIG_QTNA_WIFI
	if (config->wps_state != DEFAULT_WPS_STATE)
		fprintf(f, "wps_state=%d\n", config->wps_state);
	if (config->wps_allow_pbc_overlap)
		fprintf(f, "wps_allow_pbc_overlap=%d\n",
			config->wps_allow_pbc_overlap);
#endif /* CONFIG_QTNA_WIFI */
	...
	if (config->easymesh_backhaul_sta_profile)
		fprintf(f, "easymesh_backhaul_sta_profile=%d\n",
						config->easymesh_backhaul_sta_profile);
	if (config->easymesh_backhaul_sta && config->easymesh_backhaul_prefer_ssid_len)
		wpa_config_write_global_str(f, "easymesh_backhaul_prefer_ssid", config);
#endif /* CONFIG_QTNA_WIFI */

			     ...
			     struct os_reltime *fetch_time)
{
	...
	dst->qtn_extender_role = src->qtn_extender_role;
...
#ifdef CONFIG_QTNA_WIFI
/**
 * wpa_get_bss_by_bssid_freq_ssid - Fetch a BSS table entry based on BSSID and freq and essid
 * @wpa_s: Pointer to wpa_supplicant data
 * @bssid: BSSID
 * @freq: frequency
 * @essid: ESSID
 * @essid_len: length of essid
 * Returns: Pointer to the BSS entry or %NULL if not found
 */
struct wpa_bss * wpa_get_bss_by_bssid_freq_essid(struct wpa_supplicant *wpa_s,
				const u8 *bssid, int freq, u8 *essid, size_t essid_len)
{
	struct wpa_bss *bss;
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		if (os_memcmp(bss->bssid, bssid, ETH_ALEN) == 0 &&
				bss->freq == freq &&
				bss->ssid_len == essid_len &&
				os_memcmp(bss->ssid, essid, essid_len) == 0)
			return bss;
	}
	return NULL;
}

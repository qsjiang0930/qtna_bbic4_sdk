				   ...
				   enum wnm_oper oper)
{
	...
	return wpa_drv_wnm_oper(wpa_s, oper, addr, buf, &len);
#else
...
#ifdef CONFIG_QTNA_WIFI
	/* Disassociate with current BSS first */
	wpa_drv_disassociate(wpa_s, wpa_s->current_bss->bssid, WLAN_REASON_DEAUTH_LEAVING);
#endif /* CONFIG_QTNA_WIFI */
	wpa_s->reassociate = 1;
...
send_bss_resp_fail:
	...
	wnm_deallocate_memory(wpa_s);
#ifdef CONFIG_QTNA_WIFI
	if (wpa_s->wpa_state == WPA_DISCONNECTED || wpa_s->wpa_state == WPA_SCANNING)

...
#ifdef CONFIG_QTNA_WIFI
static inline int wpa_drv_set_fast_reassoc(struct wpa_supplicant *wpa_s,
						int value, int *cur_value)
{
	if (wpa_s->driver->set_fast_reassoc) {
		return wpa_s->driver->set_fast_reassoc(wpa_s->drv_priv,
							value, cur_value);
	}
...
static inline int wpa_drv_set_pmf(struct wpa_supplicant *wpa_s, int value)
{
	if (!wpa_s->driver->set_pmf)
		return -1;

	return wpa_s->driver->set_pmf(wpa_s->drv_priv, value);
}

static inline int wpa_drv_get_pmf(struct wpa_supplicant *wpa_s)
{
	if (!wpa_s->driver->get_pmf)
		return -1;

	return wpa_s->driver->get_pmf(wpa_s->drv_priv);
}

static inline int wpa_drv_set_pairing_hash_ie(struct wpa_supplicant *wpa_s,
						const u8 *ies, size_t ies_len)
{
	if (wpa_s->driver->set_pairing_hash_ie)
		return wpa_s->driver->set_pairing_hash_ie(wpa_s->drv_priv, ies,
								ies_len);
	return -1;
}

static inline int wpa_drv_get_pairing_hash_ie(struct wpa_supplicant *wpa_s,
						u8 *ies, size_t ies_len, u8 *addr)
{
	if (wpa_s->driver->get_pairing_hash_ie)
		return wpa_s->driver->get_pairing_hash_ie(wpa_s->drv_priv, ies,
								ies_len, addr);
	return -1;
}
...
#ifdef CONFIG_QTNA_WIFI
static inline int wpa_set_app_ie(struct wpa_supplicant *wpa_s,
			u8 subtype, struct wpabuf *appie)
{
	if (!wpa_s->driver->set_app_ie)
		return -1;
	return wpa_s->driver->set_app_ie(wpa_s->drv_priv, subtype, appie);
}
#endif /* CONFIG_QTNA_WIFI */

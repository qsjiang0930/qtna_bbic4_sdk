...
#ifdef CONFIG_QTNA_WIFI
static int
radius_server_config_diff(struct hostapd_radius_server *nserv,
				struct hostapd_radius_server *oserv)
{
	if (&nserv->addr == NULL || !nserv->port)
		return 0;

	if (hostapd_ip_diff(&nserv->addr, &oserv->addr) || nserv->port != oserv->port)
		return 1;
...
static void
radius_unregister_sock(int sock)
{
	if (sock > 0) {
...
void radius_client_reconfig(struct radius_client_data *radius,
			    struct hostapd_radius_servers *conf)
{
	int auth_update_required = 0;
	int acct_update_required = 0;

	if (radius) {
		if (conf->auth_server) {

			if (radius->auth_sock < 0)
				auth_update_required = 1;

			/* Check the IP Address and Port difference */
			if (radius->conf &&
			    radius_server_config_diff(conf->auth_server,
						radius->conf->auth_server)) {
				radius_unregister_sock(radius->auth_sock);
				auth_update_required = 1;
			}
		...
		if (conf->acct_server) {
			...
			if (radius->acct_sock < 0)
				acct_update_required = 1;

			if (radius->conf && radius_server_config_diff(conf->acct_server,
					radius->conf->acct_server)) {
				radius_unregister_sock(radius->acct_sock);
				acct_update_required = 1;
			}
		}
		...
		if (auth_update_required) {
			if (radius_client_init_auth(radius))
				wpa_printf(MSG_ERROR, "Radius client auth initialization failed");
		}

		if (acct_update_required) {
			if (radius_client_init_acct(radius))
				wpa_printf(MSG_ERROR, "Radius client acct initialization failed");
		}

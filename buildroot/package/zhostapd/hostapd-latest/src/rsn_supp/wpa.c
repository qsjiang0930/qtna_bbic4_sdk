			  ...
			  const struct wpa_eapol_key *key, struct wpa_ptk *ptk)
{
	...
	if (sm->md5_used) {
		wpa_printf(MSG_DEBUG, "%s: md5 pmk is used \n", __func__);
		wpa_hexdump(MSG_DEBUG, "sm->pmk_md5 :", sm->pmk_md5, sm->pmk_len);

		return wpa_pmk_to_ptk(sm->pmk_md5, sm->pmk_len, "Pairwise key expansion",
			      sm->own_addr, sm->bssid, sm->snonce,
			      key->key_nonce, ptk, sm->key_mgmt,
			      sm->pairwise_cipher);
	} else {
		wpa_printf(MSG_DEBUG, "%s: original pmk is used \n", __func__);
		wpa_hexdump(MSG_DEBUG, "sm->pmk :", sm->pmk, sm->pmk_len);
	}
					    ...
					    const u8 *addr, int secure)
{
	...
	if (secure) {
#ifdef CONFIG_QTNA_WIFI
		/* Message for the wireless_event manager */
		char msg[MAX_WLAN_MSG_LEN];
		/* Find the WPA version and cipher */
		const char *format_string = "Connected to AP [" MACSTR "] [%s/%s] SSID %s";
		os_memset(msg, 0, sizeof(msg));
		os_snprintf(msg, sizeof(msg)-1, format_string,
				MAC2STR(addr),
					  ...
					  size_t key_data_len)
{
	...
	if (wpa_sm_get_state(sm) == WPA_COMPLETED) {
		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Ignored duplicate message"
			" 3 of 4-Way Handshake from " MACSTR " (ver=%d)",
...
#ifdef CONFIG_QTNA_WIFI
#ifdef CONFIG_TDLS
	sm->ap_role = IEEE80211_EXTENDER_ROLE_NONE;
	os_memset(sm->mbs_bssid, 0, sizeof(sm->mbs_bssid));
	sm->rbs_num = 0;
	os_memset(sm->rbs_bssid[0], 0, sizeof(sm->rbs_bssid));
#endif
#endif /* CONFIG_QTNA_WIFI */
	return sm;
}
...
#ifdef CONFIG_QTNA_WIFI
void wpa_sm_dump_pmk_md5(struct wpa_sm *sm)
{
	wpa_hexdump(MSG_DEBUG, "wpa_sm_dump_psk_md5 :", sm->pmk_md5, sm->pmk_len);
}

/**
 * wpa_sm_set_pmk_md5 - Set PMK MD5
 * @sm: Pointer to WPA state machine data from wpa_sm_init()
 * @pmk_md5: The new PMK from MD5
 * @pmk_len: The length of the new PMK in bytes
 *
 * Configure the PMK_MD% for WPA state machine.
 */
void wpa_sm_set_pmk_md5(struct wpa_sm *sm, const u8 *pmk_md5, size_t pmk_len)
{
	if (sm == NULL)
		return;

	wpa_hexdump(MSG_DEBUG, "PMK MD5 : ", pmk_md5, pmk_len);

	sm->pmk_len = pmk_len;
	os_memcpy(sm->pmk_md5, pmk_md5, pmk_len);

	wpa_hexdump(MSG_DEBUG, "sm->PMK_MD5 :", sm->pmk_md5, pmk_len);
	wpa_sm_dump_pmk_md5(sm);

#ifdef CONFIG_IEEE80211R
	/* Set XXKey to be PSK for FT key derivation */
	sm->xxkey_len = pmk_len;
	os_memcpy(sm->xxkey, pmk_md5, pmk_len);
#endif /* CONFIG_IEEE80211R */
}

void wpa_sm_set_md5_used(struct wpa_sm *sm, int md5_used)
{
	if (sm == NULL)
		return;

	wpa_printf(MSG_DEBUG, "PSK MD5 USED is %d \n", md5_used);
	sm->md5_used = md5_used;
}
#endif /* CONFIG_QTNA_WIFI */

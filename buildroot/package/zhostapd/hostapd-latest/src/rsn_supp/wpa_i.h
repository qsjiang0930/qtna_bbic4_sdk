...
struct wpa_sm {
	...
	u8 pmk_md5[PMK_LEN];
	int md5_used;	/* Add PSK md5 flag for deriving PTK in the 4-way handshake */
...
#define	IEEE80211_EXTENDER_ROLE_RBS 2
	u8 ap_role;
	u8 mbs_bssid[ETH_ALEN];
	u8 rbs_num;
	u8 rbs_bssid[WPA_QTN_MAX_RBS_NUM][ETH_ALEN];
#endif /* CONFIG_QTNA_WIFI */
...
static inline int wpa_sm_mlme_setprotection(struct wpa_sm *sm, const u8 *addr,
					    int protect_type, int key_type, const char *msg)
{

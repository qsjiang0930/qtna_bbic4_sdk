...
#ifdef CONFIG_QTNA_WIFI
		ret = hostapd_vlan_group_add(hapd, hapd->conf->iface, vlan->vlan_id);
#else
...
#ifdef CONFIG_QTNA_WIFI
		return hostapd_vlan_group_remove(hapd, hapd->conf->iface, vlan->vlan_id);
#else
...
#ifdef CONFIG_QTNA_WIFI
static void vlan_reset_wpa_group(struct hostapd_data *hapd)
{
	struct wpa_group *group;

	/*
	 * For each VLAN group, mark first_sta_seen FALSE so that the GTK is re-calculated
	 * and distributed when it's created again
	 */
	if (hapd->wpa_auth) {
		group = hapd->wpa_auth->group;
		while (group) {
			if (group->vlan_id > 0 && group->vlan_id != VLAN_ID_WILDCARD)

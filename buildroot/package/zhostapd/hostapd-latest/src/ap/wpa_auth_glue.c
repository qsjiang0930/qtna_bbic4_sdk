				  ...
				  struct wpa_auth_config *wconf)
{
	...
	if (is_zero_ether_addr(conf->r1_key_holder)) {
		wpa_printf(MSG_ERROR, "%s R1 key holder value is 0, setting to " MACSTR,
				 __func__, MAC2STR(conf->bssid));
		os_memcpy(conf->r1_key_holder, conf->bssid, FT_R1KH_ID_LEN);
	}
...
#ifdef CONFIG_QTNA_WIFI
	wpa_printf(MSG_DEBUG, "%s key used!\n", hapd->non_wps_pp_enable ? "md5" : "original");

	if (hapd->non_wps_pp_enable)
		psk = hostapd_get_psk_md5(hapd->conf, addr, p2p_dev_addr, prev_psk);
	else
#endif /* CONFIG_QTNA_WIFI */
...
{
	if (wpa_key_mgmt_ft(hapd->conf->wpa_key_mgmt) && !hapd->l2) {
		...
		if (hapd->l2 == NULL &&
			(hapd->driver == NULL ||
			hapd->driver->send_ether == NULL)) {
			wpa_printf(MSG_ERROR, "Failed to open l2_packet interface");
			return -1;
		}
	} else if (!wpa_key_mgmt_ft(hapd->conf->wpa_key_mgmt) && hapd->l2) {

...
#ifdef CONFIG_QTNA_WIFI
		hostapd_send_wlan_msg(hapd, txt);
#endif /* CONFIG_QTNA_WIFI */
	}
...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
...
#ifdef CONFIG_QTNA_WIFI
	hapd_parse_and_write_new_config(hapd, oconf, nconf, cred, HAPD_CFG_TYPE_WPS);
#else
	 ...
	 */
#ifdef CONFIG_QTNA_WIFI
	hapd->conf->retain_wps_state = 1;
	cfg_ctx = (void *)malloc(sizeof(*cfg_ctx));
	cfg_ctx->config_type = HAPD_CFG_TYPE_WPS;
...
#ifdef CONFIG_QTNA_WIFI
static void wps_auto_ld_lock_setup(struct hostapd_data *hapd, int flag)
{
	if (hapd->wps == NULL || hapd->current_wps_lockdown != WPS_LOCKDOWN_AUTO)
		return;

	flag = !!flag;
	if (flag ^ (!!hapd->auto_ld.force_ap_setup_locked)) {
		...
		hapd->auto_ld.force_ap_setup_locked = flag;
		hapd->wps->ap_setup_locked = flag;
		wps_registrar_update_ie(hapd->wps->registrar);
	}
}
...
#ifdef CONFIG_QTNA_WIFI
	if (hapd->current_wps_lockdown == WPS_LOCKDOWN_AUTO) {
		if (hapd->auto_ld.max_fail_retry == 0 || hapd->auto_ld.force_ap_setup_locked)
			return 0;
		hapd->auto_ld.fail_count++;
		if (hapd->auto_ld.fail_count < hapd->auto_ld.max_fail_retry)
			return 0;
		wpa_printf(MSG_DEBUG, "WPS: [Auto-Lock] reach max fail count (%d), lock ap setup",
				hapd->auto_ld.max_fail_retry);
		wps_auto_ld_lock_setup(hapd, 1);
		return 0;
	}
#endif /* CONFIG_QTNA_WIFI */
				 ...
				 union wps_event_data *data)
{
	...
	case WPS_EV_FAIL:
		...
		if (data->fail.config_error == WPS_CFG_MULTIPLE_PBC_DETECTED) {
			...
			msg = "WPS AP PIN FAIL";
		} else if (data->fail.config_error == WPS_CFG_NO_ERROR
				&& data->fail.enrollee && data->fail.msg == WPS_M7) {
	...
	case WPS_EV_STA_PIN_FAIL:
		...
		new_external_state = WPS_EXT_STATE_STA_PIN_FAIL;
		break;
	/* FIXME: Handling WPS_EV_TIMEOUT here to pass compilation */
	case WPS_EV_TIMEOUT:
		break;
#endif /* CONFIG_QTNA_WIFI */
	}
...
static int hostapd_wps_init_multiap_backhaul(struct wps_context *wps,
		struct hostapd_bss_config *conf)
{
	if (conf->wps_cred_processing_multiap_backhaul == 0) {
		os_memset(wps->multiap_backhaul_bss_ssid, 0, SSID_MAX_LEN);
		wps->multiap_backhaul_bss_ssid_len = 0;
		os_memset(wps->multiap_backhaul_bss_wpa_passphrase, 0,
				sizeof(wps->multiap_backhaul_bss_wpa_passphrase));
		wps->multiap_backhaul_bss_wpa_psk = 0;
		return 0;
	}

	if ((conf->multiap_backhaul_bss.ssid_len == 0) ||
			(conf->multiap_backhaul_bss.ssid_len > SSID_MAX_LEN))
		return -1;

	if (!(conf->multiap_backhaul_bss.wpa_psk & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK)))
		return -1;

	if ((conf->multiap_backhaul_bss.wpa_passphrase[0] == 0) ||
			(strlen(conf->multiap_backhaul_bss.wpa_passphrase) >
			 sizeof(wps->multiap_backhaul_bss_wpa_passphrase)))
		return -1;

	os_memcpy(wps->multiap_backhaul_bss_ssid, conf->multiap_backhaul_bss.ssid,
			conf->multiap_backhaul_bss.ssid_len);
	wps->multiap_backhaul_bss_ssid_len = conf->multiap_backhaul_bss.ssid_len;
	wps->multiap_backhaul_bss_wpa_psk = conf->multiap_backhaul_bss.wpa_psk;
	os_memcpy(wps->multiap_backhaul_bss_wpa_passphrase,
			conf->multiap_backhaul_bss.wpa_passphrase,
			strlen(conf->multiap_backhaul_bss.wpa_passphrase));
		     ...
		     struct hostapd_bss_config *conf)
{
...
#ifdef CONFIG_OWE
			|| conf->owe_transition_ifname[0]
#endif /* CONFIG_OWE */
			) {
			if (hapd->qtn_wps_on_hidden_ssid) {
				wpa_printf(MSG_INFO, "WPS: wps_on_hidden_ssid");
				wpa_printf(MSG_INFO, "WPS: enable WPS when ignore_broadcast_ssid is on");
			} else {
				wpa_printf(MSG_INFO, "WPS: enable WPS when OWE transition mode is active");
			}
		} else {
			wpa_printf(MSG_WARNING, "WPS: ignore_broadcast_ssid");
			wpa_printf(MSG_WARNING, "WPS: configuration forced WPS to be disabled");
			conf->wps_state = 0;
		}
	}
#endif /* CONFIG_QTNA_WIFI */
	...
	if (conf->wps_vendor_spec != NULL) {
		...
		wps->vendor = WPS_VENDOR_NONE;
	}
	if (hostapd_wps_init_multiap_backhaul(wps, conf) < 0)
		goto fail;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	if (hapd->wps_third_party_band && hapd->wps_third_party_band != wps->dev.rf_bands)
		wps->dev.rf_bands |= hapd->wps_third_party_band;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	cfg.wps_cred_processing_multiap_backhaul = conf->wps_cred_processing_multiap_backhaul;
#endif /* CONFIG_QTNA_WIFI */
	...
	wps->ieee80211w = conf->ieee80211w;

	if (conf->wps_lockdown != hapd->current_wps_lockdown) {
		...
		if (hapd->current_wps_lockdown == WPS_LOCKDOWN_AUTO) {
			if (conf->auto_ld_max_retry != hapd->auto_ld.max_fail_retry) {
				hapd->auto_ld.max_fail_retry = conf->auto_ld_max_retry;
				hapd->auto_ld.fail_count = 0;
			}
			wps->ap_setup_locked = hapd->auto_ld.force_ap_setup_locked;
		}
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	os_memcpy(wps->last_wps_client, hapd->last_wps_client, ETH_ALEN);
	os_memcpy(wps->last_wps_client_wps_type,
			hapd->last_wps_client_wps_type,
			strlen((const char *)hapd->last_wps_client_wps_type));
#endif /* CONFIG_QTNA_WIFI */

	hapd->wps = wps;

#ifdef CONFIG_QTNA_WIFI
	/* 802.1x authenticator don't reconfig, update wps context here */
...
#ifdef CONFIG_QTNA_WIFI
	os_memcpy(hapd->last_wps_client, hapd->wps->last_wps_client, ETH_ALEN);
	os_memset(hapd->last_wps_client_wps_type, 0, sizeof(hapd->last_wps_client_wps_type));
	os_memcpy(hapd->last_wps_client_wps_type,
			hapd->wps->last_wps_client_wps_type,
			strlen((const char *)hapd->wps->last_wps_client_wps_type));
#endif /* CONFIG_QTNA_WIFI */
			      ...
			      const u8 *p2p_dev_addr)
{
	...
	if (hapd->iface->default_pbc_bss)
		return wps_button_pushed(hapd->iface->default_pbc_bss, &ctx);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
static int hostapd_wps_upnp_iface_check(const char *ifname)
{
	int sock;
	struct ifreq ifr;
	...
	/* WPS upnp iface must have ip address */
	if (ioctl(sock, SIOCGIFADDR, &ifr) < 0) {
		wpa_printf(MSG_ERROR, "WPS uPnP interface %s is not a valid interface", ifname);
		close(sock);
		return -1;
	}
...
#define WPS_UPNP_DEFAULT_BR_IFACE	"br0"
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	if (hostapd_wps_upnp_iface_check(hapd->conf->upnp_iface)) {
		/*
		 * When upnp_iface has invalid bridge interface, hostapd fails to come up because
		 * of this 'fail'.  So adding upnp_iface as 'br0' to bring hostapd UP and Running.
		 */
		wpa_printf(MSG_WARNING, "WPS uPnP interface %s is not up - using interface %s\n",
			hapd->conf->upnp_iface, WPS_UPNP_DEFAULT_BR_IFACE);
		os_free(hapd->conf->upnp_iface);
		hapd->conf->upnp_iface = os_strdup(WPS_UPNP_DEFAULT_BR_IFACE);
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	if (!hapd->wps) {
		wpa_printf(MSG_WARNING, "WPS: enable WPS before enabling PIN");
		return;
	}

	if (hapd->current_wps_lockdown == WPS_LOCKDOWN_AUTO) {
		if (hapd->conf->ap_setup_locked) {
			wpa_printf(MSG_WARNING, "Configuration lock AP setup");
		} else if (hapd->auto_ld.force_ap_setup_locked) {
			wps_auto_ld_lock_setup(hapd, 0);
			hapd->auto_ld.fail_count = 0;
		}
		return;
	}
...
#ifdef CONFIG_QTNA_WIFI
	if (hapd->current_wps_lockdown == WPS_LOCKDOWN_AUTO) {
		if (!hapd->auto_ld.force_ap_setup_locked) {
			wps_auto_ld_lock_setup(hapd, 1);
#ifdef CONFIG_WPS_UPNP
...
int hostapd_wps_button_pushed_interface(struct hostapd_data *hapd)
{
	...
	return wps_button_pushed(hapd, &ctx);
}

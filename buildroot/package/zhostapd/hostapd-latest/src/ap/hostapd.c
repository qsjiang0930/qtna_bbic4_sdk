...
/*
 * If the server is not reachable, add a static route to the bridge table.
 */
static void hostapd_check_radius_server_route(struct hostapd_radius_server *server,
				const char *br_ifname)
{
	...
	socklen_t addr_size;
	int ret = 0;
	char string_ipaddr[32] = {0};

	hostapd_ip_txt(&server->addr, string_ipaddr, sizeof(string_ipaddr));

	sock = socket(PF_INET, SOCK_DGRAM, 0);
	if (sock < 0) {
		wpa_printf(MSG_ERROR, "%s: socket(PF_INET,SOCK_DGRAM) failed: %s",
	...
	serverAddr.sin_addr.s_addr = inet_addr(string_ipaddr);
	...
	/* Only test for unreachable - ignore other errors */
	if (ret < 0 && errno == ENETUNREACH) {
		char command[64] = {'\0'};

		snprintf(command, sizeof(command) - 1, "route add %s %s", string_ipaddr, br_ifname);
		if (system(command) < 0)
			wpa_printf(MSG_ERROR, "Failed to add %s to interface %s",
					string_ipaddr, br_ifname);
		else
			wpa_printf(MSG_DEBUG, "Added %s to interface %s", string_ipaddr, br_ifname);
	}

	close(sock);
}

static void hostapd_add_radius_server_route(struct hostapd_data *hapd)
{
	...
	if (conf->radius->auth_servers) {
		for (i = 0; i < conf->radius->num_auth_servers; i++)
			hostapd_check_radius_server_route(
						&conf->radius->auth_servers[i],
						conf->bridge);
	}

	if (conf->radius->acct_servers) {
		for (i = 0; i < conf->radius->num_acct_servers; i++)
			hostapd_check_radius_server_route(
						&conf->radius->acct_servers[i],
						conf->bridge);
	}
}
...
static void hostapd_reload_bss(struct hostapd_data *hapd)
{
...
#ifndef CONFIG_NO_RADIUS
#ifdef CONFIG_QTNA_WIFI
	hostapd_add_radius_server_route(hapd);
#endif /* CONFIG_QTNA_WIFI */
	...
	if (is_zero_ether_addr(hapd->conf->r1_key_holder)) {
		wpa_printf(MSG_ERROR, "%s R1 key holder value is 0, setting to " MACSTR,
				 __func__, MAC2STR(hapd->own_addr));
		os_memcpy(hapd->conf->r1_key_holder, hapd->own_addr, ETH_ALEN);
	}
...
#ifdef CONFIG_IEEE80211R
			hostapd_reconfig_rrb_socket(hapd);
#endif
			wpa_ie = wpa_auth_get_wpa_ie(hapd->wpa_auth, &wpa_ie_len);
			if (hostapd_set_generic_elem(hapd, wpa_ie, wpa_ie_len))
				wpa_printf(MSG_ERROR, "Failed to configure WPA IE to driver");
		}
#else
...
int hostapd_add_bss(struct hostapd_iface *iface, char *cmd)
{
	...
	if ((cfg_idx >= 0) && (bss_idx < 0)) {
		...
		if (iface->num_bss >= g_max_bssid) {
			wpa_printf(MSG_ERROR, "exceed allowed BSS number (%d)", g_max_bssid);
			ret = -1;
			goto ab_out;
		}
		...
		hapd->msg_ctx = hapd;
		if (hostapd_setup_bss(hapd, 0) < 0) {
			bss_idx = hostapd_find_bss(iface, bss_name);
			if (bss_idx > 0)
				hostapd_remove_bss(iface, bss_idx);
			else
				iface->num_bss--;
			ret = -1;
			goto ab_out;
...
#if defined(CONFIG_QTNA_WIFI) && defined(CONFIG_OWE)
static void hostapd_update_owe_transition_mode(struct hostapd_iface *iface)
{
	int j;
	struct hostapd_data *hapd_bss;

	for (j = 0; j < iface->num_bss; j++) {
		hapd_bss = iface->bss[j];

		wpa_printf(MSG_DEBUG,
				"OWE: reset OWE transition mode info for %s",
				hapd_bss->conf->iface);
		os_memset(hapd_bss->conf->owe_transition_ssid, 0, SSID_MAX_LEN);
		hapd_bss->conf->owe_transition_ssid_len = 0;
	}
	...
	for (j = 0; j < iface->num_bss; j++) {
		...
		wpa_printf(MSG_DEBUG, "OWE: deinit-init WPS to sync IEs for %s",
				hapd_bss->conf->iface);
		hostapd_deinit_wps(hapd_bss);
		if (hostapd_init_wps(hapd_bss, hapd_bss->conf))
			wpa_printf(MSG_ERROR, "Could not reconfigure WPS");
		hostapd_init_wps_complete(hapd_bss);
	}

}
#endif /* CONFIG_QTNA_WIFI && CONFIG_OWE */
...
int hostapd_update_bss(struct hostapd_iface *iface, const char *bss_name)
{
	...
	int ret;
	int if_up = -1;
	enum wps_external_state retained_wps_ext_state = WPS_EXT_STATE_UNKNOWN;
	...
	if ((bss_idx >= 0) && (cfg_idx >= 0)) {
		...
		if (old_bss_cfg && old_bss_cfg->retain_wps_state && hapd->wps)
			retained_wps_ext_state = hapd->wps->wps_external_state;

		if (old_bss_cfg && old_bss_cfg->ctrl_interface) {
			os_free(hapd->conf->ctrl_interface);
			hapd->conf->ctrl_interface = os_strdup(old_bss_cfg->ctrl_interface);
		...
		if ((hapd->wps) && (retained_wps_ext_state != WPS_EXT_STATE_UNKNOWN) &&
		    (retained_wps_ext_state != hapd->wps->wps_external_state)) {
			hapd->wps->wps_external_state = retained_wps_ext_state;
		}
...
#if defined(CONFIG_QTNA_WIFI) && defined(CONFIG_OWE)
	hostapd_update_owe_transition_mode(iface);
#endif /* CONFIG_QTNA_WIFI && CONFIG_OWE */
...
static inline int hostapd_maclist_should_deauth(struct hostapd_bss_config *currconf,
						struct sta_info *sta)
{
	int acl_policy = currconf->macaddr_acl;
	uint8_t oui[ETH_ALEN] = {0};
#if 0
	int vlan_id = 0;
#else
	...
	if (acl_policy == ACCEPT_UNLESS_DENIED) {
		/* Deauth STAs that are in the deny list */
		if (hostapd_maclist_found(currconf->deny_mac, currconf->num_deny_mac,
					sta->addr, &vlan_id))
			return 1;
	} else if (acl_policy == DENY_UNLESS_ACCEPTED) {
		/* Deauth STAs that are not in the accept list */
		memcpy(oui, sta->addr, MAC_OUI_LEN);
		if (!(hostapd_maclist_found(currconf->accept_mac, currconf->num_accept_mac,
						sta->addr, &vlan_id)) &&
			(!(hostapd_maclist_found(currconf->accept_oui,
				currconf->num_accept_oui, oui, &vlan_id)))) {
			return 1;
		}
	}
...
int hostapd_flush_filtered_stations(struct hostapd_data *hapd)
{
	struct sta_info *sta, *next;
	struct hostapd_bss_config *currconf = hapd->conf;

	for (sta = hapd->sta_list; sta; sta = next) {
		next = sta->next;

		if (hostapd_maclist_should_deauth(currconf, sta)) {
	...
	if ((currconf->macaddr_acl == DENY_UNLESS_ACCEPTED) &&
					(currconf->num_accept_mac == 0)) {
		currconf->wps_state = 0;
		if (hapd->wps) {
			wpa_printf(MSG_WARNING, "WPS: empty whitelist and ACL "
					"configuration forced WPS to be disabled");
			hostapd_deinit_wps(hapd);
		}
	} else if ((hapd->wps == NULL) && currconf->wps_state) {
		hostapd_init_wps(hapd, currconf);
	}
...
static int hostapd_copy_mac_list(struct mac_acl_entry **curr,
					int *curr_count,
					const struct mac_acl_entry *new,
					const int new_count)
{
	struct mac_acl_entry *acl_params = NULL;

	if (new_count && new) {
		acl_params = os_zalloc(sizeof(*acl_params) * (new_count));
		if (!acl_params) {
			return -ENOMEM;
		}

		os_memcpy(acl_params->addr, new->addr, new_count * sizeof(*acl_params));
	}
...
static int hostapd_update_bss_filter_macaddress(struct hostapd_iface *iface,
				struct hostapd_bss_config *newconf,
				struct hostapd_bss_config *currconf)
{
	currconf->macaddr_acl = newconf->macaddr_acl;
	currconf->wps_state = newconf->wps_state;

	if (hostapd_copy_mac_list(&currconf->accept_mac, &currconf->num_accept_mac,
					newconf->accept_mac, newconf->num_accept_mac)) {
		return -1;
	}

	if (hostapd_copy_mac_list(&currconf->deny_mac, &currconf->num_deny_mac,
					newconf->deny_mac, newconf->num_deny_mac)) {
		return -1;
	}

	if (hostapd_copy_mac_list(&currconf->accept_oui, &currconf->num_accept_oui,
					newconf->accept_oui, newconf->num_accept_oui)) {
		return -1;
	}
...
int hostapd_update_bss_filer_mac_list(struct hostapd_iface *iface, const char *bss_name)
{
	int ret = 0;
	int bss_idx, cfg_idx;
	struct hostapd_data *hapd;
	struct hostapd_config *conf;
	struct hostapd_bss_config *newconf, *currconf;
	...
	bss_idx = hostapd_find_bss(iface, bss_name);
	cfg_idx = hostapd_find_bss_config(conf, bss_name);

	if (bss_idx != cfg_idx) {
		wpa_printf(MSG_ERROR, "Configuration file was changed");
	...
	newconf = conf->bss[cfg_idx];

#if 0
	ret = hostapd_update_bss_filter_macaddress(hapd, newconf, currconf);
#else
	ret = hostapd_update_bss_filter_macaddress(hapd->iface, newconf, currconf);
#endif
	...
	if (!ret) {
		...
		hostapd_flush_filtered_stations(hapd);
	}
...
int hostapd_deauth_all_iface(struct hostapd_iface *hapd_iface)
{
	size_t i;

	if ((hapd_iface == NULL)
		|| (hapd_iface->bss == NULL) || (hapd_iface->bss[0] == NULL)
		|| (hapd_iface->bss[0]->drv_priv == NULL)) {
		wpa_printf(MSG_ERROR, "Interface does not exist or disabled");
		return -1;
	}
	...
	for (i = 0; i < hapd_iface->num_bss; i++) {
		hostapd_flush_old_stations(hapd_iface->bss[i],
...
int hostapd_reload_config(struct hostapd_iface *iface)
{
	...
	for (j = 0; j < newconf->num_bss; j++) {
		struct hostapd_data *hapd_bss;
		int idx = hostapd_find_bss(iface, newconf->bss[j]->iface);
		int if_up = 0;

		if (idx >= 0) {
			const char *current_wps_ap_pin = NULL;
			char retained_wps_ap_pin[WPS_AP_PIN_LEN + 1];
			char *new_ctrl_interface = NULL;
			...
			if (hapd_bss->conf->ctrl_interface) {
				new_ctrl_interface = os_strdup(hapd_bss->conf->ctrl_interface);
				if (!new_ctrl_interface) {
			...
			hapd_bss->conf = newconf->bss[j];

			os_free(hapd_bss->conf->ctrl_interface);
			hapd_bss->conf->ctrl_interface = new_ctrl_interface;

			/* TODO:
			 *    compare 'iface->bss[idx].conf' with 'newconf->bss[i]'
			...
			if (current_wps_ap_pin && !hapd_bss->conf->ap_pin) {
				hostapd_wps_ap_pin_set(hapd_bss,
						(const char *) &retained_wps_ap_pin,
						WPS_AP_PIN_DEFAULT_TIMEOUT);
			}
			...
			iface->bss[k++] = hapd_bss;
			if_up = 0;
		}

		if (if_up >= 0) {
			if (hostapd_driver_commit(hapd_bss) < 0) {
				wpa_printf(MSG_ERROR, "%s: Failed to commit driver configuration",
						__func__);
				hostapd_config_free(oldconf);
				return -1;
			}
...
#ifdef CONFIG_QTNA_WIFI
	sae_faillist_deinit(hapd);
	sae_blacklist_deinit(hapd);
#endif /* CONFIG_QTNA_WIFI */
#endif /* CONFIG_SAE */
}
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	if (!first || first == -1) {
...
#ifdef CONFIG_QTNA_WIFI
			/* Set zero to let driver decides its own MACADDR */
			os_memset(hapd->own_addr, 0, ETH_ALEN);
#else
		...
		if (!addr
#ifdef CONFIG_QTNA_WIFI
			|| os_memcmp(hapd->own_addr, if_addr, ETH_ALEN)
#endif /* CONFIG_QTNA_WIFI */
			)
...
#ifdef CONFIG_QTNA_WIFI
	hapd->scs = hostapd_scs_init(hapd, hapd->conf->iface);
	if (hapd->scs == NULL) {
		wpa_printf(MSG_ERROR, "SCS initialization failed.");
		return -1;
	}
#endif /* CONFIG_QTNA_WIFI */
	 ...
	 *   3. when firt EAPOL-Start appear earlier than expected,
	 *          delay the handling function for 0.8 second.
	 */
...
#ifdef CONFIG_QTNA_WIFI
	wpa_printf(MSG_DEBUG, "setup_bss: setup ctrl_iface for %s in ctrl_iface dir %s",
			conf->iface, conf->ctrl_interface);
	if (conf->ctrl_interface && start_ctrl_iface_bss(hapd)) {
		wpa_printf(MSG_WARNING, "setup_bss: failed to setup ctrl interface");
		return -1;
	}
#endif /* CONFIG_QTNA_WIFI */
	return 0;
				...
				u8 accept_acl)
{
	...
	acl_params = os_zalloc(sizeof(*acl_params) +
			       ((n_mac_entries + n_oui_entries) * sizeof(acl_params->mac_acl[0])));
	if (!acl_params)
		return -ENOMEM;
	...
	for (i = 0; i < n_mac_entries; i++)
		os_memcpy(acl_params->mac_acl[i].addr, mac_acl[i].addr,
			  ETH_ALEN);
	for (i = 0; i < n_oui_entries; i++)
		os_memcpy(acl_params->mac_acl[i + n_mac_entries].addr, oui_acl[i].addr,
			  ETH_ALEN);

	acl_params->acl_policy = accept_acl;
	acl_params->num_mac_acl = n_mac_entries;
	acl_params->num_oui_acl = n_oui_entries;
...
void hostapd_set_acl(struct hostapd_data *hapd)
{
	...
	case DENY_UNLESS_ACCEPTED:
		err = hostapd_set_acl_list(hapd,
					   ...
					   conf->macaddr_acl);
		if (err)
			wpa_printf(MSG_ERROR, "Failed to set accept acl");
		break;
	case ACCEPT_UNLESS_DENIED:
		err = hostapd_set_acl_list(hapd, conf->deny_mac,
					   ...
					   conf->macaddr_acl);
		if (err)
			wpa_printf(MSG_ERROR, "Failed to set deny acl");
		break;
	default:
		wpa_printf(MSG_ERROR, "%s: case %d not handled", __func__, conf->macaddr_acl);
		break;
						 ...
						 int err)
{
	...
	if (hapd->iconf->total_assoc_limit >= 0 &&
		...
		if (hostapd_set_total_assoc_limit(hapd, hapd->iconf->total_assoc_limit)) {
			wpa_printf(MSG_ERROR, "Couldn't set total_assoc_limit for kernel driver");
			return -1;
		}
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	for (j = 0; j < iface->num_bss; j++) {
		wpa_printf(MSG_DEBUG, "%s: update WPS IEs", iface->bss[j]->conf->iface);
		hostapd_update_wps(iface->bss[j]);
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	wpa_printf(MSG_DEBUG, "alloc_bss_data: interface %s; ctrl_iface_dir %s", bss->iface,
			conf->hapd_ctrl_interface);
	if (conf->hapd_ctrl_interface) {
		os_free(hapd->conf->ctrl_interface);
		hapd->conf->ctrl_interface = os_strdup(conf->hapd_ctrl_interface);
		if (!hapd->conf->ctrl_interface)
			wpa_printf(MSG_WARNING, "ctrl_iface_dir init failed for %s", bss->iface);
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	hapd_iface->bss = os_calloc(g_max_bssid,

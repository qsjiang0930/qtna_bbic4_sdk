			       ...
			       struct wpabuf **assocresp_ret)
{
	...
	*beacon_ret = *proberesp_ret = *assocresp_ret = NULL;
	...
	pos = hostapd_eid_qos_map_set(hapd, buf);
	if (add_buf_data(&assocresp, buf, pos - buf) < 0)
		goto fail;
...
#ifdef CONFIG_QTNA_WIFI
int hostapd_vlan_group_add(struct hostapd_data *hapd, const char *ifname, int vlan_id)
{
	if (hapd->driver == NULL || hapd->driver->vlan_group_add == NULL)
		return 0;
	return hapd->driver->vlan_group_add(hapd->drv_priv, ifname, vlan_id);
}


int hostapd_vlan_group_remove(struct hostapd_data *hapd, const char *ifname, int vlan_id)
{
	if (hapd->driver == NULL || hapd->driver->vlan_group_remove == NULL)
		return 0;
	return hapd->driver->vlan_group_remove(hapd->drv_priv, ifname, vlan_id);
}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	ret = hapd->driver->if_remove(hapd->drv_priv, type, ifname);
	if (!ret)
		hapd->drv_priv = NULL;
	return ret;
#else

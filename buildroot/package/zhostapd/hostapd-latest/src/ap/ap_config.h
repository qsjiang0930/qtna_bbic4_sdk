...
#ifdef CONFIG_QTNA_WIFI
struct hostapd_multiap_backhaul_bss {
	u8 ssid[SSID_MAX_LEN];
	u8 ssid_len;
	int wpa_psk;
...
#ifdef CONFIG_QTNA_WIFI
	int set_assoc_limit_required; /* is setting max_num_sta to driver required */
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	int num_accept_oui;
	struct mac_acl_entry *accept_oui;
#endif /* CONFIG_QTNA_WIFI */
	int wds_sta;
	int isolate;
#ifdef CONFIG_QTNA_WIFI
	int intra_bss_isolate; /* isolation within bss */
	int bss_isolate;       /* isolation between bss */
#endif /* CONFIG_QTNA_WIFI */
	int start_disabled;
...
#ifdef CONFIG_QTNA_WIFI
	int retain_wps_state;
#endif /* CONFIG_QTNA_WIFI */
	int force_per_enrollee_psk;
	u8 *ap_settings;
	size_t ap_settings_len;
#ifdef CONFIG_QTNA_WIFI
	struct hostapd_multiap_backhaul_bss multiap_backhaul_bss;
...
#ifdef CONFIG_QTNA_WIFI
	/*QTN pairing protection features*/
	char *wps_pp_devname;
	char *wps_pp_devname_bl;
	char *wps_vendor_spec;
	int wps_lockdown;
	int pp_enable;
	int pairing_enable;
	char *pairing_id;
	u8 pairing_hash[PAIRING_HASH_LEN];
	int auto_ld_max_retry;
...
#ifdef CONFIG_QTNA_WIFI
#ifdef CONFIG_SAE
#define DEFAULT_SAE_MAX_AUTH_ATTEMPTS 0
	unsigned int sae_max_auth_attempts;
#define DEFAULT_SAE_AUTH_MONITOR_PERIOD 3
	unsigned int sae_auth_monitor_period;
#define DEFAULT_SAE_AUTH_LOCKOUT_PERIOD 30
	unsigned int sae_auth_lockout_period;
#endif /* CONFIG_SAE */
#endif /* CONFIG_QTNA_WIFI */
};
...
#ifdef CONFIG_QTNA_WIFI
	int total_assoc_limit;
	int pbc_in_srcm;
	char *hapd_ctrl_interface;
#endif /* CONFIG_QTNA_WIFI */

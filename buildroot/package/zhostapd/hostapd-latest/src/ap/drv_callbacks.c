			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
...
#ifdef CONFIG_QTNA_WIFI
		/* WAR for WPS asssociation with Intel STA */
		if (IEEE80211_IS_INTEL_MAC_ADDR(addr) &&
				hapd->wps->wps_external_state == WPS_EXT_STATE_PROCESS_START) {
			wpa_printf(MSG_DEBUG, "WAR, ignore Intel Assoc Req When WPS on");
			return 0;
		}
#endif /* CONFIG_QTNA_WIFI */
	...
	if (hapd->pbc_detect_enhance) {
		sta->eapol_start_seen = 0;
		os_get_reltime(&sta->eapol_start_delay);
		sta->eapol_start_delay.usec += hapd->pbc_detect_interval;
	}
#endif /* CONFIG_QTNA_WIFI */
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_MICHAEL_MIC_FAILURE:
		michael_mic_failure(hapd, data->michael_mic_failure.src,
#ifdef CONFIG_QTNA_WIFI
			data->michael_mic_failure.local
#else
			1
#endif /* CONFIG_QTNA_WIFI */
			);
	...
	case EVENT_RX_MGMT:
		if (!data->rx_mgmt.frame)
			break;
#ifdef CONFIG_QTNA_WIFI
#if defined(NEED_AP_MLME) || defined(CONFIG_IEEE80211R)
		if (hostapd_mgmt_rx(hapd, &data->rx_mgmt) > 0)
			break;
#endif /* NEED_AP_MLME || CONFIG_IEEE80211R */
#else
#ifdef NEED_AP_MLME
		if (hostapd_mgmt_rx(hapd, &data->rx_mgmt) > 0)
			break;

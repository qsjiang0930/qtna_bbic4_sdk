...
#ifdef CONFIG_QTNA_WIFI
enum hapd_cred_write_back_state {
	HAPD_CRED_SEARCH = 0,
	HAPD_CRED_FOUND,
	HAPD_CRED_DONE
};
...
#define SAE_AUTH_FAILLIST_LIMIT_DFLT 64
#define SAE_AUTH_BLACKLIST_LIMIT_DFLT 64

/**
 * struct sae_failure_counter - SAE failauth mac list
 */
struct sae_failure_counter {
	u8 addr[ETH_ALEN];
	int cnt;
	struct os_reltime begin_failtime;
	struct os_reltime latest_failtime;
	struct sae_failure_counter *next;
};

struct sae_failure_cnt_list {
	struct sae_failure_counter *head;
	unsigned int len;
};

/**
 * struct sae_blacklist_entry - SAE blackmac list
 */
struct sae_blacklist_entry {
	u8 addr[ETH_ALEN];
	struct os_reltime jointime;
	struct sae_blacklist_entry *next;
};

struct sae_blacklist_cnt {
	struct sae_blacklist_entry *head;
	unsigned int len;
};
...
#ifdef CONFIG_QTNA_WIFI
	struct sae_failure_cnt_list sae_faillist;
	struct sae_blacklist_cnt sae_blacklist;
#endif /* CONFIG_QTNA_WIFI */

			   ...
			   const u8 *token, size_t token_len)
{
	...
	hapd->sae_pending_token_idx[idx] = 0;
	return 0;
}
		       ...
		       const u8 *bssid, u8 auth_transaction, int allow_reuse)
{
	...
	case SAE_NOTHING:
		if (auth_transaction == 1) {
			ret = auth_sae_send_commit(hapd, sta, bssid, !allow_reuse);
			if (ret)
				return ret;
	...
	case SAE_COMMITTED:
		...
		if (auth_transaction == 1) {
			 ...
			 * additional events.
			 */
			return sae_sm_step(hapd, sta, bssid, auth_transaction, 0);
		}
		break;
	case SAE_CONFIRMED:
		sae_clear_retransmit_timer(hapd, sta);
			    ...
			    u16 auth_transaction, u16 status_code)
{
...
#ifdef CONFIG_QTNA_WIFI
	if ((hapd->conf->sae_max_auth_attempts > 0) &&
			sae_auth_blacklist_process(hapd, sta->addr,
				hapd->conf->sae_auth_lockout_period) < 0) {
		wpa_printf(MSG_DEBUG, "SAE: " MACSTR " blacklisted due to reflection attack",
				MAC2STR(sta->addr));
		resp = -1;
		goto remove_sta;
	}
	...
	if (auth_transaction == 1) {
		...
		if (!(hapd->conf->mesh & MESH_ENABLED) &&
			...
			if (end - pos >= (int) sizeof(16) &&
			    sae_group_allowed(sta->sae, hapd->conf->sae_groups,
					      WPA_GET_LE16(pos)) == WLAN_STATUS_SUCCESS) {
		...
		resp = sae_sm_step(hapd, sta, mgmt->bssid, auth_transaction, allow_reuse);
...
#ifdef CONFIG_QTNA_WIFI
				if (hapd->conf->sae_max_auth_attempts > 0)
					sae_process_auth_failure(hapd, sta->addr,
							hapd->conf->sae_max_auth_attempts,
							hapd->conf->sae_auth_monitor_period);
#endif /* CONFIG_QTNA_WIFI */
				goto reply;
			}
#ifdef CONFIG_QTNA_WIFI
			if (hapd->conf->sae_max_auth_attempts > 0)
				sae_faillist_remove_mac(hapd, sta->addr);
#endif /* CONFIG_QTNA_WIFI */
...
void auth_sae_process_commit(void *eloop_ctx, void *user_ctx)
{
	...
	dl_list_del(&q->list);
	handle_auth(hapd, (const struct ieee80211_mgmt *) q->msg, q->len, q->rssi, 1);
	os_free(q);
...
#ifdef CONFIG_QTNA_WIFI
	if (sta->aid) {
		wpa_printf(MSG_INFO, "OWE: ignore repeat AssocRequest/event");
		return WLAN_STATUS_SUCCESS;
	}
#endif /* CONFIG_QTNA_WIFI */

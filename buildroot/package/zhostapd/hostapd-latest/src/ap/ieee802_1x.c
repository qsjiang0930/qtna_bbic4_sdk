				   ...
				   struct sta_info *sta, int authorized)
{
	...
	wpa_printf(MSG_DEBUG, "%s: sta->addr: %02x:%02x:%02x:%02x:%02x:%02x\n", __func__,
	...
	if (authorized) {
#ifdef CONFIG_QTNA_WIFI
		/* Send message to iwevent to indicate sta connected */
		char msg[MAX_WLAN_MSG_LEN];
		/* Find the WPA version and cipher */
		const char *format_string = "Client connected [" MACSTR "] [%s/%s]";
		int pairwise = wpa_auth_get_pairwise(sta->wpa_sm);
		int key_mgmt = wpa_auth_sta_key_mgmt(sta->wpa_sm);
		int proto    = wpa_auth_sta_wpa_version(sta->wpa_sm);

		msg[sizeof(msg)-1] = '\0';
		snprintf(msg, sizeof(msg)-1, format_string, MAC2STR(sta->addr),
				wpa_key_mgmt_txt(key_mgmt, proto),
				wpa_cipher_txt(pairwise));
		if (hapd->driver->send_log)
			hapd->driver->send_log(hapd->drv_priv, msg);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
static void ieee802_1x_eapol_start_handle(void *eloop_ctx, void *ctx)
{
	...
	if (sta == NULL) {
		wpa_printf(MSG_ERROR, "EAPOL: sta has been removed\n");
		os_free(sta_addr);
		return;
	}
			...
			size_t len)
{
	...
	case IEEE802_1X_TYPE_EAPOL_START:
		hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X,
			       ...
			       "from STA");
#ifdef CONFIG_QTNA_WIFI
		/* workaroud: some drivers in Windows 7 may send EAPOL START right
		 * after association, quick response seems to make Windonws 7 get wrong
		 * information, postpone processing will fix this
		 */
		if (hapd->pbc_detect_enhance) {
			if (!sta->eapol_start_seen && (sta->flags & WLAN_STA_MAYBE_WPS)) {
				struct os_reltime now;

				os_get_reltime(&now);
				if (os_reltime_before(&now, &sta->eapol_start_delay)) {
					wpa_printf(MSG_DEBUG,
						"EAPOL: delay eapol start procedure for 1 second\n");
					eapol_start_delay = 1;
				}
				sta->eapol_start_seen = 1;
			}
		}

		if (eapol_start_delay) {
			sta_addr = os_malloc(6);
			if (sta_addr != NULL) {
				os_memcpy(sta_addr, sta->addr, 6);
				eloop_cancel_timeout(ieee802_1x_eapol_start_handle, hapd, sta_addr);
				eloop_register_timeout(hapd->eapol_resp_delay_s,
						hapd->eapol_resp_delay_us,
						ieee802_1x_eapol_start_handle, hapd, sta_addr);
			}
		} else {
			sta->eapol_sm->flags &= ~EAPOL_SM_WAIT_START;
			pmksa = wpa_auth_sta_get_pmksa(sta->wpa_sm);
...
#ifdef CONFIG_QTNA_WIFI
void ieee802_1x_eap_auth_update(struct hostapd_data *hapd)
{
	if (hapd->eapol_auth)
		hapd->eapol_auth->conf.eap_server = hapd->conf->eap_server;
}
#endif /* CONFIG_QTNA_WIFI */

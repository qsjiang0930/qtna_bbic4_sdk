...
#ifdef CONFIG_QTNA_WIFI
	bss->wpa_group_update_count = 8;
	bss->wpa_pairwise_update_count = 8;
#else
...
#ifdef CONFIG_QTNA_WIFI
	bss->set_assoc_limit_required = 0;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	os_memset(bss->r1_key_holder, 0, ETH_ALEN);
	bss->pmk_r1_push = 1;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	bss->wps_independent = 1;

	bss->pp_enable = WPS_PP_DISABLE;
	for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++)
		bss->wps_vendor_ext[i] = NULL;

	/* enable PBC detect enhancement by default */
	bss->pbc_detect_enhance = 1;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
#ifdef CONFIG_SAE
	bss->sae_max_auth_attempts = DEFAULT_SAE_MAX_AUTH_ATTEMPTS;
	bss->sae_auth_monitor_period = DEFAULT_SAE_AUTH_MONITOR_PERIOD;
	bss->sae_auth_lockout_period = DEFAULT_SAE_AUTH_LOCKOUT_PERIOD;
#endif /* CONFIG_SAE */
#endif /* CONFIG_QTNA_WIFI */
}
...
#ifdef CONFIG_QTNA_WIFI
	conf->total_assoc_limit = -1; /* -1: don't set it to driver */

	/* Show all supported config methods as default */
	conf->pbc_in_srcm = 1;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	pbkdf2_sha1(ssid->wpa_passphrase_md5,
			ssid->ssid, ssid->ssid_len,
			4096, ssid->wpa_psk->psk_md5, PMK_LEN);
	wpa_hexdump_key(MSG_DEBUG, "PSK_MD5 (from passphrase_md5)",
			ssid->wpa_psk->psk_md5, PMK_LEN);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	os_free(conf->ssid.wpa_passphrase_md5);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	os_free(conf->wps_pp_devname);
	os_free(conf->wps_pp_devname_bl);
	os_free(conf->pairing_id);
	os_free(conf->wps_vendor_spec);
#endif /* CONFIG_QTNA_WIFI */
				    ...
				    int full_config)
{
	...
	if (full_config && bss->wps_state &&
		...
		wpa_printf(MSG_WARNING,
			"WPS: empty whitelist and ACL configuration forced WPS to be disabled");
		bss->wps_state = 0;
	}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	if (full_config && bss->hs20 && bss->wps_state) {
		wpa_printf(MSG_WARNING, "HS 2.0: disabling WPS");
		bss->wps_state = 0;
	}
#endif /* CONFIG_QTNA_WIFI */
#endif /* CONFIG_HS20 */
...
#ifdef CONFIG_QTNA_WIFI
		/* Clear passphrase and psk, otherwise wps will fail with Vista */

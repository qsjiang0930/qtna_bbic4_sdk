...
int is_hex(const u8 *data, size_t len, int extended)
{
	size_t i;

	for (i = 0; i < len; i++) {
		if (data[i] < 32 || (extended == 0 && data[i] >= 127))
			return 1;
	}
	return 0;
}
...
#ifdef CONFIG_QTNA_WIFI
/*
 * check is current region is USA/FCC
 */
int is_current_region_us(void)
{
	char region[16];
	FILE *fp = NULL;
	int ret = 0;

	fp = fopen("/etc/region", "r");
	if (fp != NULL) {
		fgets(region, 15, fp);
		if (strncasecmp(region, "us", 2) == 0)
			ret = 1;
		fclose(fp);
	}
	return ret;

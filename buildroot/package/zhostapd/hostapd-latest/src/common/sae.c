				      ...
				      const struct crypto_bignum *y_sqr)
{
	 ...
	 * aviod branches and timing differences.
	 *
	 * If r_odd:
	 * num = (num * qr) module p
				 ...
				 struct crypto_bignum *pwe)
{
	...
	/* This is an optimization based on the used group that dose not depend
	 ...
	 * computation. PWE is avalid if pwd-value was less than prime and
			      ...
			      size_t password_len, const char *identifier)
{
	...
	for (counter = 1; counter <= k || !found; counter++) {
		...
		const_time_select_bin(found, x_bin, x_cand_bin, prime_len,
				      x_bin);
		pwd_seed_odd = const_time_select_u8(found, pwd_seed_odd,
						    pwd_seed[SHA256_MAC_LEN - 1] & 0x01);
		os_memset(pwd_seed, 0, sizeof(pwd_seed));
		if (res < 0)
			goto fail;
		/* Need to minimize diffrernces in handing res == 0 and 1 here
			      ...
			      size_t password_len, const char *identifier)
{
	...
	size_t len[3];
	size_t num_elem;
	u8 found = 0;/* 0 (false) or 0xff (true) to be used as const_time_mask */
	u8 mask;
	struct crypto_bignum *pwe;
	...
	/* Allocate a buffer to maintain selected and candidate PWE for constant time selection*/
	...
	for (counter = 1; counter <= k || !found; counter++) {
		 ...
		 * the select PWE in the beginning of pwe_buf using constant
		 * time selection. */

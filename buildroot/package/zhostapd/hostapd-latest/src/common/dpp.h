...
#define QTN_DPP_PKEX_DEFAULT_FREQ_5G	5220
#define QTN_DPP_PKEX_DEFAULT_FREQ_24G	2437
#define QTN_DPP_PKEX_TIMEOUT		11

#define MAX(_a, _b) ((_a) > (_b) ? (_a) : (_b))
enum dpp_secret_type {
	DPP_SECRET_TYPE_CONNECTOR = 1,
	DPP_SECRET_TYPE_PASSPHRASE,
	DPP_SECRET_TYPE_PSK
};

/**
 * struct dpp_credential - DPP Credential
 * @ssid: SSID
 * @ssid_len: Length of SSID
 * @secret_type: DPP secret type- CONNECTOR, PASSPHRASE, PSK
 * @secret_len: DPP secret key length
 * @secret_str: DPP secret key
 * @net_accesskey: NetAccess key
 * @csign_key: C-Sign key
 */
struct dpp_credential {
	u8 ssid[SSID_MAX_LEN];
	size_t ssid_len;
	int dpp_akm;
	int dpp_secret_type;
	struct wpabuf *secret;
	struct wpabuf *net_accesskey;
	struct wpabuf *csign_key;
};

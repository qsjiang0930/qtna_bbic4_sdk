...
static void eap_wsc_fail(struct eap_sm *sm)
{
	struct eap_wsc_data *data = (struct eap_wsc_data *)sm->eap_method_priv;

	if (sm->wps->wps_external_state < WPS_EXT_STATE_PROCESS_SUCCESS) {
		sm->wps->wps_external_state = WPS_EXT_STATE_MSG_EX_ERROR;
		eap_wsc_state(data, FAIL);
	}
}

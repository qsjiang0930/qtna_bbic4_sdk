...
#define QTN_EXTENDER_ROLE_STR	"qtn_extender_role="
#define QTN_WDS_EXT_SCRIPT "qtn_wds_ext.sh"
#define CMD_WDS_EXT_LEN		256

#define EVENT_NAME_LEN	32
struct wpa_driver_event_data {
...
wpa_driver_wext_event_wireless_custom(void *ctx, char *custom)
{
	...
	if (os_strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
		...
		if (wpa_s->ignore_hw_pbc) {
			wpa_printf(MSG_DEBUG, "WEXT: ignore WPS-BUTTON.indication "
				   "event due to ignore_hw_pbc is set");
			return;
		}
		wpas_wps_start_pbc(ctx, NULL, 0);
	} else if (os_strncmp(custom, "QTN-WDS-EXT", 11) == 0) {
		struct qtn_wds_ext_event_data *event_data =
				(struct qtn_wds_ext_event_data*)custom;

		wpa_printf(MSG_DEBUG, "WEXT: received QTN-WDS-EXT message, "
				"cmd=%d, mac=" MACSTR " peer role=%d channel=%d ssid=\"%s\"",
				event_data->cmd, MAC2STR(event_data->mac), event_data->extender_role,
				event_data->channel, (char*)event_data->ssid);

	} else if (os_strncmp(custom, "ASSOCREJECT", 11) == 0) {
		struct qtn_assoc_reject_event_data *event_data =
				(struct qtn_assoc_reject_event_data*)custom;

		wpa_printf(MSG_ERROR, "WEXT: ASSOCREJECT event"
				"reason_code=%d, bssid=" MACSTR "",
				event_data->reason_code, MAC2STR(event_data->bssid));
		os_memset(&data, 0, sizeof(data));
		data.assoc_reject.bssid = event_data->bssid;
		data.assoc_reject.status_code = event_data->reason_code;
#ifdef CONFIG_QTNA_WIFI
		if(event_data->reason_code == WLAN_STATUS_REJECTED_WITH_SUGGESTED_BSS_TRANSITION) {
			os_memcpy(data.assoc_reject.nr_item.bssid,
						event_data->nr_item.bssid, ETH_ALEN);
			data.assoc_reject.nr_item.operating_class =
						event_data->nr_item.operating_class;
			data.assoc_reject.nr_item.channel = event_data->nr_item.channel;
		}
#endif
		wpa_supplicant_event(ctx, EVENT_ASSOC_REJECT, &data);
#ifdef CONFIG_QTNA_WIFI
	} else if (os_strncmp(custom, "AUTHREJECT", 10) == 0) {
		struct qtn_auth_event_data *event_data =
				(struct qtn_auth_event_data*)custom;

		wpa_printf(MSG_ERROR, "WEXT: AUTH event"
				"reason_code=%d, bssid=" MACSTR "",
				event_data->reason_code, MAC2STR(event_data->bssid));
		if(event_data->reason_code == WLAN_STATUS_REJECTED_WITH_SUGGESTED_BSS_TRANSITION) {
			os_memset(&data, 0, sizeof(data));
			data.auth.status_code = event_data->reason_code;
			os_memcpy(data.auth.bssid, event_data->bssid, ETH_ALEN);
			data.auth.auth_event_source = AUTH_FROM_CUSTOM_EVENT;
			os_memcpy(data.auth.nr_item.bssid, event_data->nr_item.bssid, ETH_ALEN);
			data.auth.nr_item.operating_class =
						event_data->nr_item.operating_class;
			data.auth.nr_item.channel = event_data->nr_item.channel;
			wpa_supplicant_event(ctx, EVENT_AUTH, &data);
		}
#endif
	} else if (os_strncmp(custom, "REMAINONCHAN", 13) == 0) {
		struct qtn_remain_on_chan_event_data *event_data =
				(struct qtn_remain_on_chan_event_data *)custom;

		wpa_printf(MSG_ERROR, "WEXT: REMAINONCHAN event freq=%u, dur=%u, cancel=%u",
				event_data->frequency, event_data->duration,
				event_data->cancel_flag);
		os_memset(&data, 0, sizeof(data));

		data.remain_on_channel.freq = event_data->frequency;
		data.remain_on_channel.duration = event_data->duration;
		if (event_data->cancel_flag)
			wpa_supplicant_event(ctx, EVENT_CANCEL_REMAIN_ON_CHANNEL, &data);
		else
			wpa_supplicant_event(ctx, EVENT_REMAIN_ON_CHANNEL, &data);
		...
		wpa_supplicant_event(ctx, EVENT_TX_STATUS, &data);
	} else if (os_strncmp(custom, "DEAUTH", 7) == 0) {
		struct qtn_deauth_event_data *event_data =
				(struct qtn_deauth_event_data *)custom;

		wpa_printf(MSG_DEBUG, "WEXT: DEAUTH event reason_code=%d, addr=" MACSTR "",
				event_data->reason_code, MAC2STR(event_data->addr));
		os_memset(&data, 0, sizeof(data));
		data.deauth_info.addr = event_data->addr;
		data.deauth_info.reason_code = event_data->reason_code;
		wpa_supplicant_event(ctx, EVENT_DEAUTH, &data);
	} else if (os_strncmp(custom, "CSA", 4) == 0) {
		struct qtn_csa_event_data *event_data =
				(struct qtn_csa_event_data *)custom;

		wpa_printf(MSG_DEBUG, "WEXT: CSA event, chan=%d, freq=%d",
				event_data->chan, event_data->freq);
		os_memset(&data, 0, sizeof(data));
		data.ch_switch.freq = event_data->freq;
		wpa_supplicant_event(ctx, EVENT_CH_SWITCH, &data);
#endif /* CONFIG_QTNA_WIFI */
	}
}
	...
	while ((size_t) (end - pos) >= IW_EV_LCP_LEN) {
		...
		case SIOCGIWAP:
			...
			if (is_zero_ether_addr(
				...
				event_data.disassoc_info.reason_code = iwe->u.param.value;
#endif /* CONFIG_QTNA_WIFI */
				os_free(drv->assoc_req_ies);
				drv->assoc_req_ies = NULL;
				os_free(drv->assoc_resp_ies);
				    ...
				    size_t *len)
{
	...
	size_t max_buf_size = wpa_s->conf->scan_buf_max_size;
#else
	...
	for (;;) {
		...
		if (errno == E2BIG && res_buf_len < max_buf_size) {
			os_free(res_buf);
			res_buf = NULL;
			res_buf_len *= 2;
			if (res_buf_len > max_buf_size)
				res_buf_len = max_buf_size;
			wpa_printf(MSG_DEBUG, "Scan results did not fit - "
				   "trying larger buffer (%lu bytes)",
				   (unsigned long) res_buf_len);
#ifdef CONFIG_QTNA_WIFI
		} else if (errno == E2BIG && res_buf_len == max_buf_size &&
				iwr.u.data.length > 0) {
			wpa_printf(MSG_DEBUG, "Scan result list is too large"
				" - returning %zu bytes\n", max_buf_size);
			break;
#endif /* CONFIG_QTNA_WIFI */
		} else {
				 ...
				 char *end)
{
	...
	if (clen > 7 && os_strncmp(custom, "wpa_ie=", 7) == 0) {
		...
		res->ie_len += bytes;
#ifdef CONFIG_QTNA_WIFI
	/* HT Capability IE needed in scan processing */
	} else if (clen > 9 && os_strncmp(custom, "htcap_ie=", 9) == 0) {
		char *spos;
		int bytes;
		spos = custom + 9;
		bytes = custom + clen - spos;
		...
		if (tmp == NULL)
			return;
		res->ie = tmp;
		if (hexstr2bin(spos, tmp + res->ie_len, bytes)) {
			wpa_printf(MSG_INFO, "Invalid HT Capabilities IE data");
			return;
		}
		res->ie_len += bytes;
	} else if (clen > strlen(QTN_EXTENDER_ROLE_STR) &&
			os_strncmp(custom, QTN_EXTENDER_ROLE_STR,
				strlen(QTN_EXTENDER_ROLE_STR)) == 0) {
		char *spos;
		int extender_role = 0;

		spos = custom + strlen(QTN_EXTENDER_ROLE_STR);
		if (sscanf(spos, "%d", &extender_role) == 1)
		      res->res.qtn_extender_role = extender_role;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
/**
 * wpa_driver_check_freq - Check whether the specified frequency is supported by the driver
 * @drv: Pointer to wext driver data
 * @freq: Frequency to be checked
 * Returns: 0 on support, -1 on not support
 */
static int wpa_driver_check_freq(struct wpa_driver_wext_data *drv, struct iw_freq *freq)
{
	int i;

	for (i = 0; i < drv->num_frequency; ++i)
		if ( freq->e == drv->freq[i].e && freq->m == drv->freq[i].m)
			return 0;
	return -1;
}

static int wpa_get_freq_range(void *priv,
	struct ieee80211_ioctl_freq_range *freq_range)
{
	struct wpa_driver_wext_data *drv = priv;
	struct iwreq iwr;
	...
	os_memset(&iwr, 0, sizeof(iwr));
	os_strlcpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
	iwr.u.data.pointer = (caddr_t)freq_range;
	iwr.u.data.length = sizeof(*freq_range);
	iwr.u.data.flags = SIOCDEV_SUBIO_GET_FREQ_RANGE;

	if (ioctl(drv->ioctl_sock, IEEE80211_IOCTL_EXT, &iwr) < 0) {
		perror("ioctl[SIOCDEV_SUBIO_GET_FREQ_RANGE]");
		return -1;
	}

	if (iwr.u.data.length > sizeof(*freq_range)) {
		wpa_printf(MSG_ERROR, "SIOCDEV_SUBIO_GET_FREQ_RANGE:"
			" wrong data length %d", iwr.u.data.length);
		return -1;
	}
...
#ifdef CONFIG_QTNA_WIFI
			if (wpa_driver_check_freq(drv, &iwe->u.freq) < 0)
				data.res.flags |= WPA_SCAN_FREQ_NOT_AVAILABLE;
#endif /* CONFIG_QTNA_WIFI */
			break;
		case IWEVQUAL:
...
#ifdef CONFIG_QTNA_WIFI
		drv->num_frequency = range->num_frequency;
		os_memcpy(drv->freq, range->freq, MIN(sizeof(drv->freq), sizeof(range->freq)));
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	/*
	 * Overwrite the frequency lists since driver could support
	 * more than IW_MAX_FREQUENCIES (32) channels
	 */
	if (wpa_get_freq_range(priv, &freq_range) == 0) {
		drv->num_frequency = freq_range.num_freq;
		for (i = 0; i < freq_range.num_freq; i++) {
			drv->freq[i].m = freq_range.freq[i].m;
			drv->freq[i].e = freq_range.freq[i].e;
			drv->freq[i].i = freq_range.freq[i].i;
			drv->freq[i].flags = freq_range.freq[i].flags;
		}
	}
#endif /* CONFIG_QTNA_WIFI */

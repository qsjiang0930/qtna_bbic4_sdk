...
#ifdef CONFIG_QTNA_WIFI
/* BIT 6 is used for ANQP_FETCH_TRIED */
#define WPA_SCAN_FREQ_NOT_AVAILABLE	BIT(7)
...
struct wpa_driver_ap_params {
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * hs20_enable - Whether HS2.0 is enabled
	 */
	int hs20_enable;

	/**
	 * proxy_arp - Proxy ARP Service
	 */
	int proxy_arp;
	...
	/**
	 * ft_over_ds - fast transition over DS
	 */
	int ft_over_ds;
#endif /* CONFIG_QTNA_WIFI */
};
...
struct wpa_driver_ops {
	 ...
	 * @msg: optional string message passed in.
	 * Returns: 0 on success, -1 on failure
	 *
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * set_app_ie - Add APP IE to different management frames
	 * @priv: Private driver interface data
	 * @subtype: APP IE(s) for this subtype frames
	 * @appie: APP IE(s) to add or %NULL
	 *	to remove extra IE(s)
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_app_ie)(void *priv, u8 subtype, const struct wpabuf *appie);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * set_pairing_hash_ie - Set the pairing hash IE for each Bss.
	 * @priv: Private driver interface data
	 * @pairing_hash: The hash value for pairing between AP and STA.
	 * @ies_len: Length of the pairing hash
	 */
	int (*set_pairing_hash_ie)(void *priv, const u8 *pairing_hash, size_t ies_len);
#ifdef HOSTAPD
	/**
	...
	/**
	 * set_intra_per_bss - Enable/Disables intra-BSS bridging
	 * This function takes effect on the specified BSS.
	 */
	int (*set_intra_per_bss)(void *priv, int enabled);

	/**
	 * set_bss_isolate - isolate the BSS from the other BSSs
	 */
	int (*set_bss_isolate)(void *priv, int enabled);

	/**
	 * set_bss_assoc_limit - Set the max associated stations number for a BSS.
	 * @priv: Private driver interface data
	 * @limit: Max associated stations number
	 *
	 * This function can be used to limit the associated station number for a VAP.
	 */
	int (*set_bss_assoc_limit)(void *priv, int limit);

	/**
	 * set_total_assoc_limit - Set the max associated stations number for all BSSs.
	 * @priv: Private driver interface data
	 * @limit: Max associated stations number
	 *
	 * This function can be used to limit the associated station number for all VAPs.
	 */
	int (*set_total_assoc_limit)(void *priv, int limit);

	/**
	 * set_brcm_ioctl - Control the driver to support broadcom sta interference
	 * report
	 * @priv: Private driver interface data
	 * @buf: Buffer for broadcom report packets date
	 * @buflen: Maximum length of the buffer
	 * Returns: 0 on success or -1 on failure
	 */
	int (*set_brcm_ioctl)(void *priv, uint8_t *buf, uint32_t len);
	...
	/* Function to send a message directly to the underlying driver (for logging) */
	void (*send_log)(void *priv, const char *msg);

	/**
	 * Put BSS as a dynamic VLAN port
	 */
	int (*set_dyn_vlan)(void *priv, const char *, int enable);
	int (*vlan_group_add)(void *priv, const char *, int vlan_id);
	int (*vlan_group_remove)(void *priv, const char *, int vlan_id);
#else /* HOSTAPD */
	/**
	...
	/**
	 * get_pairing_hash_ie - Get the pairing hash IE from the Assoc resp
	 * @priv: Private driver interface data
	 * @pairing_hash: The hash value for pairing between AP and STA.
	 * @ies_len: The hash length - 32
	 * @addr: Peering MAC address
	 */
	int (*get_pairing_hash_ie)(void *priv, u8 *pairing_hash, size_t ies_len, u8 *addr);

	/**
	 * set_fast_reassoc - Setting value into fast reassoc.
	 * @priv: Private driver interface data
	 * @value: To enable (or) disable fast reassocation functionality
	 * @old_value: To get a current value of fast reassocation functionality
	 *
	 * This function is used to set fast reassocation functionality for fast
	 * WPS Association.
	 */
	int (*set_fast_reassoc) (void *priv, const int value, int *old_value);

	/**
	 * set_pmf - Setting pmf value
	 * @priv: Private driver interface data
	 * @pmf_value: 0=disabled, 1=enabled, 2=required
	 * Returns: 0 on success, -1 on failure
	 *
	 * This function can be used to update PMF value in driver
	 *
	 */
	int (*set_pmf)(void *priv, int pmf_value);

	/**
	 * get_pmf - Getting pmf value
	 * @priv: Private driver interface data
	 * Returns: 0=disabled, 1=enabled, 2=required
	 *
	 */
	int (*get_pmf)(void *priv);
#endif /* HOSTAPD */
#endif /* CONFIG_QTNA_WIFI */
};
...
#ifdef CONFIG_QTNA_WIFI
struct neighbor_report_trans_item {
	uint8_t bssid[ETH_ALEN];
	uint8_t operating_class;
	uint8_t channel;
};

#define AUTH_FROM_L2PCAP_EVENT 0
#define AUTH_FROM_CUSTOM_EVENT 1
#endif
/**
...
union wpa_event_data {
	...
	struct auth_info {
		...
		size_t ies_len;
#ifdef CONFIG_QTNA_WIFI
		u8 auth_event_source;
		struct neighbor_report_trans_item nr_item;
#endif
	} auth;

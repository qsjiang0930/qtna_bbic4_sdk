					  ...
					  u8 id, u8 len, const u8 *pos)
{
	...
	case WFA_ELEM_MULTIAP_BACKHAUL_EXTENSION:
		if (len != 1) {
			wpa_printf(MSG_DEBUG, "WPS: Invalid MultiAP Backhaul Extension length %u",
				   len);
			return -1;
		}
		attr->multiap_backhaul_ext = *pos;
		break;
...
#ifdef CONFIG_QTNA_WIFI
static int wps_set_vendor_ext_qtn_subelem(struct wps_parse_attr *attr,
					  u8 id, u8 len, const u8 *pos)
{
	wpa_printf(MSG_INFO, "WPS: Qtn subelement id=%u len=%u",
		   id, len);
	switch (id) {
	case WPS_ALLOW_PBC_VL_SUB_ID:
		if (len != 1) {
			wpa_printf(MSG_DEBUG, "WPS: Invalid Version2 length "
				   "%u", len);
			return -1;
		}
		attr->allow_pbc_overlap = pos;
		break;
	default:
		wpa_printf(MSG_MSGDUMP, "WPS: Skipped unknown WFA Vendor "
			   "Extension subelement %u", id);
...
static int wps_parse_vendor_ext_qtn(struct wps_parse_attr *attr, const u8 *pos,
				    u16 len)
{
	const u8 *end = pos + len;
	u8 id, elen;
	...
	while (pos + 2 < end) {
		...
		if (pos + elen > end)
			break;
		if (wps_set_vendor_ext_qtn_subelem(attr, id, elen, pos) < 0)
			return -1;
		pos += elen;
	}
...
#ifdef CONFIG_QTNA_WIFI
	case WPS_VENDOR_ID_QTN:
		return wps_parse_vendor_ext_qtn(attr, pos + 3, len - 3);
	default:
		break;
#endif /* CONFIG_QTNA_WIFI */
	}

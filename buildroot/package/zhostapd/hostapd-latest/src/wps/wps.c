...
int qtn_wps_pbc_timeout = WPS_PBC_WALK_TIME;
...
#ifdef CONFIG_QTNA_WIFI
/**
 * wps_is_allowed_pbc_overlap - Check whether WPS Quantenna
 * extension attribute is contained in WSC IE
 * @msg: WPS IE contents from Beacon or Probe Response frame
 * Returns: 1 if PBC overlap is allowed, 0 if not
 */
int wps_is_allowed_pbc_overlap(const struct wpabuf *msg)
{
	...
	if (wps_parse_msg(msg, &attr) < 0 ||
	    !attr.allow_pbc_overlap || *attr.allow_pbc_overlap == 0)
		return 0;
				       ...
				       const u8 *req_dev_types)
{
	...
	if (wps_build_version(ie) ||
	    ...
	    wps_build_wfa_ext(ie,
			      (req_type == WPS_REQ_ENROLLEE ? WPS_REQ_ENROLLEE_BIT : 0),
			      NULL, 0) ||

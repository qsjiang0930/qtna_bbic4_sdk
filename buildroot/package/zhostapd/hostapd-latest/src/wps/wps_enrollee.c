...
static struct wpabuf * wps_build_m1(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) ||
	    ...
	    wps_build_wfa_ext(msg,
			      (wps->wps->dev.easymesh_backhaul_sta_en ? WPS_REQ_EASYMESH_BIT : 0),
			      NULL, 0) ||
...
#if !defined(WPA_TKIP_SUPPORT)
	if ((wps->cred.encr_type == WPS_ENCR_TKIP) ||
		(is_current_region_us() &&
			((wps->cred.encr_type & WPS_ENCR_AES_TKIP) == WPS_ENCR_AES_TKIP))) {
		wpa_printf(MSG_ERROR, "WPS: Reject Credential. Unsupported TKIP encr_type 0x%x",
				wps->cred.encr_type);
		return -1;
	}
#endif /* WPA_TKIP_SUPPORT */
				     ...
				     struct wpabuf *attrs, int wps2)
{
	...
	if ((cred.encr_type == WPS_ENCR_TKIP) ||
		(is_current_region_us() &&
			((cred.encr_type & WPS_ENCR_AES_TKIP) == WPS_ENCR_AES_TKIP))) {
		wpa_printf(MSG_ERROR, "WPS: Reject Credential. Unsupported TKIP encr_type 0x%x",
				cred.encr_type);
		return -1;
	}
						...
						const struct wpabuf *msg)
{
	...
	if (wps->state == RECV_ACK && wps->wps->ap) {
		...
		wps->state = WPS_FINISHED;
#ifdef CONFIG_QTNA_WIFI
		os_memcpy(wps->wps->last_wps_client, wps->peer_dev.mac_addr, ETH_ALEN);
		os_memset(wps->wps->last_wps_client_wps_type,
				0, sizeof(wps->wps->last_wps_client_wps_type));
		os_memcpy(wps->wps->last_wps_client_wps_type, "ap_pin", strlen("ap_pin"));
#endif /* CONFIG_QTNA_WIFI */
		return WPS_DONE;
	}

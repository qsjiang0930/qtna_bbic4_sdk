...
#define WPS_SEC_DEVICE_TYPES 5
	...
	int pp_enable;
	char *pp_device_name;
	char *pp_device_name_bl;
	u32 easymesh_backhaul_sta_en;
	u8  multiap_backhaul_ext;
...
#ifdef CONFIG_QTNA_WIFI
int wps_is_allowed_pbc_overlap(const struct wpabuf *msg);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * wps_reg_event_cb - Callback for sending message to driver
	 * @ctx: Higher layer context data (cb_ctx)
	 * @msg: message that will be sent to driver
	 */
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * force_broadcast_uuid - Whether to broadcast uuid in beacon
	 * without dualband restriction
	 */
	int force_broadcast_uuid;

	/**
	 * wps_on_hidden_ssid - Whether wps could be turned on when ssid is off
	 */
	int wps_on_hidden_ssid;

	/**
	 * ignore_broadcast_ssid - Whether the ssid is hidden
	 */
	int ignore_brodcast_ssid;

	/**
	 * ssid_set_cb - Callback for showing or hiding SSID of this BSS
	 * @ctx: Higher layer context data (cb_ctx)
	 * @value: enable or disable SSID
	 */
	int (*ssid_set_cb)(void *ctx, int value);

	/**
	 * wps_cred_processing_multiap_backhaul: build credential with backhual BSS
	 *
	 * This option can be used to disable internal code that builds
	 * Credential attribute into M8 based on the current network
	 * configuration and Enrollee capabilities. The extra_cred data will
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * WPS_EV_INIT - WPS finish initialization
	 */
	WPS_EV_INIT,
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * WPS_EV_PBC_START - PBC start.
	 */
	WPS_EV_PBC_START,

	/**
	 * WPS_EV_PIN_START - PIN start.
	 */
	WPS_EV_PIN_START,

	/**
	 * WPS_EV_PIN_TIMEOUT - PIN timeout.
	 */
	WPS_EV_PIN_TIMEOUT,

	/**
	 * WPS_EV_TIMEOUT - Timeout
	 */
	WPS_EV_TIMEOUT,

	/**
	 * WPS_EV_EAP_M2_SEND - M2 send.
	 */
	WPS_EV_EAP_M2_SEND,

	/**
	 * WPS_EV_EAP_M8_SEND - M8 send.
	 */
	WPS_EV_EAP_M8_SEND,

	/**
	 * WPS_EV_STA_CANCEL - Station Cancel WPS
	 */
	WPS_EV_STA_CANCEL,

	/**
	 * WPS_EV_STA_PIN_FAIL - STA PIN check fail
	 */
	WPS_EV_STA_PIN_FAIL,
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
/**
 * enum wps_vendor - id to identify different vendor
 *
 * different vendors may require different actions during
 * WPS process
 */
enum wps_vendor {
	WPS_VENDOR_NONE = 0,
	WPS_VENDOR_NETGEAR = 1,
};
#endif /* CONFIG_QTNA_WIFI */
...
struct wps_context {
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * wps_external_state - Current (external) WPS state
	 */
...
#ifdef CONFIG_QTNA_WIFI
	/**
	 * last_wps_client - record last successful wps client
	 * mac address
	 */
	u8 last_wps_client[ETH_ALEN];

	/**
	 * last_wps_client_wps_type - record last successful wps client
	 * wps_type
	 */
	u8 last_wps_client_wps_type[8];

	/**
	 *  pbc_in_srcm - Do not remove PushButton config method in Selected
	 *  Registrar config methods (AP)
	 *
	 *  Some labs require SRCM must show all supported config methods
	 */
	int pbc_in_srcm;
	...
	/**
	 *  ieee80211w - PMF value. TKIP should be enabled only when we disable PMF.
	 */
	int ieee80211w;
	...
	/**
	 * multiap_backhaul_bss_ssid: MultiAP backhaul BSS SSID
	 *
	 * This optional data (set to NULL to disable) can be used to add
	 * Credential attribute(s) for other networks into M8. If
	 * wps_cred_processing_multiap_backhaul is set, this will also override the automatically
	 * generated Credential attribute.
	 */
	u8 multiap_backhaul_bss_ssid[SSID_MAX_LEN];

	/**
	 * multiap_backhaul_bss_ssid: MultiAP backhaul BSS SSID length
	 *
	 * This optional data (set to NULL to disable) can be used to add
	 * Credential attribute(s) for other networks into M8. If
	 * wps_cred_processing_multiap_backhaul is set, this will also override the automatically
	 * generated Credential attribute.
	 */
	u8 multiap_backhaul_bss_ssid_len;


	/**
	 * multiap_backhaul_bss_wpa_psk: MultiAP backhaul BSS auth type
	 *
	 * This optional data (set to NULL to disable) can be used to add
	 * Credential attribute(s) for other networks into M8. If
	 * wps_cred_processing_multiap_backhaul is set, this will also override the automatically
	 * generated Credential attribute.
	 */
	int multiap_backhaul_bss_wpa_psk;

	/**
	 * multiap_backhaul_bss_ssid: MultiAP backhaul BSS key
	 *
	 * This optional data (set to NULL to disable) can be used to add
	 * Credential attribute(s) for other networks into M8. If
	 * wps_cred_processing_multiap_backhaul is set, this will also override the automatically
	 * generated Credential attribute.
	 */
	u8 multiap_backhaul_bss_wpa_passphrase[64];
#endif /* CONFIG_QTNA_WIFI */
};

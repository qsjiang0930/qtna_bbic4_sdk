...
{
	dev->multiap_backhaul_ext = ext;
	wpa_printf(MSG_DEBUG, "WPS: extension value %02x", dev->multiap_backhaul_ext);
}
...
#ifdef CONFIG_QTNA_WIFI
	os_free(dev->pp_device_name);
	dev->pp_device_name = NULL;
	os_free(dev->pp_device_name_bl);
	dev->pp_device_name_bl = NULL;
#endif /* CONFIG_QTNA_WIFI */
}

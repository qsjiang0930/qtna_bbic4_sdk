...
#ifdef CONFIG_QTNA_WIFI
static int wps_build_vendor_ext_m1_wfa_map(struct wpabuf *msg)
{
	struct wpabuf *easymesh_backhaul_sta;

	easymesh_backhaul_sta = wpabuf_alloc(sizeof(easymesh_backhaul_sta_vendor_ext));
	if (!easymesh_backhaul_sta)
		return -1;
	wpabuf_put_data(easymesh_backhaul_sta, easymesh_backhaul_sta_vendor_ext,
	...
	wpabuf_free(easymesh_backhaul_sta);

	return 0;
}
#endif

int wps_build_wfa_ext(struct wpabuf *msg, int req_mask,
		      const u8 *auth_macs, size_t auth_macs_count)
{
	...
	if (wpabuf_tailroom(msg) <
	    7 + 3 + ((req_mask & WPS_REQ_ENROLLEE_BIT) ? 3 : 0) +
	...
	if (req_mask & WPS_REQ_ENROLLEE_BIT) {
...
#ifdef CONFIG_QTNA_WIFI
	if (req_mask & WPS_REQ_EASYMESH_BIT)
		wps_build_vendor_ext_m1_wfa_map(msg);
#endif
...
int wps_build_auth_type_flags(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (is_current_region_us())
		auth_types &= ~WPS_AUTH_WPAPSK;
#endif
	if (wps->wps->ieee80211w)
		auth_types &= ~WPS_AUTH_WPAPSK;
#endif /* CONFIG_QTNA_WIFI */

...
#ifdef CONFIG_QTNA_WIFI
	if (get_netif_info(sm->net_if, &ip_addr, &sm->ip_addr_text,
			&sm->netmask, sm->mac_addr)) {
		wpa_printf(MSG_ERROR, "WPS UPnP: Couldn't get IP/MAC address for %s",
				sm->net_if);
	}

	if (ip_addr > 0 && ip_addr != sm->ip_addr) {
		wpa_printf(MSG_DEBUG, "WPS UPnP: IP address changed to: %s for %s",
				sm->ip_addr_text, sm->net_if);
		if (setsockopt(sm->multicast_sd, IPPROTO_IP, IP_MULTICAST_IF,
				&ip_addr, sizeof(ip_addr))) {
			wpa_printf(MSG_DEBUG, "WPS: setsockopt(IP_MULTICAST_IF) %x: %d (%s)",

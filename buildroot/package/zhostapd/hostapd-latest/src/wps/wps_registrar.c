...
#ifdef CONFIG_QTNA_WIFI
#define QTN_UNHIDE_SSID		0
#define QTN_HIDE_SSID		1
#endif /* CONFIG_QTNA_WIFI */
...
struct wps_registrar {
	...
	void (*enrollee_seen_cb)(void *ctx, const u8 *addr, const u8 *uuid_e,
				 ...
				 const char *dev_name);
#ifdef CONFIG_QTNA_WIFI
	void (*wps_reg_event_cb)(void *ctx, const char *msg);
#endif /* CONFIG_QTNA_WIFI */
	...
	int wps_on_hidden_ssid;
	int (*ssid_set_cb)(void *ctx, int value);
	int wps_cred_processing_multiap_backhaul;
#endif /* CONFIG_QTNA_WIFI */
};
					...
					struct wpabuf *msg)
{
	...
	if (id != DEV_PW_PUSHBUTTON)
		return 0;
	if (!reg->force_broadcast_uuid && !reg->dualband)
		return 0;
#else
...
#ifdef CONFIG_QTNA_WIFI
	reg->wps_reg_event_cb = cfg->wps_reg_event_cb;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	reg->wps_cred_processing_multiap_backhaul = cfg->wps_cred_processing_multiap_backhaul;
#endif /* CONFIG_QTNA_WIFI */

#ifdef CONFIG_QTNA_WIFI
	reg->force_broadcast_uuid  = cfg->force_broadcast_uuid;
	reg->wps_on_hidden_ssid    = cfg->wps_on_hidden_ssid;
	reg->ignore_broadcast_ssid = cfg->ignore_brodcast_ssid;
	reg->ssid_set_cb           = cfg->ssid_set_cb;
#endif /* CONFIG_QTNA_WIFI */
			  ...
			  int timeout)
{
...
#ifdef CONFIG_QTNA_WIFI
	if ((reg->wps) && (reg->wps->event_cb))
		reg->wps->event_cb(reg->wps->cb_ctx, WPS_EV_PIN_START, NULL);

	if (reg->wps_on_hidden_ssid && reg->ignore_broadcast_ssid) {
		wpa_printf(MSG_DEBUG, "WPS: %s: Show SSID\n", __func__);
		reg->ssid_set_cb(reg->cb_ctx, QTN_UNHIDE_SSID);
	}
#endif /* CONFIG_QTNA_WIFI */
	...
	eloop_register_timeout(
				...
				WPS_PBC_WALK_TIME,
#endif /* CONFIG_QTNA_WIFI */
				0, wps_registrar_set_selected_timeout,
				reg, NULL);
...
#ifdef CONFIG_QTNA_WIFI
				wpa_printf(MSG_DEBUG, "WPS: Re-enabling used wildcard PIN");
				pin->wildcard_uuid = 1;
#else
...
#ifdef CONFIG_QTNA_WIFI
	if (reg->wps_on_hidden_ssid && reg->ignore_broadcast_ssid) {
		wpa_printf(MSG_DEBUG, "WPS: %s: Hide SSID\n", __func__);
		reg->ssid_set_cb(reg->cb_ctx, QTN_HIDE_SSID);
	}
#endif /* CONFIG_QTNA_WIFI */
}
...
#ifdef CONFIG_QTNA_WIFI
	if ((reg->wps) && (reg->wps->event_cb))
		reg->wps->event_cb(reg->wps->cb_ctx, WPS_EV_PBC_TIMEOUT, NULL);
#endif /* CONFIG_QTNA_WIFI */
}
				...
				const u8 *p2p_dev_addr)
{
	...
	eloop_register_timeout(
				...
				WPS_PBC_WALK_TIME,
#endif /* CONFIG_QTNA_WIFI */
		0, wps_registrar_pbc_timeout, reg, NULL);

#ifdef CONFIG_QTNA_WIFI
	if ((reg->wps) && (reg->wps->event_cb))
		reg->wps->event_cb(reg->wps->cb_ctx, WPS_EV_PBC_START, NULL);
				...
				int p2p_wildcard)
{
	...
	if (
#ifdef CONFIG_QTNA_WIFI
			reg->pbc && reg->selected_registrar &&
#endif /* CONFIG_QTNA_WIFI */
			wps_registrar_pbc_overlap(reg, addr, attr.uuid_e)) {
...
#ifdef CONFIG_QTNA_WIFI
	wpa_printf(MSG_DEBUG, "WPS: Invalidating used wildcard PIN");
	wps_registrar_invalidate_pin(reg, uuid_e);
#endif /* CONFIG_QTNA_WIFI */
}
...
#ifdef CONFIG_QTNA_WIFI
static void wps_setting_ntgr_map(unsigned char *input, unsigned char *output, int len)
{
	const unsigned char ntgr_table[] =
	...
	if (input != NULL && output != NULL && len > 0) {
		for (i = 0; i < len; i++) {
			...
			index = input[i] % sizeof(ntgr_table);
			output[i] = ntgr_table[index];
		}
	}
}


static int wps_ntgr_generate_new_setting(struct wps_data *wps)
{
#define NTGR_HEADER			"NTGR_"
#define NTGR_HEADER_SIZE		5
#define NTGR_SSID_SIZE_ULIMIT		28
#define NTGR_PASSWD_SIZE_ULIMIT		64
#define NTGR_PASSWD_SIZE_BASE		8

	u8 ssid_source[NTGR_SSID_SIZE_ULIMIT];
	u8 ssid[NTGR_HEADER_SIZE + NTGR_SSID_SIZE_ULIMIT];
	u8 pass_source[NTGR_PASSWD_SIZE_ULIMIT];
	u8 *pass;
	u8 ssid_len;
	u8 pass_len;

	/* we tell Client to use WPA2PSK and AES */
	wps->auth_type = WPS_AUTH_WPA2PSK;
	wps->cred.auth_type = wps->auth_type;
	wps->encr_type = WPS_ENCR_AES;
	wps->cred.encr_type = wps->encr_type;

	if (random_get_bytes(ssid_source, sizeof(ssid_source)) < 0 ||
		random_get_bytes(pass_source, sizeof(pass_source)) < 0 ||
		random_get_bytes(&ssid_len, sizeof(ssid_len)) < 0 ||
		random_get_bytes(&pass_len, sizeof(pass_len)) < 0)
		return -1;

	os_memset(ssid, 0, sizeof(ssid));
	ssid_len = ssid_len % NTGR_SSID_SIZE_ULIMIT;
	memcpy(ssid, NTGR_HEADER, NTGR_HEADER_SIZE);
	wps_setting_ntgr_map(ssid_source, ssid + NTGR_HEADER_SIZE, (int)ssid_len);
	os_memcpy(wps->cred.ssid, ssid, strlen((const char *)ssid) + 1);
	wps->cred.ssid_len = strlen((const char *)ssid);

	pass_len = pass_len % NTGR_PASSWD_SIZE_ULIMIT;
	if (pass_len < NTGR_PASSWD_SIZE_BASE)
		pass_len += NTGR_PASSWD_SIZE_BASE;
	pass = os_malloc(NTGR_PASSWD_SIZE_ULIMIT);
	if (pass == NULL)
		return -1;
	os_memset(pass, 0, NTGR_PASSWD_SIZE_ULIMIT);
	os_free(wps->new_psk);
	wps_setting_ntgr_map(pass_source, pass, pass_len);
	wps->new_psk = pass;
	wps->new_psk_len = pass_len;
...
#undef NTGR_HEADER
#undef NTGR_HEADER_SIZE
#undef NTGR_SSID_SIZE_ULIMIT
#undef NTGR_PASSWD_SIZE_ULIMIT
#undef NTGR_PASSWD_SIZE_BASE
}


static int wps_generate_new_setting(struct wps_data *wps)
{
	int ret = 0;

	switch (wps->wps->vendor) {
	case WPS_VENDOR_NETGEAR:
		ret = wps_ntgr_generate_new_setting(wps);
		break;
...
static int wps_build_multiap_backhaul_ap_settings(struct wps_data *wps_data, struct wpabuf *msg)
{
	struct wps_context *wps = wps_data->wps;
	struct wps_credential multiap_backhaul_ap_cred;
	struct wpabuf *cred;

	wpa_printf(MSG_DEBUG, "WPS: MultiAP backhaul BSS Credential");

	memset(&multiap_backhaul_ap_cred, 0, sizeof(multiap_backhaul_ap_cred));
	multiap_backhaul_ap_cred.ssid_len = wps->multiap_backhaul_bss_ssid_len;
	os_memcpy(multiap_backhaul_ap_cred.ssid, wps->multiap_backhaul_bss_ssid,
			wps->multiap_backhaul_bss_ssid_len);
	multiap_backhaul_ap_cred.auth_type = wps->multiap_backhaul_bss_wpa_psk;

	if (multiap_backhaul_ap_cred.auth_type == WPS_AUTH_WPA2PSK ||
			multiap_backhaul_ap_cred.auth_type == WPS_AUTH_WPAPSK) {
		multiap_backhaul_ap_cred.encr_type = WPS_ENCR_AES;
	} else {
		wpa_printf(MSG_ERROR,
				"WPS: MultiAP BH BSS No suitable enc type for non-WPA/WPA2");
		return -1;
	}
	 ...
	 */
	os_memcpy(multiap_backhaul_ap_cred.mac_addr, wps_data->mac_addr_e, ETH_ALEN);
	os_memcpy(multiap_backhaul_ap_cred.key, wps->multiap_backhaul_bss_wpa_passphrase,
		 os_strlen((char *)wps->multiap_backhaul_bss_wpa_passphrase));
	multiap_backhaul_ap_cred.key_len = os_strlen((char *)wps->multiap_backhaul_bss_wpa_passphrase);
	...
	if (wps_build_credential(cred, &multiap_backhaul_ap_cred)) {
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
...
#ifdef CONFIG_QTNA_WIFI
	if (wps->wps->registrar->wps_cred_processing_multiap_backhaul &&
			(wps->peer_dev.multiap_backhaul_ext == MULTIAP_BACKHAUL_STA))
		return wps_build_multiap_backhaul_ap_settings(wps, msg);
#endif /* CONFIG_QTNA_WIFI */
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
		...
		wps->cred.key_len = wps->new_psk_len;
#ifdef CONFIG_QTNA_WIFI
		if (wps_generate_new_setting(wps) < 0)
			return -1;
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	if (wps->wps->event_cb)
		wps->wps->event_cb(wps->wps->cb_ctx, WPS_EV_EAP_M2_SEND, NULL);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	if (wps->wps->event_cb)
		wps->wps->event_cb(wps->wps->cb_ctx, WPS_EV_EAP_M8_SEND, NULL);
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
static int wps_process_bcm_stb_enrc(struct wps_data *wps, struct wps_parse_attr *attr)
{
#define STB_BCM_MANUFACTURER    "Broadcom"
#define STB_BCM_DEVNAME "Cisco Client"
	int is_bcm_stb_enrc = 0;

	if (!wps || !wps->wps || !attr || !attr->manufacturer || !attr->dev_name)
		return is_bcm_stb_enrc;

	if (wps->wps->auth_types == (WPS_AUTH_WPA2PSK | WPS_AUTH_WPAPSK) &&
			wps->wps->encr_types == (WPS_ENCR_AES | WPS_ENCR_TKIP) &&
			strncmp((const char *)attr->manufacturer, STB_BCM_MANUFACTURER,
					(size_t)attr->manufacturer_len) == 0 &&
			strncmp((const char *)attr->dev_name, STB_BCM_DEVNAME,
					(size_t)attr->dev_name_len) == 0) {
...
#undef STB_BCM_MANUFACTURER
#undef STB_BCM_DEVNAME
}
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	encr_types = WPA_GET_BE16(attr->encr_type_flags);
#else
...
#ifdef CONFIG_QTNA_WIFI
	if (!wps_process_bcm_stb_enrc(wps, attr))
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
#ifdef HOSTAPD
static void wps_send_wireless_event(struct wps_context *wps,
		struct wps_device_data *dev)
{
	struct wps_registrar *reg = wps->registrar;
	char msg[256] = {'\0'};

	if (reg && reg->wps_reg_event_cb) {
		snprintf(msg, sizeof(msg), "Station " MACSTR " device name is %s",
				MAC2STR(dev->mac_addr), dev->device_name);
		reg->wps_reg_event_cb(reg->cb_ctx, msg);
	}
}
#endif /* HOSTAPD */
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
	    wps_process_encr_type_flags(wps, attr) ||
#else
...
#ifdef CONFIG_QTNA_WIFI
#ifdef HOSTAPD
	if (wps->peer_dev.device_name)
		wps_send_wireless_event(wps->wps, &wps->peer_dev);

	if (wps_verify_device_name(wps->wps, attr->dev_name, attr->dev_name_len) ==
			WPS_PP_NAME_CHK_REJECT)
		return WPS_FAILURE;
#endif /* HOSTAPD */
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
			wps_registrar_stop_pbc(wps->wps->registrar);
#endif /* CONFIG_QTNA_WIFI */
			return WPS_CONTINUE;
		}
...
#endif /* WPS_WORKAROUNDS */
#ifdef CONFIG_QTNA_WIFI
	wps_process_vendor_ext_m1(&wps->peer_dev, attr->multiap_backhaul_ext);
#endif /* CONFIG_QTNA_WIFI */
						 ...
						 const struct wpabuf *msg)
{
	...
	wps_device_store(wps->wps->registrar, &wps->peer_dev,
			 wps->uuid_e);
#ifdef CONFIG_QTNA_WIFI
	os_memcpy(wps->wps->last_wps_client, wps->mac_addr_e, ETH_ALEN);
#endif /* CONFIG_QTNA_WIFI */
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk &&
...
#ifdef CONFIG_QTNA_WIFI
		if (wps->wps->vendor == WPS_VENDOR_NETGEAR) {
			wpa_printf(MSG_DEBUG,
				"WPS: Move to Configured state based on NTGR requirement");
			os_memcpy(&cred, &wps->cred, sizeof(cred));
			/* generate mix mode in our setting */
		...
		{
			wpa_printf(MSG_DEBUG,
				"WPS: Move to Configured state based on first Enrollee connect");
...
#ifdef CONFIG_QTNA_WIFI
	os_memset(wps->wps->last_wps_client_wps_type, 0,
			sizeof(wps->wps->last_wps_client_wps_type));
#endif /* CONFIG_QTNA_WIFI */

	if (wps->pbc) {
#ifdef CONFIG_QTNA_WIFI
		os_memcpy(wps->wps->last_wps_client_wps_type, "pbc", strlen("pbc"));
#endif /* CONFIG_QTNA_WIFI */
...
#ifdef CONFIG_QTNA_WIFI
		os_memcpy(wps->wps->last_wps_client_wps_type, "sta_pin", strlen("sta_pin"));
#endif /* CONFIG_QTNA_WIFI */
		wps_registrar_pin_completed(wps->wps->registrar);
	}
...
#ifdef CONFIG_QTNA_WIFI
	if (reg->selected_registrar && reg->wps) {
		u16 methods = reg->wps->config_methods;
				   ...
				   int pk_hash_provided_oob)
{
	...
	eloop_register_timeout(
			...
			WPS_PBC_WALK_TIME,
#endif /* CONFIG_QTNA_WIFI */
			0, wps_registrar_set_selected_timeout, reg, NULL);
...
#ifdef CONFIG_QTNA_WIFI
int wps_registrar_get_peer_devname(struct wps_registrar *reg, u8 *macaddr,
				char *buf, size_t buflen)
{
	struct wps_registrar_device *d;
	int ret;

	d = wps_device_get(reg, macaddr);
	if (d == NULL)
		return -1;

	ret = os_snprintf(buf, buflen, "%s", d->dev.device_name);
	if (ret < 0 || ret >= buflen)
		return -1;
...
int wps_registrar_check_selected(struct wps_registrar *reg)
{
	return reg->selected_registrar;
}


void wps_registrar_set_dualband(struct wps_registrar *reg, int dualband)
{
	reg->dualband = dualband;
}
#endif /* CONFIG_QTNA_WIFI */

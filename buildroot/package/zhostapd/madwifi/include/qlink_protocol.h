/**
  Copyright (c) 2008 - 2016 Quantenna Communications Inc
  All Rights Reserved
...
typedef enum
{
	...
	QLINK_CMD_UPDATE_BSS_CONFIG_FILE = 0x20,
	QLINK_CMD_SET_ACL = 0x21,
	QLINK_CMD_GET_STA_EXT_CAP_IE = 0x22,
} qlink_cmd;
...
struct qlink_cmd_update_bss_config_file
{
	__le32 bss_qid;
	__le32 file_type;
	__u8 data[1];
...
struct qlink_cmd_get_sta_ext_cap_ie
{
	__le32 bss_qid;
	__u8 addr[ETH_ALEN];
	__le32 len;
...
typedef enum
{
	...
	QLINK_REPLY_GET_STA_EXT_CAP_IE = QLINK_REPLY_FIRST + 0x7,
} qlink_reply;
...
struct qlink_reply_get_sta_ext_cap_ie
{
	__u8 ext_cap_ie[1];
} __attribute__ ((packed));

...
ifeq ($(strip $(BR2_PACKAGE_QCSAPI_SYSMON)),y)
QCSAPI_EXTRA_PROGS += sysmond
QCSAPI_EXTRA_CONFS += sysmond.conf
QCSAPI_EXTRA_DEP += libinih
endif
...
qcsapi: zlib $(QCSAPI_EXTRA_DEP)
	$(MAKE) MAKEFLAGS="-j1" -C $(QCSAPI_BUILD_DIR) EXTRA_PROGS="$(QCSAPI_EXTRA_PROGS)" \
		EXTRA_CONFS="$(QCSAPI_EXTRA_CONFS)" PREFIX="$(TARGET_DIR)" \
		CC=$(TARGET_CC) STRIP=$(STRIP) XCFLAGS="$(TARGET_CFLAGS) $(EXTRA_WARNINGS) $(EXTRA_DEFINES) -fPIC \

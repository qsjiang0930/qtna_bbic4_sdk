						...
						const char *file_path_name)
{
	...
	if (!stat(file_path_name, &file_stat) && (file_stat.st_mode & S_IFREG)) {
		connect_payload->sub_type = htoqt16(QFTP_FRAME_TYPE_CONNECT);
		connect_payload->seq = htoqt16(0);
		connect_payload->image_size = htoqt32(file_stat.st_size);
...
static uint32_t qftc_compose_data_cmd(void)
{
	...
	qftc_cfg.send_payload->sub_type = htoqt16(QFTP_FRAME_TYPE_DATA);
	seq = qt16toh(qftc_cfg.send_payload->seq);
	seq++;
	qftc_cfg.send_payload->seq = htoqt16(seq);
...
static int qftc_connect(const char *file_path_name)
{
	...
	do {
		...
		if ((bytes_recv >= (ssize_t)QFTP_ACK_NACK_FRAME_LEN) ||
			(qt16toh(qftc_cfg.recv_payload->sub_type) == QFTP_FRAME_TYPE_ACK)) {
			break;
		}

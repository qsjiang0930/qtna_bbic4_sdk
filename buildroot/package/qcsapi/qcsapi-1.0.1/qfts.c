...
int main(void)
{
	...
	if (retry_count > QFTS_RECV_RETRY_LIMIT ||
			qt16toh(connect_payload->sub_type) != QFTP_FRAME_TYPE_CONNECT) {
		qfts_clean();
		return -1;
	}

	image_size = qt32toh(connect_payload->image_size);
	...
	reply_payload->sub_type = htoqt16(QFTP_FRAME_TYPE_ACK);
	...
	do {
		...
		if (!op_failed && (qt16toh(data_payload->sub_type) == QFTP_FRAME_TYPE_DATA)) {
		...
		if (img_state != IMG_CRC_ERROR)
			...
			reply_payload->sub_type = htoqt16(QFTP_FRAME_TYPE_NACK);

...
else
{
	if($_POST['cmb_interface']!="")
	{
		...
		$interface_id='wifi'.substr($_POST['cmb_interface'],0,1);
	}
	else
	{
		$page_id="0";
...
function get_value()
{
	...
	{
		//Get the status of HS2.0, 1-enable 0-disable
		$get_hs20_status = trim(shell_exec("call_qcsapi get_hs20_status wifi1"));
		if (is_qcsapi_error($get_hs20_status))
			$get_hs20_status = 0;
		$ssid1=exec("call_qcsapi get_SSID wifi1");
		if(is_qcsapi_error($ssid1))
...
function set_value()
{
	...
	if ($_POST['action'] == 0)
	{
		exec("call_qcsapi set_wps_configured_state $interface_id $new_wpsstate");
		if($new_wpsstate == "2")
		{
			$curr_appin=exec("call_qcsapi get_wps_ap_pin $interface_id");
		}
	...
	else if($_POST['action'] == 3)
	{
		...
		$curr_wpspin_esc = escapeshellarg($curr_wpspin);

		if($curr_mode == "Access point")
		{
			$tmp=exec("hostapd_cli wps_check_pin $curr_wpspin_esc");
			if($tmp==$curr_wpspin)
			{
				exec("call_qcsapi registrar_report_pin $interface_id $curr_wpspin_esc");
		...
		{
			$tmp=exec("wpa_cli wps_check_pin $curr_wpspin_esc");
			if($tmp==$curr_wpspin)
			{
				exec("call_qcsapi enrollee_report_pin wifi0 $curr_wpspin_esc");
				echo "<script langauge=\"javascript\">self.open(\"config_wps_info.php?id=$page_id\",'WPS_STATUS','alwaysRaised,resizable,scrollbars,width=600,height=150').focus();</script>";
				//$wps_session=1;
			}
			else

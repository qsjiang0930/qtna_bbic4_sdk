...
function read_default_gw()
{
	$default_gw = exec("ip route show | awk '/^default via/ {print $3}'");
...
function read_dns()
{
	$dns = exec("awk '/^nameserver / {print $2}' /etc/resolv.conf 2>/dev/null | head -1");
...
function read_emac_addr()
{
	...
	if((strpos($emac_addr, "API error") === FALSE))
	{
		$res=$emac_addr;
	} else {
		$res="";
...
function read_pciemac_addr()
{
	...
	if((strpos($pciemac_addr, "API error") === FALSE))
	{
		$res=$pciemac_addr;
	} else {
		$res="";
...
function read_wmac_addr()
{
	...
	if((strpos($wmac_addr, "API error") === FALSE))
	{
		$res=$wmac_addr;
	} else {
		$res="";

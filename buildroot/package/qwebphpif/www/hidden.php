...
function load_value()
{
    ...
    if ($curr_scs_status == 1)
    {
        $scs_report_only=exec("call_qcsapi get_scs_params wifi0 | grep 'scs_report_only' | awk '{print $2}'");
        if ($scs_report_only != 0)
        {
            $curr_scs_status = 2;
        }
    }
    ...
    $curr_mu_mimo_status = exec("call_qcsapi get_enable_mu wifi0") == "0"? 0:1;
    if($curr_mode == "Repeater"){
        $curr_qtm_status = exec("call_qcsapi qtm $curr_ap_interface show") == "QCS API error 1: Operation not permitted"? 0:1;
        $curr_airtime_status = exec("call_qcsapi get_airfair $curr_ap_interface") == "0"? 0:1;
    }
    else{
        $curr_qtm_status = exec("call_qcsapi qtm wifi0 show") == "QCS API error 1: Operation not permitted"? 0:1;
        $curr_airtime_status = exec("call_qcsapi get_airfair wifi0") == "0"? 0:1;
    }
	...
	$file_mu = exec("call_qcsapi get_config_param wifi0 mu");
	if(is_qcsapi_error($file_mu))
	{
		$file_mu="";
	}
}
...
function set_value()
{
	global $curr_hw_wps_pbc,$curr_mode,$curr_pp_name,$curr_pp_status,$curr_nonwps_pp_status,$curr_mspp_name,$curr_mspp_status,$curr_region,$file_region,$curr_scs_status,$curr_qtm_status,$curr_bf_status,$arr,$curr_coc_status,$curr_mu_mimo_status,$curr_airtime_status,$interface_id,$curr_ap_interface,$file_mu;
	//set att pp status
    ...
    if(strcmp($new_scs_status,$curr_scs_status)!=0)
    {
        ...
        else if(1==$new_scs_status)
        {
            exec("call_qcsapi enable_scs wifi0 1");
            // confirm("Warning: Channel can't be changed manually when scs is enabled.");
            exec("call_qcsapi set_scs_report_only wifi0 0");
        }
        else
        {
            exec("call_qcsapi enable_scs wifi0 1");
            exec("call_qcsapi set_scs_report_only wifi0 1");
        }
    ...
    if($curr_mode == "Repeater"){
        ...
        if(strcmp($new_qtm_status,$curr_qtm_status)!=0)
        {
            ...
            if(0==$new_qtm_status)
            {
                exec("call_qcsapi qtm $curr_ap_interface set enabled 0");
            }
            else
            {
                exec("call_qcsapi qtm $curr_ap_interface set enabled 1");
            }
        ...
        $new_airtime_status = $_POST['airStatus'];
        if(strcmp($new_airtime_status,$curr_airtime_status)!=0)
        {
            if(0==$new_airtime_status)
            {
                exec("call_qcsapi set_airfair $curr_ap_interface 0");
            }
            else
            {
                exec("call_qcsapi set_airfair $curr_ap_interface 1");
            }

        }

        //set MU-MIMO
        $new_mu_mimo_status = $_POST['muStatus'];
        if(strcmp($new_mu_mimo_status,$curr_mu_mimo_status)!=0)
        {
            if($file_mu == "")
            {
                $contents = file_get_contents("/mnt/jffs2/wireless_conf.txt");
                $mu_str = "&mu=".$new_mu_mimo_status;
                $contents = $contents.$mu_str;
                file_put_contents("/mnt/jffs2/wireless_conf.txt", $contents);
                $_SESSION['qtn_can_reboot']=TRUE;
                header('Location: system_rebooted.php');
                exit();
                ...
                if(0==$new_mu_mimo_status)
                {
                    exec("call_qcsapi set_enable_mu $curr_ap_interface 0");
                    exec("call_qcsapi update_config_param $curr_ap_interface mu 0");
                }
                else
                {
                    exec("call_qcsapi set_enable_mu $curr_ap_interface 1");
                    exec("call_qcsapi update_config_param $curr_ap_interface mu 1");
                }
            }
        }
    }
        ...
        if(strcmp($new_airtime_status,$curr_airtime_status)!=0)
        {
            if(0==$new_airtime_status)
            {
                exec("call_qcsapi set_airfair wifi0 0");
            }
            else
            {
                exec("call_qcsapi set_airfair wifi0 1");
            }
        ...
        if(strcmp($new_mu_mimo_status,$curr_mu_mimo_status)!=0)
        {
            if($file_mu == "")
            {
                ...
                if(0==$new_mu_mimo_status)
                {
                    exec("call_qcsapi set_enable_mu wifi0 0");
                    exec("call_qcsapi update_config_param wifi0 mu 0");
                }
                else
                {
                    exec("call_qcsapi set_enable_mu wifi0 1");
                    exec("call_qcsapi update_config_param wifi0 mu 1");
                }
            }
        }
    }
...
function validate()
{
	...
	else if (cf.enblscs[1].checked)
	{
		cf.scsStatus.value = 1;
	}
	else
	{
		cf.scsStatus.value = 2;
	}
	...
	if (cf.enblair[0].checked)
	{
		cf.airStatus.value = 0;
	}
	else
	{
		cf.airStatus.value = 1;
	}
...
function populate_interface_list(index)
{
	...
	for (var i=0; i < cmb_if.options.length; i++)
	{
        if(mode == "Repeater"){
            ...
            if(i == 0){
                cmb_if.options[i].style.display="none";
            }
        }
        else{
            cmb_if.options[i].text = tmp_text[i]; cmb_if.options[i].value = interface_index[i];
		...
		<div class="rightmain">
			<table class="tablemain" style=" height:auto">
				...
				<tr>
					...
					<td>
						...
						<input type="radio" name="enblscs" id="enblscs"  <?php if(1==$curr_scs_status){echo "checked=\"checked\"";}?>>
							Enable&nbsp;&nbsp;
						<input type="radio" name="enblscs" id="enblscs"  <?php if(2==$curr_scs_status){echo "checked=\"checked\"";}?>>
							Report Only
				...
				<tr>
					<td>MU-MIMO:</td>
					<td>
						<input type="radio" name="enblmu" id="enblcoc"  <?php if(0==$curr_mu_mimo_status){echo "checked=\"checked\"";}?>>
							Disable&nbsp;&nbsp;
						<input type="radio" name="enblmu" id="enblcoc"  <?php if(1==$curr_mu_mimo_status){echo "checked=\"checked\"";}?>>
							Enable
						<input type="hidden" name="muStatus" />
					</td>
				</tr>
				<tr>
					<td>Airtime Fairness:</td>
					<td>
						<input type="radio" name="enblair" id="enblcoc"  <?php if(0==$curr_airtime_status){echo "checked=\"checked\"";}?>>
							Disable&nbsp;&nbsp;
						<input type="radio" name="enblair" id="enblcoc"  <?php if(1==$curr_airtime_status){echo "checked=\"checked\"";}?>>
							Enable
						<input type="hidden" name="airStatus" />
					</td>
				</tr>
				<tr id="tr_l2">

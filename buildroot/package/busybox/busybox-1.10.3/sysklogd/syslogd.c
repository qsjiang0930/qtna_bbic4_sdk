...
#define OPTION_PARAM &opt_m, &G.logFilePath, &opt_l \
	USE_FEATURE_REMOTE_LOG(    ,&G.ifname) \
	USE_FEATURE_ROTATE_LOGFILE(,&opt_s) \
	USE_FEATURE_ROTATE_LOGFILE(,&opt_b) \
...
static void send_to_pktlogger(char *buffer, int size)
{
...
#define QDRV_NETLINK_PKTLOGGER 30
	...
	if (sock_fd < 0) {
		printf("syslogd: Failed to create netlink socket\n");
		return;
	}
	...
	nlh = calloc(1, NLMSG_SPACE(size + 1)); /* Reserve space for a trailing '\n' */
	if (nlh == NULL) {
		close(sock_fd);
		return;
	}

	nl_data = (char *) NLMSG_DATA(nlh);
	memcpy(nl_data, buffer, size);
	*(nl_data + size) = '\n';

	nlh->nlmsg_pid = getpid();
	nlh->nlmsg_len = size + 1;
#define QDRV_NETDEBUG_TYPE_SYSMSG 6
	nlh->nlmsg_type = QDRV_NETDEBUG_TYPE_SYSMSG;
	...
	iov.iov_base = nlh;
	iov.iov_len = NLMSG_SPACE(nlh->nlmsg_len);

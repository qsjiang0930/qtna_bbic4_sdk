	...
	<div class="rightmain">
		<table class="tablemain">
			<tr>
				...
				<?php
				$mode=exec("get_qfdr_running_mode");
			...
			<pre><?php
				...
				while(1)
				{
					...
					if(!(strpos($assoc_count, "QCS API error") === false))
					{$assoc_count="0";}
					if( $assoc_count > 0)
					{
						$index = 0;
						while( $index<$assoc_count )
						{
							...
							if( !(strpos($station_mac, "QCS API error") === false ) )
							{
								$current_wifi_mode = trim(shell_exec("get_qfdr_running_mode"));
							...
							if( !(strpos($time_associated, "QCS API error") === false ) ||

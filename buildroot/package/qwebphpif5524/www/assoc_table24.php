	...
	<div class="rightmain">
		<table class="tablemain">
			...
			<pre><?php
				...
				$assoc_count=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_num $interface");
				if( $assoc_count > 0)
				{
					$index = 1;
					while( $index<=$assoc_count )
					{
						...
						{
							$station_mac=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_info $index | grep mac | awk '{print$2}'");
						}
						else {
							$station_mac = "call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 status rootap | grep mac | awk '{print $2}'";
						}
						$tmp_rssi=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_info $index | grep rssi | awk '{print$2}'");
						$rssi=$tmp_rssi-100;
						$rx_bytes=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_info $index | grep rx_bytes | awk '{print$2}'");
						$tx_bytes=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_info $index | grep tx_bytes | awk '{print$2}'");
						$bw_assoc=trim(exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_info $index | grep bandwidth | awk -F ':' '{print$2}'"));
						$time_associated=trim(exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_info $index | grep duration | awk -F ':' '{print$2}'"))." s";
						$index++;
						echo "<tr>";

...
function get_value()
{
	...
	if($curr_mode=="0")
	{
		...
		$mac0=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 showhwaddr ap");
		$arr_ssid[0]="ap"."("."$mac0".")";
		$arr_index=0;
		...
		else
		{
			...
			$mac1=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 showhwaddr vap0");
			$sum_active_interface++;
		...
		else
		{
			...
			$mac2=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 showhwaddr vap1");
			$sum_active_interface++;
		...
		else
		{
			...
			$mac3=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 showhwaddr vap2");
		...
		else
		{
			...
			$mac4=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 showhwaddr vap3");
...
{
	$bssid = exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 showhwaddr $interface");
...
function read_ssid($interface)
{
	...
	{
		$ssid=exec("qweconfig get ssid.$interface.wlan1");
	}
	return $ssid;
}
...
function read_assoc($interface)
{
	...
	if ($curr_mode == "0")
	{
		$res=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 sta_num $interface");
	}
	else
	{
		$res=trim(exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 status rootap | grep mac | awk '{print$2}'"));
...
function read_rssi($interface)
{
	...
	if ($curr_mode != "0")
	{
		$res=exec("call_qcsapi_sockrpc --host $client_ip qwe qweaction wlan1 status rootap | grep rssi | awk '{print$2}'");
		if($res == "not-associated")
		{
			$res="Not Associated";
		}
	...
	<div class="right">
		...
		<div class="rightmain">
			<table class="tablemain">
				...
				<tr>
					<td>SSID:</td>
					<td><?php $tmp=read_ssid($interface_id); echo $tmp;?></td>
				</tr>
				<tr>
					<td>Channel:</td>

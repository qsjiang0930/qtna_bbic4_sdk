...
function get_value()
{
	global $curr_mode,$curr_qfdrmode,$curr_fixed_ap,$curr_ssid,$curr_channel,$curr_channel_list,$curr_proto,$curr_psk,$curr_band,$curr_broadcast,$curr_wmm,$curr_bintval,$curr_dtim_period,$curr_short_gi,$curr_bw,$curr_coexist,$curr_frag_threshold,$curr_rts_threshold;
	//==========================================================================================================
	$curr_mode=exec("qweconfig get mode.wlan1");
	if ($curr_mode == "0")//AP
	{
		$curr_qfdrmode=exec("get_qfdr_running_mode");
		if ($curr_qfdrmode == "Station")
		{
			$ssidmode=exec("call_qcsapi get_qfdr_param 24g_ap_mode");
			if ($ssidmode == "fixed")
				$curr_fixed_ap=1;
			else
				$curr_fixed_ap=0;
	...
	{
		{
			...
			if ($curr_mode == "0")//AP
			{
				...
				if ($new_fixed_ap != $curr_fixed_ap)
				{
					...
					if ($new_fixed_ap == "1")
					{
						exec("call_qcsapi set_qfdr_param 24g_ap_mode fixed");
					}
					else
					{
						exec("call_qcsapi set_qfdr_param 24g_ap_mode clone");
					}
				}
				if ($new_fixed_ap == "1")
				{
			...
			if ($change_flag>0)
			{
				exec("perform_cmd_on_remote \"qfdr_sync_config qwe\"");
				exec("perform_cmd_on_remote \"qfdr_sync_config qfdr\"");
				exec("rebuild_qfdr_ap 2.4g");
			}
		}
	}
}
...
function onload_event()
{
	...
	var curr_qfdrmode = "<?php echo $curr_qfdrmode; ?>";
	if (curr_qfdrmode == "Access point")

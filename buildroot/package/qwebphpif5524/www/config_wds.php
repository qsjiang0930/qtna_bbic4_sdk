...
function setValue($old_chk,$new_chk,$old_wds,$new_wds,$new_psk,$old_vlan,$new_vlan,$wds_if)
{
	...
	if($new_chk!=$old_chk)
	{
		...
		{
			$tmp=exec("call_qcsapi wds_add_peer wifi0 $new_wds");
			if($tmp!="complete")
			{
			...
			//sleep(3);
			if($new_psk!="")
			{exec("call_qcsapi wds_set_psk wifi0 $new_wds $new_psk");}
			//add vlan
			if($new_vlan!="")
			{
	...
	else if ($new_chk==$old_chk && $new_chk==1)
	{
		if ($new_wds!=$old_wds)
		{
			...
			exec("qvlan unbind $wds_if $old_vlan");
			exec("call_qcsapi wds_remove_peer wifi0 $old_wds");
			exec("call_qcsapi wds_add_peer wifi0 $new_wds");
			if($old_vlan!="")
		...
		{exec("call_qcsapi wds_set_psk wifi0 $new_wds NULL");}

...
function get_value()
{
	...
	global $hs20_conn_capab_list,$hs20_conn_arr_len,$curr_disable_dgaf,$curr_proxy_arp;
	...
	if (!(strpos($hs20_conn_capab_list, "QCS API error") === FALSE))
	{
		$hs20_conn_arr_len = 0;
	}
	else
	...
	if (!(strpos($curr_oper_band, "QCS API error") === FALSE))
	{
		$curr_2_4_ghz_band=0;
		$curr_5_ghz_band=0;
	}
...
function set_value()
{
	global  $curr_hotspot,$curr_disable_dgaf,$curr_proxy_arp,$curr_wan_link_status;
	...
	if ($_POST['action'] == 0)
	{
		...
		$get_wps_status="";
                //Get the status of WPS State
                session_start();
                $get_wps_status = $_SESSION['wps_state'];
		...
		//Disable WPS IE when HS2.0 is enabled
                if ($new_hotspot == 1 && $curr_hotspot == 0)
                {
			if ($get_wps_status != 0 || $get_wps_status == "")
				exec("call_qcsapi set_wps_configured_state wifi0 0");
                }
                //Enable WPS IE when HS2.0 is disabled
                if ($new_hotspot == 0 && $curr_hotspot == 1)
                {
                        if ($get_wps_status == NULL || $get_wps_status == "")
			{
                                //Default value of wps_state param is 2 in hostapd.conf
                                $get_wps_status = 2;
                        }
                        exec("call_qcsapi set_wps_configured_state wifi0 $get_wps_status");
                }
                //To keep the new HS2.0 configuration.
                // Based on HS2.0 configuration, WPS can be enabled or disabled through UI.
                session_start();
                $_SESSION['hs20_status']=$new_hotspot;
...
function modechange(obj)
{
	if (obj.name == "cmb_hotspot")
	{
		if (isset('cmb_hotspot', '0'))
		{
			...
			set_disabled('chk_proxy_arp', true);
		...
		else
		{
			...
			set_disabled('chk_proxy_arp', false);
...
<div class="container">
	...
	<div class="right">
		<div class="righttop">5G WI-FI - HOTSPOT</div>

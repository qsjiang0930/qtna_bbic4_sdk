...
function setValue()
{
	...
	if ($_POST['action'] == 1)
	{
		...
		if ($new_chk == 1)
		{
			...
			exec("call_qcsapi set_SSID $device \"$escaped_new_ssid\"");
			//Set Broadcast
			exec("call_qcsapi set_option $device SSID_broadcast $new_br");
			//Set Priority
			exec("call_qcsapi set_priority $device $new_priority");
			$old_line=exec("cat $file | grep $device");
			$new_line="$device:priority=$new_priority";
			//Set VLAN
			if($new_vlan!="")
			{
				exec("call_qcsapi vlan_config $device bind $new_vlan");
				$new_line=$new_line."&vlan=$new_vlan";
			}
			$content=str_replace($old_line,$new_line,$content);
			$tmp=file_put_contents($file, $content);
			//Set PMF
			exec("call_qcsapi set_pmf $device $new_pmf");
			//Set Encryption
			set_proto($device,$new_proto);
			//Set PSK
			$escaped_new_psk=escape_any_characters($new_psk);
			$tmp_return=exec("call_qcsapi set_passphrase $device 0 \"$escaped_new_psk\"");
			if($tmp_return=="QCS API error 22: Invalid argument")
			exec("call_qcsapi set_pre_shared_key $device 0 \"$escaped_new_psk\"");
			//Set Radius server
			if ($add_radius == 1) {
		...
		# sync slave config and apply
		exec("perform_cmd_on_remote \"qfdr_sync_config hostapd\"");
		exec("perform_cmd_on_remote \"call_qcsapi reload_in_mode wifi0 ap\"");
		exec("perform_cmd_on_remote \"rfenable on\"");
	}
}
...
function modechange(obj)
{
	...
	if (obj.name == "cmb_proto")
	{
		...
		if (isset('cmb_pmf', '0'))
		{
			...
			else if (document.mainform.cmb_proto.selectedIndex > 2)
			{
				flush_radius_table();
				set_visible('radius_display_table', true);
				set_visible('radius_table', true);

...
{
	$local_mode=exec("call_qcsapi get_mode wifi0");
	if ($local_mode == "Station")
	{
		$encryption=exec("call_qcsapi SSID_get_proto wifi0 \"$ssid\"");
		$authentication=exec("call_qcsapi SSID_get_authentication_mode wifi0 \"$ssid\"");
	}
	else
	{
		$encryption=exec("call_qcsapi_sockrpc --host $client_ip SSID_get_proto wifi0 \"$ssid\"");
		$authentication=exec("call_qcsapi_sockrpc --host $client_ip SSID_get_authentication_mode wifi0 \"$ssid\"");
	}
	if(!(strpos($encryption, "QCS API error") === FALSE))
		$proto="NONE";
...
{
	if ($proto == "NONE")
		exec("call_qcsapi SSID_set_authentication_mode wifi0 \"$ssid\" NONE");
	elseif ($proto == "11i")
	{
		exec("call_qcsapi SSID_set_authentication_mode wifi0 \"$ssid\" PSKAuthentication");
		exec("call_qcsapi SSID_set_proto wifi0 \"$ssid\" $proto");
		exec("call_qcsapi SSID_set_encryption_modes wifi0 \"$ssid\" AESEncryption");
	}
	elseif ($proto == "11i_pmf")
	{
		exec("call_qcsapi SSID_set_authentication_mode wifi0 \"$ssid\" SHA256PSKAuthenticationMixed");
		exec("call_qcsapi SSID_set_proto wifi0 \"$ssid\" $proto");
		exec("call_qcsapi SSID_set_encryption_modes wifi0 \"$ssid\" AESEncryption");
	}
	...
	{
		exec("call_qcsapi SSID_set_authentication_mode wifi0 \"$ssid\" PSKAuthentication");
		exec("call_qcsapi SSID_set_proto wifi0 \"$ssid\" $WPAand11i");
		exec("call_qcsapi SSID_set_encryption_modes wifi0 \"$ssid\" TKIPandAESEncryption");
	}
}
...
function get_value()
{
	...
	if ($curr_mode == "Station")
	{
		if ($local_mode == "Station")
		{
			$curr_ssid=trim(exec("call_qcsapi get_ssid wifi0"));
			if($curr_ssid == "") $curr_ssid=trim(exec("call_qcsapi_sockrpc --host $client_ip get_ssid wifi0"));
		}
		else
			$curr_ssid=trim(exec("call_qcsapi_sockrpc --host $client_ip get_ssid wifi0"));
	}
	else
		$curr_ssid=trim(exec("call_qcsapi get_ssid wifi0"));
	...
	$curr_channel=read_wireless_conf("channel");
	//Get bw
	$curr_bw=trim(shell_exec("call_qcsapi get_bw wifi0"));
	// Get Current  band - 11ng, 11na, 11ac etc.
	$curr_band = trim(shell_exec("iwpriv wifi0 get_mode | sed 's?^.*:??'| cut -c1-4"));
	//Get Current Channel
	$curr_auto_chan=trim(shell_exec("call_qcsapi get_channel wifi0"));
	if(!(strpos($curr_auto_chan, "QCS API error") === FALSE))
		$curr_auto_chan="";
	$curr_auto_chan_remote=trim(shell_exec("call_qcsapi_sockrpc --host $client_ip get_channel wifi0"));
	if(!(strpos($curr_auto_chan_remote, "QCS API error") === FALSE))
		$curr_auto_chan_remote="";
	...
	if ($curr_scan_status == "1")
		$curr_channel_str="Scanning:".$curr_auto_chan.",".$curr_auto_chan_remote;
	else
		$curr_channel_str="Current Channel:".$curr_auto_chan.",".$curr_auto_chan_remote;
	// Get RF Chip ID
	...
	if ($curr_mode == "Access point")
	{
		...
		if ($curr_proto != "NONE")
		{
			...
			if(!(strpos($curr_psk, "QCS API error") === FALSE))
		...
		if ($curr_proto == "WPA2-EAP" || $curr_proto == "WPAand11i-EAP")
		{
			...
			if (!(strpos($curr_radius, "QCS API error") === FALSE))
			{
				 $radius_arr_len=0;
			}
			else
	...
	elseif ($curr_mode == "Station")
	{
		...
		if ($local_mode == "Station")
			...
			$curr_pmf=trim(shell_exec("call_qcsapi_sockrpc --host $client_ip SSID_get_pmf wifi0 $curr_ssid_esc"));
		if(!(strpos($curr_pmf, "QCS API error") === FALSE))
		{$curr_pmf="0";}
		//Get Current Proto and PSK
		if ($curr_ssid != "")
		{
			...
			if ($curr_proto != "NONE")
			{
				...
				{
					$curr_encryption = trim(shell_exec("call_qcsapi_sockrpc --host $client_ip SSID_get_encryption_modes wifi0 $curr_ssid"));
					$curr_psk = trim(shell_exec("call_qcsapi_sockrpc --host $client_ip SSID_get_key_passphrase wifi0 \"$curr_ssid_esc\" 0"));
				}
				if($curr_psk == "")
				{
					if ($local_mode == "Station")
						...
						$curr_psk = trim(shell_exec("call_qcsapi_sockrpc --host $client_ip SSID_get_pre_shared_key wifi0 \"$curr_ssid_esc\" 0"));
				}
			}
		}
	}
...
function set_value()
{
	...
	if ($_POST['action'] == 0)
	{
		...
		{
			$s_curr_mode=($curr_mode == "Station") ? "sta" : "ap";
			$s_new_mode=($new_mode == "Station") ? "sta" : "ap";
			write_wireless_conf("mode",$s_curr_mode,$s_new_mode);
		...
		else
		{
			...
			if ($curr_mode == "Access point")
			{
				...
				{
					exec("iwpriv wifi0 mode	$new_band");
					write_wireless_conf("band","$curr_band",$new_band);
					$need_reboot=1;
				}
				//set bw
				if ($new_bw!=$curr_bw)
				{
					exec("call_qcsapi set_bw wifi0 $new_bw");
					write_wireless_conf("bw",$curr_bw,$new_bw);
				}
				//Set Channel
				if ($new_channel != $curr_channel)
				{
					if ($curr_region == "none")
					{
						exec("call_qcsapi set_channel wifi0 $new_channel");
					}
					else
					{
						exec("call_qcsapi set_regulatory_channel wifi0 $new_channel $curr_region 0");
					}

					write_wireless_conf("channel",$curr_channel,$new_channel);
					sleep(1);
				}
				//Set PMF
				if ($new_pmf!=$curr_pmf)
				{
					exec("call_qcsapi update_persistent_param wifi0 pmf $new_pmf");
					exec("call_qcsapi set_pmf wifi0 $new_pmf");
				}
				//Set Protocol
				...
				if ($new_broadcast != $curr_broadcast){
					exec("call_qcsapi set_option wifi0 broadcast_SSID $new_broadcast");
				}
				//Set Priority
				if($curr_priority != $new_priority)
				{
					exec("call_qcsapi set_priority wifi0 $new_priority");
					$modify_flag=1;
				}
				//Set Beacon Interval
				if ($new_bintval != $curr_bintval)
					exec("call_qcsapi set_beacon_interval wifi0 $new_bintval");
				//Set DTIM Period
				if ($new_dtim_period != $curr_dtim_period)
					exec("call_qcsapi set_dtim wifi0 $new_dtim_period");
				//Set Short GI
				if ($new_short_gi != $curr_short_gi)
					exec("call_qcsapi set_option wifi0 short_gi $new_short_gi");
				//Set VLAN
				if ($curr_vlan!=$new_vlan)
				{
					...
					else
					{
						if($curr_vlan=="")
						{
							exec("call_qcsapi vlan_config wifi0 bind $new_vlan");
						}
						else
						{
							exec("call_qcsapi vlan_config wifi0 unbind $curr_vlan");
				...
				if($need_sync == 1) {
					exec("perform_cmd_on_remote \"qfdr_sync_config hostapd with-apply\"");
				}
			}
			else if ($curr_mode == "Station")
			{
				$local_mode = exec("call_qcsapi get_mode wifi0");
				if ($local_mode != "Station")
					exec("call_qcsapi reload_in_mode wifi0 sta");
				...
				if ($new_ssid != $curr_ssid)
				{
					...
					$tmp=exec("call_qcsapi verify_ssid wifi0 \"$new_ssid\"");
					if(!(strpos($tmp, "QCS API error") === FALSE))
						exec("call_qcsapi create_ssid wifi0 \"$new_ssid\"");
				}
				//set band
				if ($new_band!=$curr_band)
				{
				...
				{
					$modify_flag=1;
					exec("iwpriv wifi0 setparam 71 $new_bw");
					write_wireless_conf("bw",$curr_bw,$new_bw);
				}
				//Set PMF
				if ($new_pmf!=$curr_pmf)
				{
					$modify_flag=1;
					exec("call_qcsapi update_persistent_param wifi0 pmf $new_pmf");
					exec("call_qcsapi SSID_set_pmf wifi0 $new_ssid $new_pmf");
				}
				//Set Protocol
				if ($new_proto != $curr_proto)
				{
				...
				if ($new_psk != $curr_psk)
				{
					...
					$ssid= escape_any_characters($new_ssid);
					if($new_is_psk == 1)
						exec("call_qcsapi SSID_set_pre_shared_key wifi0 \"$ssid\" 0 \"$new_psk\"");
					else
						exec("call_qcsapi SSID_set_key_passphrase wifi0 \"$ssid\" 0 \"$new_psk\";");
				}
				if ($modify_flag == 1)
					exec("perform_cmd_on_remote \"qfdr_sync_config supplicant with-apply\"");
			}
		}
		//Set MCS Index
		if (strcmp($new_mcs,$curr_mcs)!=0 || strcmp($new_nss,$curr_nss)!=0)
		{
			...
			else
			{
				...
				exec("call_qcsapi set_mcs_rate wifi0 $new_mcs");
			}
		}
		if ($need_reboot == 1)
		{
...
function populate_encryptionlist(pmf_value)
{
	...
	else if (mode == "Access point")
	{
		if (pmf_value == "0") //Disabled
		{
			...
			cmb_encryption.options[4].text = "WPA2 + WPA Enterprise"; cmb_encryption.options[4].value = "WPAand11i-EAP";
		}
...
function modechange(obj)
{
	...
	if(obj.name == 'cmb_encryption')
	{
		...
		if (isset('cmb_pmf', '0'))
		{
			if (document.mainform.cmb_encryption.selectedIndex > 0
				&& document.mainform.cmb_encryption.selectedIndex <= 2)
			{
				set_visible('tr_passphrase', true);
			}
			else if (document.mainform.cmb_encryption.selectedIndex > 2)
			{
	...
	if(obj.name == 'cmb_wirelessmode')
	{
		...
		else //11ng - Temp - Supports only 20MHz bw
		{
			cmb_bw.options.length = 1;
			cmb_bw.options[0].text = "20MHz"; cmb_bw.options[0].value = "20";
			cmb_bw.options[0].selected = true;
		...
		//Set mcslist
		if (isset('cmb_wirelessmode', '11na') || isset('cmb_wirelessmode', '11ng'))
		{
			set_visible('tr_divline',true);
			set_visible('tr_txrate', true);
			populate_mcslist();
...
function onload_event()
{
	...
	else if (curr_band == "11na" || curr_band == "11ng")
	{
		populate_mcslist();
	}
	else
...
<div class="container">
	...
	<div class="right">
		<div class="righttop">5G WI-FI - CONFIG</div>
						...
						<tr>
							...
							<td width="60%">
								<select name="cmb_devicemode" class="combox" id="cmb_devicemode" onchange="modechange(this)">
									...
									{
									?>
										<option value="Station">Extender</option></option>
									<?php
									}
									?>
									<?php
									...
									{
									?>
										<option value="Access point">Access Point</option></option>
									<?php
									}
									?>
							...
							<td>
								<select name="cmb_encryption" id="cmb_encryption" class="combox" onchange="modechange(this)">
									...
									?>
										...
										<option value="WPAand11i-EAP"> WPA2 + WPA Enterprise </option>
									<?php
									}
									?>
								</select>

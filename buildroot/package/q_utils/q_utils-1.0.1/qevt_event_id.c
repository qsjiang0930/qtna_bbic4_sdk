...
static int qevt_parse_conf_file(char *str)
{
	...
	data = strtok(NULL, ",");
	if (data == NULL) {
		fprintf(stderr, "Failed to read event Name\n");
		goto failure;
	}

	if (strlen(data) >= EVENT_NAME_LEN) {
		fprintf(stderr, "Event name length is too long: %s\n", data);
		goto failure;
	}

	strncpy(tmp->name, data, EVENT_NAME_LEN - 1);

	/* Read Event compare string */
	data = strtok(NULL, ",");
	if (data == NULL) {
		fprintf(stderr, "Failed to read event compare string\n");
		goto failure;
	}

	if (strlen(data) >= EVENT_SUB_STRING_LEN) {
		fprintf(stderr, "Event compare string length is too long: %s\n", data);
		goto failure;
	}
	...
	while (data != NULL) {
		if (tmp->num_of_params >= MAX_PARAMS) {
			fprintf(stderr, "Too many params exceeds limit\n");
			goto failure;
		}

		if (strlen(data) >= MAX_PARAM_NAME_LEN) {
			fprintf(stderr, "Event param name length is too long: %s\n", data);
			goto failure;
		}
		...
		data = strtok(NULL, ",");
		if (data == NULL) {
			fprintf(stderr, "Failed to read pattern for the param\n");
			goto failure;
		}

		if (strlen(data) >= MAX_PARAM_PATTERN_LEN) {
			fprintf(stderr, "Pattern length of param is too long: %s\n", data);
			goto failure;
		}

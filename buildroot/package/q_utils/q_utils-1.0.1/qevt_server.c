...
static void qevt_del_interface_data(int ifindex)
{
	struct qevt_wifi_iface *curr;
	struct qevt_wifi_iface *prev = NULL;
	struct qevt_wifi_iface *next;

	curr = qevt_server_cfg.if_cache;
	while (curr) {
		next = curr->next;

		if (curr->ifindex == ifindex) {
			if(!prev)
				qevt_server_cfg.if_cache = next;
			else
				prev->next = next;
...
static void qevt_wpa_remove_iface(struct qevt_wpa_iface **if_wpa_head,
				struct qevt_wpa_iface *if_wpa)
{
	struct qevt_wpa_iface *temp, *p;

	temp = *if_wpa_head;

	if (if_wpa->socket_is_alive || if_wpa->event_check) {
		if_wpa->socket_is_alive = 0;
		return;
	}

	qevt_wpa_close(if_wpa);

	if (temp == if_wpa) {
		*if_wpa_head = temp->next;
		free(if_wpa);
		return;
	} else {
		while (temp->next) {
...
static int qevt_wpa_scan(struct qevt_server_config * const config)
{
	...
	while (if_wpa_head) {
		if_wpa_head->iface_is_missing = 1;
		...
		if (wifi) {
			...
			while (if_wpa_head) {
				if (!strcmp(wifi, if_wpa_head->ifname)) {
					found = 1;
					if_wpa_head->iface_is_missing = 0;
					break;
				}
				if_wpa_head = if_wpa_head->next;
			}
			...
			if (found) {
				...
				if (!qevt_wpa_ping(if_wpa_head)) {
					if_wpa_head->socket_is_alive = 0;
					qevt_wpa_close(if_wpa_head);
				} else {
					if_wpa_head->socket_is_alive = 1;
				}
			} else {
	...
	while (if_wpa_head) {
		struct qevt_wpa_iface *next = if_wpa_head->next;

		if (if_wpa_head->iface_is_missing)
			qevt_wpa_remove_iface(&config->if_wpa, if_wpa_head);
		if_wpa_head = next;
	}
...
static void qevt_wpa_event(const struct qevt_wpa_iface * const wpa)
{
	...
	ret = wpa_ctrl_recv(wpa->wpa_event, wpa_event_msg, &reply_len);
	if (ret < 0)
		return;
...
static int qevt_core_dump_watch_init(void)
{
	...
	if (mkdir(DEFAULT_CORE_FILE_PATH, S_IRWXU | S_IRWXG) < 0) {
		if (errno != EEXIST)
...
static int qevt_server_init(void)
{
	...
	openlog("qevt_server", LOG_CONS | LOG_NDELAY, LOG_DAEMON);
...
static int qevt_client_connected(struct qevt_client * const client)
{
	...
	if (count > 0) {
		...
		/* buffer is not tainted, it seems a limit of coverity, add suppress comment */
...
static int qevt_netlink_read(void)
{
	...
	for (nlh = (struct nlmsghdr *)buf; NLMSG_OK(nlh, len);
		...
		if (nlh->nlmsg_type == RTM_DELLINK)
		{
			...
			qevt_del_interface_data(ifim->ifi_index);
			break;
		}
		if (nlh->nlmsg_type == RTM_NEWLINK)
...
static int qevt_read_system_event(void)
{
	...
	buffer[MIN(sizeof(buffer) - 1, bytes)] = 0;
...
static void qevt_wait_for_event(void)
{
	...
	while (qevt_server_cfg.running) {
		...
		while (wpa) {
			if (wpa->wpa_event && wpa->fd && wpa_event_check && wpa->socket_is_alive) {
				FD_SET(wpa->fd, &rfds);

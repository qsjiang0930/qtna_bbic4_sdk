 ...
 */
	...
	#define CONFIG_ARC_MUC_STACK_OFFSET_UBOOT	(0x0003F7C0 - RUBY_CRUMBS_PRESERVED_SIZE)
	/* MuC stack, included in CONFIG_ARC_MUC_SRAM_SIZE */
	#define CONFIG_ARC_MUC_STACK_SIZE		(4 * 1024)
#else
	/* Must be equal to CONFIG_ARC_MUC_STACK_OFFSET */
	#define CONFIG_ARC_MUC_STACK_OFFSET_UBOOT	(0x0003FFA0 - RUBY_CRUMBS_PRESERVED_SIZE)
	/* MuC stack, included in CONFIG_ARC_MUC_SRAM_SIZE */
...
#define RUBY_CRUMBS_OFFSET_UBOOT	(0x0003FFC0 - RUBY_CRUMBS_PRESERVED_SIZE)
...
#if !defined (RUBY_CRUMBS_PRESERVED_SIZE)
	#define RUBY_CRUMBS_PRESERVED_SIZE	(64)
#endif

#if (RUBY_CRUMBS_PRESERVED_SIZE > 0 && RUBY_CRUMBS_PRESERVED_SIZE < 8) || (RUBY_CRUMBS_PRESERVED_SIZE % 4) != 0
	#error RUBY_CRUMBS_PRESERVED_SIZE invalid
#endif

#define RUBY_CRUMBS_SIZE		(64 + RUBY_CRUMBS_PRESERVED_SIZE)

#ifndef __ASSEMBLY__
	#if RUBY_CRUMBS_PRESERVED_SIZE
		struct ruby_crumbs_preserved {
			char misc[RUBY_CRUMBS_PRESERVED_SIZE - 4];
			unsigned long	magic;
		};
		#define RUBY_CRUMBS_PRESERVED_MAGIC	0x244bea0e
	#endif
	...
	#if RUBY_CRUMBS_PRESERVED_SIZE
		/*
		* preserved segment comes before 'classic' crumbs structure to preserve
		* compatibility, so that classic crumbs remains located in same place
		*/
		struct ruby_crumbs_preserved	preserved;
	#endif
		/* lhost marks beginning of 'classic' crumbs structure */

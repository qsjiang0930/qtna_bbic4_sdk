...
#define QDPC_BDA_FW_QLINK_DONE		0x14
...
#define PCIE_BDA_HOST_CALCMD		BIT(3)
...
#define PCIE_BDA_HOST_QLINK_DRV		BIT(7) /* Host advertises driver type */
...
#define PCIE_BDA_HOST_MEMALLOC_ERR	BIT(13) /* Host malloc failed on firmware download */
#define PCIE_BDA_HOST_MEMMAP_ERR	BIT(14) /* Host pci map failed on firmware download */
...
typedef struct qdpc_pcie_bda {
	...
	uint32_t	bda_ep2h_irqstatus;	/* allow boot loader to use irqs if desired */
	uint32_t	bda_h2ep_irqstatus;	/* allow boot loader to use irqs if desired */
	uint32_t	bda_msi_addr;
	uint8_t		reserved1[56];		/* Reserve 56 bytes for backwards compatibility */
	...
	uint8_t		reserved2;
	uint32_t	bda_ep_next_pkt;	/* RC pointer to next packet to EP */

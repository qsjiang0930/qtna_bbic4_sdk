...
static int add_del_if(struct net_bridge *br, int ifindex, int isadd)
{
	...
	if (ret == 0)
		call_netdevice_notifiers(
			(isadd) ? NETDEV_BRIDGE_ADDIF : NETDEV_BRIDGE_DELIF,
			dev);
	return ret;
}

static void get_mc_group(struct net_bridge *br)
{
...
static int old_dev_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	case BRCTL_SET_IGMP_SNOOP_STATE:
		if (args[1] > BR_IGMP_SNOOP_ENABLED)
			return -EINVAL;
	...
	case BRCTL_SET_MC_DEBUG:
		g_br_igmp_snoop_dbg = args[1] ? 1 : 0;
		printk("%sabling multicast debug\n", args[1] ? "En" : "Dis");
		return 0;

	case BRCTL_GET_MC_GROUP:
		get_mc_group(br);
		return 0;
	}

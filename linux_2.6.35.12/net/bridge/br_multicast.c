				 ...
				 struct sk_buff *skb)
{
	...
	ih = igmp_hdr(skb2);
	if (M_FLAG_ISSET(skb, M_L2_EXT_FILTER)) {
		if (ih->type == IGMP_HOST_MEMBERSHIP_REPORT ||
				ih->type == IGMPV2_HOST_MEMBERSHIP_REPORT)
			BR_INPUT_SKB_CB(skb)->mrouters_only = 1;
		else if (ih->type == IGMP_HOST_MEMBERSHIP_QUERY && ih->group)
			BR_INPUT_SKB_CB(skb)->ucast_fwd = 1;
		goto out;
	}
...
/* Added by Quantenna */
void br_mdb_update_by_sub_port(struct net_bridge *br,
			       const struct net_bridge_port *port,
			       port_id sub_port, port_id new_sub_port)
{
	struct net_bridge_mdb_htable *mdb;
	struct net_bridge_mdb_entry *mp;
	struct net_bridge_port_group *p;
	...
	for (i = 0; i < mdb->max; i++) {
		hlist_for_each_entry_safe(mp, node, node2, &mdb->mhash[i], hlist[mdb->ver]) {
			for (pp = &mp->ports; (p = *pp); pp = &p->next) {
				if (p->port != port || p->sub_port != sub_port)
					continue;
				if (g_fwt_mult_join_hook && g_fwt_mult_leave_hook) {
					g_fwt_mult_join_hook(new_sub_port, port->dev->if_port,
								&p->addr);
					g_fwt_mult_leave_hook(p->sub_port, port->dev->if_port,
								&p->addr);
				}
				p->sub_port = new_sub_port;
				break;
			}
		}
	}

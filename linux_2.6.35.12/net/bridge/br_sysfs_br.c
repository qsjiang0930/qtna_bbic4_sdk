...
static ssize_t show_igmp_snoop_state(struct device *d,
			      struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static ssize_t store_igmp_snoop_state(struct device *d,
			       struct device_attribute *attr, const char *buf,
			       size_t len)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(igmp_snoop_state, S_IRUGO | S_IWUSR, show_igmp_snoop_state,
		   store_igmp_snoop_state);

static ssize_t show_ssdp_flood_state(struct device *d,
			      struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	return sprintf(buf, "%d\n", br->ssdp_flood_state);
...
static ssize_t store_ssdp_flood_state(struct device *d,
			       struct device_attribute *attr, const char *buf,
			       size_t len)
{
	struct net_bridge *br = to_bridge(d);
...
static DEVICE_ATTR(ssdp_flood_state, S_IRUGO | S_IWUSR, show_ssdp_flood_state,
		   store_ssdp_flood_state);

static ssize_t show_report_flood_interval(struct device *d,
			      struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
...
static ssize_t store_report_flood_interval(struct device *d,
			       struct device_attribute *attr, const char *buf,
			       size_t len)
{
	struct net_bridge *br = to_bridge(d);
...
static ssize_t show_multicast_group(struct device *d, struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	...
	for (idx = 0; idx < BR_HASH_SIZE; idx++) {
		...
		hlist_for_each_entry(me, h, &mdb->mhash[idx], hlist[mdb->ver]) {
			br_multicast_mdb_dump(me);
		}
	}
...
static DEVICE_ATTR(igmp_snoop_group, S_IRUGO | S_IWUSR, show_multicast_group, NULL);

static ssize_t set_snoop_debug(struct device *d, struct device_attribute *attr,
	const char *buf, size_t len)
{
	if (!capable(CAP_NET_ADMIN))
		return -EPERM;

	if (sscanf(buf, "%u", &g_br_igmp_snoop_dbg) != 1)
		return -EINVAL;

	return len;
}

static ssize_t get_snoop_debug(struct device *d, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", g_br_igmp_snoop_dbg);
}
static DEVICE_ATTR(igmp_snoop_debug, S_IRUGO | S_IWUSR, get_snoop_debug, set_snoop_debug);
#endif

static ssize_t show_mcast_ip_flood_list(struct device *d,
		struct device_attribute *attr, char *buf)
{
	struct net_bridge *br = to_bridge(d);
	ssize_t ret = 0;
	...
	rtnl_lock();
	for (i = 0; i < br->ip_flood_list_count; i++)
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "%pI4\n", &br->ip_flood_list[i]);
	rtnl_unlock();

	return ret;
}

static void br_ip_flood_list_add_entry(struct net_bridge *br, __be32 ip)
{
	int i;

	for (i = 0; i < br->ip_flood_list_count; i++) {
		if (br->ip_flood_list[i] == ip)
			return;
	}

	if (br->ip_flood_list_count >= BR_MAX_IP_FLOOD_ENTRIES) {
		printk("[%s] IP flood list is full\n",
			br->dev->name);
		return;
	}

	spin_lock_bh(&br->lock);
	br->ip_flood_list[br->ip_flood_list_count] = ip;
	br->ip_flood_list_count++;
	spin_unlock_bh(&br->lock);
}

static void br_ip_flood_list_del_entry(struct net_bridge *br, __be32 ip)
{
	int i;
	...
	for (i = 0; i < br->ip_flood_list_count; i++) {
		if (br->ip_flood_list[i] == ip) {
			spin_lock_bh(&br->lock);
			br->ip_flood_list_count--;
			br->ip_flood_list[i] = br->ip_flood_list[br->ip_flood_list_count];
			spin_unlock_bh(&br->lock);
			break;
		}
	}
...
static ssize_t store_mcast_ip_flood_list(struct device *d,
		struct device_attribute *attr, const char *buf,
		size_t len)
{
	struct net_bridge *br = to_bridge(d);
	...
	if (!ipv4_is_multicast(ip)) {
		printk("[%s] addr: %s is not a valid multicast ip address\n",
			br->dev->name, buf + 1);
		return len;
	}
	...
	switch (buf[0]) {
	case '-':
		br_ip_flood_list_del_entry(br, ip);
		break;
	case '+':
		br_ip_flood_list_add_entry(br, ip);
		break;
	default:
		printk("[%s] command: \"%c\" is not recognised. Valid commands are - and +\n",
			br->dev->name, buf[0]);
		break;
	}
...
static DEVICE_ATTR(mcast_ip_flood_list, S_IRUGO | S_IWUSR, show_mcast_ip_flood_list,
		   store_mcast_ip_flood_list);
...
static struct attribute *bridge_attrs[] = {
	...
	&dev_attr_mcast_ip_flood_list.attr,

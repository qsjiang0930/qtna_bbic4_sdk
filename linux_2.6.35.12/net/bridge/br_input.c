...
int br_set_l2_ext_filter_port(struct net_device *dev)
{
	...
	br = p->br;
	br->l2_ext_filter_port = p;

	return 0;
}
EXPORT_SYMBOL(br_set_l2_ext_filter_port);
...
static int br_handle_mcast_should_flood(struct net_bridge *br,
		const unsigned char *dest, void *data)
{
	const struct iphdr *ipv4h = data;
	uint32_t daddr;
	int i;

	if (!iputil_mac_is_v4_multicast(dest))
		return 0;

	daddr = get_unaligned((uint32_t *)&ipv4h->daddr);

	for (i = 0; i < br->ip_flood_list_count; i++) {
		if (daddr == br->ip_flood_list[i])
			return 1;
	}
		...
		struct net_bridge *br, const unsigned char *dest)
{
	...
	void *data;
	int should_flood;
	int to_ext_filter = QTN_FLAG_IS_L2_EXT_FILTER(skb->dev->qtn_flags) &&
				br->l2_ext_filter_port;
	int from_ext_filter = M_FLAG_ISSET(skb, M_L2_EXT_FILTER);
	...
	should_flood = br_handle_mcast_should_flood(br, dest, data);
	if (should_flood)
		M_FLAG_SET(skb, M_IPFF);

	if (br_handle_to_protoclstack(dest, data) && !from_ext_filter)
		skb2 = skb;
	...
	if (!iputil_is_lncb(dest, data) &&
		...
		if (BR_INPUT_SKB_CB_MROUTERS_ONLY(skb)) {
			...
			mdst = br_mdb_get_ext(br, skb);
			if (mdst && !to_ext_filter) {
				br_multicast_copy_to_sub_ports(br, mdst, skb);

				if (skb2 == NULL)
					kfree_skb(skb);
				skb = NULL;
			} else if (!mdst && br->igmp_snoop_enabled) {
				goto drop;
			}
		} else {
...
fwd:
	if (skb && to_ext_filter) {
		br_forward(br->l2_ext_filter_port, 0, skb, skb2);
	} else if (skb) {
		skb->dest_port = 0;
...
int __sram_text br_handle_frame_finish(struct sk_buff *skb)
{
	...
	if (br_artsmng_enabled && (p->state == BR_STATE_BLOCKING)) {
	...
	if (from_ext_filter) {
		...
		/* prevent pkt from being forwarded back to original sender */
		if (src)
			skb->src_port = src->sub_port;
		/* prevent FDB flip */
	...
	if (skb) {
		to_ext_filter = QTN_FLAG_IS_L2_EXT_FILTER(skb->dev->qtn_flags) &&
			br->l2_ext_filter_port;
		if (to_ext_filter) {
			br_forward(br->l2_ext_filter_port, 0, skb, skb2);
		} else if (dst) {
			br_forward(dst->dst, dst->sub_port, skb, skb2);
		} else {
			skb->dest_port = 0;
			br_flood_forward(br, skb, skb2);
		}
...
#ifdef ARTSMNG_SUPPORT
		if (br_artsmng_enabled)
			break;
#endif /* ARTSMNG_SUPPORT */
		/* fall through to drop the packet */
	default:
		goto drop;
	}

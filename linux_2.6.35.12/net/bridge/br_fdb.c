...
/* Added by Quantenna */
unsigned char br_fdb_update_by_sub_port(struct net_bridge *br,
			       const struct net_bridge_port *p,
			       port_id sub_port, port_id new_sub_port)
{
		...
		hlist_for_each_safe(h, g, &br->hash[i]) {
			...
			if ((f->dst == p) &&
				...
				if (g_add_fwt_entry_hook)
					g_add_fwt_entry_hook(f->addr.addr, p->dev->if_port,
							new_sub_port, NULL);
				f->sub_port = new_sub_port;
			}
		}
	}
	...
	br_mdb_update_by_sub_port(br, p, sub_port, new_sub_port);
					       ...
					       int is_local, port_id sub_port)
{
	...
	if (!source) {
		br_warn(br, "creating fdb entry failed because of empty source port!\n");
		return NULL;
	}

...
struct clock_event_device __attribute__((section(".sram.data"))) arc_clockevent_device;

static irqreturn_t timer_irq_handler(int irq, void *dev_id);
#if defined(CONFIG_TOPAZ_FILTER_HBM_LEVEL2)
irqreturn_t timer_irq_handler_lv2(int irq);
#endif
...
static void __always_inline call_watchdog_softirq(void)
{
    static unsigned int counter;

    counter++;
    if (unlikely(counter >= HZ)) {
        watchdog_softirq();
        counter = 0;
    }
}

irqreturn_t __attribute__((section(".sram.text"))) timer_irq_handler(int irq, void *dev_id)
{
    struct clock_event_device *evt;

#if defined(CONFIG_TOPAZ_FILTER_HBM_LEVEL2)
    evt = &arc_clockevent_device;

    write_new_aux_reg(AUX_IRQ_HINT, 0);
    topaz_hbm_handle_deferred_level2();
#else
    evt = dev_id;

    pet_watchdog_timer();
    arc_timer0_ack_event(evt->mode == CLOCK_EVT_MODE_PERIODIC);
    topaz_hbm_filter_txdone_pool(1);
#endif
    call_watchdog_softirq();
    evt->event_handler(evt);
    return IRQ_HANDLED;
}

#if defined(CONFIG_TOPAZ_FILTER_HBM_LEVEL2)
irqreturn_t __attribute__((section(".sram.text"))) timer_irq_handler_lv2(int irq)
{
	...
	val = read_new_aux_reg(AUX_IRQ_HINT);
	if (likely(val == 0))
		write_new_aux_reg(AUX_IRQ_HINT, TOPAZ_IRQ_TIMER0_LEVEL1);
...
#if defined(CONFIG_TOPAZ_FILTER_HBM_LEVEL2)
    request_irq(TIMER0_INT, timer_irq_handler_lv2, 0, "ARC Timer0 LV2", NULL);
    setup_irq(TOPAZ_IRQ_TIMER0_LEVEL1, &arc_timer_irq);
#else

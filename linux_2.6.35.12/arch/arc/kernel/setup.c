...
volatile unsigned long int2_saved_sp;
#endif
...
static int __init parse_tag_cmdline(struct tag *tag)
{
    ...
    strncpy(command_line, tag->u.cmdline.cmdline, sizeof(command_line) - 1);
    command_line[sizeof(command_line) - 1] = '\0';
    parse_board_config(command_line);
    parse_flash_config(command_line);
    return 0;
...
void __init setup_arch(char **cmdline_p)
{
    ...
    *end_of_stack(&init_task) = STACK_END_MAGIC;
    ...
    if (atag_head) {
	    if (__in_mem_range(atag_head, RUBY_SRAM_FLIP_BEGIN, RUBY_SRAM_SIZE)) {
		    atag_head = atag_head - RUBY_SRAM_FLIP_BEGIN + RUBY_SRAM_BEGIN;
	    }
